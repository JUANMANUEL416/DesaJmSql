CREATE OR ALTER PROCEDURE DBO.SPK_FACTURACE_N
@NOAUT        VARCHAR(16),
@NIT          VARCHAR(20),
@COMPANIA     VARCHAR(2),
@IDSEDE       VARCHAR(5),
   ...
   IF @PROC = 'CI'  --JEDM 20240209   COLOCO ESTE IF PQ ESTABA HACIEN UPDATE A CIT A SI NO VINIERA CON PROCEDENCIA CIT
	...
	BEGIN           
		...
		BEGIN
         ...
         OR @IDTERCERO=LTRIM(RTRIM(DBO.FNK_VALORVARIABLE('IDTERCEROPARTICULAR')))
			BEGIN
				DECLARE CUR_FTR CURSOR FOR
	...
	BEGIN  
		...
		BEGIN   
			...
			BEGIN
				...
				INSERT INTO #FTRD1(CNSFTR, FECHA, DB_CR, AREAPRESTACION, UBICACION, VR_TOTAL,
						...
						NOPRESTACION, NOITEM, AREAFUNCONT, N_FACTURA, SUBCCOSTO, PCOSTO, FECHAPREST, IDPLAN,IDIMPUESTO, IDCLASE,ITEM , VLRIMPUESTO,
                  PIVA, VIVA,CUENTA_FIMPDV,TCOPAGO)
				SELECT @CNSFTR, GETDATE(), 'DB', AUT.IDAREA, NULL, 
						...
						CASE WHEN @NODESCUENTACOPAGO = 1 THEN 0 ELSE 
																...
																END
						END,  
						0, AUT.IDPROVEEDOR, @NOAUT,'CE',AUTD.IDAUT, AUTD.NO_ITEM, NULL, 
		...
		BEGIN
			...
			SELECT @CNSFTR, GETDATE(), 'DB', CIT.IDAREA, NULL, 
				...
				0, CIT.IDMEDICO, @NOAUT,'CI', @NOAUT, 1, NULL, 
      ...
      BEGIN
         ...
         CONTINUE
      END
      IF (SELECT SUM(VLR_SERVICI-VLR_COPAGOS) FROM #FTRD1)<=0
      BEGIN 
			IF @PROC = 'CE'
			BEGIN
				UPDATE AUT SET FACTURADA=1,N_FACTURA='' WHERE NOAUT = @NOAUT  
            UPDATE AUTD SET FACTURADA=1,N_FACTURA='' FROM AUTD INNER JOIN AUT ON AUT.IDAUT=AUTD.IDAUT
            WHERE AUT.IDAUT=@NOAUT
			END
			IF @PROC = 'CI'
			BEGIN
				UPDATE CIT SET FACTURADA=1, N_FACTURA='' WHERE CONSECUTIVO=@IDAUT 
			END
			RAISERROR('El Valor del copago supera o es Igual al valor del Servicio, no debo Generar Factura ',16,1)
			FETCH NEXT FROM CUR_FTR
		   INTO @IDTERCEROCA,@IDPLAN,@N_FACTURAORI       
         CONTINUE
      END
		...
		BEGIN
			...
			BEGIN
				...
				SELECT @OBSERVACION='#$'+COALESCE(SIIFCODIGOPCI,'')+';'+REPLACE(COALESCE(SIIFCONTRATO,''),'@N_FACTURA',@NVOCONSEC)+';'+COALESCE(SIIFCORREOSUPER,'')+'#$'
		...
		BEGIN
			...
			END
         IF DBO.FNK_VALORVARIABLE('MABONOAUTORIZA_PARTI')='SI'
         BEGIN 
            IF EXISTS(SELECT * FROM AUT WHERE NOAUT=@NOAUT AND COALESCE(VLRDEPOSITOS,0)>0)
            BEGIN
               PRINT 'DESMARCO EL ENVIO A CAJA PARA GENERAR DE NUEVO Y LEGALIZAR DEPOSITOS'
               UPDATE AUT SET GENEROCAJA=0 WHERE NOAUT=@NOAUT

               EXEC SPK_PAGOSCAJA_AUT @NOAUT,@SYS_COMPUTERNAME,@COMPANIA,@IDSEDE,@USUARIO,1
            END
         END
		END  
		IF @PROC = 'CI'
