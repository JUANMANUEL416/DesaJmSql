...
BEGIN   
   ...
   PRINT 'Cuentas por Pagar de: '+@PROCEDENCIA+' - '+@TIPOCXP+ ' ; @CXP_TIPO: '+@CXP_TIPO
	...
	SELECT @CUENTA_IVA = CASE WHEN DBO.FNK_VALORVARIABLE('CUENTA_IVA_CXP') = 'SI' THEN CUENTA_IVA ELSE NULL END FROM #TH WHERE CNSFCXP = @CNSFCXP AND COALESCE(#TH.CUENTA_IVA,'')<>''
       ...
       END
		 ...
		 INSERT INTO MCHE(NROCOMPROBANTE,COMPANIA,TIPO,CUENTA,IDTERCERO,CCOSTO,DETALLE,VALOR,REFERENCIA1,
		   			 ...
		   			 PROCESO,CNSPAGO,GENERA,VALOR_PORCIMP,BASE_IMP,
						 N_FACTURA,F_FACTURAREF, F_VENCE, CLASE_GENERA, PROCEDENCIA, REFERENCIA_PRO, CODUNG, CODPRG,IDSEDE)
		 SELECT   @NROCOMPROBANTE,@COMPANIA,'CR',FIMPDV.CUENTA,CASE WHEN COALESCE(FCXPI.TERDIF,0)=1 THEN FCXPI.IDTERCEROIMP ELSE #T.IDTERCERO END,@CCOSTO,
		 ...
		 INSERT INTO MCHE(NROCOMPROBANTE,COMPANIA,TIPO,CUENTA,IDTERCERO,CCOSTO,DETALLE,VALOR,REFERENCIA1,
						 ...
						 PROCESO,CNSPAGO,GENERA,VALOR_PORCIMP,BASE_IMP,
						 N_FACTURA,F_FACTURAREF, F_VENCE, CLASE_GENERA, PROCEDENCIA, REFERENCIA_PRO, CODUNG, CODPRG,IDSEDE)
		 SELECT   @NROCOMPROBANTE,@COMPANIA,'CR',FCXPI.CUENTA,CASE WHEN COALESCE(FCXPI.TERDIF,0)=1 THEN FCXPI.IDTERCEROIMP ELSE #T.IDTERCERO END,@CCOSTO, 'IMPUESTO POR '+UPPER(FIMPD.DESCRIPCION), 
