...
DECLARE @PARAMETROS       NVARCHAR(MAX) 
      ...
      ,@VLR_LEGALIZADO DECIMAL(14,2)   ,@VLR_ANTICIPOS DECIMAL(14,2)       ,@CNSANT VARCHAR(20)    ,@CRUCETER BIT ,@DATOS VARCHAR(MAX)
DECLARE @IDMODELOCONT VARCHAR(20) , @IDTIPOCONT VARCHAR(20), @IDAREA VARCHAR(20), @PREFIJO VARCHAR(20), @CCOSTO VARCHAR(20), @CLASECONT VARCHAR(20), @IDSERVICIO VARCHAR(20)
		,@TITULO  NVARCHAR(MAX),@ESTADODIAN SMALLINT
...
BEGIN
	...
	BEGIN
	   ...
	   WITH		 (
		...
		,PROCEDENCIA		VARCHAR(10)			'$.PROCEDENCIA' ,TERDIF  VARCHAR(20)   '$.TERDIF',IDTERCEROIMP  VARCHAR(20)   '$.IDTERCEROIMP' )
		...
		BEGIN TRY 
            ...
            ,TERDIF           = CASE WHEN  COALESCE(#datoFCXPI.TERDIF,'False')='false' THEN 0 ELSE 1 END
            ,IDTERCEROIMP      =CASE WHEN  COALESCE(#datoFCXPI.TERDIF,'False')='false' THEN NULL ELSE #datoFCXPI.IDTERCEROIMP END 
            ...
            ,IDTERCEROIMP
				...
				,#datoFCXPI.CUENTA			
				,'Otros'
            ,CASE WHEN  COALESCE(#datoFCXPI.TERDIF,'False')='false' THEN 0 ELSE 1 END
            ,CASE WHEN  COALESCE(#datoFCXPI.TERDIF,'False')='false' THEN NULL ELSE #datoFCXPI.IDTERCEROIMP END 
				FROM #datoFCXPI
	   ...
	   WITH		 (
		...
		BEGIN TRY 
			...
			BEGIN
				...
				,CUENTA_DB			  = #datoFCXPDBCR.CUENTA_DB			
				,CUENTA_CR			  = #datoFCXPDBCR.CUENTA_CR			
			...
			BEGIN
				...
				INSERT INTO FCXPDBCR (CNSFCXP,ITEM,IDNOTA,TIPO,DECRUCES,FECHA,GIVA ,GIMPUESTOS,VALOR,PORCENTAJEIVA,VALORIVA,GENCXP,VLR_IMPUESTOS,
            VLR_NETO	,CAMTER,IDTERCEROGENCXP	,IDSERVICIO,NODOCUMENTO	,OBSERVACION,PROCEDENCIA,CUENTA_DB	,CUENTA_CR	,CCOSTO,NOREFERENCIA	,CODUNG
            ,CODPRG ,USUARIO ,SYS_COMPUTERNAME	,NROCOMPROBANTE, ESTADO)
				SELECT #datoFCXPDBCR.CNSFCXP	,@ITEM,#datoFCXPDBCR.IDNOTA,#datoFCXPDBCR.TIPO ,#datoFCXPDBCR.DECRUCES,REPLACE(JSON_VALUE(@datoFCXPDBCR ,'$.FECHA'),'-','') 					
                   ,#datoFCXPDBCR.GIVA	,#datoFCXPDBCR.GIMPUESTOS	,#datoFCXPDBCR.VALOR	,DBO.FNK_VALORVARIABLE('IDIVA')	,#datoFCXPDBCR.VALORIVA	,#datoFCXPDBCR.GENCXP				
                   ,#datoFCXPDBCR.VLR_IMPUESTOS	,#datoFCXPDBCR.VLR_NETO	,#datoFCXPDBCR.CAMTER	,#datoFCXPDBCR.IDTERCEROGENCXP	,#datoFCXPDBCR.IDSERVICIO			,#datoFCXPDBCR.NODOCUMENTO		
                   ,#datoFCXPDBCR.OBSERVACION		,#datoFCXPDBCR.PROCEDENCIA		,#datoFCXPDBCR.CUENTA_DB		,#datoFCXPDBCR.CUENTA_CR		,#datoFCXPDBCR.CCOSTO				,#datoFCXPDBCR.NOREFERENCIA		
                   ,#datoFCXPDBCR.CODUNG				,#datoFCXPDBCR.CODPRG				,@USUARIO			,@SYS_COMPUTERNAME,#datoFCXPDBCR.NROCOMPROBANTE	, 'N'	
            FROM #datoFCXPDBCR
			END
		END TRY
		BEGIN CATCH
...
--query3
	...
	BEGIN
      ...
      PRINT 'INGRESO '
		...
		BEGIN
			...
			RETURN
		END
      IF EXISTS(SELECT * FROM FCXP WHERE CNSFCXP=@CNSFCXP AND DOCEQUIVALENTE=1 AND COALESCE(ESTADODIAN,0) IN(0,1,3))
      BEGIN
		   BEGIN TRY 
			   --SELECT COALESCE(ESTADODIAN,0) FROM FCXP WHERE CNSFCXP= @CNSFCXP
			   UPDATE FCXP SET ESTADODIAN=CASE WHEN COALESCE(ESTADODIAN,0) IN(0,3) THEN 1 ELSE 0 END WHERE CNSFCXP = @CNSFCXP
		   END TRY
		   BEGIN CATCH
			   INSERT INTO @TBLERRORES(ERROR)
			   SELECT ERROR_MESSAGE()
      ...
      BEGIN
         INSERT INTO @TBLERRORES(ERROR)
         SELECT 'Estado Dian no valido, Verifique e intente de nuevo '
      END
		...
		BEGIN
			...
			RETURN
		END
      SELECT @ESTADODIAN=COALESCE(ESTADODIAN,0) FROM FCXP WHERE CNSFCXP=@CNSFCXP
		SELECT 'OK' AS OK,@ESTADODIAN AS ESTADODIAN
		RETURN
	END
	...
	BEGIN
		...
		BEGIN
         ...
         BEGIN
            UPDATE FCXPDBCR SET ESTADO='N',CONTABILIZADA=0,NROCOMPROBANTE=NULL WHERE  CNSFCXP=@CNSFCXP AND ITEM=@ITEM
         END
		END
		ELSE
		BEGIN
   ...
   BEGIN         
      ...
      IF NOT EXISTS(SELECT * FROM FCXPD WHERE CNSFCXP=@CNSFCXP)
      BEGIN
         INSERT INTO @TBLERRORES(ERROR)
         SELECT 'Cuenta por Pagar sin detalles'
      END
      IF NOT EXISTS(SELECT * FROM FCXP INNER JOIN CUE ON FCXP.CUECREDITO=CUE.CUENTA WHERE FCXP.CNSFCXP=@CNSFCXP AND CUE.TIPO='Detalle')
      BEGIN
         INSERT INTO @TBLERRORES(ERROR)
         SELECT 'La cuenta contable de la cxp no es de tipo detalle, no se puede continuar'
      END
      IF(SELECT COUNT(*) FROM @TBLERRORES)>0
      BEGIN
         SELECT 'KO' OK, ERROR FROM @TBLERRORES
         RETURN
      END
      IF NOT EXISTS(SELECT * FROM FCXPI  WHERE CNSFCXP=@CNSFCXP)
      BEGIN
         SELECT 'OK'OK,0 AS IMPUESTOS
         RETURN
      END
      ELSE
      BEGIN
      ...
      END
   END  
   IF @METODO='TRAER_VALORES_ANTI'     
   BEGIN         
      SELECT @CNSFCXP=CNSFCXP,@CNSFCOM=CNSFCOM,@PROCESO=PROCESO,@CNSANT=CNSANT   
      FROM   OPENJSON (@PARAMETROS)
      WITH (           
      CNSFCXP  VARCHAR(20)   '$.CNSFCXP',
      CNSFCOM  VARCHAR(20)   '$.CNSFCOM',
      PROCESO  VARCHAR(20)   '$.PROCESO',
      CNSANT  VARCHAR(20)    '$.CNSANT'
      )
       IF (SELECT COUNT(*) FROM FCXPP INNER JOIN FCXP ON FCXP.CNSFCXP=FCXPP.CNSFCXP WHERE CNSFCOM=@CNSFCOM AND FCXP.ESTADO<>'A'
           AND EXISTS(SELECT * FROM FCJ WHERE FCXPP.CODCAJA=FCJ.CODCAJA AND FCXPP.CNSFACJ=FCJ.CNSFACJ AND CERRADA=0) ) >0
       BEGIN 
          SELECT 'KO'OK,'Existen anticipos con ordenes de pago sin confirmar en caja. Verique e intente de nuevo'ERROR
          RETURN 
       END
       IF @PROCESO='Nuevo'
       BEGIN
          SELECT @VLR_LEGALIZADO=SUM(VALOR_LEG)  FROM ANT  WHERE CNSFCOM = @CNSFCOM AND ESTADO<>'A'
       END
       ELSE
       BEGIN
          SELECT @VLR_LEGALIZADO=SUM(VALOR_LEG)  FROM ANT  WHERE CNSFCOM = @CNSFCOM AND ESTADO<>'A' AND CNSANT<>@CNSANT
       END
       SELECT @VLR_ANTICIPOS=SUM(VALOR) FROM FCXP WHERE PROCEDENCIA = 'Anticipos' AND ESTADO<>'A' AND CNSFCOM = @CNSFCOM

       SELECT 'OK'OK,COALESCE(@VLR_LEGALIZADO,0) AS VLR_LEGALIZADO, COALESCE(@VLR_ANTICIPOS,0) AS VLR_ANTICIPO, (COALESCE(@VLR_ANTICIPOS,0)-COALESCE(@VLR_LEGALIZADO,0)) AS SALDO
       RETURN 

   END  
   IF @METODO='APLICAR_ANTICIPO'     
   BEGIN         
      SELECT @DATOS=DATOS        
      FROM   OPENJSON (@PARAMETROS)
      WITH (           
      ...
      SELECT @CNSFCXP=CNSFCXP,@VLR_LEGALIZADO=VALOR_LEG,@FECHA=FECHA_LEG,@OBSERVACION=DESCRIPCION ,@CNSFCOM=CNSFCOM,@CRUCETER=CRUCETER
      FROM   OPENJSON (@DATOS)
      WITH   (   
      CNSFCXP  VARCHAR(20)      '$.CNSFCXP',
      CNSFCOM  VARCHAR(20)   '$.CNSFCOM',
      CRUCETER  BIT   '$.CRUCETER',
      VALOR_LEG  DECIMAL(14,2)  '$.VALOR_LEG',
      FECHA_LEG  DATE           '$.FECHA_LEG',
      DESCRIPCION  VARCHAR(520) '$.DESCRIPCION'      
      )   
      IF EXISTS(SELECT * FROM FCXP WHERE CNSFCXP=@CNSFCXP AND ESTADO<>'A' AND SALDO>=@VLR_LEGALIZADO)
      BEGIN
         BEGIN TRY
            PRINT 'AQUI LLAMO A GENCONSECUTIVO @IDSEDE='+COALESCE(@IDSEDE,'SIN SEDE')  +COALESCE(@COMPANIA,'CIA') 
            SELECT @CNSANT=''
		      EXEC SPK_GENCONSECUTIVO @COMPANIA,@IDSEDE, '@NAT', @CNSANT OUTPUT  
            PRINT '@CNSANT='+COALESCE(@CNSANT,'')   
		      SELECT @CNSANT = @IDSEDE + REPLACE(SPACE(8 - LEN(@CNSANT))+LTRIM(RTRIM(@CNSANT)),SPACE(1),0)
		      PRINT '@CNSANT='+COALESCE(@CNSANT,'')                    
         END TRY
         BEGIN CATCH
            INSERT INTO @TBLERRORES(ERROR) SELECT ERROR_MESSAGE()
            ...
            RETURN
         END CATCH  
         INSERT INTO ANT(CNSANT, CNSFCOM, DESCRIPCION, FECHA, VALORCXP, VALOR_LEG, IDRESPONSABLE, CCOSTO, IDAREA, NOREFERENCIA, F_FACTURAREF, F_VENCE, USUARIO, ESTADO, CNSFCXP, CONTABILIZADO, NROCOMPROBANTE,  ENPPTO, CNSPPTO, CNSCDP, CNSRP, CRUCETER)
         SELECT @CNSANT, @CNSFCOM, @OBSERVACION, @FECHA, @VLR_LEGALIZADO, @VLR_LEGALIZADO, @USUARIO, '', '', NOREFERENCIA, F_FACTURAREF, F_VENCE, @USUARIO, 'I', CNSFCXP, 0, NULL,  0, NULL, NULL, NULL, @CRUCETER
         FROM FCXP 
         WHERE CNSFCXP=@CNSFCXP
      END
      ELSE
      BEGIN
         INSERT INTO @TBLERRORES(ERROR)
         SELECT 'No se Encontro la cuenta por pagar o no tiene saldo '
      END
      IF(SELECT COUNT(*) FROM @TBLERRORES)>0
      BEGIN
         SELECT 'KO' OK, ERROR FROM @TBLERRORES
      ...
      END
      BEGIN TRY           
          EXEC SPK_ANTICIPO_LEG_CXP @CNSFCOM, @CNSANT, @COMPANIA,@USUARIO,@IDSEDE                
      END TRY
      BEGIN CATCH
              INSERT INTO @TBLERRORES(ERROR) SELECT ERROR_MESSAGE()
              RETURN
      ...
      SELECT 'OK' OK, CONTABILIZADO,NROCOMPROBANTE FROM ANT WHERE CNSANT=@CNSANT
      RETURN 
   END  
   IF @METODO='ANULAR_ANTICIPO'     
   BEGIN         
      SELECT @CNSANT=CNSANT        
      FROM   OPENJSON (@PARAMETROS)
      WITH (           
      CNSANT  VARCHAR(20)   '$.CNSANT'
      )
      IF EXISTS(SELECT * FROM ANT WHERE CNSANT=@CNSANT AND ESTADO='L')
      BEGIN
         SELECT @NROCOMPROBANTE=NROCOMPROBANTE,@CONTABILIZADA=CONTABILIZADO  FROM ANT WHERE CNSANT=@CNSANT
         IF @CONTABILIZADA <>1
         BEGIN
            DELETE MCHE WHERE NROCOMPROBANTE=@NROCOMPROBANTE
            DELETE MCPE WHERE NROCOMPROBANTE=@NROCOMPROBANTE

            UPDATE ANT SET ESTADO='A' WHERE CNSANT=@CNSANT
         END
         ELSE
         BEGIN
            IF EXISTS(SELECT * FROM MCP INNER JOIN PRI ON MCP.ANOMES=PRI.ANOMES  WHERE MCP.NROCOMPROBANTE=@NROCOMPROBANTE AND PRI.CERRADO=0 AND COALESCE(MCP.ANULADO,0)<>1)
            BEGIN
               UPDATE MCP SET ESTADO=1,ANULADO=1 WHERE NROCOMPROBANTE=@NROCOMPROBANTE
               UPDATE ANT SET ESTADO='A' WHERE CNSANT=@CNSANT
            END
            ELSE
            BEGIN
               INSERT INTO @TBLERRORES(ERROR)
               SELECT 'El Periodo contable de la legalización de este Anticipo se encuentra cerrado, no se permiten Cambios'
            END
         END
      END
      ELSE
      BEGIN
         INSERT INTO @TBLERRORES(ERROR)
         SELECT 'Este Anticipo no se encuentra Legalizado, no se puede continuar. Verifique e intente de nuevo' 
      END
      IF(SELECT COUNT(*) FROM @TBLERRORES)>0
      BEGIN
         SELECT 'KO' OK, ERROR FROM @TBLERRORES
         RETURN
      END
      SELECT @CNSFCXP=CNSFCXP FROM ANT WHERE CNSANT=@CNSANT
      EXEC SPK_CALCULOVALORESCXP @CNSFCXP
      SELECT 'OK' OK
      RETURN 
   END  
