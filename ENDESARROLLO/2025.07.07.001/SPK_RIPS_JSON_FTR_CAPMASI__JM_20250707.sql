...
BEGIN
   ...
   END
	...
	BEGIN 
			...
			causaMotivoAtencion='38',CIT.VALORTOTAL,IIF(COALESCE(CIT.VALORCOPAGO,0)>0,IIF(COALESCE(AFI.TIPOUSUARIO,'')IN('01','02','03'),@conceptoRecaudo,IIF(COALESCE(AFI.TIPOUSUARIO,'')='04','01',@conceptoRecaudo)),'05'),COALESCE(CIT.VALORCOPAGO,0), MED.TIPO_ID, MED.IDMEDICO,
	...
	BEGIN
		...
		causaMotivoAtencion='38',AUTD.VALOR,IIF(COALESCE(AUTD.VALORCOPAGO,0)>0,@conceptoRecaudo,'05'),COALESCE(AUTD.VALORCOPAGO,0), COALESCE(MED.TIPO_ID, @TIPODOC,''), COALESCE(MED.IDMEDICO, @DOCIDAFILIADO,''),
	...
	BEGIN 
		...
		SELECT @IDPRESTADOR,CIT.IDAFILIADO,CIT.CONSECUTIVO,REPLACE(CONVERT(VARCHAR,CIT.FECHA,102),'.','-')+' '+LEFT(CONVERT(VARCHAR,CIT.FECHA,108),5),null,
				...
				CASE WHEN COALESCE(CIT.IDDX,'')='' THEN 'null' END,CASE WHEN COALESCE(CIT.IDDX,'')='' THEN 'null' END,CIT.VALORTOTAL,IIF(COALESCE(CIT.VALORCOPAGO,0)>0,@conceptoRecaudo,'05'),'04',COALESCE(CIT.VALORCOPAGO,0),@N_FACTURA
		FROM  FTRDC 
			INNER JOIN CIT ON FTRDC.NOADMISION=CIT.CONSECUTIVO
			INNER JOIN SER ON CIT.IDSERVICIO=SER.IDSERVICIO
			INNER JOIN AFI ON FTRDC.IDAFILIADO=AFI.IDAFILIADO
