...
--QUERY 2			
	...
	BEGIN
		...
		END
		IF NOT EXISTS (SELECT 1 FROM FCXPDBCR WHERE CNSFCXP = @CNSFCXP AND ITEM=@ITEM AND COALESCE(ESTADODIAN,0) IN (0,3) )
		BEGIN
		...
		BEGIN
			BEGIN TRY
                UPDATE FCXPDBCR SET ESTADODIAN=1 WHERE CNSFCXP=@CNSFCXP AND ITEM=@ITEM
			END TRY
			BEGIN CATCH 
				INSERT INTO @TBLERRORES(ERROR) SELECT ERROR_MESSAGE() ;  
			END CATCH  
	...
	BEGIN
		...
		BEGIN
			BEGIN TRY
				...
				FROM FCXPDNOT WHERE FCXPDNOT.CNSFCXP = @CNSFCXP AND ITEM_DBCR = @ITEM
