...
DECLARE  @PARAMETROS   NVARCHAR(MAX)                  ,@MODELO           VARCHAR(100)                  ,@METODO           VARCHAR(100)  
        ...
        ,@CNSPPTO VARCHAR(20)                         ,@CNSCDP           VARCHAR(20)                   ,@NROCOMPROBANTE   VARCHAR(50)
		  ,@ESTADO_FCXP VARCHAR(20)                     ,@USUARIO          VARCHAR(20)    
...
BEGIN 
	...
	BEGIN
			...
			BEGIN
				...
				BEGIN
					...
					BEGIN
						...
						SELECT @VLR_ABONO = (SELECT COALESCE(SUM(ABONO),0) FROM FCXPP WHERE ESTADO<>'A' AND CNSFCXP=@CNSFCXP )
		...
		BEGIN
			BEGIN TRY
				...
				SELECT @CNSFCXPP, @CNSFCXP, @CNSFCXPPM, @VALOR, @VALOR-@VLR_ABONO, @SALDOANTE, (@VALOR-@VLR_ABONO)- @SALDOANTE,@IDTERCERO, DBO.FNK_GETDATE(), @USUARIO,NULL , NULL
		...
		BEGIN
			BEGIN TRY
				...
				BEGIN
					...
					, FCXPPM.OBSERVACION = #dataFCXPPM_Sel.OBSERVACION,  FCXPPM.IDTERCERO_AFAVOR = #dataFCXPPM_Sel.IDTERCERO_AFAVOR
					FROM FCXPPM, #dataFCXPPM_Sel WHERE FCXPPM.CNSFCXPPM = #dataFCXPPM_Sel.CNSFCXPPM
				END
			END TRY  
			BEGIN CATCH 
   ...
   BEGIN         
      ...
      BEGIN
         ...
         RETURN
      END
      SELECT  COUNT(*) FROM RPDX WHERE CNS=@CNSRPDX
      SELECT 'OK' OK
      RETURN 
   END  
END
