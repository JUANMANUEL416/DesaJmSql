...
DECLARE @XML AS NVARCHAR(MAX)               ,@NIT_ADQUIRIENTE VARCHAR(20)=''         ,@CNSFCT VARCHAR(20)                               ,@N_FACTURA VARCHAR(20)
,@NOREFERENCIA VARCHAR(20)                  ,@VR_FACTURA DECIMAL(14,2)               ,@VR_BASEIMPONIBLE DECIMAL(14,2)                   ,@VR_IVA DECIMAL(14,2)=0
,@FECHA_FACT DATE=GETDATE()                 ,@FECHA_FACT_VENCE DATE                  ,@F_NOTA DATETIME                                  ,@HORA_FACT TIME=GETDATE()
,@TIPOVENTA INT                             ,@CNSRESOL VARCHAR(20)                   ,@VENCIDA INT=0                                    ,@FTRELECTRONICA INT=0
,@MENSAJE_ERROR VARCHAR(MAX)                ,@TIPO_DOCUMENTO VARCHAR(2)              ,@TIPO_OPERACION VARCHAR(2)                        ,@CANT_TOTAL_ITEMS INT  
,@NOMBRE_CONTACTO VARCHAR(1000)             ,@TELEFONO_CONTACTO VARCHAR(1000)        ,@EMAIL_CONTACTO VARCHAR(1000)                     ,@FAX_CONTACTO VARCHAR(1000) 
,@FINIRESOL DATE                            ,@FFINRESOL DATE                         ,@PREFIJO VARCHAR(10)                              ,@CNSINICIAL VARCHAR(20) 
,@CNSFINAL VARCHAR(20)                      ,@CNSACTUAL VARCHAR(20)                  ,@CLAVE_TECNICA VARCHAR(100)                       ,@TIPO_PERSONA CHAR(1) 
,@NITFACTU VARCHAR(20)                      ,@DV_FACTURADOR VARCHAR(1)               ,@RSOCIALFACTU VARCHAR(1000)                       ,@DIRFACTU VARCHAR(1000) 
,@NOMCIUFACTU VARCHAR(100)                  ,@CODPOSTALFACTU VARCHAR(20)             ,@CIUFACTU VARCHAR(100)                            ,@CODDEPTOFACTU VARCHAR(2) 
,@NOMDEPFACTU VARCHAR(100)                  ,@CODDEPTOADQUI VARCHAR(2)               ,@COD_RESPONSABILIDAD_FISCAL VARCHAR(1000)         ,@IDCONCEPTO VARCHAR(10)
,@schemeName_FACTURADOR VARCHAR(2)          ,@TIPO_PERSONA_ADQUIRIENTE INT           ,@DV_ADQUIRIENTE VARCHAR(1)                        ,@RAZONSOCIAL_ADQUIRIENTE VARCHAR(256)
,@CIUDAD_ADQUIRIENTE VARCHAR(100)           ,@CODPOSTALADQUI VARCHAR(20)             ,@NOMCIUADQUI VARCHAR(100)                         ,@DIRADQUI VARCHAR(512)
,@CODRESPFISCALADQ VARCHAR(10)='O-99'       ,@TELCONTACTOADQUI VARCHAR(100)          ,@FAXCONTACTOADQUI VARCHAR(100)                    ,@EMAIL_ADQUIRIENTE VARCHAR(200)
,@NOMCONTACTOADQUI VARCHAR(256)             ,@schemeName_ADQUIRIENTE VARCHAR(2)      ,@IDTERCEROADQUI VARCHAR(20)                       ,@IDPLAN VARCHAR(6)
-- DATOS DEL SOFTWARE,
,@SoftwareID VARCHAR(100)                   ,@SoftwarePIN VARCHAR(100)               ,@SoftwareSecurityCode NVARCHAR(1000) 
-- DETALLES DE LA FACTURA
,@CANT_LINEAS INT                           ,@ROW_ID INT=0                           ,@N_CUOTA INT                                      ,@ANEXO VARCHAR(1024)
,@CANTIDAD DECIMAL(14,2)                    ,@VALOR DECIMAL(14,2)                    ,@VLR_SERVICI DECIMAL(14,2)                        ,@REFERENCIA VARCHAR(20)
,@IDIMPUESTO VARCHAR(10)                    ,@IDCLASE VARCHAR(10)                    ,@PIVA DECIMAL(14,2)                               ,@VIVA DECIMAL(14,2)
,@FACTURA VARCHAR(20)                       ,@NIT_DIAN VARCHAR(20)='800197268'       ,@CUFE VARCHAR(1000)                               ,@CUDE VARCHAR(1000)
,@QR_CODE NVARCHAR(MAX)                     ,@CANTIARCHENV INT=0                     ,@PREFIJO_USCXS VARCHAR(20)                        ,@OBSERVACION_FNOT NVARCHAR(1000)
,@VR_NOTA DECIMAL(14,2)                     ,@VALORCOPAGO DECIMAL(14,2)              ,@VALORMODERADORA DECIMAL(14,2)                    ,@IDMONEDA VARCHAR(3)
,@IDMONEDA_DESTINO VARCHAR(3)               ,@IDSGSSS VARCHAR(20)                    ,@IDSEDE VARCHAR(5)                                ,@IDSEDEPPAL VARCHAR(5)
,@IPS BIT                                   ,@COPAGOIPS BIT=0                        ,@PROCEDENCIA VARCHAR(20)                          ,@IDAFILIADO VARCHAR(20)
,@TIPOVOLUNTARIO VARCHAR(50)                ,@NOAUTORIZACION VARCHAR(50)             ,@NOMBRE_USUARIO VARCHAR(40)                       ,@NROCONTRATO VARCHAR(20)
,@AUTORIZACIONES_MIPRES VARCHAR(20)         ,@CAPITADA BIT=0                         ,@TIPOCAP SMALLINT=0                               ,@FDESDE_FACT DATE
,@FHASTA_FACT DATE                          ,@NOPOLIZA VARCHAR(20)                   ,@MODALIDAD_CNT VARCHAR(2)                         ,@DESC_MODALIDAD_CNT VARCHAR(100)
,@NOMBREUSUARIO VARCHAR(40)                 ,@IDTERCERO_OFE VARCHAR(20)              ,@CalculationRate DECIMAL(14,2)                    ,@ENDDATE DATETIME
,@STARTDATE DATETIME                        ,@TIPOFAC VARCHAR(2)                     ,@TERIPS VARCHAR(20)                               ,@NOTAS VARCHAR(2000)
,@VLPQFMAS DECIMAL(14,2)                    ,@TIPONOTA VARCHAR(2)                    ,@COD_ANTICIPO VARCHAR(20)                         ,@NOM_ANTICIPO VARCHAR(100)
,@DESCUENTO DECIMAL(14,2)                   ,@CODHABILITA VARCHAR(20)                ,@DESC_TIPOUSUARIO VARCHAR(100)                    ,@CobertuPlanBeneficio VARCHAR(200)
,@CODSER VARCHAR(3)                         ,@COPAPROPIO BIT                         ,@PORCENTAJE_DESCUENTO DECIMAL(14, 10) = 0 
-- Porcentaje descuento global para aplicar en cada linea con más decimales para mayor precisión
...
BEGIN
	...
	BEGIN -- Facturador Electrónico OFE
		...
		SELECT @TIPO_PERSONA=CASE WHEN NATJURIDICA='Juridica' THEN '1' ELSE '2' END 
			,@NITFACTU=LTRIM(RTRIM(NIT)) ,@DV_FACTURADOR=DV  ,@RSOCIALFACTU=dbo.FNK_LIMPIATEXTO(RAZONSOCIAL,'0-9 A-Z().;:,'),@DIRFACTU=dbo.FNK_LIMPIATEXTO(TER.DIRECCION, '0-9 A-Z().;:,') --='CARRERA 19 No 14-47'
			,@CIUFACTU=TER.CIUDAD ,@NOMCIUFACTU=CIU.NOMBRE ,@NOMDEPFACTU=DEP.NOMBRE	,@CODPOSTALFACTU=CIU.CODPOSTAL,@COD_RESPONSABILIDAD_FISCAL=TER.COD_RESP_FISCAL
			,@CODDEPTOFACTU=DEP.COD_DIAN,@schemeName_FACTURADOR=(SELECT TOP 1 DATO1 FROM TGEN WHERE TABLA='GENERAL' AND CAMPO='TIPOIDENT' AND CODIGO=TER.TIPO_ID)
	...
	BEGIN
		...
		SELECT TOP 1 @CNSFCT=CNSFCT, @N_FACTURA=N_FACTURA/*dbo.FNK_LIMPIATEXTO(N_FACTURA,'A-Z0-9_-')*/
			, @VR_FACTURA=CAST(VALORSERVICIOS AS DECIMAL(14,2)), @VR_IVA=CAST(COALESCE(VIVA,0) AS DECIMAL(14,2)), @NOREFERENCIA=NOREFERENCIA, @FECHA_FACT= F_FACTURA 
			, @FECHA_TRM = F_FACTURA, @FECHA_FACT_VENCE=F_VENCE, @HORA_FACT=F_FACTURA, @TIPOVENTA=CASE WHEN TIPOVENTA='Credito' OR FTR.IDPLAN NOT IN (SELECT DATO FROM USVGS WHERE IDVARIABLE LIKE 'IDPLANPART%') THEN 2 ELSE 1 END
			, @CNSRESOL=CNSRESOL, @TIPO_DOCUMENTO=CASE WHEN COALESCE(TIPO_DOCUMENTO, '')='' THEN '01' ELSE TIPO_DOCUMENTO END	, @TIPO_OPERACION=CASE WHEN COALESCE(TIPO_OPERACION, '')='' THEN '10' ELSE TIPO_OPERACION END
          ...
          ,@VALORMODERADORA=COALESCE(VALORMODERADORA,0) ,@COPAPROPIO =COALESCE(COPAPROPIO,0), @IDMONEDA = 'COP', @IDMONEDA_DESTINO=CASE WHEN COALESCE(MONEDA,'01')='01' THEN 'COP' ELSE MONEDA END
			, @IDSEDE=CASE WHEN COALESCE(FTR.IDSEDE,'') IN('Todas','') THEN dbo.FNK_VALORVARIABLE('IDSEDEPRINCIPAL') ELSE FTR.IDSEDE END --STORRES
			, @IDAFILIADO=IDAFILIADO, @PROCEDENCIA=PROCEDENCIA, @IDPLAN=IDPLAN, @NOMBRE_USUARIO=DBO.FNK_LIMPIATEXTO(USUSU.NOMBRE,'A-Z '), @CAPITADA=COALESCE(FTR.CAPITADA, 0)
         , @TIPOCAP=COALESCE(FTR.TIPOCAP,0), @FDESDE_FACT=FECHACAP_INI, @FHASTA_FACT=FECHACAP_FIN, @TIPOFAC=FTR.TIPOFAC, @NOTAS=OBSERVACION, @NOMBREUSUARIO = USUSU.NOMBRE
		...
		BEGIN
			...
			SELECT @NROCONTRATO=IIF(COALESCE(CNT.NOPOLIZA,'')<>'',CNT.NOPOLIZA,IIF(COALESCE(CNT.IDALTERNA,'')<>'',CNT.IDALTERNA,COALESCE(PPT.NROCONTRATO,'')))
				,   @MODALIDAD_CNT= CASE WHEN COALESCE(MODALIDAD, '') IN ('01','02','03','04')  THEN MODALIDAD
                                     ELSE  CASE COALESCE(CAPITADO, 0) 
						                          WHEN 1  THEN CASE WHEN '0' <> 4 THEN  '03' ELSE '04' END -- Pago po capitacion // TIPOCAP 4 -> oFTALMO PIÑERES
						                          WHEN 0 THEN
			...
			SELECT @NROCONTRATO = dbo.FNK_LIMPIATEXTO(@NROCONTRATO,'0-9 A-Z().;:,-/_'),@MODALIDAD_CNT = dbo.FNK_LIMPIATEXTO(@MODALIDAD_CNT,'0-9 A-Z().;:,-'),@CODSER = dbo.FNK_LIMPIATEXTO(@CODSER,'0-9 A-Z().;:,-')
		...
		BEGIN 
			...
			SELECT @DOCIDAFILIADO=DBO.FNK_LIMPIATEXTO(DOCIDAFILIADO,'0-9A-Z')	,@PAPELLIDO=DBO.FNK_LIMPIATEXTO(PAPELLIDO,'A-Z ')
				,@SAPELLIDO=DBO.FNK_LIMPIATEXTO(SAPELLIDO,'A-Z ')	,@PNOMBRE	= DBO.FNK_LIMPIATEXTO(PNOMBRE,'A-Z ')
				,@SNOMBRE	= DBO.FNK_LIMPIATEXTO(SNOMBRE,'A-Z ')	,@TIPOUSUARIO = COALESCE(TIPOUSUARIO, '05')
               ...
               BEGIN
                  ...
                  BEGIN -- FEV-RIPS Resol. 1036 de 2022 Las consultas en facturas de procedencia asistencial (archivo AC) 
                     ...
                     BEGIN
                        IF EXISTS(SELECT * FROM PLN WHERE IDPLAN=@IDPLAN AND TIPOSISTEMA<>'Prepagado') 
                        BEGIN                          
                           SELECT @VLR_COPAGO=SUM(X.VLR_COPA),@VALORMODERADORA=SUM(VLR_MODE)
                           FROM (
                              SELECT FTR.N_FACTURA,  VLR_MODE = CASE WHEN EXISTS(SELECT *  FROM MOCCD WHERE TIPODEPAGO='Moderadora' AND FTRD.VLR_COPAGOS=MOCCD.VALOR) THEN FTRD.VLR_COPAGOS ELSE 0 END , 
                                       VLR_COPA = CASE WHEN NOT EXISTS(SELECT *  FROM MOCCD WHERE TIPODEPAGO='Moderadora' AND FTRD.VLR_COPAGOS=MOCCD.VALOR)  THEN FTRD.VLR_COPAGOS ELSE 0 END
                              FROM FTRD INNER JOIN FTR ON FTR.CNSFCT=CNSFTR
                                 INNER JOIN SER ON FTRD.REFERENCIA=SER.IDSERVICIO
                                 INNER JOIN RIPS_CP ON SER.CODIGORIPS=RIPS_CP.IDCONCEPTORIPS
                              WHERE FTR.N_FACTURA=@N_FACTURA 
                              AND RIPS_CP.ARCHIVO<>'AC'
                              GROUP BY FTR.N_FACTURA,FTRD.VLR_COPAGOS
                           )X
                        IF EXISTS(SELECT * FROM PLN WHERE IDPLAN=@IDPLAN AND TIPOSISTEMA='Subsidiado') 
                        BEGIN
                           SELECT @VLR_COPAGO=COALESCE(@VLR_COPAGO,0)+COALESCE(@VALORMODERADORA,0)
                        ...
                        BEGIN
				               SET @XML+=@E+@T+@T+@T+@T+@T+@T+@T+@T+'<AdditionalInformation>' 
					            SET @XML+=@E+@T+@T+@T+@T+@T+@T+@T+@T+@T+'<Name>COPAGO</Name>' 
					            SET @XML+=@E+@T+@T+@T+@T+@T+@T+@T+@T+@T+'<Value>'+CAST(COALESCE(@VLR_COPAGO,0) AS VARCHAR)+'</Value>' 
					            SET @XML+=@E+@T+@T+@T+@T+@T+@T+@T+@T+'</AdditionalInformation>' 
                        END
                     ...
                     BEGIN
                        ...
                        BEGIN
				               SET @XML+=@E+@T+@T+@T+@T+@T+@T+@T+@T+'<AdditionalInformation>' 
					            SET @XML+=@E+@T+@T+@T+@T+@T+@T+@T+@T+@T+'<Name>COPAGO</Name>' 
					            SET @XML+=@E+@T+@T+@T+@T+@T+@T+@T+@T+@T+'<Value>'+CAST(COALESCE(@VALORCOPAGO,0) AS VARCHAR)+'</Value>' 
					            SET @XML+=@E+@T+@T+@T+@T+@T+@T+@T+@T+'</AdditionalInformation>' 
                        END
                        IF COALESCE(@VALORMODERADORA,0)>0
                        BEGIN
...
--Query2
		...
		BEGIN
			...
			--CFR Costo y flete 	--CIF Costo, flete y seguro	--CIP Transporte y Seguro Pagados hasta --CPT Transporte Pagado Hasta--DAP Entregado en un Lugar
			--DAT Entregado en Terminal--DDP Entregado con Pago de Derechos	--EXW En Fábrica	--FAS Franco al costado del buque--FCA Franco transportista	--FOB Franco a bordo
		...
		BEGIN -- Legalizaciones de depositos
         ...
         IF EXISTS(SELECT * FROM PLN WHERE IDPLAN=@IDPLAN AND TIPOSISTEMA='Prepagado')
         BEGIN
            SELECT  @VALORCOPAGO = SUM(VLR_COPAGOS) FROM FTRD INNER JOIN FTR ON FTR.CNSFCT=CNSFTR
               INNER JOIN SER ON FTRD.REFERENCIA=SER.IDSERVICIO
               INNER JOIN RIPS_CP ON SER.CODIGORIPS=RIPS_CP.IDCONCEPTORIPS
            WHERE FTR.N_FACTURA=@N_FACTURA 
				SET @COD_ANTICIPO = '03'
         ...
         BEGIN
            ...
            BEGIN
               ...
               BEGIN -- FEV-RIPS Resol. 1036 de 2022 Las consultas en facturas de procedencia asistencial (archivo AC) 
                  ...
                  WHERE FTR.N_FACTURA=@N_FACTURA 
                  AND RIPS_CP.ARCHIVO='AC' 
                  IF @VLR_COPAGO IS NULL SET @VLR_COPAGO =0
                  ...
                  BEGIN
                     ...
                     SET @XML+=@E+@T+@T+'<cbc:ID schemeID="' + @COD_ANTICIPO + '">'+@NOM_ANTICIPO+'</cbc:ID>'
				         SET @XML+=@E+@T+@T+'<cbc:PaidAmount currencyID="'+COALESCE(@IDMONEDA,'COP')+'">'+CAST(@VLR_COPAGO AS VARCHAR)+'</cbc:PaidAmount>'
				         SET @XML+=@E+@T+@T+'<cbc:ReceivedDate>'+REPLACE(CONVERT(VARCHAR,@FECHA_FACT,102),'.','-')+'</cbc:ReceivedDate>'
				         SET @XML+=@E+@T+@T+'<cbc:PaidDate>'+REPLACE(CONVERT(VARCHAR,@FECHA_FACT,102),'.','-')+'</cbc:PaidDate>'
				         SET @XML+=@E+@T+@T+'<cbc:PaidTime>'+CONVERT(VARCHAR,@HORA_FACT,108)+'-05:00</cbc:PaidTime>'
               ...
               BEGIN
                  ...
                  IF @VLR_COPAGO IS NULL SET @VLR_COPAGO =0
                  IF @VALORMODERADORA IS NULL  SET @VALORMODERADORA=0
                  IF EXISTS(SELECT * FROM PLN WHERE IDPLAN=@IDPLAN AND TIPOSISTEMA='Subsidiado')
                  BEGIN
                     SELECT @VLR_COPAGO=COALESCE(@VLR_COPAGO,0)+COALESCE(@VALORMODERADORA,0)
                     SELECT @VALORMODERADORA=0
                  ...
                  BEGIN
				         ...
				         SET @XML+=@E+@T+@T+'<cbc:InstructionID>Prepago recibido</cbc:InstructionID>'
				         SET @XML+=@E+@T+'</cac:PrepaidPayment>'			   
                     SET @VALORCOPAGO =COALESCE(@VALORCOPAGO,0)+@VLR_COPAGO
                  END
                  IF COALESCE(@VALORMODERADORA,0)>0
                  BEGIN
                     SELECT  @COD_ANTICIPO='02',@NOM_ANTICIPO='CUOTA MODERADORA'
                     ...
                     SET @XML+=@E+@T+@T+'<cbc:ID schemeID="' + @COD_ANTICIPO + '">'+@NOM_ANTICIPO+'</cbc:ID>'
				         ...
				         SET @XML+=@E+@T+@T+'<cbc:PaidDate>'+REPLACE(CONVERT(VARCHAR,@FECHA_FACT,102),'.','-')+'</cbc:PaidDate>'
				         SET @XML+=@E+@T+@T+'<cbc:PaidTime>'+CONVERT(VARCHAR,@HORA_FACT,108)+'-05:00</cbc:PaidTime>'
				         SET @XML+=@E+@T+@T+'<cbc:InstructionID>Cuota Moderadoras 3</cbc:InstructionID>'
                     SET @XML+=@E+@T+'</cac:PrepaidPayment>'
			            SET @VALORCOPAGO =COALESCE(@VALORCOPAGO,0)+@VALORMODERADORA
                  END
               END
            END
            ELSE
               ...
               BEGIN
                  ...
                  BEGIN
                     SET @XML+=@E+@T+'<cac:PrepaidPayment>' -- Anticipos a la factura / Copagos
				         IF COALESCE(@COD_ANTICIPO, '') <> '' SET @XML+=@E+@T+@T+'<cbc:ID schemeID="' + @COD_ANTICIPO + '">'+@NOM_ANTICIPO+'</cbc:ID>'
				         ELSE SET @XML+=@E+@T+@T+'<cbc:ID>ANTICIPOS</cbc:ID>'
               ...
               BEGIN
                  ...
                  BEGIN
                     ...
                     SET @XML+=@E+@T+@T+'<cbc:ID schemeID="' + @COD_ANTICIPO + '">'+@NOM_ANTICIPO+'</cbc:ID>'
				         ...
				         SET @XML+=@E+@T+@T+'<cbc:PaidDate>'+REPLACE(CONVERT(VARCHAR,@FECHA_FACT,102),'.','-')+'</cbc:PaidDate>'
				         SET @XML+=@E+@T+@T+'<cbc:PaidTime>'+CONVERT(VARCHAR,@HORA_FACT,108)+'-05:00</cbc:PaidTime>'
				         SET @XML+=@E+@T+@T+'<cbc:InstructionID>Cuota Moderadoras 5</cbc:InstructionID>'
                     SET @XML+=@E+@T+'</cac:PrepaidPayment>'
      ...
      END     
		...
		BEGIN
			...
			-- 00	Descuento no condicionado *	-- 01	Descuento condicionado **	-- 02	Recargo no condicionado *		-- 03	Recargo condicionado **
		...
		BEGIN -- Tasa de cambio cuando es una moneda extranjera 	-- @CalculationRate -- Cuanto vale una USD en Pesos
		...
		BEGIN -- Detalles de la Factura
			SELECT	ROW_ID=ROW_NUMBER() OVER(ORDER BY N_CUOTA ASC),N_CUOTA, dbo.FNK_LIMPIATEXTO(ANEXO,'0-9 A-Z') ANEXO, FTRD.CANTIDAD,
				CASE WHEN COALESCE(@TERIPS,'')='CENDI' AND COALESCE(@VLPQFMAS,0)>0 
					THEN @VLPQFMAS ELSE VALOR END VALOR , CASE WHEN COALESCE(@TERIPS,'')='CENDI' AND COALESCE(@VLPQFMAS,0)>0 
					THEN @VLPQFMAS ELSE VLR_SERVICI END VLR_SERVICI	,IIF(@PROCEDENCIA IN ('SALUD','CE','CI') AND SER.MEDICAMENTOS=0,
					...
					,dbo.FNK_LIMPIATEXTO(REFERENCIA,'0-9 A-Z _-'))REFERENCIA ,COALESCE(FTRD.IDIMPUESTO,'') IDIMPUESTO, COALESCE(FTRD.IDCLASE,'') IDCLASE
				,CAST(COALESCE(PIVA,0) AS decimal(14,2)) PIVA ,CAST(COALESCE(VIVA,0) AS decimal(14,2)) VIVA,DESCUENTO = 0,NOPRESTACION,NOITEM	,MED=0
			INTO #FTRD 
			   ...
			   BEGIN --ACTULIZO CODIGOS
					SELECT CNSFTR,N_FACTURA,N_CUOTA
						,IIF(@PROCEDENCIA IN ('SALUD','CE','CI') AND SER.MEDICAMENTOS=0,
							...
							,dbo.FNK_LIMPIATEXTO(REFERENCIA,'0-9 A-Z _-'))REFERENCIA,NOPRESTACION,NOITEM,MED=0
                     INTO #FTRD1
					FROM FTRD WITH(NOLOCK) LEFT JOIN SER ON SER.IDSERVICIO=FTRD.REFERENCIA
			...
			BEGIN
				...
				SELECT @N_CUOTA=ROW_ID-- N_CUOTA -- Se quitó N_CUOTA porque la DIAN solo admite servicios con valores mayores a cero, krystalos guarda valores en cero en ftrd y al excluirlo se saltan los consecutivos y viola la regla del xml FAV02b
					,@ANEXO=#FTRD.ANEXO	,@CANTIDAD=#FTRD.CANTIDAD	,@VALOR=#FTRD.VALOR
					...
					,@IDIMPUESTO=#FTRD.IDIMPUESTO ,@IDCLASE=#FTRD.IDCLASE	,@PIVA=#FTRD.PIVA	,@VIVA=#FTRD.VIVA,@DESCUENTO_LINEA = #FTRD.DESCUENTO
