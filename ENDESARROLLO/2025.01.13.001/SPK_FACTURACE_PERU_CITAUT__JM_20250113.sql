CREATE OR ALTER PROCEDURE DBO.SPK_FACTURACE_PERU_CITAUT
@NOAUT			VARCHAR(16),
@NIT			VARCHAR(20),
@COMPANIA		VARCHAR(2),
@IDSEDE			VARCHAR(5),
...
BEGIN
   ...
   INSERT INTO #FTRD1(CNSFTR, FECHA, DB_CR, AREAPRESTACION, UBICACION, VR_TOTAL,
         ...
         VLRIMPUESTO, PIVA, VIVA,CUENTA_FIMPDV,PROCEDENCIA)
                  ...
                  AND 1=CASE WHEN COALESCE(AUTD.ESDELAB,0)=1 THEN CASE WHEN COALESCE(AUTD.ENLAB,0) > 0 THEN 
                        CASE WHEN EXISTS(SELECT * FROM  LING INNER JOIN LORD ON LING.NOINGRESO=LORD.NOINGRESO 
                              WHERE LING.NOPRESTACION=AUT.NOAUT AND LORD.ESTADO='R') THEN 1 ELSE 0 END ELSE 0  END ELSE 1 END
