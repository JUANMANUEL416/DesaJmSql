   ...
   END
	...
	BEGIN
		...
		SELECT @PROCESO=PROCESO,  @CNSFTR=CNSFTR,@N_FACTURA=N_FACTURA,@IDSEDEF=IDSEDEF,	@F_FACTURA=F_FACTURA,@F_VENCE=F_VENCE,	@IDTERCEROF=IDTERCERO,	@IDPLAN=IDPLAN,	
         ...
         @FTRAUTO =CASE WHEN COALESCE(FTRAUTO,'false')='false' THEN 0 ELSE 1 END,@MONEDA =MONEDA,@TIPOCAP =TIPOCAP,@PGPEVE =CONVERT(INT,PGPEVE)
		FROM   OPENJSON (@DATOS)
		WITH   (
		...
		BEGIN
         ...
         END
			...
			BEGIN TRY
				...
				@VLRCOPAPRO,@CAPIPGP,@CP_CONVENIO,@CP_MODALIDAD,@CP_MES,@FECHACAP_INI,@FECHCACA_FIN, @VALORMODERADORA,COALESCE(@SINCNT,0),COALESCE(@CUOTAS,0),
            @TIPOCAP
...
--QUERY 3
	...
	BEGIN 
		...
		SELECT @N_FACTURA=N_FACTURA, @CAPITA=COALESCE(CAPITADA,0), @TIPOFACT=TIPOFAC FROM FTR WHERE CNSFCT=@CNSFACT
		...
		BEGIN
			...
			IF @CAPITA= 1 
         BEGIN
            IF EXISTS(SELECT * FROM FTR INNER JOIN PPT ON FTR.IDTERCERO=PPT.IDTERCERO AND FTR.IDPLAN=PPT.IDPLAN AND COALESCE(PPT.PGP,0)=1)
            BEGIN
               PRINT 'VOY POR PGP'
