...
BEGIN
	...
	SELECT @CNSFCT=CNSFCT,@TIPOCAP=COALESCE(TIPOCAP,0) FROM FTR WHERE N_FACTURA=@N_FACTURA
   IF @TIPOCAP=0
   BEGIN
	   ...
	   BEGIN 
		   ...
		   BEGIN --AP
			   ...
			   SELECT @IDPRESTADOR,AUT.IDAFILIADO,AUT.NOAUT,REPLACE(CONVERT(VARCHAR, AUT.FECHA, 102), '.', '-') + ' ' + LEFT(CONVERT(VARCHAR, AUT.FECHA, 108), 5),NULL
				   ,AUT.NUMAUTORIZA,left(REPLACE(REPLACE(LTRIM(RTRIM(SER.CODCUPS)),CHAR(13),''),CHAR(10),''),6),'02','01','02',325,'16',COALESCE(MED.TIPO_ID, @TIPODOC), COALESCE(MED.IDMEDICO,@DOCIDAFILIADO)
				   ,AUT.DXPPAL,AUT.DXRELACIONADO,AUT.DXPPAL,0,IIF(COALESCE(AUT.VALORCOPAGO,0)>0,'02',@conceptoRecaudo),FTRDC.CANTIDAD,'04'
		   ...
		   FROM  FTRDC INNER JOIN HADM ON FTRDC.NOADMISION=HADM.NOADMISION
					   ...
					   LEFT JOIN MED ON MED.IDMEDICO = CASE WHEN COALESCE(HPRE.IDMEDICO,'') = '' THEN COALESCE(HADM.IDMEDICOALTA,HADM.IDMEDICOTRA) ELSE HPRE.IDMEDICO END
		   WHERE FTRDC.CNSFTR=@CNSFCT
		   AND FTRDC.PROCEDENCIA='HADM'
		   AND HPRED.N_FACTURA=@N_FACTURA
		   AND COALESCE(HPRED.VALOR,0)>0
