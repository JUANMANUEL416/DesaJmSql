...
BEGIN
	...
	BEGIN
		...
		BEGIN
			SELECT @QUERY = 
				'SELECT CEN.CCOSTO value, CONCAT(CEN.CCOSTO,'' - '',CEN.DESCRIPCION, '' -- '',CEN.IDAREA, '' - '',AFU.DESCRIPCION) label, AFU.DESCRIPCION DESCAREA, CEN.IDAREA
							FROM   CEN 
	...
	BEGIN
		...
		RETURN
	END
	IF @METODO = 'CJR' --CP 20250724
	BEGIN
		IF COALESCE(@WHERES, '') <> ''
		BEGIN
			SELECT @QUERY = 'SELECT CODCAJERO value, CONCAT(CODCAJERO,'' - '', DESCRIPCION) label FROM CJR
							WHERE 1 = 1 AND' + @WHERES
			EXEC (@QUERY)
		END
		ELSE
			SELECT CODCAJERO value, CONCAT (CODCAJERO, ' - ', DESCRIPCION) label FROM CJR
		RETURN
	END
