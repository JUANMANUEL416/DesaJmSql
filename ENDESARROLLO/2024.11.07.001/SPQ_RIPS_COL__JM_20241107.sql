   ...
   BEGIN         
      ...
      BEGIN
         ...
         BEGIN
            ...
            BEGIN
               BEGIN TRY           
                   EXEC SPK_GEN_TRAMAS_PERU @ENVIO                
               END TRY
               BEGIN CATCH
                       INSERT INTO @TBLERRORES(ERROR) SELECT ERROR_MESSAGE()
               END CATCH
   ...
   BEGIN   
     SELECT @ENVIO=ENVIO, @IDTERCERO=IDTERCERO        
      ...
      BEGIN
         ...
         SELECT @NOMBREBASE='_'+TER.IDALTERNA2+'_0'+RIPS.ENVIO+'_'+CAST(YEAR(RIPS.FECHAENVIO) AS VARCHAR(4))+CASE WHEN MONTH(RIPS.FECHAENVIO)<10 THEN '0' ELSE '' END
         ...
         SELECT 'OK' OK,@NIT+'_'+@IDALTERNA2+LTRIM(RTRIM(@NOMBREBASE)) NOMBREBASE
         SELECT TRAMATEX AS PLANO FROM TRAMA_DATE WHERE IDTRAMA=@ENVIO
         SELECT TRAMATEX AS PLANO FROM TRAMA_DFAC WHERE IDTRAMA=@ENVIO
         SELECT TRAMATEX AS PLANO FROM TRAMA_DSER WHERE IDTRAMA=@ENVIO
         SELECT TRAMATEX AS PLANO FROM TRAMA_DFAC WHERE IDTRAMA=@ENVIO
         SELECT '' AS PLANO 
         RETURN
      END
      ...
      BEGIN
         ...
         +','+COALESCE(ACTOQ,'1')+','+CASE WHEN COALESCE(@VL_ENTERO,'')='SI' THEN CAST(CONVERT(BIGINT,CONVERT(DECIMAL(14,2),VALORPROCED)) AS VARCHAR(20)) ELSE VALORPROCED END,CHAR(13),'')+CHAR(13)+CHAR(10) PLANO
