CREATE OR ALTER PROCEDURE DBO.SPK_RIPS_JSON_FTR_IND_PR
...
BEGIN
   ...
   */ --Se conoce que los partoculares SI SE REPORTAN STORRES_20250403
	...
	BEGIN
		...
		RETURN
	END
   SELECT 'AC'ARCHIVO,COALESCE(SUM(CAST(vrServicio AS DECIMAL(14,2))),0) FROM @CONSULTAS
   UNION ALL
   SELECT 'AP', COALESCE(SUM(CAST(vrServicio AS DECIMAL(14,2))),0) FROM @PROCEDIMIENTOS
      UNION ALL
   SELECT 'AM', COALESCE(SUM(CAST(vrServicio AS DECIMAL(14,2))),0) FROM @MEDICAMENTOS
      UNION ALL
   SELECT 'AT', COALESCE(SUM(CAST(vrServicio AS DECIMAL(14,2))),0) FROM @OTROSSER
	...
	BEGIN
		...
		BEGIN
			...
			SET @PLANO+= '"conceptoRecaudo":"'+ CASE WHEN TRY_CAST(@valorPagoModerador AS decimal(14,2))<=0 THEN '05'  ELSE @conceptoRecaudo /* IIF(@conceptoRecaudo='01','03',@conceptoRecaudo)*/ END +'",'
