CREATE OR ALTER PROCEDURE DBO.SPK_NC_CONTAB_FTR
...
BEGIN
	...
	SET DATEFORMAT dmy
   /*STORRES -- 20250523 -- SE AGRGA VALIDACION PARA CUANDOLA FACTURA ESTA ANULADA*/
   IF (SELECT ESTADO FROM FTR WHERE N_FACTURA = @N_FACTURA)='A'
   BEGIN
      PRINT 'FACTURA ANULADA CAMBIO ESTADO AL COMPROBANTE EN MCPE'
   ...
   BEGIN
      ...
      UPDATE FTR SET CP_VLR_SERVICIOS = V.VALORTOTAL, CP_VLR_COPAGOS   =ROUND(CASE WHEN COALESCE(COPAPROPIO,0)=1 THEN CP_VLR_COPAGOS ELSE COALESCE(V.VALORCOPAGO,0)+COALESCE(VALORMODERADORA,0) END,0), CP_VLR_PAGCOMP = 0
   ...
   BEGIN
      IF EXISTS(SELECT * FROM MCP WHERE NOREFERENCIA=@N_FACTURA AND COALESCE(ANULADO,0)<>1 
                AND COMPROBANTE =RTRIM(LTRIM(DBO.FNK_VALORVARIABLE('IDCON_TCOM_FTR'))))
      BEGIN
		   PRINT 'ME REGRESO POSIBLE DUPLICIDAD POR FAVOR REVISAR  PARO 1'  --20230215 OSOLANO
         RAISERROR('ERROR DE DUPLICIDAD EN CONTABILIDAD, VERIFIQUE ',1,16)
		   RETURN
      END
   END
   ...
   BEGIN
      ...
      FROM FTR WHERE N_FACTURA = @N_FACTURA
	   ...
	   IF EXISTS (SELECT * FROM MCP WHERE PROCEDENCIA='FACTURA' AND NOREFERENCIA=@N_FACTURA  
   ...
   ELSE
   BEGIN
      IF EXISTS(SELECT * FROM MCP WHERE NROCOMPROBANTE=@NROCOMPROBANTE AND COALESCE(ANULADO,0)<>1)
      BEGIN
         RAISERROR('COMPORBANTE YA SE ENCUENTRA EN CONTABILIDAD, VERIFIQUE ',1,16)
         RETURN
      END
   ...
   BEGIN
      ...
      -- DEDUCCIONES
      IF (@PROCEDENCIA IN ('FINANCIERO','CE','CI','ACTIVOS','CAJA')) --OR (@NOREFERENCIA='MASIVA')
      BEGIN
         ...
         END
         ELSE
            IF @PROCEDENCIA='CI' OR @PROCEDENCIA='CAJA'
            BEGIN
               PRINT @PROCEDENCIA
               IF @PROCEDENCIA='CI'
               BEGIN
               ...
               BEGIN
                  INSERT INTO #T (TIPO, CUENTA, IDAREA, CCOSTO, VLR_TOTAL, VLR_SERVICI, VLR_COPAGOS, VLR_PAGCOMP, 
                             VLR_DESCUENTO, VLR_ABONOS, IDPLAN, IDAFILIADO, NOMBREAFILIADO, VLR_IVA, VLR_IMPUESTOS, NOINGRESO, 
                             CODUNG, CODPRG)
                  SELECT 'DB', FTR.CUENTACXC,'', '', 0 AS VLR_TOTAL,SUM(FTRD.VLR_SERVICI) AS VLR_SERVICI, SUM(FTRD.VLR_COPAGOS) AS VLR_COPAGOS, 
                          SUM(FTRD.VLR_PAGCOMP) AS VLR_PAGCOMP, 0 AS VLR_DESCUENTO, 0 AS VLR_ABONOS,
                          FTR.IDPLAN,FTR.IDTERCERO,TER.RAZONSOCIAL, SUM(COALESCE(FTRD.VIVA,0)), 0, FTRD.NOADMISION,
                        COALESCE(FTRD.CODUNG,''), COALESCE(FTRD.CODPRG,'') 
                  FROM	  FTR INNER JOIN FTRD ON FTR.N_FACTURA   = FTRD.N_FACTURA 
                              LEFT JOIN TER ON FTR.IDTERCERO=TER.IDTERCERO
                  WHERE  FTR.N_FACTURA = @N_FACTURA
                  AND    COALESCE(FTRD.PAQUETE,0)=CASE WHEN COALESCE(FTR.PAQUETE,0)=0  AND FTR.PROCEDENCIA='CI' AND COALESCE(FTR.CNSFMAS,'')<>'' THEN 0 ELSE COALESCE(FTRD.PAQUETE,0)  END
                  GROUP BY FTRD.NOADMISION, FTR.CUENTACXC,COALESCE ( FTRD.CODUNG,''), COALESCE( FTRD.CODPRG,''),FTR.IDPLAN,FTR.IDTERCERO,TER.RAZONSOCIAL
               END
            END
            ELSE
            BEGIN
               ...
               BEGIN
                  ...
                  SELECT 'DB' TIPO, TTEC1.CUENTA, '', '', 0 AS VLR_TOTAL, 
                    SUM(FTRD.VLR_SERVICI) AS VLR_SERVICI, ROUND(CASE WHEN COALESCE(FTR.COPAPROPIO,0)=1 THEN CP_VLR_COPAGOS ELSE SUM(FTRD.VLR_COPAGOS)+COALESCE(FTR.VALORMODERADORA,0) END,0) VLR_COPAGOS, 
                  ...
                  GROUP BY TTEC1.CUENTA, FTRD.IDPLAN, COALESCE ( FTRD.CODUNG,''), COALESCE( FTRD.CODPRG,''),COALESCE(FTR.COPAPROPIO,0),FTR.CP_VLR_COPAGOS,
                            FTR.CP_VLR_PAGCOMP,COALESCE(FTR.VALORMODERADORA,0)

                  IF DBO.FNK_VALORVARIABLE('FACTURAPOS_ACTIVA')='SI'
                  BEGIN
                     PRINT 'RETIRO EL VALOR DEL COPAGO '
                     UPDATE #T SET VLR_SERVICI=VLR_SERVICI-VLR_COPAGOS,VLR_COPAGOS=0,@DIFECOPAGOS=0,@VALORCOPAGO =0 WHERE 1=1
                  END
               END
            END
      END
      ...
      SET @IDCATEGORIA = DBO.FNK_VALORVARIABLE('TERCATAFILIADO')      
	   ...
	   BEGIN
         PRINT 'Inserta Tercero con SPK_INSERT_TERDEAFI '+@IDAFILIADO+','+COALESCE(@IDCATEGORIA,'NO TENGO CATEGORIA')
      ...
      BEGIN 
         ...
         BEGIN
            ...
            AND  1=CASE WHEN DBO.FNK_VALORVARIABLE('FACTURAPOS_ACTIVA')='SI' THEN 0 ELSE 1 END
...
--query2
      ...
      BEGIN
         ...
         BEGIN
            ...
            PRINT 'CREDITOS MCH FACTURA JOSER'
            ...
            SELECT @MAUX, @COMPANIA, 'CR', 
                   CASE WHEN COALESCE(DBO.FNK_VALORVARIABLE('IDTERCEROINSTALADO'),'') <> ''
                        THEN
                            CASE WHEN COALESCE(FTRD.IDPROVEEDOR,'') = DBO.FNK_VALORVARIABLE('IDTERCEROINSTALADO') 
                                 ...
                                 ELSE CASE WHEN @PROCEDENCIA='CAJA' THEN (SELECT CUENTA  FROM KCDE WHERE TIPO='FTR_COPAGOS_INGRESO')
                                             ELSE DBO.FNK_NC_CUENTA_KMCOM(@IDMODELOCONT,'VENTAS','CR',FTRD.PREFIJO, FTRD.CCOSTO,'S',FTRD.REFERENCIA,@PGP) END
                            END
                   ...
                   CASE WHEN DBO.FNK_VALORVARIABLE('IXCOUNTRY') = 'PERU' OR DBO.FNK_VALORVARIABLE('FACTURAPOS_ACTIVA') = 'SI'  THEN COALESCE(FTRD.VLR_SERVICI,0) -COALESCE(FTRD.VIVA,0)-COALESCE(FTRD.VLR_COPAGOS,0)
                   ELSE
	    ...
	    -- MOD.JQUIROGA 20090620 - En Venezuela IDTERCERO es el mismo proveedor
         ...
         SELECT @MAUX, @COMPANIA, 'CR', FTRD.CUENTA_FIMPDV, 
                CASE WHEN DBO.FNK_VALORVARIABLE('IXCOUNTRY') = 'VENEZUELA' OR DBO.FNK_VALORVARIABLE('IXCOUNTRY') = 'PERU' THEN FTR.IDTERCERO ELSE DBO.FNK_VALORVARIABLE('IDCON_TERCEROIVA') END, 
      ...
      BEGIN
         ...
         BEGIN
            ...
            SELECT  @VALORFTR=CASE WHEN DBO.FNK_VALORVARIABLE('FACTURAPOS_ACTIVA')='SI' THEN VR_TOTAL ELSE VALORSERVICIOS END FROM FTR WHERE N_FACTURA=@N_FACTURA
            ...
            FROM
                  (SELECT NROCOMPROBANTE=@MAUX, COMPANIA=@COMPANIA, TIPO='CR', 
                          CUENTA=DBO.FNK_NC_CUENTA_KMCOM(@IDMODELOCONT,'VENTAS','CR',FTRDC.PREFIJO, FTRDC.CCOSTO,'S','',@PGP),
                          FTR.IDTERCERO, FTRDC.CCOSTO, DETALLE=UPPER(PRE.NOM_PREFIJO)+' FACTURA No.'+@N_FACTURA, 
                          VALOR=ABS(FTRDC.VALORTOTAL-CASE WHEN DBO.FNK_VALORVARIABLE('FACTURAPOS_ACTIVA')='SI' THEN FTRDC.VLR_COPAGO ELSE 0 END) , 
                          REFERENCIA1=FTRDC.PREFIJO, REFERENCIA2=NULL, ITEMREF=NULL, USUARIO=@USUARIO, FECHA=DBO.FNK_FECHA_SIN_MLS(GETDATE()), 
                          FTRDC.PREFIJO, FTRDC.IDAREA, IDPROVEEDOR=
                          CASE FTRDC.PROCEDENCIA 
                           WHEN 'HADM' THEN (SELECT IDPROVEEDOR FROM HPRED WHERE NOPRESTACION=FTRDC.NOPRESTACION AND NOITEM=FTRDC.NOITEM AND ISNULL(HPRED.ANULADO,0) = 0)  --SJUNCO REQ 1803
                   ...
                   FROM   FTR LEFT JOIN FTRDC ON FTR.CNSFCT    = FTRDC.CNSFTR 
                              LEFT JOIN   PRE ON FTRDC.PREFIJO = PRE.PREFIJO
                   WHERE  ABS(FTRDC.VALORTOTAL-CASE WHEN DBO.FNK_VALORVARIABLE('FACTURAPOS_ACTIVA')='SI' THEN FTRDC.VLR_COPAGO ELSE 0 END)>0
                   AND    FTR.N_FACTURA     = @N_FACTURA) X
            GROUP BY NROCOMPROBANTE, COMPANIA, TIPO, CUENTA, IDTERCERO, CCOSTO, DETALLE, 
                  REFERENCIA1 ,REFERENCIA2 ,ITEMREF, USUARIO, FECHA, PREFIJO, IDAREA, IDPROVEEDOR,
                  CLASECONT, ESTADO, PROCESO, GENERA, PROCEDENCIA, REFERENCIA_PRO, N_FACTURA, F_FACTURAREF, F_VENCE,
