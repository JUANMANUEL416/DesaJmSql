   ...
   BEGIN
	PRINT '@TIPOCAP='+CAST(COALESCE(@TIPOCAP,0) AS VARCHAR(2))
      ...
      BEGIN
         ...
         AND IDSEDE=CASE WHEN DBO.FNK_VALORVARIABLE('FACTSEDE')='SI' THEN @IDSEDEFAC ELSE FTR.IDSEDE END
      ...
      PRINT '@conceptoRecaudo='+@conceptoRecaudo
		   ...
		   SELECT DISTINCT  AFI.TIPO_DOC,AFI.DOCIDAFILIADO,AFI.IDAFILIADO,
                  CASE WHEN COALESCE(AFI.TIPOUSUARIO,'')='' THEN 
					   CASE   WHEN AFI.TIPOAFILIADO = 'C' THEN '01'
									   ...
									   WHEN AFI.TIPOAFILIADO = 'S/N' THEN '05' ELSE '05' END ELSE  COALESCE(AFI.TIPOUSUARIO,'') END ,
	   ...
	   BEGIN 
			   ...
			   causaMotivoAtencion='38',0,IIF(COALESCE(CIT.VALORCOPAGO,0)>0,@conceptoRecaudo,'05'),COALESCE(CIT.VALORCOPAGO,0), MED.TIPO_ID, MED.IDMEDICO
			   FROM FTRDC 
	   ...
	   BEGIN
		   ...
		   causaMotivoAtencion='38',0,IIF(COALESCE(AUTD.VALORCOPAGO,0)>0,@conceptoRecaudo,'05'),COALESCE(AUTD.VALORCOPAGO,0), COALESCE(MED.TIPO_ID, @TIPODOC,''), COALESCE(MED.IDMEDICO, @DOCIDAFILIADO,''),
		   ...
		   1,0,IIF(COALESCE(HPRED.VALORCOPAGO,0)>0,@conceptoRecaudo,'05'),COALESCE(HPRED.VALORCOPAGO,0), MED.TIPO_ID, MED.IDMEDICO 
		   ...
		   causaMotivoAtencion='38',COALESCE(HADM.DXINGRESO,HADM.DXEGRESO),COALESCE(HADM.DXSALIDA1,''),COALESCE(HADM.DXSALIDA2,''),COALESCE(HADM.DXSALIDA3,''),
		   1,0,IIF(COALESCE(HPRED.VALORCOPAGO,0)>0,@conceptoRecaudo,'05'),COALESCE(HPRED.VALORCOPAGO,0),CONVERT(INT,HPRED.CANTIDAD), MED.TIPO_ID, MED.IDMEDICO 
		   FROM  FTRDC INNER JOIN HADM ON FTRDC.NOADMISION=HADM.NOADMISION  
			   INNER JOIN  HPRE ON HADM.NOADMISION=HPRE.NOADMISION
				   INNER JOIN HPRED ON HPRE.NOPRESTACION=HPRED.NOPRESTACION AND HPRED.NOITEM = FTRDC.NOITEM
				   INNER JOIN SER ON HPRED.IDSERVICIO=SER.IDSERVICIO  AND FTRDC.IDSERVICIO = SER.IDSERVICIO
	   ...
	   BEGIN
		   ...
		   SELECT @IDPRESTADOR,AUT.IDAFILIADO,AUT.NOAUT,numAutorizacion=AUT.NUMAUTORIZA,null,fechaDispensAdmon=REPLACE(CONVERT(VARCHAR,AUT.FECHA,102),'.','-')+' '+LEFT(CONVERT(VARCHAR,AUT.FECHA,108),5),
			   ...
			   IIF(COALESCE(AUTD.VALORCOPAGO,0)>0,@conceptoRecaudo,'05'),'04',COALESCE(AUTD.VALORCOPAGO,0),@N_FACTURA
		   FROM FTRDC INNER JOIN AUT ON FTRDC.NOADMISION=AUT.IDAUT  
		   ...
		   SELECT @IDPRESTADOR,HADM.IDAFILIADO,HADM.NOADMISION,numAutorizacion=HADM.NOAUTORIZACION,null,fechaDispensAdmon=REPLACE(CONVERT(VARCHAR,FTRDC.FECHA,102),'.','-')+' '+LEFT(CONVERT(VARCHAR,FTRDC.FECHA,108),5),
               ...
               CONVERT(VARCHAR,CONVERT(INT,HPRED.CANTIDAD),10),1, COALESCE(MED.TIPO_ID, @TIPODOC,'CC'), COALESCE(MED.IDMEDICO,'',''),CONVERT(VARCHAR,CONVERT(DECIMAL(14,2),HPRED.VALOR),20),
			   CONVERT(VARCHAR,CONVERT(DECIMAL(14,2),HPRED.VALOR*HPRED.CANTIDAD),20),IIF(COALESCE(HPRED.VALORCOPAGO,0)>0,@conceptoRecaudo,'05'),'04',CONVERT(DECIMAL(14,2),HPRED.VALORCOPAGO),@N_FACTURA
		   FROM FTRDC INNER JOIN HADM ON FTRDC.NOADMISION=HADM.NOADMISION
			   ...
			   LEFT JOIN IFFA ON IART.IDFORFARM=IFFA.IDFORFARM
			   LEFT JOIN IUNI ON IUNI.IDUNIDAD = IART.IDUNIDAD
            LEFT JOIN MED ON MED.IDMEDICO = CASE WHEN COALESCE(HADM.IDMEDICOING,'') = '' THEN COALESCE(HADM.IDMEDICOALTA,HADM.IDMEDICOTRA,DBO.FNK_VALORVARIABLE('EPI_MEDICODEFAULT')) ELSE HADM.IDMEDICOING END
		   WHERE FTRDC.CNSFTR=@CNSFCT
		   AND FTRDC.PROCEDENCIA='HADM'
		   AND HPRED.N_FACTURA=@N_FACTURA
		   AND COALESCE(HPRED.VALOR,0)>0
	   ...
	   BEGIN 
		   ...
		   SELECT @IDPRESTADOR,CIT.IDAFILIADO,CIT.CONSECUTIVO,REPLACE(CONVERT(VARCHAR,CIT.FECHA,102),'.','-')+' '+LEFT(CONVERT(VARCHAR,CIT.FECHA,108),5),null,
				   ...
				   CASE WHEN COALESCE(CIT.IDDX,'')='' THEN 'null' END,CASE WHEN COALESCE(CIT.IDDX,'')='' THEN 'null' END,0,IIF(COALESCE(CIT.VALORCOPAGO,0)>0,@conceptoRecaudo,'05'),'04',COALESCE(CIT.VALORCOPAGO,0),@N_FACTURA
		   FROM  FTRDC 
			   INNER JOIN CIT ON FTRDC.NOADMISION=CIT.CONSECUTIVO
			   INNER JOIN SER ON CIT.IDSERVICIO=SER.IDSERVICIO
			   INNER JOIN AFI ON FTRDC.IDAFILIADO=AFI.IDAFILIADO
		   ...
		   BEGIN --AP
			   ...
			   SELECT @IDPRESTADOR,AUT.IDAFILIADO,AUT.NOAUT,REPLACE(CONVERT(VARCHAR, AUT.FECHA, 102), '.', '-') + ' ' + LEFT(CONVERT(VARCHAR, AUT.FECHA, 108), 5),NULL
				   ...
				   ,AUT.DXPPAL,IIF(COALESCE(AUT.DXRELACIONADO,'')='',AUT.DXPPAL,AUT.DXRELACIONADO),AUT.DXPPAL,0,IIF(COALESCE(AUT.VALORCOPAGO,0)>0,@conceptoRecaudo,'05'),'04'
			   ...
			   SELECT @IDPRESTADOR,AUT.IDAFILIADO,AUT.NOAUT,REPLACE(CONVERT(VARCHAR, AUT.FECHA, 102), '.', '-') + ' ' + LEFT(CONVERT(VARCHAR, AUT.FECHA, 108), 5),NULL
				   ,AUT.NUMAUTORIZA,left(REPLACE(REPLACE(LTRIM(RTRIM(SER.CODCUPS)),CHAR(13),''),CHAR(10),''),6),'02','01','02',325,'16',COALESCE(MED.TIPO_ID, 'CC'), COALESCE(MED.IDMEDICO,@DOCIDAFILIADO)
				   ,AUT.DXPPAL,IIF(COALESCE(AUT.DXRELACIONADO,'')='',AUT.DXPPAL,AUT.DXRELACIONADO),AUT.DXPPAL,0,IIF(COALESCE(AUT.VALORCOPAGO,0)>0,@conceptoRecaudo,'05'),FTRDC.CANTIDAD,'04'
				   ,CASE WHEN AUT.IDPLAN IN (DBO.FNK_VALORVARIABLE ('IDPLANPART'),DBO.FNK_VALORVARIABLE ('IDPLANPART2')    ----VGARCIA
										   ,DBO.FNK_VALORVARIABLE ('IDPLANPART3'),DBO.FNK_VALORVARIABLE ('IDPLANPART4')  ----VGARCIA
										   ,DBO.FNK_VALORVARIABLE ('IDPLANPART5'))                                       ----VGARCIA
						   THEN '0.00' 
			   ...
			   UPDATE @PROCEDIMIENTOS1 SET restoPagoModerador=TRY_CAST(valorPagoModerador AS decimal(14,2))%cantidad where 2=2
			   UPDATE @PROCEDIMIENTOS1 SET valorPagoModerador=TRY_CAST(valorPagoModerador AS decimal(14,2))-TRY_CAST(restoPagoModerador AS decimal(14,2))  where 2=2
		   ...
		   SELECT @IDPRESTADOR,HADM.IDAFILIADO,HADM.NOADMISION,REPLACE(CONVERT(VARCHAR,HPRE.FECHA,102),'.','-')+' '+LEFT(CONVERT(VARCHAR,HPRE.FECHA,108),5),null,
			   ...
			   CASE WHEN COALESCE(HADM.DXSALIDA1,'')='' THEN 'null' END,CASE WHEN COALESCE(HADM.COMPLICACION,HADM.DXINGRESO,HADM.DXEGRESO,'')='' THEN 'null' END,0,IIF(COALESCE(HPRED.VALORCOPAGO,0)>0,@conceptoRecaudo,'05'),'04',COALESCE(HPRED.VALORCOPAGO,0),@N_FACTURA
		   FROM  FTRDC INNER JOIN HADM ON FTRDC.NOADMISION=HADM.NOADMISION
					   INNER JOIN AFI  ON FTRDC.IDAFILIADO=AFI.IDAFILIADO
					   INNER JOIN HPRED ON FTRDC.NOPRESTACION=HPRED.NOPRESTACION AND FTRDC.NOITEM=HPRED.NOITEM
		   ...
		   SELECT @IDPRESTADOR,HADM.IDAFILIADO,HADM.NOADMISION,REPLACE(CONVERT(VARCHAR,HPRE.FECHA,102),'.','-')+' '+LEFT(CONVERT(VARCHAR,HPRE.FECHA,108),5),null,
			   ...
			   CASE WHEN COALESCE(HADM.DXSALIDA1,'')='' THEN 'null' END,CASE WHEN COALESCE(HADM.COMPLICACION,HADM.DXINGRESO,HADM.DXEGRESO,'')='' THEN 'null' END,0,IIF(COALESCE(HPRED.VALORCOPAGO,0)>0,@conceptoRecaudo,'05'),HPRED.CANTIDAD, '04',COALESCE(HPRED.VALORCOPAGO,0),@N_FACTURA
		   FROM  FTRDC INNER JOIN HADM ON FTRDC.NOADMISION=HADM.NOADMISION
					   INNER JOIN AFI  ON FTRDC.IDAFILIADO=AFI.IDAFILIADO
					   INNER JOIN HPRED ON FTRDC.NOPRESTACION=HPRED.NOPRESTACION AND FTRDC.NOITEM=HPRED.NOITEM
					   INNER JOIN HPRE  ON HADM.NOADMISION=HPRE.NOADMISION AND FTRDC.NOPRESTACION=HPRE.NOPRESTACION
	   ...
	   BEGIN
		   ...
		   SELECT @IDPRESTADOR,CIT.IDAFILIADO,CIT.CONSECUTIVO,numAutorizacion=CIT.NOAUTORIZACION,null,
			   ...
			   CIT.VALORCOPAGO,0)>0,@conceptoRecaudo,'05'),tipoPagoModerador='04',COALESCE(CIT.VALORCOPAGO,0),
			   @N_FACTURA
         ...
         PRINT 'TERMINO CIT'
		   ...
		   SELECT @IDPRESTADOR,AUT.IDAFILIADO,AUT.NOAUT,numAutorizacion=AUT.NUMAUTORIZA,null,
			   ...
			   IIF(COALESCE(AUT.VALORCOPAGO,0)>0,@conceptoRecaudo,'05'),
         ...
         PRINT 'TERMINO AUT'
		   ...
		   SELECT @IDPRESTADOR,HADM.IDAFILIADO,HADM.NOADMISION                                                                   
			   ...
			   ,IIF(COALESCE(HPRED.VALORCOPAGO,0)>0,@conceptoRecaudo,'05')
			   ,tipoPagoModerador='04'
         ...
         PRINT 'TERMINO HADM OTROS SER'
	   END 
      PRINT 'TERMINO LA RECOLECCION DE DATOS '+CAST(GETDATE() AS VARCHAR(20))
...
--QUERY2	
      ...
      PRINT 'TERMINO LA RECOLECCION DE DIAGNOSTICOS '+CAST(GETDATE() AS VARCHAR(20))
	   BEGIN
		   PRINT 'Diagnosticos Consultas'+CAST(GETDATE() AS VARCHAR(20))
		   UPDATE AC
		   SET 
		   ...
		   PRINT 'Diagnosticos medicamentos'+CAST(GETDATE() AS VARCHAR(20))
		   UPDATE AM
		   SET 
		   ...
		   WHERE COALESCE(DX.IDDX, '') <> '';
          PRINT 'Diagnosticos procedimientos '+CAST(GETDATE() AS VARCHAR(20))
		   PRINT 'URGENCIAS'
		   UPDATE AU
		   SET 
		   ...
		   WHERE COALESCE(DX.IDDX, '') <> '';
	   END
      PRINT 'TERMINO LA ACTUALIZAR DX  '+CAST(GETDATE() AS VARCHAR(20))
      ...
      ELSE
      BEGIN
         UPDATE @CONSULTAS SET conceptoRecaudo=@conceptoRecaudo WHERE valorPagoModerador>=0 AND conceptoRecaudo='05'
         UPDATE @PROCEDIMIENTOS SET conceptoRecaudo=@conceptoRecaudo WHERE valorPagoModerador>=0 AND conceptoRecaudo='05'
         UPDATE @MEDICAMENTOS SET conceptoRecaudo=@conceptoRecaudo WHERE valorPagoModerador>=0 AND conceptoRecaudo='05'
         UPDATE @OTROSSER SET conceptoRecaudo=@conceptoRecaudo WHERE valorPagoModerador>=0 AND conceptoRecaudo='05'
      END
      UPDATE @CONSULTAS SET vrServicio=0 WHERE 1=1
      UPDATE @PROCEDIMIENTOS SET vrServicio=0 WHERE 1=1
      UPDATE @MEDICAMENTOS SET vrServicio=0,vrUnitMedicamento=0 WHERE 1=1
      UPDATE @OTROSSER SET vrServicio=0, vrUnitOS=0 WHERE 1=1

      
    --  SELECT 'AC', SUM(valorPagoModerador) FROM @CONSULTAS
    --  UNION ALL
    --  SELECT 'AM', SUM(valorPagoModerador) FROM @MEDICAMENTOS
    --  UNION ALL
    --  SELECT 'AP', SUM(valorPagoModerador) FROM @PROCEDIMIENTOS
    --  UNION ALL
    --  SELECT 'AT', SUM(valorPagoModerador) FROM @OTROSSER
	   --PRINT 'TERMINE DE PREPARAR LOS DATOS '+CAST(GETDATE() AS VARCHAR(20))
    --  RETURN
   END
   ...
   FROM @PROCEDIMIENTOS
    ...
    --MEDICAMENTOS
      ...
      INSERT INTO @MEDICAMENTOS2(codPrestador ,	numAutorizadon, IDAFILIADO ,	CONSECUTIVO,	idMIPRES ,	fechaDispensAdmon,	codDiagnosticoPrincipal ,	codDiagnosticoRelacionado,
      ...
      SELECT codPrestador , numAutorizadon,	IDAFILIADO ,	CONSECUTIVO,	idMIPRES ,	fechaDispensAdmon,	codDiagnosticoPrincipal ,	codDiagnosticoRelacionado,
	         tipoMedicamento,	codTecnologiaSalud,	nomTecnologiaSalud,	concentracionMedicamento ,	unidadMedida ,	formaFarmaceutica,	unidadMinDispensa ,
	         cantidadMedicamento,	diasTratamiento,	tipoDocumentoIdentificacion,	numDocumentoIdentificacion,	vrUnitMedicamento,	vrServicio,	conceptoRecaudo,
