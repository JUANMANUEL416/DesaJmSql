...
DECLARE	 @PARAMETROS   NVARCHAR(MAX),@MODELO           VARCHAR(100) ,@METODO    VARCHAR(100)	,@USUARIO      VARCHAR(12)
		...
		,@FECHAFACT    DATE			,@CNSNOCIT			VARCHAR(20)	,@CONSECUTIVO_NEW  VARCHAR(20)
DECLARE @TIPOCITA_ANT VARCHAR(20), @MODALIDAD_ANT VARCHAR(20), @TIPOCITA_NUE VARCHAR(20), @MODALIDAD_NUE VARCHAR(20)	,@SERVICIO_2 NVARCHAR(MAX)
        ...
        ,@AM BIT
        ,@VIRTUAL BIT
        ,@TELEFONOAVISO VARCHAR(20)
        ,@IDMEDICO_CUR VARCHAR(20), @IDSEDE_CUR VARCHAR(20)
DECLARE @TBLERRORES TABLE(ERROR VARCHAR(MAX));
DECLARE @FECHAMAX DATETIME, @FRECUENCIA VARCHAR(5), @FUERARUTA BIT
BEGIN
	SET DATEFORMAT dmy
	...
	BEGIN
		...
		SELECT 'OK' OK
		IF DBO.FNK_VALORVARIABLE ('CIT_DELETE_CITEXTRA') = 'SI'
		BEGIN
			DECLARE CURSOR_MEDICO CURSOR FOR
			SELECT  CIT_EXTRA.CONSECUTIVO FROM CIT_EXTRA 
				INNER JOIN MED ON CIT_EXTRA.IDMEDICO = MED.IDMEDICO
			WHERE CIT_EXTRA.FECHA < GETDATE()
			OPEN CURSOR_MEDICO;
				
			FETCH NEXT FROM CURSOR_MEDICO INTO @CONSECUTIVO;

			WHILE @@FETCH_STATUS = 0
			BEGIN
				SELECT @FECHAFINAL = FECHA, @IDMEDICO_CUR = IDMEDICO, @IDSEDE_CUR = IDSEDE FROM CIT_EXTRA WHERE CONSECUTIVO = @CONSECUTIVO
				IF NOT EXISTS (SELECT 1 FROM CIT WHERE CIT.FECHA = @FECHAFINAL AND CIT.IDMEDICO = @IDMEDICO_CUR  )
				BEGIN
					EXEC SPK_GENCONSECUTIVO '01',@IDSEDE_CUR,'@CIT', @CONSECUTIVO_NEW OUTPUT  
					SELECT @CONSECUTIVO_NEW = @IDSEDE_CUR + REPLACE(SPACE(10 - LEN(@CONSECUTIVO_NEW))+LTRIM(RTRIM(@CONSECUTIVO_NEW)),SPACE(1),0) 
					...
					FROM CIT_EXTRA WHERE CONSECUTIVO  = @CONSECUTIVO
					DELETE FROM CIT_EXTRA WHERE CONSECUTIVO = @CONSECUTIVO
				END
		...
		SELECT MED.IDMEDICO, MED.NOMBRE
		  , (SELECT COUNT(1) FROM CIT C WHERE CIT.IDMEDICO = C.IDMEDICO AND CIT.IDEMEDICA = C.IDEMEDICA AND CONVERT(DATE, CIT.FECHA) = CONVERT(DATE,C.FECHA) AND C.IDAFILIADO IS NOT NULL  ) [OCUPADO]
		  , (SELECT COUNT(1) FROM CIT C WHERE CIT.IDMEDICO = C.IDMEDICO AND CIT.IDEMEDICA = C.IDEMEDICA AND CONVERT(DATE, CIT.FECHA) = CONVERT(DATE,C.FECHA) AND C.IDAFILIADO IS NULL  ) [DISPONIBLE]
		  , (SELECT COUNT(1) FROM CIT C WHERE CIT.IDMEDICO = C.IDMEDICO AND CIT.IDEMEDICA = C.IDEMEDICA AND CONVERT(DATE, CIT.FECHA) = CONVERT(DATE,C.FECHA)  ) [TOTAL]
		FROM CIT 
		...
		SELECT @SQL ='
		SELECT  CIT.FECHA, NAME=MED.IDMEDICO, 
            VALUE=CONVERT(VARCHAR(8000)
                ,(CASE 
                    WHEN COALESCE(CIT.IDAFILIADO,'''')=''''
                    THEN (SELECT ''LIBRE'' ESTADO, CIT.CONSECUTIVO 
			                    ,CASE 
                                    WHEN (SELECT COUNT(1) FROM CIT C INNER JOIN MAGPRP ON C.IDMEDICO = MAGPRP.IDMEDICO AND C.FECHA  >= MAGPRP.FECHAINICIAL AND C.FECHA< MAGPRP.FECHAFINAL WHERE  C.CONSECUTIVO = CIT.CONSECUTIVO) >0 OR COALESCE(CIT.BLOQUEADO,0) = 1 THEN ''BLOQUEO''
                                ...
                                ,CIT.IDSEDE
                                ,CIT.FACTURADA
                                ,CIT.TIPOCITA [TIPOCITA]
                                ,T3.DESCRIPCION [TIPOCITA_DESC]
                                ,CIT.MODELOS
                                ,CIT.IDMEDICO
                                ,CIT.FECHA
                                ...
                                ,(SELECT LEFT(STUFF(( SELECT CHAR(10) + MPE.IDPESPECIAL+ '' - '' + MPE.DESCESPECIAL FROM DBO.FNK_SPLIT(( SELECT MODELOS FROM CIT C WHERE C.CONSECUTIVO = CIT.CONSECUTIVO),'','',0,NULL) A  INNER JOIN MPE ON MPE.IDPESPECIAL=A.ITEM FOR XML PATH('''')),1,0,''''),1040) 
                                   ) [NOMBREMODELO] 
                             FOR JSON PATH ) 
                    ELSE (SELECT ''OCUPADO'' ESTADO
                            ...
                            , CIT.FECHA, CIT.NOADMISION, CIT.CCOSTO, CIT.CONSECUTIVO, DBO.FNK_ESTADO_CITA(CIT.CONSECUTIVO) [ESTADOCITA]
                            ...
                            , COALESCE(CIT.VALORTOTAL,0) * CASE WHEN CANTIDADC = 0 THEN 1 WHEN COALESCE(CANTIDADC,'''') = '''' THEN 1 ELSE CANTIDADC END [VALORTOTAL]
                            , CIT.OBSERVACIONES, CIT.TIPOCITA [TIPOCITA], T3.DESCRIPCION [TIPOCITA_DESC], CIT.MODELOS, CIT.CANTIDADC
			                ...
			                , MED_SEC.NOMBRE [MED_SEC_NOM], MES_SEC.DESCRIPCION [MES_SEC_DESC]
			                ...
			                , AFI.SEXO, AFI.EDAD, COALESCE(AFI.EMAIL,'''') EMAIL
                            , TGEN_TI_AFI.DESCRIPCION [TIPO_AFI_DESC], COALESCE(CIT.FACTURABLE,0 ) FACTURABLE
			                ...
			                , COALESCE(CIT.IDKPAGE,0) [IDKPAGE], COALESCE(KPAGE.LINKPAGO,'''') [LINKPAGO], KPAGE.ESTADO [KPAGE_ESTADO], KPAGE.F_EXPIRA
			                ...
			                , CASE WHEN EXISTS(SELECT 1 FROM ALR WHERE ALR.IDAFILIADO = CIT.IDAFILIADO) THEN ''SI'' ELSE ''NO'' END [ALERTA]
                            , (SELECT LEFT(STUFF(( SELECT CHAR(10) + MPE.IDPESPECIAL+ '' - '' + MPE.DESCESPECIAL 
                                FROM DBO.FNK_SPLIT(( SELECT MODELOS FROM CIT C WHERE C.CONSECUTIVO = CIT.CONSECUTIVO),'','',0,NULL) A  
                                    INNER JOIN MPE ON MPE.IDPESPECIAL=A.ITEM FOR XML PATH('''')),1,0,''''),1040) 
                              ) [NOMBREMODELO]
                            , CASE WHEN CONVERT (DATE,CIT.FECHA) = CONVERT(DATE,GETDATE()) THEN ''1'' ELSE ''0'' END [TIEMPO_LLEGADA]
                            , CASE WHEN CIT.IDPLAN IN (SELECT DATO FROM USVGS WHERE IDVARIABLE LIKE ''IDPLANPART%'')
                                    THEN CASE  WHEN CIT.TIPOCAJA = ''TFCJ'' THEN  ((COALESCE(CIT.VALORTOTAL,0))-(COALESCE(CASE WHEN  TIPODTO = ''V'' THEN CIT.DESCUENTO WHEN TIPODTO=''P'' THEN (CIT.DESCUENTO * CIT.VALORTOTAL) / 100 END,0))) 
							                    ...
							                    ELSE (CIT.VALORCOPAGO-COALESCE(CASE WHEN  TIPODTO = ''V'' THEN CIT.DESCUENTO WHEN TIPODTO=''P'' THEN (CIT.DESCUENTO * CIT.VALORTOTAL) / 100 END,0)) 
                                    END [VALORCOPAGO]
			                , MSGPAGO =  CASE WHEN DBO.FNK_VALORVARIABLE(''IXCOUNTRY'') = ''PERU''  AND (EXISTS (SELECT 1 FROM PCJ WHERE PCJ.CODCAJA = CIT.CODCAJA AND PCJ.CNSFACJ = CIT.NORECIBOCAJA) 
                                            ...
                                            WHEN DBO.FNK_VALORVARIABLE(''IXCOUNTRY'') <> ''PERU'' AND COALESCE(CIT.N_FACTURA, '''') <> '''' AND COALESCE(CIT.VALORCOPAGO,0) >0 THEN ''Pago''  
                                            WHEN COALESCE(CIT.VALORCOPAGO,0) = 0 THEN ''Pago''				 
				                            ELSE CASE WHEN CIT.TIPOCAJA<>''FCJ'' THEN ''Sin Pagar'' WHEN  EXISTS(SELECT * FROM FCJ WHERE CODCAJA=CIT.CODCAJA AND CNSFACJ=CIT.NORECIBOCAJA AND CERRADA=1 AND ESTADO=''P'') THEN ''Pago'' ELSE  ''Sin Pagar'' END
                                            END 
                            , CASE WHEN CIT.GENEROCAJA <> 1  THEN ''''
                                ...
                                END [ESTADO_REGISTRO]
                            , CASE WHEN CIT.IDPLAN IN (SELECT DATO FROM USVGS WHERE IDVARIABLE LIKE ''IDPLANPART%'') THEN ''PARTICULAR'' ELSE ''NOPARTICULAR'' END [PARTICULAR]
                            , CASE WHEN CIT.IDPLAN IN (SELECT DATO FROM USVGS WHERE IDVARIABLE LIKE ''IDPLANPART%'') AND CIT.IDTERCEROCA=DBO.FNK_VALORVARIABLE(''IDTERCEROPARTICULAR'') THEN 1 ELSE 0 END AS [PART]
                            ...
                            , COALESCE(CASE WHEN  TIPODTO = ''V'' THEN CIT.DESCUENTO WHEN TIPODTO=''P'' THEN (CIT.DESCUENTO * CIT.VALORTOTAL) / 100 END,NULL) DESCUENTO
			                , CASE WHEN EXISTS (SELECT 1 FROM CITSITEDS WHERE CITSITEDS.CONSECUTIVO = CIT.CONSECUTIVO) THEN ''SI'' ELSE ''NO'' END [ESTADO_SITEDS]
			                , IIF((SELECT COUNT(1) FROM GTUR WHERE GTUR.CONSECUTIVO = CIT.CONSECUTIVO AND GTUR.ESTADO = 0)>0,1,0) AS [LLAMADO]
				        FROM AFI
				            INNER JOIN MED ON CIT.IDMEDICO = MED.IDMEDICO
				            ...
				            LEFT JOIN MED MED_SEC ON CIT.IDMEDICO_GEN = MED_SEC.IDMEDICO
				            LEFT JOIN MES MES_SEC ON MED_SEC.IDEMEDICA = MES_SEC.IDEMEDICA
				            LEFT JOIN TGEN ON AFI.NIVELSOCIOEC = TGEN.CODIGO AND TGEN.TABLA = ''General'' AND CAMPO = ''NIVELSOCIOECO''
				            LEFT JOIN TGEN TGEN_TI_AFI ON AFI.TIPOUSUARIO = TGEN_TI_AFI.CODIGO AND TGEN_TI_AFI.TABLA = ''AFI'' AND TGEN_TI_AFI.CAMPO = ''TIPO_USUARIO''
		...
		FROM
            CIT INNER JOIN MED ON CIT.IDMEDICO = MED.IDMEDICO
		...
		IF  @DISPONIBILIDAD = 'No Asistió' 
			SELECT @SQL += ' AND CIT.IDAFILIADO IS NOT NULL AND ((COALESCE(CIT.CUMPLIDA,0) = 0 AND CIT.FECHA <= GETDATE()) OR COALESCE(CIT.CUMPLIDA,0) = 2 )'
   ...
   IF @METODO = 'FACTURAR_CITA'
	  ...
	  --SELECT @TIPOFAC
		SELECT @CUMPLIDA = CUMPLIDA,@TIPOCAJA=TIPOCAJA,@NORECIBOCAJA=NORECIBOCAJA,@IDAFILIADO=IDAFILIADO,@IDSERVICIO=IDSERVICIO,
		       ...
		       @IDPLAN= IDPLAN,  @PART = CASE WHEN CIT.IDPLAN  IN (SELECT DATO FROM USVGS 
                                                                ...
                                                                OR CIT.IDTERCEROCA=LTRIM(RTRIM(DBO.FNK_VALORVARIABLE('IDTERCEROPARTICULAR')))
                                                                THEN 1 
                                            ELSE 0 
                                       END 
		FROM   CIT 
		...
		BEGIN
			...
			END
		END
		IF COALESCE(@CUMPLIDA,0) <> 1 AND DBO.FNK_VALORVARIABLE('FACT_CIT_CUMPLIDA') = 'SI' AND NOT EXISTS(SELECT * FROM USVGS WHERE IDVARIABLE LIKE 'IDPLANPART%' AND DATO=@IDPLAN) --STORRES -- 20250526 -- SE AGRFEGA VALIDACION PARA QUE NO VALIDE CUANDO EL PALN ES PARTCULAR
		BEGIN
			PRINT 'La cita debe de estar atendida'  
			INSERT INTO @TBLERRORES(ERROR)
			SELECT 'La cita debe de estar atendida'            
		END
		...
		BEGIN
			INSERT INTO @TBLERRORES(ERROR) SELECT 'El plan del Paciente esta Inactivo'
			SELECT 'KO' OK
			SELECT ERROR FROM @TBLERRORES
			RETURN
		END
		...
		BEGIN
			INSERT INTO @TBLERRORES(ERROR) SELECT 'La aseguradora del Paciente esta Inactiva'
			SELECT 'KO' OK
			SELECT ERROR FROM @TBLERRORES
			RETURN
		END
		...
		BEGIN
			INSERT INTO @TBLERRORES(ERROR) SELECT 'Revisar el estado en Planes por Tercero.'
			SELECT 'KO' OK
			SELECT ERROR FROM @TBLERRORES
			RETURN
		END
		...
		BEGIN
			INSERT INTO @TBLERRORES(ERROR) SELECT 'Revisar en Planes por Tercero. Es capitado'
			SELECT 'KO' OK
			SELECT ERROR FROM @TBLERRORES
			RETURN
		END
		...
		BEGIN
			INSERT INTO @TBLERRORES(ERROR) SELECT 'Revisar en Planes por Tercero. Es PGP'
			SELECT 'KO' OK
			SELECT ERROR FROM @TBLERRORES
			RETURN
		END
	...
	BEGIN
		...
		BEGIN TRY 
			...
			BEGIN
				...
				WHERE CONSECUTIVO = @CONSECUTIVO
			END
			IF @TIPOFIRMA = 'PRESTACION_ACUDIENTE'	
			BEGIN
				...
				WHERE CONSECUTIVO = @CONSECUTIVO
			END
			IF @TIPOFIRMA = 'PACIENTE_EXONERACION'	
			BEGIN
				DELETE FROM DOCS WHERE DOCUMENTOID = (SELECT FIRMA_EXO_PX FROM CIT WHERE CONSECUTIVO=@CONSECUTIVO)
				
				UPDATE CIT SET FIRMA_EXO_PX = @IDFIRMA
				WHERE CONSECUTIVO = @CONSECUTIVO
			END
			IF @TIPOFIRMA = 'MOV_FIRMA_CLIENTE'	
			BEGIN
				DELETE FROM DOCS WHERE DOCUMENTOID = (SELECT FIRMA_CLIENTE FROM IMOV WHERE CNSMOV=@CONSECUTIVO)
				
				UPDATE IMOV SET FIRMA_CLIENTE = @IDFIRMA
	...
	BEGIN
		...
		FROM   CIT INNER JOIN MED ON CIT.IDMEDICO   = MED.IDMEDICO
		            LEFT JOIN AFI ON CIT.IDAFILIADO = AFI.IDAFILIADO
					LEFT JOIN MES ON MES.IDEMEDICA = CIT.IDEMEDICA
		WHERE  CIT.IDEMEDICA = COALESCE(@IDEMEDICA, CIT.IDEMEDICA) AND MED.TIPO_USUARIO <> 'Silla' 
		AND    CONVERT(DATE, CIT.FECHA) = CONVERT(DATE, JSON_VALUE(@PARAMETROS,'$.DIA'))
		AND CIT.IDSEDE = CASE WHEN COALESCE(@IDSEDE,'') <> '' THEN @IDSEDE ELSE CIT.IDSEDE END
		GROUP BY MES.IDEMEDICA, MES.DESCRIPCION
	END

	IF @METODO = 'DISPONIBILIDAD_MED'
	BEGIN
		SELECT @FECHA = FECHA, @IDEMEDICA = IDEMEDICA 
			,@MODALIDAD = MODALIDAD, @IDSEDE = IDSEDE, @DISPONIBILIDAD = DISPONIBILIDAD, @IDTERCERO = IDTERCERO, @PROCEDENCIA = PROCEDENCIA
		FROM OPENJSON(@PARAMETROS)
		...
		SELECT OK = 'OK',@FECHA AS FECHA, @IDEMEDICA AS IDEMEDICA,@MODALIDAD AS MODALIDAD, @IDSEDE AS IDSEDE, @DISPONIBILIDAD AS DISPONIBILIDAD, @IDTERCERO AS IDTERCERO
		SELECT MED.IDMEDICO, MED.NOMBRE
		  , (SELECT COUNT(1) FROM CIT C WHERE CIT.IDMEDICO = C.IDMEDICO AND CIT.IDEMEDICA = C.IDEMEDICA AND CONVERT(DATE, CIT.FECHA) = CONVERT(DATE,C.FECHA) AND C.IDAFILIADO IS NOT NULL  ) [OCUPADO]
		  , (SELECT COUNT(1) FROM CIT C WHERE CIT.IDMEDICO = C.IDMEDICO AND CIT.IDEMEDICA = C.IDEMEDICA AND CONVERT(DATE, CIT.FECHA) = CONVERT(DATE,C.FECHA) AND C.IDAFILIADO IS NULL  ) [DISPONIBLE]
		  , (SELECT COUNT(1) FROM CIT C WHERE CIT.IDMEDICO = C.IDMEDICO AND CIT.IDEMEDICA = C.IDEMEDICA AND CONVERT(DATE, CIT.FECHA) = CONVERT(DATE,C.FECHA)  ) [TOTAL]
		...
		FROM   CIT INNER JOIN MED ON CIT.IDMEDICO   = MED.IDMEDICO
		            LEFT JOIN AFI ON CIT.IDAFILIADO = AFI.IDAFILIADO
		WHERE  CIT.IDEMEDICA = @IDEMEDICA AND (@PROCEDENCIA = 'APLIM' OR MED.TIPO_USUARIO <> 'Silla') 
		AND    CONVERT(DATE, CIT.FECHA) = CONVERT(DATE, @FECHA)
		AND    CIT.IDMEDICO = CASE WHEN COALESCE(@IDMEDICO,'') <> '' THEN @IDMEDICO ELSE CIT.IDMEDICO END
		AND		CIT.IDSEDE = CASE WHEN COALESCE(@IDSEDE,'') <> '' THEN @IDSEDE ELSE CIT.IDSEDE END
		AND	  (
			COALESCE(CIT.MODALIDAD,'') = ''
			OR CIT.MODALIDAD = IIF(COALESCE(@MODALIDAD,'') = '', CIT.MODALIDAD , @MODALIDAD)
		)
		GROUP BY CIT.IDMEDICO, CIT.IDEMEDICA, MED.IDMEDICO, MED.NOMBRE, CONVERT(DATE, CIT.FECHA)
	END
	IF @METODO = 'EDITA_COPAGO'
	BEGIN
		SELECT @VALORCOPAGO =  VALORCOPAGO, @CONSECUTIVO = CONSECUTIVO
		FROM OPENJSON (@PARAMETROS)
		WITH		 ( VALORCOPAGO		DECIMAL(14,2)	'$.VALORCOPAGO', CONSECUTIVO		VARCHAR(50)		'$.CONSECUTIVO')

		
		IF EXISTS (SELECT * FROM CIT WHERE CONSECUTIVO  = @CONSECUTIVO AND COALESCE(FACTURADA,0) = 1)
		BEGIN
			INSERT INTO @TBLERRORES(ERROR) SELECT 'El registro ya se encuentra Facturado'
		END
		IF EXISTS (SELECT * FROM CIT WHERE CONSECUTIVO = @CONSECUTIVO AND COALESCE(NFACTURA,'')<> '')
		BEGIN
			INSERT INTO @TBLERRORES(ERROR) SELECT 'El Registro tiene una boleta asignada.'
		END
		...
		BEGIN
			...
			BEGIN
				...
				RETURN
			END
			SELECT 'OK'OK, VALORCOPAGO FROM CIT WHERE CONSECUTIVO = @CONSECUTIVO
    END

		END 
    IF @METODO='GETPROXIMACITAS'
    BEGIN
        SELECT @CONSECUTIVO = JSON_VALUE(@PARAMETROS,'$.CONSECUTIVO')
            , @IDAFILIADO = JSON_VALUE(@PARAMETROS,'$.IDAFILIADO')
            , @IDEMEDICA = JSON_VALUE(@PARAMETROS,'$.IDEMEDICA')
            , @IDPESPECIAL = JSON_VALUE(@PARAMETROS,'$.IDPESPECIAL')
            , @FUERARUTA = JSON_VALUE(@PARAMETROS,'$.FUERARUTA')
            , @FRECUENCIA = JSON_VALUE(@PARAMETROS,'$.FRECUENCIA')

        SELECT @FECHA2=FECHA, @IDMEDICO=IDMEDICO, @IDSEDE=IDSEDE, @I=0, @I_TO=1 FROM CIT WHERE CONSECUTIVO=@CONSECUTIVO
        
        IF @FUERARUTA=1
        BEGIN
            SELECT @FECHAMAX = DATEADD(MONTH, TRY_CAST(@FRECUENCIA AS SMALLINT),CAST(@FECHA2 AS DATE))
        END
        ELSE
        BEGIN
            SELECT TOP 1 @FECHAMAX=CAST(F_DEFAULT AS DATE)
            FROM AFIPROT
            INNER JOIN MES ON MES.IDSERVICIO_CITACONTROL=AFIPROT.IDSERVICIO
            WHERE IDAFILIADO=@IDAFILIADO AND IDPESPECIAL=@IDPESPECIAL AND EOMONTH(F_DEFAULT)>EOMONTH(@FECHA2) AND MES.IDEMEDICA=@IDEMEDICA
            ORDER BY F_DEFAULT
        END

        DECLARE @CIT_CONTROL TABLE (CONSECUTIVO VARCHAR(13), FECHA DATETIME, IDMEDICO VARCHAR(12), NOMBRE VARCHAR(45), DESCRIPCION VARCHAR(45))
        SELECT @FECHA2=@FECHAMAX-3, @FECHAMAX+=7

        WHILE @I<4
        BEGIN
            INSERT INTO @CIT_CONTROL (CONSECUTIVO, FECHA, IDMEDICO, NOMBRE, DESCRIPCION)
            SELECT TOP 1 CONSECUTIVO, FECHA, MED.IDMEDICO, MED.NOMBRE, MES.DESCRIPCION
            FROM CIT 
                INNER JOIN MED ON MED.IDMEDICO=CIT.IDMEDICO
                LEFT JOIN MES ON MES.IDEMEDICA=COALESCE(CIT.IDEMEDICA,MED.IDEMEDICA)
            WHERE IDAFILIADO IS NULL AND CIT.IDEMEDICA=@IDEMEDICA AND CIT.IDMEDICO=@IDMEDICO
            AND CAST(FECHA AS DATE)=DATEADD(DAY,@I_TO, CAST(@FECHA2 AS DATE)) AND CAST(FECHA AS DATE)<@FECHAMAX+1
            AND CIT.IDSEDE=@IDSEDE
            ORDER BY FECHA ASC
        ...
        BEGIN
            ...
            FROM CIT 
                INNER JOIN MED ON MED.IDMEDICO=CIT.IDMEDICO
                LEFT JOIN MES ON MES.IDEMEDICA=COALESCE(CIT.IDEMEDICA,MED.IDEMEDICA)
            WHERE IDAFILIADO IS NULL AND CIT.IDEMEDICA=@IDEMEDICA 
            AND CAST(FECHA AS DATE)=DATEADD(DAY,@I_TO, CAST(@FECHA2 AS DATE)) AND CAST(FECHA AS DATE)<@FECHAMAX+1
            AND CIT.IDSEDE=@IDSEDE
            AND NOT EXISTS (SELECT 1 FROM @CIT_CONTROL B WHERE CAST(B.FECHA AS DATE)=CAST(CIT.FECHA AS DATE))
            ORDER BY FECHA ASC
        ...
        END
        SELECT 'OK' OK        
        SELECT TOP 3 * FROM @CIT_CONTROL ORDER BY FECHA
    ...
    BEGIN
        ...
        SELECT @IDAFILIADO=IDAFILIADO, @IDPESPECIAL=COALESCE(@IDPESPECIAL,IDPESPECIAL,'') FROM CIT WHERE CONSECUTIVO=@CONSECUTIVO

        SELECT 'OK' OK
        SELECT MES.IDEMEDICA AS value, MES.DESCRIPCION AS label, 
            FRECUENCIAS = (SELECT O.OPCION AS value, O.DESCRIPCION AS label
                            FROM TGENOP O 
                            WHERE O.TABLA='MES' AND O.CAMPO='FRECUENCIA' AND O.CODIGO=MES.IDEMEDICA
                            FOR JSON PATH
                            )
        FROM MPEMES INNER JOIN MES ON MES.IDEMEDICA=MPEMES.IDEMEDICA
        WHERE IDPESPECIAL=@IDPESPECIAL

        SELECT TOP 1 MES.IDEMEDICA
        FROM AFIPROT
        INNER JOIN MES ON MES.IDSERVICIO_CITACONTROL=AFIPROT.IDSERVICIO
        INNER JOIN MPEMES ON MES.IDEMEDICA=MPEMES.IDEMEDICA
        WHERE AFIPROT.IDAFILIADO=@IDAFILIADO AND AFIPROT.IDPESPECIAL=@IDPESPECIAL AND AFIPROT.ESTADO IN ('Inicial')
        ORDER BY F_DEFAULT
        RETURN
    END
END
