CREATE OR ALTER PROCEDURE  DBO.SPK_CIERRERECIBO_CAJA  
@CODCAJA          VARCHAR(4),
@CNSFACJ          VARCHAR(20),
@COMPANIA         VARCHAR(2),
@SEDE             VARCHAR(5), 
...
BEGIN
   ...
   BEGIN   
      ...
      IF DBO.FNK_VALORVARIABLE('FACTURAPOS_ACTIVA')='SI' 
         AND EXISTS(SELECT * FROM FCJ WITH (NOLOCK) WHERE CODCAJA=@CODCAJA AND CNSFACJ=@CNSFACJ 
         AND COALESCE(IDPLAN,'') NOT IN(SELECT DATO FROM USVGS WHERE IDVARIABLE LIKE'IDPLANPART%') 
         AND PROCEDENCIA<>'CAJA' AND ESTADO='P')
      BEGIN
         PRINT 'VOY A GENERAR LA FACTURA'
         EXEC SPK_CREA_FACTURA_POS_CAJA @CNSFACJ,@CODCAJA,@COMPANIA,@SEDE,@USUARIO,@SYS_COMPUTERNAME,0,@ESPAGARE
      END
      ELSE
      BEGIN
         ...
         BEGIN
            ...
            FROM FCJ WITH (NOLOCK) WHERE CODCAJA=@CODCAJA AND CNSFACJ=@CNSFACJ
            IF EXISTS(SELECT * FROM CIT WHERE CONSECUTIVO=@NOADMISION AND VALORTOTAL<=VALORCOPAGO)
            BEGIN
               PRINT 'VOY A GENERAR LA FACTURA'
               EXEC SPK_CREA_FACTURA_POS_CAJA @CNSFACJ,@CODCAJA,@COMPANIA,@SEDE,@USUARIO,@SYS_COMPUTERNAME,1,@ESPAGARE
            END
         END
      END
      IF @PROCEDENCIA='VENTA' AND COALESCE(@N_FACTURA,'')<>''
      BEGIN
         SELECT @PCJTERCERO=FCJ.IDTERCERO,@VALORTPAGO=VALORTOTAL,@CNSDOCPCJ=NOADMISION
         FROM   FCJ 
         ...
         EXEC SPK_ADMIN_CXC_CONTAB
               ...
               INSERT INTO FCJPCXC (CNSFACJ, CODCAJA, CNSCXC, N_FACTURA, ITEM_FCXCDV, IDTERCERO, VALOR, 
                                    VLR_IMPUESTOS, ESTADO, CNSFPAG, TIPOCXC)
               SELECT @CNSFACJ, @CODCAJA, @CNSCXC, @N_FACTURA, @ITEM_FCXCDV, @IDTERCERO,@VALORTPAGO,
                        0, 'P', @CNSFPAG, 0
      END
   END
   ELSE
...
--query2         
            ...
            BEGIN--CAJ
               ...
               AND FCXP.PROCEDENCIA='CAJAMENOR'                    
                ...
                PRINT  'CAAMENOR'+ @CAJAMENOR   
                PRINT  '@NVOCONSEC2'+ @NVOCONSEC2   
                           
                  SELECT @CNSPCJ = NULL
                  EXEC SPK_GENCONSECUTIVO @COMPANIA, @CAJAMENOR, '@PCJ', @CNSPCJ OUTPUT  
                     SELECT @CNSPCJ = @CAJAMENOR + REPLACE(SPACE(8 - LEN(@CNSPCJ))+LTRIM(RTRIM(@CNSPCJ)),SPACE(1),0)
            ...
            BEGIN                
               ...
               BEGIN
                  ...
                  INSERT INTO FCJ(CNSFACJ, CODCAJA, CLASE_FAC, NOADMISION, NOPRESTACION, 
                                  ...
                                  TIPOEGRESO, IMPRESOTRAS, BANCO, SUCURSAL, CTA_BCO, USUARIO, 
                                  DEAPERTURA)
                  SELECT @NVOCONSEC2, @IDTERCERO, 'COBRO', CNSFACJ, CNSFACJ, 
