...
DECLARE   @PARAMETROS       NVARCHAR(MAX),      @MODELO         VARCHAR(100)	,@METODO		VARCHAR(100)
		...
		,@FECHAVENCE		DATETIME			,@CNSPRESELL VARCHAR(20)	,@datoCreaCliente NVARCHAR(MAX)  ,@CCOSTO VARCHAR(20)
		,@NATJURIDICA		VARCHAR(20)			,@TIPOREGIMEN		VARCHAR(20)		,@TIPO_ID		VARCHAR(20), @IDAREA VARCHAR(20)
		,@RAZONSOCIAL		VARCHAR(500)		,@EMAIL				VARCHAR(200)	,@CODCAJERO VARCHAR(20)	,@NOMBRECLIENTE VARCHAR(500)
		,@DIRECCION			VARCHAR(500)		,@TELEFONOS			VARCHAR(20)		,@CNSACJ VARCHAR(20)	,@CNSFACJ VARCHAR(20),@IDTARIFA VARCHAR(5)
DECLARE @VALORTOTAL DECIMAL(14,2), @VALORRECAUDADO DECIMAL(14,2), @TIPOPAGO VARCHAR(20), @BANCO VARCHAR(20)	   , @CIUDAD VARCHAR(20)
   ...
   --  
	...
	BEGIN
		...
		BEGIN
			BEGIN TRY
				SELECT @IDITAR = '01'
            SELECT @IDAREA = IDAREA, @CCOSTO = CCOSTO FROM IBOD WHERE IDBODEGA = @IDBODEGA
				--BEGIN TRANSACTION;
				...
				BEGIN TRY
						...
						SELECT @IDTERCERO, NULL ,@IDTERCERORESP, @NVOCONSEC, @IDBODEGA, DBO.FNK_VALORVARIABLE('IDINVENTA_PARTICULAR') , DBO.FNK_GETDATE(), @CCOSTO, @USUARIO, @USUARIO, 0, 0, @IDAREA, 0, 'INV'
                  ...
                  --@N_FACTURA
						...
						,BANCO, SUCURSAL, CTA_BCO, NOADMISION, NOPRESTACION, CERRADA, ARCHIVADO, IDPLAN, IDAFILIADO, NOMBRECLIENTE, N_FACTURA, RECAUDO)
						...
						, null,null,null, @NVOCONSEC, @NVOCONSEC, 0, 0, @IDPLAN, @IDAFILIADO, @NOMBRECLIENTE, @N_FACTURA, 1
		...
		BEGIN
			BEGIN TRY
				...
				BEGIN
					...
					, IART.CODBARRA, IE.ID, IART.IDFABRICANTE, TER.RAZONSOCIAL, IART.UNID_VENTA
					FROM IEXI IE     
					INNER JOIN IART ON IE.IDARTICULO = IART.IDARTICULO           
					INNER JOIN TER ON IART.IDFABRICANTE = TER.IDTERCERO
					WHERE IE.IDARTICULO IN (
	...
	END
	IF @METODO = 'EDITA_TERCERO'
	BEGIN
		SELECT @IDTERCERO = IDTERCERO, @DIRECCION = DIRECCION, @TELEFONOS = TELEFONOS, @CIUDAD = CIUDAD, @EMAIL = EMAIL
		FROM OPENJSON(@PARAMETROS)
		WITH(	IDTERCERO	VARCHAR(20)		'$.IDTERCERO', DIRECCION	VARCHAR(60)		'$.DIRECCION', TELEFONOS	VARCHAR(35)		'$.TELEFONOS'
            ,CIUDAD	VARCHAR(20)		'$.CIUDAD', EMAIL	VARCHAR(100)		'$.EMAIL')
		...
		BEGIN
			BEGIN TRY
				UPDATE TER SET DIRECCION = @DIRECCION , TELEFONOS = @TELEFONOS, EMAIL = @EMAIL , CIUDAD = @CIUDAD WHERE TER.IDTERCERO = @IDTERCERO
			END TRY  
			BEGIN CATCH 
				INSERT INTO @TBLERRORES(ERROR) SELECT ERROR_MESSAGE() ;  
			END CATCH  
		...
		END 
	END 
   IF @METODO='LISTA_TARIFAS'     
   BEGIN         
      SELECT 'OK'OK                    

      SELECT DISTINCT IDTARIFA AS value,DESCTARIFA as label
      FROM TAR 
      WHERE 
      EXISTS(SELECT * FROM TARDV INNER JOIN SER ON TARDV.IDSERVICIO=SER.IDSERVICIO
                                 INNER JOIN IART ON SER.IDSERVICIO=IART.IDARTICULO
                                 AND SER.IDARTICULO<>DBO.FNK_VALORVARIABLE('DEINVNODESER'))
      RETURN
   END  
   IF @METODO='LISTA_PRECIOS'     
   BEGIN         
      SELECT @IDTARIFA = IDTARIFA      
      FROM   OPENJSON (@PARAMETROS)
      WITH (           
       IDTARIFA  VARCHAR(20)   '$.IDTARIFA'
      )
      SELECT 'OK'OK
      SELECT SER.IDARTICULO,IART.DESCRIPCION,CONVERT(DECIMAL(14,2),TARDV.VALOR) VLR_UNITARIO,DBO.FNK_FECHA_DDMMAA(TARDV.FECHAINI)FECHAINI
      ,DBO.FNK_FECHA_DDMMAA(TARDV.FECHAFIN)FECHAFIN,COALESCE(IART.UNID_VENTA,1)UNID_VENTA,
      CONVERT(DECIMAL(14,2),TARDV.VALOR*COALESCE(IART.UNID_VENTA,1))VLR_UNIDAD
      FROM TARDV INNER JOIN SER ON TARDV.IDSERVICIO=SER.IDSERVICIO
                 INNER JOIN IART ON SER.IDSERVICIO=IART.IDARTICULO
      WHERE TARDV.IDTARIFA=@IDTARIFA
      AND TARDV.FECHAINI<=GETDATE()
      AND TARDV.FECHAFIN>=GETDATE()-1
      RETURN
