IF EXISTS(SELECT NAME FROM SYSOBJECTS WHERE NAME= 'SPK_FACTURACE_ESCMA' AND XTYPE='P')
BEGIN
   DROP PROCEDURE SPK_FACTURACE_ESCMA
END

GO
CREATE PROCEDURE DBO.SPK_FACTURACE_ESCMA
@CNSRPDX       VARCHAR(20),
@NIT           VARCHAR(20),
@COMPANIA      VARCHAR(2),
@IDSEDE        VARCHAR(5),
   ...
   BEGIN
      ...
      BEGIN  
         ...
         BEGIN
            ...
            AND CNSINICIAL <= CONVERT(INT,LTRIM(RTRIM(REPLACE(@NVOCONSEC,@PRE,'')))) 
            AND CNSFINAL >= CONVERT(INT,LTRIM(RTRIM(REPLACE(@NVOCONSEC,@PRE,''))))
         END
