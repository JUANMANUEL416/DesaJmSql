CREATE OR ALTER PROCEDURE  DBO.SPK_FACTURACI_ESCMA
@CNSRPDX      VARCHAR(20),
@NIT          VARCHAR(20),
@COMPANIA     VARCHAR(2),
@IDSEDE       VARCHAR(5),
     ...
     SELECT @VRDTO = SUM(CASE WHEN CIT.TIPODTO='P' THEN VALORTOTAL* COALESCE(DESCUENTO,0)/100 ELSE COALESCE(DESCUENTO,0)END )
      ...
      BEGIN
         ...
         BEGIN
            ...
            WHERE IDTERCERO=@IDTERCEROCA AND ESTADO='L' AND TIPOFAC='M' 
            AND (NOREFERENCIA='VARIOS' OR NOREFERENCIA<>'VARIOS')
            AND PROCEDENCIA='CI'
            AND DATEDIFF(DAY,F_FACTURA,DBO.FNK_GETDATE())<=1
            ...
            ELSE
            BEGIN
                SELECT @IDFORMATOFTR=DBO.FNK_VALORVARIABLE('IDFORMATOFTR_M')
            ...
            BEGIN
               ...
               BEGIN
                  ...
                  SELECT @CNSRESOL  = CNSRESOL FROM FDIAN WHERE IDENTIFICADOR = @IDSEDE AND VENCIDA = 0 AND PROCEDENCIA = 'FTR' AND CNSINICIAL <= CONVERT(INT,LTRIM(RTRIM(REPLACE(@NVOCONSEC,@PRE,'')))) AND CNSFINAL >= CONVERT(INT,LTRIM(RTRIM(REPLACE(@NVOCONSEC,@PRE,''))))
               END
