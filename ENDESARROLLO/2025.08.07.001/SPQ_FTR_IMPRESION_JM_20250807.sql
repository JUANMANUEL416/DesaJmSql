...
BEGIN  
	...
	BEGIN
		...
		BEGIN
			IF @PROCEDENCIA='MAD'
			BEGIN
				SELECT @VLR_AFECTO=FTRD.VALOR_UNITARIO_SIN_IGV, 
				@VLR_INAFECTO=0,
				@VLR_SUBTOTAL=FTRD.VALOR_UNITARIO_SIN_IGV
				FROM FTRD 
			...
			BEGIN
				...
				BEGIN
					...
					BEGIN
						...
						WHERE @N_FACTURA=IIF(@NOREFERENCIA='MAD_CPM',MAD.N_FACTURA,MAD.N_FACTURA_PPS)
					END
					ELSE
					BEGIN
				...
				BEGIN
					...
					BEGIN        
						...
						WHEN 2 THEN CASE WHEN  COALESCE(SER.MEDICAMENTOS,0)<>1 THEN  COALESCE(SER.CODCUPS,SER.IDSERVICIO) ELSE COALESCE(SER.CODCUM,SER.IDALTERNA) END --CUM
						WHEN 3 THEN CASE WHEN  COALESCE(SER.MEDICAMENTOS,0)<>1 THEN  SER.CODCUPS ELSE SER.IDALTERNA END --ATC
						WHEN 4 THEN CASE WHEN  COALESCE(SER.MEDICAMENTOS,0)<>1 THEN  SER.IDSERVICIO ELSE SER.IDALTERNA END --ADRES
						WHEN 5 THEN CASE WHEN  COALESCE(SER.MEDICAMENTOS,0)<>1 THEN  SER.IDSERVICIO ELSE SER.IDSERVICIO END--SAVIA
						WHEN 6 THEN CASE WHEN  COALESCE(SER.MEDICAMENTOS,0)<>1 THEN  SER.IDSERVICIO ELSE SER.CODSISPRO END -- SISPRO
						WHEN 7 THEN CASE WHEN  COALESCE(SER.MEDICAMENTOS,0)<>1 THEN  SER.IDSERVICIO ELSE SER.IDALTERNA END --INVINA
						WHEN 8 THEN CASE WHEN  COALESCE(SER.MEDICAMENTOS,0)<>1 THEN  SER.CODCUPS ELSE SER.IDALTERNA END -- SOLOCUPS
						WHEN 9 THEN CASE WHEN  COALESCE(SER.MEDICAMENTOS,0)<>1 THEN  SER.CODCUPS ELSE SER.CODCUM END --CUM CUPS
						WHEN 10 THEN CASE WHEN COALESCE(SER.MEDICAMENTOS,0)<>1 THEN  SER.CODCUPS ELSE SER.CODCUM END -- CODIGO SOAT
						END IDSERVICIO,
						CASE  @IMPRIMEIDALTERNA    
...
--                     ROUND(CASE WHEN COALESCE(PIVA,0)>0 AND COALESCE(FTRD.VLR_COPAGOS,0)>0 THEN (FTRD.VLR_COPAGOS/(1+(PIVA/100))) ELSE  COALESCE(FTRD.VLR_COPAGOS,0) END,2) COPA_SINIGV,
			...
			BEGIN
				IF @TIPOFAC='I'
				BEGIN
				  IF EXISTS (SELECT * FROM FTR WHERE N_FACTURA=@N_FACTURA AND OBSERVACION1 LIKE '%CITAUT%')
				  BEGIN 
					...
					WHERE AUT.CONSECUTIVOCIT=@NOREFERENCIA  
					      AND AUT.N_FACTURA=@N_FACTURA
...
--query2
		...
		BEGIN
			...
			SELECT 'Item '+REPLACE(SPACE(2-LEN(FTRD.N_CUOTA)),SPACE(1),'0')+LTRIM(RTRIM(CAST(FTRD.N_CUOTA AS VARCHAR(3))))ITEM,
					...
					CASE  @IMPRIMEIDALTERNA    
						...
						CASE  @IMPRIMEIDALTERNA    
							...
							ELSE SER.CODCUPS
						END CODCUPS,
						CASE  WHEN @IMPRIMEIDALTERNA IN (8,9) THEN COALESCE(SER.DESCRIPCION_CUPS,FTRD.ANEXO)
					...
					FTRD.ANEXO ANEX,
						...
						CASE WHEN @PROCEDENCIA='INVENTARIO' THEN 'Nro.Orden Compra:'+COALESCE(IEXI.CNSFCOM,'')+' | Registro Invima: '+COALESCE(IART.REGINVIMA,'')+' | Lote:'+COALESCE(IEXI.NOLOTEPEDIDO,'')+' | Fecha Vcto.:'+
								DBO.FNK_FECHA_DDMMAA(IEXI.FECHAVENCE)+' | Código CUM:'+COALESCE(IART.CODCUM,'')+' | Fabricante:'+LTRIM(RTRIM(TER.RAZONSOCIAL)) 
						ELSE '' END PERFILFAR,
					FTRD.CANTIDAD,FTRD.VALOR VLR_UNI,FTRD.VLR_SERVICI,COALESCE(FTRD.VLR_COPAGOS,0) VLR_COPA,VR_TOTAL VLRTOTAL,
					VLRTOT_SINIGV=(FTRD.VALOR*FTRD.CANTIDAD),
					CONVERT(VARCHAR, CONVERT(VARCHAR, CAST(FTRD.VALOR  AS MONEY), 1))VALOR,
			...
			FROM FTRD LEFT JOIN SER ON FTRD.REFERENCIA=SER.IDSERVICIO
						LEFT JOIN PRE ON FTRD.PREFIJO=PRE.PREFIJO
						LEFT JOIN IART ON FTRD.REFERENCIA=IART.IDARTICULO AND FTRD.PROCEDENCIA='INV'
						LEFT JOIN IMOV ON FTRD.NOADMISION=IMOV.CNSMOV  AND FTRD.PROCEDENCIA='INV'
						LEFT JOIN IEXI ON FTRD.IDARTICULO=IEXI.IDARTICULO AND IEXI.NOLOTE=FTRD.NOLOTE AND FTRD.PROCEDENCIA='INV'
	...
	IF @METODO = 'GET_FTR_BASE64'
	BEGIN
		DECLARE @BASE64 NVARCHAR(MAX)
		SELECT @BASE64 = DOCBASE64
		FROM dbo.FTR_DOCS
		WHERE N_FACTURA = JSON_VALUE(@PARAMETROS, '$.N_FACTURA')
		SELECT OK = 'OK', DOCBASE64 = @BASE64
	END
END