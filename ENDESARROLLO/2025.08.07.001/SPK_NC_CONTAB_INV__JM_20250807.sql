CREATE OR ALTER PROCEDURE  DBO.SPK_NC_CONTAB_INV   
	@CNSMOV            VARCHAR(20),   
	@USUARIO           VARCHAR(12),  
...
BEGIN 
	...
	IF NOT EXISTS(SELECT COMPANIA,ANO,MES FROM PRI WHERE ANO=@ANO AND MES=@MES)  
	BEGIN
	...
	BEGIN  
		...
		BEGIN
			...
			SELECT @NROCOMPROBANTE,@COMPANIA,'DB',  
				...
				COALESCE(IEXI.IDTERCERO,IMOV.IDTERCERO),IMOV.CCOSTO, @CONCETOART+' '+IART.DESCRIPCION,           
			...
			FROM   IMOV INNER JOIN IMOVH ON IMOV.CNSMOV=IMOVH.CNSMOV 
                  ...
                  LEFT  JOIN IEXI ON IMOVH.IDARTICULO=IEXI.IDTERCERO AND IMOVH.NOLOTE=IEXI.NOLOTE AND IMOVH.NOLOTEPEDIDO=IEXI.NOLOTEPEDIDO 
			WHERE IMOV.CNSMOV=@CNSMOV
			GROUP BY COALESCE(IEXI.IDTERCERO,IMOV.IDTERCERO),IMOV.CCOSTO,IMOV.OBSERVACION,IMOV.IDAREA,IMOV.CNSMOV,IART.IDITAR, IMOV.IDBODEGA,  
	...
	BEGIN  
      PRINT 'CONTAB_INV_IVA_ITAR ='+ DBO.FNK_VALORVARIABLE('CONTAB_INV_IVA_ITAR')
		...
		BEGIN
         ...
         IF EXISTS(SELECT * FROM ISAL LEFT JOIN IEXI ON ISAL.NOLOTE=IEXI.NOLOTE AND ISAL.IDARTICULO=IEXI.IDARTICULO 
                  WHERE ISAL.CNSMOV=@CNSMOV AND COALESCE(ISAL.VR_ANTESIVA,0)<=0)
         BEGIN
            PRINT 'Encontre algo malo'
            UPDATE ISAL SET VR_ANTESIVA=CASE WHEN COALESCE(IEXI.VR_IVA,0)>0 THEN IEXI.VR_ANTESIVA ELSE COALESCE(IEXI.PCOSTO,ISAL.PCOSTO) END,
                            VR_IVA=  CASE WHEN COALESCE(IEXI.VR_IVA,0)>0 THEN IEXI.VR_IVA ELSE 0 END
            
            FROM ISAL LEFT JOIN IEXI ON ISAL.NOLOTE=IEXI.NOLOTE AND ISAL.IDARTICULO=IEXI.IDARTICULO 
            WHERE ISAL.CNSMOV=@CNSMOV 
            AND COALESCE(ISAL.VR_ANTESIVA,0)<=0
            AND COALESCE(IEXI.CNSFCOM,'')<>''
            PRINT 'ULTIMO UPDATE'
            UPDATE ISAL SET VR_ANTESIVA=CASE WHEN COALESCE(VR_ANTESIVA,0)=0 THEN PCOSTO ELSE VR_ANTESIVA END
            WHERE ISAL.CNSMOV=@CNSMOV 
         END
