	...
	BEGIN 
      PRINT 'CIT'
		...
		SELECT @IDPRESTADOR,CIT.IDAFILIADO,CIT.CONSECUTIVO,REPLACE(CONVERT(VARCHAR,CIT.FECHA,102),'.','-')+' '+LEFT(CONVERT(VARCHAR,CIT.FECHA,108),5),null,
				 LEFT(COALESCE(CIT.NOAUTORIZACION,'null'),30),left(REPLACE(REPLACE(LTRIM(RTRIM(SER.CODCUPS)),CHAR(13),''),CHAR(10),''),6),'02','01','02',325,'16',MED.TIPO_ID,MED.IDMEDICO,CIT.IDDX,
      ...
      PRINT 'AP HADM'
		...
		SELECT @IDPRESTADOR,HADM.IDAFILIADO,HADM.NOADMISION,REPLACE(CONVERT(VARCHAR,HPRE.FECHA,102),'.','-')+' '+LEFT(CONVERT(VARCHAR,HPRE.FECHA,108),5),null,
			HADM.NOAUTORIZACION,left(REPLACE(REPLACE(LTRIM(RTRIM(SER.CODCUPS)),CHAR(13),''),CHAR(10),''),6),COALESCE(REPLACE(TGEN.DATO2,' ',''),HADM.VIAINGRESO,'02'),'01','02',325,'16',
      ...
      DECLARE @F_INIPERIODO DATETIME
		...
		REPLACE(CONVERT(VARCHAR,CASE WHEN HADM.FECHA < @F_INIPERIODO THEN @F_INIPERIODO ELSE HADM.FECHA END,102),'.','-')+' '+LEFT(CONVERT(VARCHAR,HADM.FECHA,108),5)
         ...
         ,HADM.DXSALIDA3
		   ...
		   +' '+LEFT(CONVERT(VARCHAR, CASE WHEN HADM.FECHAALTAMED IS NULL OR HADM.FECHAALTAMED > @F_FINPERIODO THEN DATEADD(MINUTE,-2,DATEADD(DAY,1,@F_FINPERIODO)) ELSE HADM.FECHAALTAMED END,108),5) f_egreso -- 20250613 STORRES SE AGREGA CONDICION OR PARA VALIDARF FECHA
		FROM FTRDC INNER JOIN HADM ON FTRDC.NOADMISION=HADM.NOADMISION
...
--QUERY2	
	...
	BEGIN
		...
		FROM FTRDC INNER JOIN AUT   ON FTRDC.NOADMISION= AUT.NOAUT
					...
					INNER JOIN SER     ON AUTD.IDSERVICIO=SER.IDSERVICIO AND FTRDC.IDSERVICIO = SER.IDSERVICIO
					INNER JOIN RIPS_CP ON SER.CODIGORIPS=RIPS_CP.IDCONCEPTORIPS
					LEFT  JOIN MED		 ON MED.IDMEDICO    = IIF(COALESCE(AUT.IDMEDICOSOLICITA,'')='',DBO.FNK_VALORVARIABLE('EPI_MEDICODEFAULT'),AUT.IDMEDICOSOLICITA) --20250625 -- STORRES -- SE AGREGA UN IFF AL PRIMER Y SEGUNDO CAMPO YA QUE SE ENCUENTRA QUE LOS DATOS ESTAN EN BLANCO Y EL COALESCE NO CONTINUA CON EL PROCESO
		WHERE FTRDC.CNSFTR=@CNSFCT
		AND FTRDC.PROCEDENCIA='AUT'
		AND AUTD.N_FACTURA=@N_FACTURA
		AND RIPS_CP.ARCHIVO='AT'
   ...
   FROM @PROCEDIMIENTOS
    ...
    --OTROS RECIEN NACIDOS
     ...
     UPDATE @CONSULTAS2 SET vrServicio=0,valorPagoModerador=CASE WHEN @CPROPIO=1 THEN 0 ELSE valorPagoModerador END
                               --20250613 STORRES SE QUITA COMENTARIO YA QUE CYANDO EL COPAGO ES PROPIO EN LA FACTURA EXISTEN SERVICIOS SIN VALORES A FACTURAR PERO 
                               -- CON VALORES DE COPAGOS. 
/*20250613 STORRES -- SE QUITA COMENTARIO YA QUE CUANDO EL COPAGO ES PROPIO EN LA FACTURA EXISTEN SERVICIOS SIN VALORES A FACTURAR PERO  CON VALORES DE COPAGOS. */
