...
DECLARE  @IDKPAGE INT			,@paymentOrderId VARCHAR(20),@NOADMISION VARCHAR(20)
		...
		,@TIPOFAC VARCHAR(10)	,@FECHAFACT  DATETIME		,@USUCAJA_2 VARCHAR(20)
...
BEGIN
	...
	DECLARE PG_CURSOR CURSOR FOR 
		...
		AND 1=CASE WHEN @IDKAPAGE IS NOT NULL  THEN 1 ELSE CASE WHEN COALESCE(ESTADO,'SEND') IN('SEND','RUNNING','PAID','ERROR') THEN 1 ELSE 0 END END
		AND 1=CASE WHEN @IDKAPAGE IS NOT NULL THEN 1 ELSE CASE WHEN ESTADO IN('SEND','RUNNING') THEN 1 ELSE CASE WHEN COALESCE(REF_PAGO,'')='' THEN 1 ELSE 0 END END END
		ORDER BY IDKPAGE
	OPEN PG_CURSOR    
				...
				BEGIN
					...
					BEGIN
						...
						BEGIN
							...
							SELECT @SQL='{"MODELO":"RECAUDO_PE","METODO":"COBRAR","PARAMETROS":{"CONSECUTIVO":"'+@NOADMISION+'","CODCAJA":"'+@CODCAJA+'","CNSFACJ":"'+@CNSFACJ+'","PROCEDENCIA":"HADM","TIPOFAC":"Boleta"}, "USUARIO":"'+@USUCAJA+'"}'
							EXEC SPQ_JSON @SQL
