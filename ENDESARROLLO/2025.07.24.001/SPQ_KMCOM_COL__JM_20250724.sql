...
BEGIN
   ...
   BEGIN   
      ...
      SELECT @IDMODELOCONT=IDMODELOCONT, @IDTIPOCONT=IDTIPOCONT, @IDAREA=IDAREA, @CCOSTO=CCOSTO, @CLASECONT=CLASECONT,
             ...
             @MIVA=CASE WHEN COALESCE(MIVA,'false')='true' THEN 1 ELSE 0 END,   @CUENTAIVA=CUENTAIVA
      FROM   OPENJSON (@DATOS)
      WITH   ( 
      ...
      END
      IF NOT EXISTS(SELECT * FROM CUE WHERE CUENTA=@CUENTADB AND TIPO='Detalle') AND @IDTIPOCONT NOT IN('VENTAS','NOMINA')
      BEGIN
         INSERT INTO @TBLERRORES(ERROR)
         SELECT 'Cuenta contable Debito No Valida, Verifique e intente de nuevo'
      END
      IF NOT EXISTS(SELECT * FROM CUE WHERE CUENTA=@CUENTACR AND TIPO='Detalle') AND @IDTIPOCONT NOT IN('VENTAS','CXP_SERVICIOS')
      BEGIN
         INSERT INTO @TBLERRORES(ERROR)
      ...
      BEGIN
         IF @IDTIPOCONT<>'VENTAS'
         BEGIN
            IF NOT EXISTS(SELECT * FROM KMCOM WHERE IDTIPOCONT=@IDTIPOCONT AND PREFIJO=@PREFIJO AND CCOSTO=@CCOSTO AND CUENTA=@CUENTADB) AND COALESCE(@CUENTADB,'')<>''
            BEGIN
               INSERT INTO KMCOM(IDMODELOCONT, IDTIPOCONT, TIPO, PREFIJO, IDAREA, CCOSTO, CUENTA, CLASECONT, ESTADO, CUENTAIPS, NIIF, CTA_PASIVOEST, MPGP, CUECRPGP, MCAPITA, CUECRCAP,MIVA,CUENTAIVA)
               SELECT @IDMODELOCONT,@IDTIPOCONT,'DB',@PREFIJO,@IDAREA,@CCOSTO,@CUENTADB,@CLASECONT,NULL,NULL,0,NULL,NULL,NULL,NULL,NULL,@MIVA,@CUENTAIVA
            END
         END
         ...
         BEGIN
            ...
            INSERT INTO KMCOM(IDMODELOCONT, IDTIPOCONT, TIPO, PREFIJO, IDAREA, CCOSTO, CUENTA, CLASECONT, ESTADO, CUENTAIPS, NIIF, 
                              CTA_PASIVOEST, MPGP, CUECRPGP, MCAPITA, CUECRCAP,MIVA,CUENTAIVA)
            SELECT @IDMODELOCONT,@IDTIPOCONT,'CR',@PREFIJO,@IDAREA,@CCOSTO,@CUENTACR,@CLASECONT,NULL,
                  ...
                  ,IIF(@IDTIPOCONT='VENTAS',@CAPITA,NULL),IIF(@IDTIPOCONT='VENTAS',@CUENTACAPI,NULL),@MIVA,@CUENTAIVA
        END
      END
      IF @PROCESO='Editar'
      BEGIN
         ...
         BEGIN
            ...
            BEGIN
               ...
               BEGIN
                  UPDATE KMCOM SET CUENTA=@CUENTADB,MIVA=@MIVA,CUENTAIVA=@MIVA
            ...
            BEGIN
               IF COALESCE(@CUENTACR,'')<>''
               BEGIN
                  UPDATE KMCOM SET CUENTA=@CUENTACR,MIVA=@MIVA,CUENTAIVA=@MIVA
                  WHERE KMCOM.IDTIPOCONT=@IDTIPOCONT AND PREFIJO=@PREFIJO
                  AND CCOSTO=@CCOSTO 
                  AND IDAREA=@IDAREA
                  AND TIPO='CR'
         ...
         BEGIN
            UPDATE KMCOM SET CUENTAIPS=@CUENTAIPS,MPGP=@PGP,CUECRPGP=CASE WHEN COALESCE(@PGP,0)=1 THEN @CUENTAPGP ELSE NULL END,MCAPITA=@CAPITA,
                             CUECRCAP=CASE WHEN COALESCE(@CAPITA,0)=1 THEN  @CUENTACAPI ELSE NULL END,MIVA=@MIVA,CUENTAIVA=@MIVA
            WHERE KMCOM.IDTIPOCONT=@IDTIPOCONT AND PREFIJO=@PREFIJO
  ...
  IF @METODO='CRUD_KMCOMS'     
   BEGIN   
      ...
      BEGIN
         ...
         BEGIN
            BEGIN TRY           
               INSERT INTO KMCOMS(IDMODELOCONT, IDTIPOCONT, TIPO, PREFIJO, IDAREA, CCOSTO, IDSERVICIO, CUENTA, ESTADO, CUENTAIPS, CTA_PASIVOEST, PGP, CUEPGP, CAPITA, CUECAPITA,MIVA,CUENTAIVA)
               SELECT @IDMODELOCONT, @IDTIPOCONT, @TIPO, @PREFIJO, @IDAREA, @CCOSTO, @IDSERVICIO, @CUENTADB,'Activo', @CUENTAIPS, NULL, NULL, NULL, NULL, NULL,@MIVA,@CUENTAIVA              
            END TRY
            BEGIN CATCH
      ...
      BEGIN
         BEGIN TRY                     
            UPDATE KMCOMS SET CUENTA=@CUENTADB,IDSERVICIO=@IDSERVICIO,MIVA=@MIVA,CUENTAIVA=@CUENTAIVA  WHERE TIPO=@TIPO AND IDMODELOCONT=@IDMODELOCONT AND IDTIPOCONT=@IDTIPOCONT AND  PREFIJO=@PREFIJO AND  IDAREA=@IDAREA
