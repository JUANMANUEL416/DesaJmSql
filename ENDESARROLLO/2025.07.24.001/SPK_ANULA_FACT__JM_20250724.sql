...
BEGIN
   ...
   IF @PART=1
   BEGIN
      IF EXISTS(SELECT * FROM FPAGD WHERE N_FACTURA=@N_FACTURA)
      BEGIN
         ...
         --IF EXISTS(SELECT * FROM FPAGD WHERE CNSFPAG=@CNSFPAG AND N_FACTURA=@N_FACTURA )
         --BEGIN
         --   DELETE FPAGD WHERE CNSFPAG=@CNSFPAG AND N_FACTURA=@N_FACTURA
         --END
      ...
      IF EXISTS(SELECT * FROM FCJ INNER JOIN ACJ ON FCJ.CNSACJ=ACJ.CNSACJ WHERE FCJ.CODCAJA=@CODCAJA AND FCJ.CNSFACJ=@CNSFACJ AND ACJ.ABIERTA=1)
      BEGIN
   ...
   BEGIN
      ...
      BEGIN
         ...
         IF DBO.FNK_VALORVARIABLE('IXCOUNTRY')='PERU' AND EXISTS(SELECT * FROM FTRD WHERE N_FACTURA=@N_FACTURA AND PROCEDENCIA='CE')
         BEGIN
            UPDATE AUT SET FACTURADA = 0, MARCAFAC = 0, N_FACTURA = NULL
	         WHERE  AUT.N_FACTURA =  @N_FACTURA AND FACTURADA = 1
