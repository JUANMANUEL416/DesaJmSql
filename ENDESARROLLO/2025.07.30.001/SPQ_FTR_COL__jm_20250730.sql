...
DECLARE @PARAMETROS NVARCHAR(MAX)			,@MODELO VARCHAR(100)			   ,@METODO VARCHAR(100)
	  ...
	  ,@WHERE_DATA VARCHAR(500)		,@QUERY NVARCHAR(max)           ,@ARSTIPOCONTRATO VARCHAR(10)
BEGIN
	SELECT *
	INTO #JSON
	...
	BEGIN
		PRINT 'VOY A ANULAR'
		SELECT @USUANULA= USUARIO,@N_FACTURA=N_FACTURA,@RAZONANULA=OBSANULA,@PART=CASE WHEN COALESCE(PART,'false')='true' THEN 1 ELSE 0 END
      ...
      SELECT @NROCOMPROBANTE=NROCOMPROBANTE,@FACTE=FACTE FROM FTR WHERE N_FACTURA=@N_FACTURA
      ...
      IF COALESCE(@FACTE,0)= 2 AND NOT EXISTS(SELECT * FROM MCP WHERE NROCOMPROBANTE=@NROCOMPROBANTE AND ESTADO=2)
			INSERT INTO @TBLERRORES(ERROR)
			SELECT 'La Factura no se Encuentra correctamente contabilizada y ya fue enviada a la Dian. Es necesaria su Contabilización para generar la nota Crédito correspondiente'          
      ...
      print 'PASE TODAS LAS VALIDACIONES ENTRO AL BEGIN TRY'
		BEGIN TRY 
			...
			IF @PROCEDENCIA NOT IN('FINANCIERO')
			    EXEC SPK_ANULA_FACT @N_FACTURA,'01',@IDSEDE,@USUARIO,@RAZONANULA,@SYS_COMPUTERNAME,@USUANULA,@PART
			ELSE
			BEGIN
				IF @CAPITA=1
	...
	BEGIN
		...
		IF @IDDEP<> DBO.FNK_VALORVARIABLE('IDFDEPFACTURACION')
			INSERT INTO @TBLERRORES(ERROR) SELECT 'La Factura No esta en el Departamento de Facturación. No se puede liberar' 
		IF @INDCARTERA=1 
		...
		IF @PROCEDENCIA='AUT' AND NOT EXISTS(SELECT * FROM AUT WHERE IDAUT=@NOREFERENCIA)
			INSERT INTO @TBLERRORES(ERROR) SELECT 'No se encontro Nro de Autorización Asociada a la Factura. No se puede liberar' 
	...
	BEGIN
      ...
      IF @F_FACTURA >DBO.FNK_GETDATE()
		...
		BEGIN
			INSERT INTO @TBLERRORES(ERROR) SELECT 'Periodo Contable No Existe o no Esta Abierto, Revise Configuración Peridos Contables'
		END
      ...
      END
		...
		BEGIN
			...
			IF @IXCOUNTRY='PERU' AND @SINCNT=1 AND COALESCE(@IDPLAN,'')=''
				SELECT @TTEC=DBO.FNK_VALORVARIABLE('IDTERCONTABLEPART'),@IDPLAN=DBO.FNK_VALORVARIABLE('IDPLANPART'),@EC = 1
			PRINT 'DV='+STR(@DV)
			PRINT '@N_FACTURA = '+@N_FACTURA +' // @CNSFTR '+ @CNSFTR
			BEGIN TRY
         ...
         END
			BEGIN TRY 
				INSERT INTO FTRD(CNSFTR,N_CUOTA,AREAPRESTACION,PREFIJO,REFERENCIA,ANEXO,CANTIDAD,VALOR,VR_TOTAL,VLR_SERVICI,
									IDPLAN,CCOSTO,N_FACTURA,FECHA,COPAGOS_CP,VIVA,PIVA,ARSTIPOCONTRATO)
				SELECT @CNSFTR,@N_CUOTA,@IDAREA,@PREFIJO,@IDSERVICIO,@ANEXO,@CANTIDAD,
					...
					CASE WHEN @IXCOUNTRY='PERU' THEN 18 ELSE NULL  END,@ARSTIPOCONTRATO 
			END TRY 
			BEGIN CATCH
		...
		BEGIN
			BEGIN TRY
				...
				UPDATE FTRD SET AREAPRESTACION=@IDAREA, PREFIJO=@PREFIJO, REFERENCIA=@IDSERVICIO, ANEXO=@ANEXO, CANTIDAD=@CANTIDAD,
					             ...
					             VR_TOTAL=@VR_TOTAL, VLR_SERVICI=@VR_TOTAL, CCOSTO=@CCOSTO,
	...
	BEGIN 
		...
		BEGIN
         ...
         BEGIN
            ...
            BEGIN
               INSERT INTO @TBLERRORES(ERROR) SELECT 'El Nro de Autorización debe ser Obligatorio y cumplir con el mínimo de caracteres'
               SELECT 'KO' OK
			      SELECT ERROR FROM @TBLERRORES
			      RETURN
            END
      ...
      END
		...
		BEGIN TRY
         ...
         BEGIN
            ...
            --COALESCE(FACTURADA,0) = 0 AND CONVERT(DATE, FECHA) >= '2025-01-01' AND CONVERT(DATE, FECHA) <= '2025-02-18' AND IDTERCERO= '800251440' AND IDPLAN= 'SANITA' AND PROCEDENCIA = 'AUT'AND COALESCE(NOCOBRABLE,0)=0
...
--QUERY 3   
	...
	BEGIN 
      ...
      IF @TIPODOC='FACTURA'
		   SELECT @N_FACTURA=N_FACTURA, @CAPITA=COALESCE(CAPITADA,0), @TIPOFACT=TIPOFAC,@TIPOCAP=TIPOCAP  FROM FTR WHERE CNSFCT=@CNSFACT
      ELSE
         IF @TIPODOC='AJUSTE'
            ...
            WHERE FGLO.CNSGLO=@CNSFACT
         ELSE
            SELECT @N_FACTURA=FTR.N_FACTURA, @CAPITA=COALESCE(CAPITADA,0), @TIPOFACT=TIPOFAC,@TIPOCAP=TIPOCAP  
            FROM FNOT INNER JOIN  FTR ON FNOT.N_FACTURA=FTR.N_FACTURA 
            WHERE FNOT.CNSFNOT=@CNSFACT
      --- INFORMACION DE RIPS SI YA ESTA VALIDADO
      SELECT  @CUV  = COALESCE(CUV,'') , @FRADICA_RIPS = COALESCE(FRADICA_RIPS,'') , @JSON_RESPUESTA  = COALESCE(JSON_RESPUESTA,'')   
      FROM FTRJSON  WHERE CNSFCT=@CNSFACT

      IF @TIPODOC='AJUSTE' AND COALESCE(@CUV,'')=''
		...
		BEGIN
            ...
            SELECT @NOMBREJSON=@CNSFACT+'.json'
               ...
               RETURN
				END
            IF NOT EXISTS(SELECT * FROM FNOT WHERE N_fACTURA=@N_FACTURA AND CNSFNOT=@CNSFACT AND COALESCE(FACTE,0)=2)
		      BEGIN 
			      SELECT 'KO' KO
			...
			IF @CAPITA= 1 
         BEGIN
            IF EXISTS(SELECT * FROM FTR INNER JOIN PPT ON FTR.IDTERCERO=PPT.IDTERCERO AND FTR.IDPLAN=PPT.IDPLAN AND COALESCE(PPT.PGP,0)=1 AND FTR.N_FACTURA=@N_FACTURA)
            BEGIN
               PRINT 'VOY POR PGP'
	...
	BEGIN
		BEGIN TRY 
			...
			BEGIN
				SELECT 'KO' OK
				SELECT 'El documento en Base64 y el número de factura son obligatorios' ERROR
				RETURN
			END
