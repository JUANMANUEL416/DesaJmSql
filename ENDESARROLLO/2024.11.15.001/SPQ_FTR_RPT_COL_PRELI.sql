...
BEGIN
   ...
   BEGIN         
      ...
      BEGIN
         SELECT @SQL=
                     'SELECT PLN.DESCPLAN,DBO.FNK_FECHA_GRINGA(F_FACTURA)F_FACTURA,CASE WHEN FTR.TIPOFIN=''N'' THEN ''03'' ELSE ''01'' END TIPO,
                     CASE WHEN FTR.TIPOFIN=''N'' THEN ''BOLETA ELECTRONICA'' ELSE ''FACTURA ELECTRONICA'' END TIPO,LEFT(FTR.N_FACTURA,4)SERIE,
                     RIGHT(FTR.N_FACTURA,8)NRO,CASE WHEN FTR.TIPOFIN=''C''  THEN 6 ELSE CONVERT(VARCHAR(1),CONVERT(INT,COALESCE(TGEN.VALOR1,0))) END,
                     TER.TIPO_ID,TER.NIT,TER.RAZONSOCIAL,
                     (SELECT SUM(CASE WHEN COALESCE(FTRD.VIVA,0)>0 THEN FTRD.VLR_SERVICI ELSE  0 END)
