...
BEGIN         
   ...
   SELECT 'OK'OK
     ...
     SELECT DATOS=( SELECT CIT.CONSECUTIVO,DBO.FNK_FECHA_GRINGA(FECHA)FECHA,LEFT(CONVERT(VARCHAR,FECHA,108),5)HORA,MES.DESCRIPCION AS ESPECIALIDAD,
      MED.NOMBRE AS NOMBRE_MED,SED.DESCRIPCION AS SEDE,COALESCE(CIT.VALORCOPAGO,0)VLRCOPAGO,COALESCE(AFI.CELULAR,TELEFONORES)CELULAR,AFI.NOMBREAFI AS NOMBEPACIENTE,
      CIT.ATENCION,
   ...
   BEGIN                   
      ...
      BEGIN
         ...
         BEGIN
            ...
            BEGIN
               IF EXISTS(SELECT * FROM CIT WHERE CONSECUTIVO=@CONSECUTIVO AND COALESCE(CONFIRMADA,0)=0 AND FECHACONFIRMA IS NULL)
               BEGIN
               ...
               BEGIN
                  ...
                  SELECT 'Cita ya esta confirmada, no se permiten cambios'
               END
            END
            ELSE
            BEGIN
