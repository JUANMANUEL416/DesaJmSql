...
--QUERY2
	...
	BEGIN
		...
		BEGIN
			SELECT @QUERY = 
				'SELECT IDSERVICIO value, CONCAT(FCTSERD.IDTIPOSER, '' - '', FCTSERD.IDSERVICIO,'' - '',FCTSERD.DESCRIPCION)   label, FCTSERD.IDTIPOSER, FCTSER.UNSPSC  FROM FCTSERD
		...
		END
		ELSE
			SELECT FCTSERD.IDSERVICIO value, CONCAT (FCTSERD.IDTIPOSER, ' - ', FCTSERD.IDSERVICIO, ' - ', FCTSERD.DESCRIPCION) label, 
	...
	IF @METODO = 'VW_PXS'
	BEGIN
		SELECT @QUERY = 'SELECT IDTERCERO value ,RAZONSOCIAL label
							FROM VW_PXS '
		IF @WHERES<>''
		BEGIN
			SET @QUERY += ' WHERE '+@WHERES
