CREATE OR ALTER PROCEDURE DBO.SPK_RIPS_JSON_FTR_CAPMASI
...
BEGIN
	...
	PRINT 'SPK_RIPS_JSON_FTR_CAPMASI ='+@N_FACTURA

	SELECT @CNSFCT=CNSFCT,@IDPLAN=IDPLAN,
   ...
   END
	...
	BEGIN 
			INSERT INTO @CONSULTAS(codPrestador,CONSECUTIVO,IDAFILIADO,fechaInicioAtencion,numAutorizacion,codConsulta,modalidadGrupoServicioTecSal,grupoServicios,codServicio,
									finalidadTecnologiaSalud,causaMotivoAtencion,vrServicio,conceptoRecaudo,valorPagoModerador,numFEVPagoModerador,tipoDocumentoIdentificacion, numDocumentoIdentificacion,
			...
			causaMotivoAtencion='38',CIT.VALORTOTAL,IIF(COALESCE(CIT.VALORCOPAGO,0)>0,IIF(COALESCE(AFI.TIPOUSUARIO,'')IN('01','02','03'),@conceptoRecaudo,IIF(COALESCE(AFI.TIPOUSUARIO,'')='04','01',@conceptoRecaudo)),'05'),COALESCE(CIT.VALORCOPAGO,0),IIF(COALESCE(CIT.VALORCOPAGO,0)>0,COALESCE(CIT.NFACTURA,@N_FACTURA),'null'), MED.TIPO_ID, MED.IDMEDICO,
         NULL,NULL
			FROM FTRDC 
	...
	BEGIN
		...
		causaMotivoAtencion='38',AUTD.VALOR,IIF(COALESCE(AUTD.VALORCOPAGO,0)>0,@conceptoRecaudo,'05'),COALESCE(AUTD.VALORCOPAGO,0),IIF(COALESCE(AUTD.VALORCOPAGO,0)>0,COALESCE(AUTD.NFACTURA,@N_FACTURA),'null'), COALESCE(MED.TIPO_ID, @TIPODOC,''), COALESCE(MED.IDMEDICO, @DOCIDAFILIADO,''),
      ...
      AND COALESCE(AUTD.VALOR,0)>0
		...
		1,HPRED.VALOR,IIF(COALESCE(HPRED.VALORCOPAGO,0)>0,@conceptoRecaudo,'05'),COALESCE(HPRED.VALORCOPAGO,0), MED.TIPO_ID, MED.IDMEDICO 
		...
		codServicio=325,finalidadTecnologiaSalud=LEFT(CASE WHEN HPRE.FINALIDAD IS NULL OR HPRE.FINALIDAD='' OR HPRE.FINALIDAD='10'THEN '44' ELSE HPRE.FINALIDAD END,2),
		causaMotivoAtencion='38',COALESCE(HADM.DXINGRESO,HADM.DXEGRESO),COALESCE(HADM.DXSALIDA1,''),COALESCE(HADM.DXSALIDA2,''),COALESCE(HADM.DXSALIDA3,''),
		1,HPRED.VALOR,IIF(COALESCE(HPRED.VALORCOPAGO,0)>0,@conceptoRecaudo,'05'),COALESCE(HPRED.VALORCOPAGO,0),CONVERT(INT,HPRED.CANTIDAD), MED.TIPO_ID, MED.IDMEDICO 
		FROM  FTRDC INNER JOIN HADM ON FTRDC.NOADMISION=HADM.NOADMISION  
			INNER JOIN  HPRE ON HADM.NOADMISION=HPRE.NOADMISION
				INNER JOIN HPRED ON HPRE.NOPRESTACION=HPRED.NOPRESTACION AND HPRED.NOITEM = FTRDC.NOITEM
				INNER JOIN SER ON HPRED.IDSERVICIO=SER.IDSERVICIO  AND FTRDC.IDSERVICIO = SER.IDSERVICIO
	...
	BEGIN
		...
		SELECT @IDPRESTADOR,AUT.IDAFILIADO,AUT.NOAUT,null,fechaDispensAdmon=REPLACE(CONVERT(VARCHAR,AUT.FECHA,102),'.','-')+' '+LEFT(CONVERT(VARCHAR,AUT.FECHA,108),5),
			AUT.DXPPAL,IIF(COALESCE(AUT.DXRELACIONADO,'')='',AUT.DXPPAL,AUT.DXRELACIONADO),'01',COALESCE(SER.CODCUM,IART.CODCUM),LEFT( dbo.FNK_LIMPIATEXTO(IART.DESCRIPCION,'0-9 A-Z();:.,'),30),
         CASE WHEN ISNUMERIC(dbo.FNK_LIMPIATEXTO(LEFT(COALESCE(ICCN.DESCRIPCION,'0'),3),'0-9'))=1 THEN dbo.FNK_LIMPIATEXTO(LEFT(COALESCE(ICCN.DESCRIPCION,'0'),3),'0-9') ELSE 0 END,
			COALESCE(IUNI.HOMOLOGO_RIPS,247),COALESCE(IFFA.HOMOJSON,'null'),'11',COALESCE(AUTD.CANTIDAD,0),CASE WHEN COALESCE(AUTD.DIAS,0)=0 THEN 1 ELSE AUTD.DIAS END,MED.TIPO_ID,MED.IDMEDICO,AUTD.VALOR,AUTD.VALOR*AUTD.CANTIDAD,
			IIF(COALESCE(AUTD.VALORCOPAGO,0)>0,@conceptoRecaudo,'05'),'04',COALESCE(AUTD.VALORCOPAGO,0),IIF(COALESCE(AUTD.VALORCOPAGO,0)>0,COALESCE(AUTD.NFACTURA,@N_FACTURA),'null')
		FROM FTRDC INNER JOIN AUT ON FTRDC.NOADMISION=AUT.IDAUT  
      ...
      AND IART.PRINCIPAL=1
		...
		SELECT @IDPRESTADOR,HADM.IDAFILIADO,HADM.NOADMISION,null,fechaDispensAdmon=REPLACE(CONVERT(VARCHAR,FTRDC.FECHA,102),'.','-')+' '+LEFT(CONVERT(VARCHAR,FTRDC.FECHA,108),5),
            ...
            COALESCE(HPRED.CANTIDAD,0),1,MED.TIPO_ID,MED.IDMEDICO,CONVERT(VARCHAR,CONVERT(DECIMAL(14,2),HPRED.VALOR),20),
	...
	BEGIN 
		...
		SELECT @IDPRESTADOR,CIT.IDAFILIADO,CIT.CONSECUTIVO,REPLACE(CONVERT(VARCHAR,CIT.FECHA,102),'.','-')+' '+LEFT(CONVERT(VARCHAR,CIT.FECHA,108),5),null,
				...
				CASE WHEN COALESCE(CIT.IDDX,'')='' THEN 'null' END,CASE WHEN COALESCE(CIT.IDDX,'')='' THEN 'null' END,CIT.VALORTOTAL,IIF(COALESCE(CIT.VALORCOPAGO,0)>0,@conceptoRecaudo,'05'),'04',COALESCE(CIT.VALORCOPAGO,0),IIF(COALESCE(CIT.VALORCOPAGO,0)>0,COALESCE(CIT.NFACTURA,@N_FACTURA),'null')
		FROM  FTRDC 
			INNER JOIN CIT ON FTRDC.NOADMISION=CIT.CONSECUTIVO
			INNER JOIN SER ON CIT.IDSERVICIO=SER.IDSERVICIO
			INNER JOIN AFI ON FTRDC.IDAFILIADO=AFI.IDAFILIADO
		...
		BEGIN --AP
			...
			SELECT @IDPRESTADOR,AUT.IDAFILIADO,AUT.NOAUT,REPLACE(CONVERT(VARCHAR, AUT.FECHA, 102), '.', '-') + ' ' + LEFT(CONVERT(VARCHAR, AUT.FECHA, 108), 5),NULL
				...
				,AUT.DXPPAL,COALESCE(AUT.DXRELACIONADO,'null'),AUT.DXPPAL,AUTD.VALOR,IIF(COALESCE(AUT.VALORCOPAGO,0)>0,@conceptoRecaudo,'05'),FTRDC.CANTIDAD,'04'
				,CASE WHEN AUT.IDPLAN IN (DBO.FNK_VALORVARIABLE ('IDPLANPART'),DBO.FNK_VALORVARIABLE ('IDPLANPART2')    ----VGARCIA
						...
						END
				,IIF(COALESCE(AUTD.VALORCOPAGO,0)>0,COALESCE(AUTD.NFACTURA,@N_FACTURA),'null')
			FROM FTRDC 
      ...
      PRINT 'VALIDO TOTALES DE DIVISION'
      DECLARE @VALORS DECIMAL(14,2)
      DECLARE @VALORORI DECIMAL(14,2)

			DECLARE JSPROCE_CURSOR_AUTD CURSOR FOR 
			SELECT CONSECUTIVO,SUM(valorPagoModerador) VALOR FROM @PROCEDIMIENTOS
         WHERE COALESCE(valorPagoModerador,0)>0
         GROUP BY CONSECUTIVO
			ORDER BY CONSECUTIVO
			OPEN JSPROCE_CURSOR_AUTD
			FETCH NEXT FROM JSPROCE_CURSOR_AUTD INTO @CNSCONSULTA,@VALORS
			WHILE @@FETCH_STATUS = 0
			BEGIN
            SELECT @VALORORI=VALORCOPAGO FROM AUT WHERE NOAUT=@CNSCONSULTA
            IF ABS(@VALORORI-@VALORS)>0 AND  ABS(@VALORORI-@VALORS)<10
            BEGIN
               SELECT TOP 1 @CONSE=id FROM @PROCEDIMIENTOS WHERE CONSECUTIVO=@CNSCONSULTA
               IF @VALORORI-@VALORS<0
               BEGIN
                  UPDATE @PROCEDIMIENTOS SET valorPagoModerador=valorPagoModerador+(@VALORORI-@VALORS) WHERE ID=@CONSE
               END
               ELSE
               BEGIN
                  UPDATE @PROCEDIMIENTOS SET valorPagoModerador=valorPagoModerador-(@VALORORI-@VALORS) WHERE ID=@CONSE
		...
		SELECT @IDPRESTADOR,HADM.IDAFILIADO,HADM.NOADMISION,REPLACE(CONVERT(VARCHAR,HPRE.FECHA,102),'.','-')+' '+LEFT(CONVERT(VARCHAR,HPRE.FECHA,108),5),null,
			...
			CASE WHEN COALESCE(HADM.DXSALIDA1,'')='' THEN 'null' END,CASE WHEN COALESCE(HADM.COMPLICACION,HADM.DXINGRESO,HADM.DXEGRESO,'')='' THEN 'null' END,HPRED.VALOR,IIF(COALESCE(HPRED.VALORCOPAGO,0)>0,@conceptoRecaudo,'05'),'04',COALESCE(HPRED.VALORCOPAGO,0),@N_FACTURA
		FROM  FTRDC INNER JOIN HADM ON FTRDC.NOADMISION=HADM.NOADMISION
					INNER JOIN AFI  ON FTRDC.IDAFILIADO=AFI.IDAFILIADO
					INNER JOIN HPRED ON FTRDC.NOPRESTACION=HPRED.NOPRESTACION AND FTRDC.NOITEM=HPRED.NOITEM
		...
		SELECT @IDPRESTADOR,HADM.IDAFILIADO,HADM.NOADMISION,REPLACE(CONVERT(VARCHAR,HPRE.FECHA,102),'.','-')+' '+LEFT(CONVERT(VARCHAR,HPRE.FECHA,108),5),null,
			...
			CASE WHEN COALESCE(HADM.DXSALIDA1,'')='' THEN 'null' END,CASE WHEN COALESCE(HADM.COMPLICACION,HADM.DXINGRESO,HADM.DXEGRESO,'')='' THEN 'null' END,HPRED.VALOR,IIF(COALESCE(HPRED.VALORCOPAGO,0)>0,@conceptoRecaudo,'05'),HPRED.CANTIDAD, '04',COALESCE(HPRED.VALORCOPAGO,0),@N_FACTURA
		FROM  FTRDC INNER JOIN HADM ON FTRDC.NOADMISION=HADM.NOADMISION
					INNER JOIN AFI  ON FTRDC.IDAFILIADO=AFI.IDAFILIADO
					INNER JOIN HPRED ON FTRDC.NOPRESTACION=HPRED.NOPRESTACION AND FTRDC.NOITEM=HPRED.NOITEM
					INNER JOIN HPRE  ON HADM.NOADMISION=HPRE.NOADMISION AND FTRDC.NOPRESTACION=HPRE.NOPRESTACION
   ...
   END
	...
	BEGIN
		...
		SELECT @IDPRESTADOR,CIT.IDAFILIADO,CIT.CONSECUTIVO,numAutorizacion=CIT.NOAUTORIZACION,null,
			...
			IIF(COALESCE(CIT.VALORCOPAGO,0)>0,COALESCE(CIT.NFACTURA,@N_FACTURA),'null')
	...
	BEGIN
		...
		WHERE HCA.CLASE='HC'
			...
			AND EXISTS(SELECT * FROM FTRDC WHERE  FTRDC.CNSFTR=@CNSFCT AND FTRDC.NOADMISION=CIT.CONSECUTIVO AND FTRDC.PROCEDENCIA='CIT')
		UNION 
		SELECT HCA.IDAFILIADO,HCA.NOADMISION,HCA.CONSECUTIVOCIT,
				CASE HCA.TIPODX 
							WHEN 'Presuntivo'   THEN '01'
   ...
   --UPDATE @USUARIOS SET @TIPOUSU='03' WHERE 1=1
	...
	BEGIN
		...
		UPDATE AP
		SET 
			AP.codDiagnosticoPrincipal = CASE WHEN COALESCE(AP.codDiagnosticoPrincipal, '') = '' THEN COALESCE(DX.IDDX,'Z000') ELSE AP.codDiagnosticoPrincipal END,
			AP.codDiagnosticoRelacionado = CASE WHEN COALESCE(AP.codDiagnosticoRelacionado, '') = '' THEN COALESCE(DX.DX1,'null') ELSE AP.codDiagnosticoRelacionado END
...
--QUERY2	
    ...
    --OTROS RECIEN NACIDOS
     ...
     END
     --SELECT 'AC', SUM(vrServicio) FROM @CONSULTAS
     --SELECT 'AP', SUM(vrServicio) FROM @PROCEDIMIENTOS
     --SELECT 'AM', SUM(vrServicio) FROM @MEDICAMENTOS
     ...
     SELECT @VALORCOPAGOACU=COALESCE(SUM(valorPagoModerador),0) FROM @CONSULTAS
      ...
      IF ABS(@VALORCOPAGO-@VALORCOPAGOACU)>0 AND ABS(@VALORCOPAGO-@VALORCOPAGOACU)<10
      BEGIN
         PRINT 'INGRESO---@VALORCOPAGO'+CAST(@VALORCOPAGO AS VARCHAR(20))
         PRINT '@VALORCOPAGOACU='+CAST(@VALORCOPAGOACU AS VARCHAR(20))
         PRINT 'ABS(@VALORCOPAGO-@VALORCOPAGOACU) ='+CAST((ABS(@VALORCOPAGO-@VALORCOPAGOACU)) AS VARCHAR(20))
	...
	END
	PRINT 'FINALIZO EN SPK_RIPS_JSON_FTR_CAPMASI'
END
