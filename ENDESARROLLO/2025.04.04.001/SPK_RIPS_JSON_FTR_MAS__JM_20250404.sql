...
BEGIN
   ...
   PRINT 'CONSULTAS CIT'
		   ...
		   CASE WHEN COALESCE(CIT.VALORCOPAGO,0)>0 THEN  CASE WHEN COALESCE(AFI.TIPOUSUARIO,'') IN('01','02','03')  THEN '02' ELSE IIF(AFI.TIPOUSUARIO IN ('05','11'),'03','01') END ELSE @conceptoRecaudo END,
		   COALESCE(CIT.VALORCOPAGO,0), MED.TIPO_ID, MED.IDTERCERO   --STORRES 20250326
		   FROM  FTRD INNER JOIN CIT ON FTRD.NOADMISION=CIT.CONSECUTIVO
					  INNER JOIN AFI ON CIT.IDAFILIADO=AFI.IDAFILIADO
					  INNER JOIN SER ON CIT.IDSERVICIO=SER.IDSERVICIO
				      INNER JOIN MED ON CIT.IDMEDICO = MED.IDMEDICO --STORRES 20250326
     ...
     --                         AND CIT.CONSECUTIVO= COALESCE (HCA.CONSECUTIVOCIT,HCA.NOADMISION) ---- VGARCIA
        ...
        PRINT 'CONSULTAS AUT'
			...
			CASE WHEN COALESCE(AUTD.VALORCOPAGO,0)>0 OR COALESCE(AUT.VALORCOPAGO,0)> 0   THEN  CASE WHEN COALESCE(AFI.TIPOUSUARIO,'') IN('01','02','03')  THEN '02' ELSE IIF(AFI.TIPOUSUARIO IN('11','05'),'03','01') END ELSE @conceptoRecaudo END,
         ...
         PRINT 'CONSULTAS HADM...'
			...
			1,COALESCE(HPRED.VALOR,0),IIF(COALESCE(HPRED.VALORCOPAGO,0)>0,'01',@conceptoRecaudo),COALESCE(HPRED.VALORCOPAGO,0), MED.TIPO_ID, MED.IDTERCERO --STORRES 20250326
			FROM  FTRD INNER JOIN HADM ON FTRD.NOADMISION=HADM.NOADMISION  
               INNER JOIN  HPRE ON HADM.NOADMISION=HPRE.NOADMISION
					...
					INNER JOIN RIPS_CP ON SER.CODIGORIPS=RIPS_CP.IDCONCEPTORIPS
               INNER JOIN MED ON MED.IDMEDICO = CASE WHEN COALESCE(HPRE.IDMEDICO,'') = '' THEN COALESCE(HADM.IDMEDICOALTA,HADM.IDMEDICOTRA) ELSE HPRE.IDMEDICO END  --STORRES 20250326
			WHERE FTRD.CNSFTR=@CNSFCT
         ...
         PRINT 'REVISO CANTIDADES MAYORES A 1 CONSULTAS HADM'
			...
			codServicio=325,finalidadTecnologiaSalud=LEFT(CASE WHEN HPRE.FINALIDAD IS NULL OR HPRE.FINALIDAD='' OR HPRE.FINALIDAD='10'THEN '44' ELSE HPRE.FINALIDAD END,2),
			causaMotivoAtencion='38',COALESCE(HADM.DXINGRESO,HADM.DXEGRESO),COALESCE(HADM.DXSALIDA1,''),COALESCE(HADM.DXSALIDA2,''),COALESCE(HADM.DXSALIDA3,''),
		    1,COALESCE(HPRED.VALOR,0),IIF(COALESCE(HPRED.VALORCOPAGO,0)>0,'01',@conceptoRecaudo),COALESCE(HPRED.VALORCOPAGO,0),CONVERT(INT,HPRED.CANTIDAD), MED.TIPO_ID, MED.IDTERCERO --STORRES 20250326 
			FROM  FTRD INNER JOIN HADM ON FTRD.NOADMISION=HADM.NOADMISION  
               INNER JOIN  HPRE ON HADM.NOADMISION=HPRE.NOADMISION
					INNER JOIN HPRED ON HPRE.NOPRESTACION=HPRED.NOPRESTACION AND HPRED.NOITEM = FTRD.NOITEM
					INNER JOIN SER ON HPRED.IDSERVICIO=SER.IDSERVICIO AND FTRD.REFERENCIA = SER.IDSERVICIO
      ...
      BEGIN
		   ...
		   SELECT @IDPRESTADOR,AUT.IDAFILIADO,AUT.NOAUT,null,fechaDispensAdmon=REPLACE(CONVERT(VARCHAR,AUT.FECHA,102),'.','-')+' '+LEFT(CONVERT(VARCHAR,AUT.FECHA,108),5),
            ...
            CASE WHEN ISNUMERIC(dbo.FNK_LIMPIATEXTO(LEFT(COALESCE(ICCN.DESCRIPCION,'0'),3),'0-9'))=1 THEN dbo.FNK_LIMPIATEXTO(LEFT(COALESCE(ICCN.DESCRIPCION,'0'),3),'0-9') ELSE 0 END,
			   ...
			   CASE WHEN AUT.VALORCOPAGO = 0 THEN '05' WHEN COALESCE(AUTD.VALORCOPAGO,0)>0 OR COALESCE(AUT.VALORCOPAGO,0)> 0   THEN  CASE WHEN COALESCE(AFI.TIPOUSUARIO,'') IN('01','02','03')  THEN '02' ELSE '01' END ELSE @conceptoRecaudo END, -- STORRES_20250327
			   '04',CASE WHEN AUT.VALORCOPAGO = 0 THEN 0 ELSE AUTD.VALORCOPAGO END,@N_FACTURA -- STORRES_20250327
     ...
     PRINT 'PROCEDIMIENTOS'
		...
		SELECT @IDPRESTADOR,CIT.IDAFILIADO,CIT.CONSECUTIVO,REPLACE(CONVERT(VARCHAR,CIT.FECHA,102),'.','-')+' '+LEFT(CONVERT(VARCHAR,CIT.FECHA,108),5),null,
				COALESCE(CIT.NOAUTORIZACION,'null'),LEFT(SER.CODCUPS,6),'02','01','02',325,'16',MED.TIPO_ID,MED.IDTERCERO,CIT.IDDX, --STORRES 20250326
