...
DECLARE @CODCAJA VARCHAR(20), @OK SMALLINT, @CNSACJ VARCHAR(20)	,@VALORTOTAL DECIMAL(14,2)	,@CONTADOR INT	,@DESCSERVICIO VARCHAR(200)
		,@IDALTERNA VARCHAR(50)
DECLARE @ITEM_AUTD INT		, @LIMITE_AU INT ,@IDPLAN VARCHAR(7)
BEGIN   
   SET DATEFORMAT dmy
   --SELECT @JSON
   SELECT *
   ...
   END
	...
	BEGIN
	   ...
	   WITH		 (
		...
		,OBSERVACION		VARCHAR(200)	'$.OBSERVACION', IDPLAN VARCHAR(7) '$.IDPLAN'	)
		...
		BEGIN TRY 
            ...
            ,GENFACTURA = CASE WHEN @IDTIPOMOV=DBO.FNK_VALORVARIABLE('IDINVMOVVENTAFACTURA') THEN 1 ELSE 0 END
            ,IDPLAN = #datoIMOV_SAL.IDPLAN	
            ...
            ,IDPLAN
               ...
               ,CASE WHEN @IDTIPOMOV=DBO.FNK_VALORVARIABLE('IDINVMOVVENTAFACTURA') THEN 1 ELSE 0 END
               ,#datoIMOV_SAL.IDPLAN
				FROM #datoIMOV_SAL
			END
		END TRY
...
--QUERY3
   ...
   BEGIN         
      ...
      IF EXISTS(SELECT * FROM IMOV WHERE CNSMOV=@CNSMOV AND ESTADO=1 AND COALESCE(GENFACTURA,0)=1 AND COALESCE(FACTURADA,0)=0)
      BEGIN
         BEGIN TRY           
      ...
      BEGIN
         INSERT INTO @TBLERRORES(ERROR)
         SELECT 'Movimiento no esta preparado para facturar, Verifique e intente de nuevo'
      END
      IF(SELECT COUNT(*) FROM @TBLERRORES)>0
      BEGIN
         ...
         RETURN
      END
      SELECT 'OK' OK,N_FACTURA FROM IMOV WHERE CNSMOV=@CNSMOV
      RETURN 
   END 
   IF @METODO='ITRAH_REGRESAR'     
   BEGIN         
      ...
      IF EXISTS (SELECT * FROM IMOV WHERE CNSITRA=@CNSITRA AND ESTADO<>2)
      BEGIN
         INSERT INTO @TBLERRORES(ERROR)
         SELECT 'este transito ya fue traido, no se puede regresar '
      END
      RETURN                    
   END  
END
