         ...
         BEGIN
            ...
            SELECT 'OK'OK,FTR.N_FACTURA,TER.TIPO_ID,TER.NIT,TER.DV,TER.RAZONSOCIAL,TER.IDALTERNA2,TRIM(TER.DIRECCION) DIRECCION,COALESCE(TER.TELEFONOS,'')TELEFONOS,
                  ...
                  --dbo.FNK_MontoEscrito(FTR.VR_TOTAL)LETRAS,
                   ...
                   @ENTERO AS ENTERO,CASE TIPOFIN WHEN 'N' THEN 'B' ELSE 'F' END TIPOFACT,
                    ...
                    FTR.OBSERVACION OBSERVACIONES,COALESCE(EMP.RAZONSOCIAL,'') AS COMPANIA
            ...
            BEGIN
               ...
               BEGIN
                  IF  @PROCEDENCIA='MAD'
                  BEGIN
                     SELECT MED.NOMBRE,MES.DESCRIPCION,MED.NO_REGISTRO,MAD.NOAUTORIZACION,DBO.FNK_FECHA_DDMMAA(MAD.FECHA)FECHAING,
                            DBO.FNK_FECHA_DDMMAA(CIT.FECHA)FECHAEGR,MAD.PROCEDENCIA,''IDDX,''DESCMDX
...
--                     ROUND(CASE WHEN COALESCE(PIVA,0)>0 AND COALESCE(FTRD.VLR_COPAGOS,0)>0 THEN (FTRD.VLR_COPAGOS/(1+(PIVA/100))) ELSE  COALESCE(FTRD.VLR_COPAGOS,0) END,2) COPA_SINIGV,
            ...
            BEGIN
               IF @TIPOFAC='I'
               BEGIN
					--PRINT 'INGRESE PARA FECHA FINAL DE PAD'
					--DECLARE @FAUX01 DATE, @PLAAUX01 VARCHAR(10), @IDPLANAUX01 VARCHAR(20)
					--SELECT @FAUX01 = CONVERT(VARCHAR,EOMONTH(AUT.FECHA),103 ) 
					--      ,@PLAAUX01= PLN.TIPOPLAN 
					--	  ,@IDPLANAUX01 = AUT.IDPLAN
				 --  FROM AUT LEFT JOIN PLN ON AUT.IDPLAN = PLN.IDPLAN WHERE NOAUT = @NOREFERENCIA
					--PRINT @FAUX01
					--PRINT @PLAAUX01
					--PRINT @IDPLANAUX01
					
                  SELECT COALESCE(MED.NOMBRE,'')NOMBRE,COALESCE(MES.DESCRIPCION,'')DESCRIPCION,COALESCE(AUT.NUMAUTORIZA,'')NOAUTORIZACION
				  ,CONVERT(VARCHAR,AUT.FECHA,103) FECHAING
			     ...
			     ELSE
                  ...
                  FTRD.VALOR_COPAGO_TOTAL_SIN_IGV
						FROM FTRD INNER JOIN SER ON FTRD.REFERENCIA=SER.IDSERVICIO
									INNER JOIN PRE ON FTRD.PREFIJO=PRE.PREFIJO
						WHERE N_FACTURA=@N_FACTURA
						AND 1=CASE WHEN DBO.FNK_VALORVARIABLE('ANEXOIMPNOCOBRABLE')='SI' THEN 1 ELSE CASE WHEN  FTRD.VALOR>0 THEN 1 ELSE 0 END END
						ORDER BY CASE WHEN DBO.FNK_VALORVARIABLE('ANEXOIMPNOCOBRABLE')='SI' THEN FTRD.VR_TOTAL ELSE PRE.PREFIJO END DESC ,SER.IDSERVICIO
					END
				END
            END
            IF @PROCEDENCIA='CAJA'
