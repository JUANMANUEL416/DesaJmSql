...
PLN.DESCPLAN,CIT.IDSERVICIO,SER.DESCSERVICIO,SER.PREFIJO,PRE.NOM_PREFIJO,TER.NIT,TER.RAZONSOCIAL,'1' CANTIDAD,
...
0 RESUL,SED.NIT AS PROVEEDOR,SED.DESCRIPCION AS N_PROVEEDOR
...
CIT.IDMEDICO,MED.NOMBRE,MES.DESCRIPCION,CIT.CLASEORDEN,CIT.IDPLAN,
PLN.DESCPLAN,AUTD.IDSERVICIO,SER.DESCSERVICIO,SER.PREFIJO,PRE.NOM_PREFIJO,TER.NIT,TER.RAZONSOCIAL,AUTD.CANTIDAD,
COALESCE(AUTD.VALOR,0)VALOR,COALESCE(AUTD.VALORCOPAGO,0)VALORCOPAGO,
COALESCE(AUTD.VALOREXCEDENTE,0),
COALESCE(CIT.FACTURADA,0) FACTURADA,AUTD.N_FACTURA,AUTD.NFACTURA AS NBOLETA,COALESCE(AUT.ESPROGRAMAS,0),AUTD.PROTOCOLO,
CASE WHEN COALESCE(AUTD.ESDELAB,0)=1 THEN CASE WHEN COALESCE(AUTD.ENLAB,0)=1 THEN
CASE WHEN EXISTS(SELECT * FROM  LING INNER JOIN LORD ON LING.NOINGRESO=LORD.NOINGRESO
WHERE LING.NOPRESTACION=AUT.NOAUT AND LORD.ESTADO='R') THEN 1 ELSE 2 END ELSE 3 END ELSE 0 END RESUL,AUT.IDFARMACIA ,TGEN.DESCRIPCION AS DISTRIBUIDOR 
FROM [dbo].[CIT] INNER JOIN  AFI ON CIT.IDAFILIADO=AFI.IDAFILIADO 
      INNER JOIN  MED ON CIT.IDMEDICO=MED.IDMEDICO 
      INNER JOIN TER ON CIT.IDTERCEROCA=TER.IDTERCERO 
      INNER JOIN PLN ON CIT.IDPLAN=PLN.IDPLAN 
