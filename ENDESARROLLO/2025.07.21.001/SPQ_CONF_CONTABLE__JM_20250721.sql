...
DECLARE  @PARAMETROS NVARCHAR(MAX)			,@MODELO VARCHAR(100)			   ,@METODO VARCHAR(100)
      ...
      ,@CODDISTRIBUCION VARCHAR(20)       ,@CODDISTRIBUCIONANT VARCHAR(20) ,@CUENTA VARCHAR(20)
      ,@VALOR DECIMAL(7,2)                ,@EXTRAMURAL BIT                 ,@IDBODEGACUSTODIA VARCHAR(20)
      ,@IDVARIABLE VARCHAR(20)            ,@DATO VARCHAR(20)               ,@MCUEORDEN_GLOSAS  VARCHAR(20)
      ,@CTA_CAJ_REC_NOLEG  VARCHAR(20)    ,@CIERRECOSTOS  BIT              ,@CIERREIMP  BIT
      ,@MESDECIERRE SMALLINT
BEGIN
	SELECT *
	INTO #JSON
   ...
   BEGIN         
      ...
      SELECT @IDSEDE=IDSEDE, @DESCRIPCION=DESCRIPCION,@DIRECCION=DIRECCION,@TELEFONOS=TELEFONOS,@CIUDAD=CIUDAD,@RESPONSABLE=RESPONSABLE,
             ...
             @MEFARMACIA=CASE COALESCE(MEFARMACIA,'') WHEN 'true' THEN 1 ELSE 0 END,@PROCESO=PROCESO,@EXTRAMURAL=CASE COALESCE(EXTRAMURAL,'') WHEN 'true' THEN 1 ELSE 0 END,
             @IDBODEGACUSTODIA=IDBODEGACUSTODIA
      FROM   OPENJSON (@DATOS)
      WITH   ( 
      ...
      BEGIN
         ...
         BEGIN
            INSERT INTO SED (IDSEDE, DESCRIPCION, DIRECCION, TELEFONOS, CIUDAD, IDREGIONAL, RESPONSABLE, MONTO_MAX, MONTO_MIN, 
                              ...
                              IDBODHEMO, IDSGSSS, IPS, MEFARMACIA, CRONICO, EXTRAMURAL, EMAIL,IDBODEGACUSTODIA)
           SELECT @IDSEDE, @DESCRIPCION, @DIRECCION, @TELEFONOS, @CIUDAD, NULL, @RESPONSABLE, 0, 0, 
                              ...
                              @IDBODHEMO, @IDSGSSS, @IPS, @MEFARMACIA, 0, @EXTRAMURAL, NULL,@IDBODEGACUSTODIA
      ...
      BEGIN
         ...
         BEGIN
            UPDATE SED SET DESCRIPCION=@DESCRIPCION, DIRECCION=@DIRECCION, TELEFONOS=@TELEFONOS, CIUDAD=@CIUDAD,RESPONSABLE=@RESPONSABLE,
                           ...
                           CODHABILITA =@CODHABILITA,IDBODHEMO=@IDBODHEMO, IDSGSSS=@IDSGSSS, IPS=@IPS, MEFARMACIA=@MEFARMACIA,EXTRAMURAL=@EXTRAMURAL,
                           IDBODEGACUSTODIA=@IDBODEGACUSTODIA
            WHERE IDSEDE=@IDSEDE                          
         END
         ELSE
   ...
   BEGIN         
      ...
      BEGIN
         ...
         BEGIN
            --SELECT DBO.FNK_COLUMNAS_TABLAS('AFU')   ---20250710 -- STORRES -- SE COMENTARIZA SELECT PARA EVITAR MENSAJE DE ERROR
   ...
   BEGIN         
      ...
      IF @PROCESO='Nuevo'
      BEGIN
         IF NOT EXISTS(SELECT * FROM HARE WHERE IDAREA=@IDAREA)
         BEGIN
      ...
      IF @PROCESO='Editar'
      BEGIN
         IF EXISTS(SELECT * FROM HARE WHERE IDAREA=@IDAREA)
         BEGIN
            UPDATE HARE SET DESCRIPCION=@DESCRIPCION WHERE IDAREA=@IDAREA
         END
         ELSE
         BEGIN
            INSERT INTO @TBLERRORES(ERROR)
   ...
   BEGIN         
      ...
      IF @PROCESO='Nuevo'
      BEGIN
         IF NOT EXISTS(SELECT * FROM KUNG WHERE CODUNG=@CODUNG)
         BEGIN
      ...
      IF @PROCESO='Editar'
      BEGIN
         IF EXISTS(SELECT * FROM KUNG WHERE CODUNG=@CODUNG)
         BEGIN
            UPDATE KUNG SET DESCRIPCION=@DESCRIPCION,LOGO=@LOGO WHERE CODUNG=@CODUNG
         END
         ELSE
         BEGIN
            INSERT INTO @TBLERRORES(ERROR)
   ...
   BEGIN         
      ...
      FROM   OPENJSON (@PARAMETROS)
      WITH (           
         CODDISTRIBUCION  VARCHAR(20)   '$.CODDISTRIBUCION'
      )
      SELECT 'OK'OK, COALESCE(SUM(COALESCE(VALOR,0)),0) VALOR
      FROM FDISD WHERE CODDISTRIBUCION=@CODDISTRIBUCION
      RETURN                  
   END  
   IF @METODO='FDIS_CRUD'     
   BEGIN         
      ...
      WITH   (   
      CODDISTRIBUCION  VARCHAR(20)   '$.CODDISTRIBUCION',
      CODDISTRIBUCIONANT  VARCHAR(20)   '$.CODDISTRIBUCIONANT',
      DESCRIPCION  VARCHAR(61)   '$.DESCRIPCION',
      PROCESO  VARCHAR(20)   '$.PROCESO'
      )    

      IF @PROCESO='Nuevo'
      BEGIN
         IF NOT EXISTS(SELECT * FROM FDIS WHERE CODDISTRIBUCION=@CODDISTRIBUCION)
         BEGIN
      ...
      BEGIN
         ...
         BEGIN
            IF EXISTS(SELECT * FROM FDIS WHERE CODDISTRIBUCION=@CODDISTRIBUCION)
            BEGIN
         ...
         ELSE
         BEGIN
            UPDATE FDIS SET CODDISTRIBUCION=@CODDISTRIBUCION,DESCRIPCION=@DESCRIPCION WHERE CODDISTRIBUCION=@CODDISTRIBUCIONANT
         END
      END
      IF @PROCESO='Borrar'
      BEGIN
         IF EXISTS(SELECT * FROM FDISD WHERE CODDISTRIBUCION=@CODDISTRIBUCION)
         BEGIN
            INSERT INTO @TBLERRORES(ERROR)
         ...
         ELSE
         BEGIN
            DELETE FDIS WHERE CODDISTRIBUCION=@CODDISTRIBUCION
         END
      END
      IF(SELECT COUNT(*) FROM @TBLERRORES)>0
      BEGIN
   ...
   BEGIN         
      ...
      END
      IF NOT EXISTS(SELECT * FROM CUE WHERE CUENTA=@CUENTA AND TIPO='Detalle')
      BEGIN
         INSERT INTO @TBLERRORES(ERROR)
      ...
      IF @PROCESO='Editar'
      BEGIN
         UPDATE FDISD SET VALOR=@VALOR WHERE CODDISTRIBUCION=@CODDISTRIBUCION AND CCOSTO=@CCOSTO AND IDAREA=@IDAREA AND CUENTA=@CUENTA
      END
      IF @PROCESO='Borrar'
      BEGIN
         DELETE FDISD WHERE CODDISTRIBUCION=@CODDISTRIBUCION AND CCOSTO=@CCOSTO AND IDAREA=@IDAREA AND CUENTA=@CUENTA
      END
      SELECT 'OK'OK, COALESCE(SUM(COALESCE(VALOR,0)),0) VALOR
      FROM FDISD WHERE CODDISTRIBUCION=@CODDISTRIBUCION
      RETURN  
   ...
   BEGIN         
      ...
      END
      IF NOT EXISTS(SELECT * FROM COM WHERE COMPROBANTE=@DATO)
      BEGIN
         INSERT INTO @TBLERRORES(ERROR)
      ...
      END
      BEGIN TRY           
         UPDATE USVGS SET DATO=@DATO WHERE IDVARIABLE=@IDVARIABLE          
      END TRY
      BEGIN CATCH
   ...
   BEGIN         
      ...
      SELECT @MESDECIERRE =MESDECIERRE,  @CIERRECOSTOS =CASE WHEN COALESCE(CIERRECOSTOS,'false')='false' THEN 0 ELSE 1 END ,  @CIERREIMP =CASE WHEN COALESCE(CIERREIMP,'false')='false' THEN 0 ELSE 1 END ,
             @CTA_CAJ_REC_NOLEG =CTA_CAJ_REC_NOLEG, @MCUEORDEN_GLOSAS =MCUEORDEN_GLOSAS
      FROM   OPENJSON (@DATOS)
      WITH   (  
      MESDECIERRE  SMALLINT   '$.MESDECIERRE',
      CIERRECOSTOS  VARCHAR(20)   '$.CIERRECOSTOS',
      CIERREIMP  VARCHAR(20)   '$.CIERREIMP',
      CTA_CAJ_REC_NOLEG  VARCHAR(20)   '$.CTA_CAJ_REC_NOLEG',
      MCUEORDEN_GLOSAS  VARCHAR(20)   '$.MCUEORDEN_GLOSAS'         
      ) 
      IF EXISTS(SELECT * FROM CUE WHERE CUENTA=@CTA_CAJ_REC_NOLEG AND TIPO='Detalle')
      BEGIN
         UPDATE PAR SET CTA_CAJ_REC_NOLEG=@CTA_CAJ_REC_NOLEG,MESDECIERRE=@MESDECIERRE,CIERRECOSTOS=@CIERRECOSTOS,
         CIERREIMP=@CIERREIMP,MCUEORDEN_GLOSAS=@MCUEORDEN_GLOSAS WHERE COMPANIA='01'
      END
      ELSE
      BEGIN
         INSERT INTO @TBLERRORES(ERROR)
