...
DECLARE @IDPLAN VARCHAR(10),@CONCEPTORECAUDOICI VARCHAR(20)
BEGIN
   ...
   */ --Se conoce que los partoculares SI SE REPORTAN STORRES_20250403
   ...
   SELECT @CONCEPTORECAUDOICI = DATO1 FROM TGEN WHERE TABLA = 'RIPS_JSON' AND CAMPO = 'conceptoRecaudo' AND CODIGO = 'ICI' 
	...
	BEGIN	
		...
		BEGIN
			SELECT @fechaInicioAtencion=REPLACE(CONVERT(VARCHAR,MIN(FECHA),102),'.','-')+' '+LEFT(CONVERT(VARCHAR,MIN(FECHA),108),5) FROM HPRE WHERE NOADMISION=@NOADMISION 
		END
		ELSE
		BEGIN
			SELECT @fechaInicioAtencion=REPLACE(CONVERT(VARCHAR,HADM.FECHA,102),'.','-')+' '+LEFT(CONVERT(VARCHAR,HADM.FECHA,108),5) FROM HADM WHERE NOADMISION=@NOADMISION
		END
		SELECT @fechaEgreso=REPLACE(CONVERT(VARCHAR,HADM.FECHAALTAMED,102),'.','-')+' '+LEFT(CONVERT(VARCHAR,HADM.FECHAALTAMED,108),5) FROM HADM WHERE NOADMISION=@NOADMISION 				
		IF 1=1
		BEGIN --AC
