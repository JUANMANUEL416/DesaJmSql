CREATE OR ALTER PROCEDURE DBO.SPK_INSERT_TERDEAFI  
    ...
    BEGIN
       ...
       SET @COD_RESP_FISCAL=DBO.FNK_VALORVARIABLE('DEFAULT_PART_RF')
        ...
        INSERT INTO TER (
            ...
            PNOMBRE, SNOMBRE, PAPELLIDO, SAPELLIDO,RUC, EMAILRECIBOFE
        )  
        SELECT @IDTERCERO, NOMBREAFI, DOCIDAFILIADO, DBO.FNK_CALCULA_DV(DOCIDAFILIADO), TIPO_DOC,
            ...
            PNOMBRE, SNOMBRE, PAPELLIDO, SAPELLIDO,RUC, COALESCE(AFI.EMAIL,'')
        FROM AFI 
        WHERE IDAFILIADO=@IDAFILIADO

        UPDATE AFI SET RUC=@IDTERCERO WHERE IDAFILIADO=@IDAFILIADO
    END
    IF EXISTS(SELECT * FROM TER WHERE IDTERCERO=@IDTERCERO AND TIPO_ID='RUC') AND DBO.FNK_VALORVARIABLE('IXCOUNTRY')='PERU'
    BEGIN
       PRINT 'VALIDO DE NUEVO'
       UPDATE TER SET NIT=CASE WHEN  LEN(NIT)<=8 THEN 'DNI' WHEN LEN(NIT)=11 THEN 'RUC' WHEN LEN(NIT)=12 AND TIPO_ID NOT IN('CE','PS') THEN 'PS'  ELSE 'PS' END
