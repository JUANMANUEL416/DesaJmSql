...
DECLARE @PROCESO VARCHAR(20)              ,@CNSFNOT VARCHAR(20)            ,@CLASE VARCHAR(20)
      ...
      ,@VIVA DECIMAL(14,2)               ,@VLR_ITEM DECIMAL(14,2)
BEGIN
	SELECT *
	INTO #JSON
   ...
   BEGIN         
      ...
      BEGIN
         SELECT 'KO'OK,'Nota No Encontrada, Revise e intente de nuevo'ERROR
         RETURN
      END
      ...
      IF @PROCESO='Nuevo'
      BEGIN
         IF EXISTS(SELECT * FROM FNOTD WHERE CNSFNOT=@CNSFNOT AND N_FACTURA=@N_FACTURA AND N_CUOTA=@N_CUOTA)
         BEGIN
            SELECT 'KO'OK,'Este item ya esta Asociado a esta Nota, No se permiten items Duplicados'ERROR
            RETURN            
         END
      ...
      BEGIN
         ...
         BEGIN
            ...
            BEGIN
               IF EXISTS(SELECT * FROM FTRD WHERE N_FACTURA=@N_FACTURA AND N_CUOTA=@N_CUOTA 
                         AND VLR_SERVICI-CASE WHEN @IXCOUNTRY='PERU' THEN COALESCE(VLR_COPAGOS,0) ELSE 0 END < @VR_TOTAL)
               BEGIN               
                  SELECT 'KO'OK, 'Inconsistencia entre el valor del item de la facura vs item de la nota cr; Verifique e intente de nuevo 'ERROR
                  RETURN
               END
               UPDATE FTRD SET VLRNOTACR=@VR_TOTAL WHERE N_FACTURA=@N_FACTURA AND N_CUOTA=@N_CUOTA
            END
         ...
         BEGIN
            PRINT 'Borrando item'
            SELECT @N_fACTURA=N_FACTURA,@N_CUOTA=N_CUOTA FROM FNOTD WHERE CNSFNOT=@CNSFNOT AND CLASE=@CLASE AND ITEM=@ITEM
            IF EXISTS(SELECT * FROM FTRD WHERE N_FACTURA=@N_FACTURA AND N_CUOTA=@N_CUOTA)
            BEGIN
               UPDATE FTRD SET VLRNOTACR=0 WHERE N_FACTURA=@N_FACTURA AND N_CUOTA=@N_CUOTA
            END
