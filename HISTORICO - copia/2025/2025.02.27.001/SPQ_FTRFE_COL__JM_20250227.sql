...
BEGIN
	   ...
	   BEGIN
         ...
         BEGIN
            ...
            SELECT @F_ENVIO=CONVERT(VARCHAR,FECHAENVIO,112) FROM RIPS WHERE ENVIO=@ENVIO
		      SELECT 'OK'OK,'ddig_'+NIT+'_'+COALESCE(IDALTERNA2,NIT)+'_'+LTRIM(RTRIM(@ENVIO))+'_'+LTRIM(RTRIM(@F_ENVIO)) AS N_SOPORTE 
   ...
   BEGIN         
      ...
      IF EXISTS(SELECT * FROM FTR WHERE N_FACTURA=@N_FACTURA AND (PROCEDENCIA='CAJA' OR IDPLAN IN(SELECT DATO FROM USVGS WHERE IDVARIABLE LIKE 'IDPLANPART%')))
      BEGIN
