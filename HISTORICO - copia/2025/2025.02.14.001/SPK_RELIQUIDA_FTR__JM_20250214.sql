...
DECLARE @NTDBAJUSTE DECIMAL(14,2)
BEGIN 
   SET @VLRCESIONES=0                                                               
   SET @VLRCESION=0                                                               
   ...
   IF NOT EXISTS(SELECT  * FROM FCXCDV WHERE N_FACTURA=@N_FACTURA AND CNSCXC=@CNSCXC  AND TIPO<>'F')
   BEGIN
      SELECT @TOTALND =@TOTALND+ SUM(FNOT.VR_TOTAL)
      FROM   FNOT
      ...
      AND    FNOT.ESTADO='O'
      PRINT 'NOTAS DEBITO AJUSTES '+CAST(COAlESCE(@TOTALND,0) AS VARCHAR(20))
   END
   IF @TOTALND IS NULL  
      SELECT @TOTALND = 0 
   ...
   BEGIN 
      ...
      PRINT 'VALIDO NOTAS CREDITO DESPUES DE CONCILIACION....'
      ...
      BEGIN
         ...
         IF NOT EXISTS(SELECT  * FROM FCXCDV WHERE N_FACTURA=@N_FACTURA AND CNSCXC=@CNSCXC  AND TIPO<>'F') OR(COALESCE(@CNSGLO,'')='')
         BEGIN
            SELECT @VRNOTASD =COALESCE(@VRNOTASD,0)+ SUM(FNOT.VR_TOTAL)
            FROM   FNOT
            WHERE  FNOT.N_FACTURA   = @N_FACTURA  
            AND    FNOT.CERRADA     = 1 
            AND    FNOT.CLASE       = 'D'    
      ...
      BEGIN
         ...
         BEGIN
            ...
            SELECT @NTDBAJUSTE = COALESCE(SUM(FNOT.VR_TOTAL),0)
            FROM   FNOT
            WHERE  FNOT.N_FACTURA   = @N_FACTURA  
            AND    FNOT.CERRADA     = 1 
            AND    FNOT.CLASE       = 'D'    
            ...
            SELECT @VLRRECUPERAR = VLRRECUPERAR+COALESCE(@NTDBAJUSTE,0) FROM FCONCID 
                   WHERE  CNSCXC = @CNSCXC AND N_FACTURA = @N_FACTURA 
				   AND IDFCONCI = @IDFCONCI 
                   AND COALESCE(CNSGLO,'')=COALESCE(@CNSGLO,'')
				   AND ESTADO='Cerrada'
            PRINT '@IDFCONCI='+@IDFCONCI+'//@SALDOINICIAL='+CONVERT(VARCHAR,@SALDOINICIAL)+'//@VLRRECUPERAR= '+CONVERT(VARCHAR,@VLRRECUPERAR)+' //@CNSGLO'+COALESCE(@CNSGLO,'NO GLOSA')+'@NTDBAJUSTE ='+CAST(@NTDBAJUSTE AS VARCHAR(20))
           ...
           -- SELECT @VRPAGOS = @VRPAGOS + COALESCE(@VRPAGOS1,0), @VRDEDUC = @VRDEDUC + COALESCE(@VRDEDUC1,0)
			...
			PRINT 'VALOR DEL PAGO:'+LTRIM(RTRIM(STR(@VRPAGOS + @VRDEDUC+@VRNOTAS)))
            UPDATE FCXCDV 
