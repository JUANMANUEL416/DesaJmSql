...
DECLARE @TIPOCIERRE VARCHAR(20)
DECLARE @TOTALCR  DECIMAL(14,2)
DECLARE @TOTALDB  DECIMAL(14,2)
DECLARE @TOTALTRAS DECIMAL(14,2)
DECLARE @TOTALAPER DECIMAL(14,2)
BEGIN  
   PRINT 'EMPIEZO'
   ...
   BEGIN
      PRINT 'Es de caja menor no se contabiliza'
      BEGIN
   ...
   SELECT @TIPOCIERRE='00'
   IF EXISTS(SELECT * FROM FCJ INNER JOIN PCJ ON FCJ.CODCAJA=PCJ.CODCAJA AND FCJ.CNSFACJ=PCJ.CNSFACJ
              ...
              WHERE  FCJ.CNSACJ=@CNSACJ   
                     ...
                     AND FPA.FISICO=1
                     AND FPA.USAR_CUENTACAJA=1
                     AND COALESCE(FCJ.VALORTOTAL,0)>0
                     )
   BEGIN
      IF NOT EXISTS(SELECT * FROM FCJ INNER JOIN PCJ ON FCJ.CODCAJA=PCJ.CODCAJA AND FCJ.CNSFACJ=PCJ.CNSFACJ
                                INNER JOIN FPA ON PCJ.TIPOPAGO=FPA.FORMAPAGO
              WHERE  FCJ.CNSACJ=@CNSACJ   
                     AND FCJ.DEAPERTURA=0   
      ...
      BEGIN
         PRINT 'Tiene movimientos de ingreso, pero cerro la caja sin trasladar'
         SET @TIPOCIERRE='01' --CIERRE SIN TRASLADO
      END
      ELSE
      BEGIN
         PRINT 'Tiene movimientos de ingreso y de egreso'
         SET @TIPOCIERRE='02'
      END
   END
   ELSE
   BEGIN
      ...
      BEGIN 
         PRINT 'No tiene movimientos de ingreso pero si de salida '
         SET @TIPOCIERRE='03'
      END
   END
   IF @TIPOCIERRE='00'
   BEGIN
      PRINT 'Cierre de caja sin movimientos, me regreso'
      IF COALESCE(@NROCOMPROBANTE,'')<>''
      BEGIN
         DELETE MCHE WHERE NROCOMPROBANTE=@NROCOMPROBANTE
         DELETE MCPE WHERE NROCOMPROBANTE=@NROCOMPROBANTE
   ...
   SELECT  @NROCOMPROBANTE,MCH.COMPANIA,'CR',MCH.CUENTA,MCH.IDTERCERO,MCH.CCOSTO,
           'RECLASIFICACION DE TERCERO Y DOCUMENTO  CIERE DE CAJA',VALOR,MCH.REFERENCIA1,    
                       MCH.REFERENCIA2,MCH.REFERENCIA3,ITEMREF,@USUARIO,DBO.FNK_GETDATE(),PREFIJO,IDAREA,MCH.CLASECONT,1,'CAJA_CIERRE',  
                       CNSPAGO,MCH.N_FACTURA,DBO.FNK_GETDATE(),DBO.FNK_GETDATE(),GENERA,MCH.PROCEDENCIA, REFERENCIA_PRO, MCH.CODUNG, MCH.CODPRG
   ...
   PRINT 'Valido los traslado de caja -- recepcion de traslados'
   IF EXISTS(SELECT * FROM FCJ WHERE CNSACJ=@CNSACJ AND DEAPERTURA=2 AND CERRADA=1 AND ESTADO<>'A')
   BEGIN
      INSERT INTO MCHE(NROCOMPROBANTE,COMPANIA,TIPO,CUENTA,IDTERCERO,CCOSTO,DETALLE,VALOR,REFERENCIA1,    
                        REFERENCIA2,REFERENCIA3,ITEMREF,USUARIO,FECHA,PREFIJO,IDAREA,CLASECONT,ESTADO,PROCESO,  
                        CNSPAGO,N_FACTURA,F_FACTURAREF,F_VENCE,GENERA, PROCEDENCIA, REFERENCIA_PRO, CODUNG, CODPRG) 
      SELECT  @NROCOMPROBANTE,'01','CR',CAJ.CUENTA,FCJ.CODCAJA,NULL,
               'RECLASIFICACION DE TERCERO Y DOCUMENTO  RECEPCION DE TRASLADO CIERE DE CAJA',FCJ.VALORTOTAL,FCJ.CNSACJ,    
                           FCJ.CNSFACJ,FCJ.CODCAJA,1,@USUARIO,DBO.FNK_GETDATE(),FCJ.TIPOEGRESO,NULL,'S',1,'CAJA_CIERRE',  
                           @CNSACJ,FCJ.CNSFACJ,DBO.FNK_GETDATE(),DBO.FNK_GETDATE(),'Movimiento','CAJA', @CNSACJ, '', ''
      FROM FCJ INNER JOIN CAJ ON FCJ.CODCAJA=CAJ.CODCAJA 
      WHERE FCJ.CNSACJ=@CNSACJ
      AND FCJ.DEAPERTURA=2
      AND FCJ.CLASE_FAC='COBRO'    
   END

   PRINT 'Valido Apertura de Caja --'
   IF EXISTS(SELECT * FROM FCJ WHERE CNSACJ=@CNSACJ AND DEAPERTURA=1 AND CERRADA=1 AND ESTADO<>'A' AND COALESCE(VALORTOTAL,0)>0)
   BEGIN
      INSERT INTO MCHE(NROCOMPROBANTE,COMPANIA,TIPO,CUENTA,IDTERCERO,CCOSTO,DETALLE,VALOR,REFERENCIA1,    
                        REFERENCIA2,REFERENCIA3,ITEMREF,USUARIO,FECHA,PREFIJO,IDAREA,CLASECONT,ESTADO,PROCESO,  
                        CNSPAGO,N_FACTURA,F_FACTURAREF,F_VENCE,GENERA, PROCEDENCIA, REFERENCIA_PRO, CODUNG, CODPRG) 
      SELECT  @NROCOMPROBANTE,'01','CR',CAJ.CUENTA,FCJ.CODCAJA,NULL,
               'RECLASIFICACION APERTURA  DE TERCERO Y DOCUMENTO   CIERE DE CAJA',FCJ.VALORTOTAL,FCJ.CNSACJ,    
                           FCJ.CNSFACJ,FCJ.CODCAJA,1,@USUARIO,DBO.FNK_GETDATE(),FCJ.TIPOEGRESO,NULL,'S',1,'CAJA_CIERRE',  
                           @CNSACJ,FCJ.CNSFACJ,DBO.FNK_GETDATE(),DBO.FNK_GETDATE(),'Movimiento','CAJA', @CNSACJ, '', ''
      FROM FCJ INNER JOIN CAJ ON FCJ.CODCAJA=CAJ.CODCAJA 
      WHERE FCJ.CNSACJ=@CNSACJ
   ...
   SELECT @TOTALCR=SUM(VALOR)
   FROM MCHE WHERE NROCOMPROBANTE=@NROCOMPROBANTE

   PRINT '@TOTALCR ='+STR(COALESCE(@TOTALCR,0))

   PRINT 'Voy por los Debitos'

   INSERT INTO MCHE(NROCOMPROBANTE,COMPANIA,TIPO,CUENTA,IDTERCERO,CCOSTO,DETALLE,VALOR,REFERENCIA1,    
                     REFERENCIA2,REFERENCIA3,ITEMREF,USUARIO,FECHA,PREFIJO,IDAREA,CLASECONT,ESTADO,PROCESO,  
                     CNSPAGO,N_FACTURA,F_FACTURAREF,F_VENCE,GENERA, PROCEDENCIA, REFERENCIA_PRO, CODUNG, CODPRG) 
   SELECT  @NROCOMPROBANTE,MCH.COMPANIA,'DB',MCH.CUENTA,MCH.IDTERCERO,MCH.CCOSTO,
            'RECLASIFICACION DE TERCERO Y DOCUMENTO  CIERE DE CAJA',VALOR,MCH.REFERENCIA1,    
   ...
   SELECT @TOTALDB=SUM(VALOR)
   FROM MCHE WHERE NROCOMPROBANTE=@NROCOMPROBANTE AND TIPO='DB'

   PRINT '@TOTALDB ='+STR(COALESCE(@TOTALDB,0))

   IF COALESCE(@TOTALDB,0)-COALESCE(@TOTALCR,0) < 0
   BEGIN
      PRINT 'los movimientos de Egreso no superan el valor de los ingreso, se los Asocio a la Caja, para cuando se abra'
      SELECT @TOTALTRAS=ABS(COALESCE(@TOTALDB,0)-COALESCE(@TOTALCR,0))
      INSERT INTO MCHE(NROCOMPROBANTE,COMPANIA,TIPO,CUENTA,IDTERCERO,CCOSTO,DETALLE,VALOR,REFERENCIA1,    
                        REFERENCIA2,REFERENCIA3,ITEMREF,USUARIO,FECHA,PREFIJO,IDAREA,CLASECONT,ESTADO,PROCESO,  
                        CNSPAGO,N_FACTURA,F_FACTURAREF,F_VENCE,GENERA, PROCEDENCIA, REFERENCIA_PRO, CODUNG, CODPRG) 
      SELECT  @NROCOMPROBANTE,'01','DB',CAJ.CUENTA,FCJ.CODCAJA,NULL,
               'VALORES NO TRASLADADOS SALDO EN CAJA   CIERE DE CAJA',@TOTALTRAS,FCJ.CNSACJ,    
                           FCJ.CNSFACJ,FCJ.CODCAJA,1,@USUARIO,DBO.FNK_GETDATE(),FCJ.TIPOEGRESO,NULL,'S',1,'CAJA_CIERRE',  
                           @CNSACJ,FCJ.CNSFACJ,DBO.FNK_GETDATE(),DBO.FNK_GETDATE(),'Movimiento','CAJA', @CNSACJ, '', ''
      FROM FCJ INNER JOIN CAJ ON FCJ.CODCAJA=CAJ.CODCAJA 
      WHERE FCJ.CNSACJ=@CNSACJ
   ...
   PRINT 'Sumo los movimientos para restarlos del traslado  o cierre'
