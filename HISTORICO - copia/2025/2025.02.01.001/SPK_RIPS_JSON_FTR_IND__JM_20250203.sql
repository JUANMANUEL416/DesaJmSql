...
BEGIN
   ...
   PRINT 'INICIO EN SPK_RIPS_JSON_FTR_IND'
	...
	BEGIN
		...
		RETURN
	END
   IF EXISTS(SELECT  * FROM FTR WHERE N_FACTURA=@N_FACTURA AND (TIPOFAC='M' OR COALESCE(CAPITADA,0)=1))
   BEGIN
      RAISERROR('Este proceso solo aplica para factura individual, no se puede continuar',1,16)
      RETURN
   END
	...
	BEGIN
		SELECT @DOCIDAFILIADO=AFI.DOCIDAFILIADO,@TIPODOC=AFI.TIPO_DOC,
		 @TIPOUSU= CASE WHEN COALESCE(AFI.TIPOUSUARIO,'')<>'' THEN AFI.TIPOUSUARIO ELSE CASE   WHEN AFI.TIPOAFILIADO = 'C' THEN '01'
	...
	END
   SELECT @IDPRESTADOR=REPLACE(REPLACE(LTRIM(RTRIM(@IDPRESTADOR)),CHAR(13),''),CHAR(10),'')
   PRINT  '@IDPRESTADOR='+COALESCE(@IDPRESTADOR,'NO TENGO @IDPRESTADOR')
	SELECT @PLANO='{'
	...
	BEGIN
		...
		causaMotivoAtencion='38',COALESCE(CIT.VALORTOTAL,0),COALESCE(CIT.VALORCOPAGO,0), COALESCE(MED.TIPO_ID,'CC'),COALESCE(MED.IDMEDICO,CIT.IDMEDICO)
	...
	BEGIN 
		...
		BEGIN
			...
			causaMotivoAtencion='38',COALESCE(AUTD.VALOR,0),CASE WHEN COALESCE(AUTD.VALORCOPAGO,0) <=0 THEN COALESCE(AUT.VALORCOPAGO,0) ELSE AUTD.VALORCOPAGO END, COALESCE(MED.TIPO_ID,'CC'),COALESCE(MED.IDMEDICO,AUT.IDMEDICOSOLICITA)
		...
		BEGIN
			...
			CASE HCA.TIPODX 
							...
							ELSE 1
							END,COALESCE(HPRED.VALOR,0),COALESCE(HPRED.VALORCOPAGO,0), MED.TIPO_ID, MED.IDMEDICO --STORRES
         ...
         AND HPRED.CANTIDAD=1
			...
			CASE HCA.TIPODX 
							...
							WHEN 'Conf Repet'   THEN 3
							ELSE 1
							END,COALESCE(HPRED.VALOR,0),COALESCE(HPRED.VALORCOPAGO,0),CONVERT(INT,HPRED.CANTIDAD), MED.TIPO_ID, MED.IDMEDICO --STORRES
			FROM HADM INNER JOIN  HPRE ON HADM.NOADMISION=HPRE.NOADMISION
					INNER JOIN HPRED ON HPRE.NOPRESTACION=HPRED.NOPRESTACION
					INNER JOIN SER ON HPRED.IDSERVICIO=SER.IDSERVICIO
					LEFT JOIN (SELECT TOP 1 HCA.NOADMISION,HCA.TIPODX,HCA.IDDX,COALESCE(HCA.DX1,'')DX1,COALESCE(HCA.DX2,'')DX2,COALESCE(DX3,'')DX3 
	...
	BEGIN
		...
		FROM HADM 
			...
			INNER JOIN HPRED ON HPRE.NOPRESTACION=HPRED.NOPRESTACION
			INNER JOIN SER ON HPRED.IDSERVICIO=SER.IDSERVICIO --AND SER.PREFIJO NOT IN (dbo.FNK_VALORVARIABLE('PREFIJOOXIGENO'))
			INNER JOIN RIPS_CP ON SER.CODIGORIPS=RIPS_CP.IDCONCEPTORIPS
	...
	BEGIN
		...
		BEGIN
			...
			SELECT @IDPRESTADOR,REPLACE(CONVERT(VARCHAR,HPRE.FECHA,102),'.','-')+' '+LEFT(CONVERT(VARCHAR,HPRE.FECHA,108),5),null,
				HADM.NOAUTORIZACION,SER.CODCUPS,COALESCE(REPLACE(TGEN.DATO2,' ',''),HADM.VIAINGRESO,'02'),'01','02','325','16',MED.TIPO_ID,MED.IDMEDICO,COALESCE(HCA.IDDX,HADM.DXINGRESO,HADM.DXEGRESO), --STORRES
				COALESCE(HCA.DX1,HADM.DXSALIDA1),COALESCE(HCA.IDDX,HADM.DXINGRESO,HADM.DXEGRESO),COALESCE(HPRED.VALOR,0),'04',COALESCE(HPRED.VALORCOPAGO,0),@N_FACTURA
			FROM HADM INNER JOIN HPRE ON HADM.NOADMISION=HPRE.NOADMISION
					INNER JOIN HPRED ON HPRE.NOPRESTACION=HPRED.NOPRESTACION
					LEFT JOIN (SELECT TOP 1 HCA.NOADMISION,HCA.TIPODX,HCA.IDDX,COALESCE(HCA.DX1,'')DX1,COALESCE(HCA.DX2,'')DX2,COALESCE(DX3,'')DX3 
                           FROM HCA WHERE NOADMISION=@NOADMISION AND COALESCE(IDDX,'')<>'' 
               ...
               LEFT JOIN MED ON MED.IDMEDICO = CASE WHEN COALESCE(HPRE.IDMEDICO,'') = '' THEN COALESCE(HADM.IDMEDICOALTA,HADM.IDMEDICOTRA) ELSE HPRE.IDMEDICO END-- STORRES
			WHERE HADM.NOADMISION=@NOADMISION
			AND HPRED.N_FACTURA=@N_FACTURA
			 AND COALESCE(HPRED.VALOR,0)>0
			 AND COALESCE(HPRED.NOCOBRABLE,0)=0
			...
			SELECT @IDPRESTADOR,REPLACE(CONVERT(VARCHAR,HPRE.FECHA,102),'.','-')+' '+LEFT(CONVERT(VARCHAR,HPRE.FECHA,108),5),null,
				HADM.NOAUTORIZACION,SER.CODCUPS,COALESCE(REPLACE(TGEN.DATO2,' ',''),HADM.VIAINGRESO,'02'),'01','02','325','16',MED.TIPO_ID,MED.IDMEDICO,COALESCE(HCA.IDDX,HADM.DXINGRESO,HADM.DXEGRESO),
				COALESCE(HCA.DX1,HADM.DXSALIDA1),COALESCE(HADM.COMPLICACION,HCA.IDDX,HADM.DXINGRESO,HADM.DXEGRESO),COALESCE(HPRED.VALOR,0),HPRED.CANTIDAD,'04',COALESCE(HPRED.VALORCOPAGO,0),@N_FACTURA
			FROM HADM INNER JOIN HPRE ON HADM.NOADMISION=HPRE.NOADMISION
					INNER JOIN HPRED ON HPRE.NOPRESTACION=HPRED.NOPRESTACION
					LEFT JOIN (SELECT TOP 1 HCA.NOADMISION,HCA.TIPODX,HCA.IDDX,COALESCE(HCA.DX1,'')DX1,COALESCE(HCA.DX2,'')DX2,COALESCE(DX3,'')DX3 
                           FROM HCA WHERE NOADMISION=@NOADMISION AND COALESCE(IDDX,'')<>'' 
	...
	BEGIN
		...
		BEGIN
         ...
         IF COALESCE(@codDiagnosticoRelacionado, '')<>''
			...
			SET @PLANO+=' "numFEVPagoModerador":"'+@numFEVPagoModerador+'",'
			SET @PLANO+=' "consecutivo":'+@consecutivo+','
			SET @PLANO+= '"conceptoRecaudo":"'+CASE WHEN TRY_CAST(@valorPagoModerador AS decimal(14,2) ) IS NULL THEN '05' ELSE
						  CASE WHEN CAST(@valorPagoModerador AS decimal(14,2)) <=0 THEN '05' ELSE COALESCE(@conceptoRecaudo,'05') END  END+'"'   ----VGARCIA
			SET @PLANO+=' }'
			SELECT @NRO+=1
		END
...
--QUERY2
	...
	BEGIN
		...
		@consecutivo=1
		FROM HADM 
			LEFT JOIN HCA ON HADM.NOADMISION=HCA.NOADMISION AND HCA.CLASE='HC' AND HCA.PROCEDENCIA='QX' AND HCA.CLASEPLANTILLA <> DBO.FNK_VALORVARIABLE('HCPLANTILLAEPI')
	...
	BEGIN
		...
		SELECT @codPrestador=@IDPRESTADOR
			...
			,@codDiagnosticoRelacionadoE2=COALESCE(HCA.DX2,HADM.DXSALIDA2)
			,@codDiagnosticoRelacionadoE3=COALESCE(HCA.DX3,HADM.DXSALIDA3)
			,@codComplicacion=COALESCE(HADM.COMPLICACION,HCA.IDDX,HADM.DXINGRESO,HADM.DXEGRESO)
			,@condicionDestinoUsuarioEgreso=CASE WHEN HADM.ESTADOPSALIDA=1 THEN '01' ELSE '02' END
			,@codDiagnosticoCausaMuerte=CASE WHEN HADM.ESTADOPSALIDA=1 THEN 'null' ELSE CAUSABMUERTE END
			,@fechaEgreso=REPLACE(CONVERT(VARCHAR,HADM.FECHAALTAMED,102),'.','-')+' '+LEFT(CONVERT(VARCHAR,HADM.FECHAALTAMED,108),5)
			,@consecutivo=1
   ...
   --SELECT SUM(vrServicio) FROM @CONSULTAS
   --SELECT SUM(CONVERT(DECIMAL(14,2),vrServicio)) FROM @MEDICAMENTOS
   --SELECT SUM(vrServicio) FROM @PROCEDIMIENTOS
   --SELECT SUM(vrServicio) FROM @OTROSSER
