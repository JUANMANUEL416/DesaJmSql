...
ELSE CASE WHEN COALESCE(APOYODG_AMBITO,'')<> '' AND COALESCE(AUTD.ENLAB,0)=2  THEN 0 
      WHEN COALESCE(APOYODG_AMBITO,'')<> '' AND COALESCE(AUTD.ENLAB,0)<> 2  THEN 1 ELSE 1 END END DX_PEND
...
GROUP BY AUT.CONSECUTIVOCIT,AUT.NOAUT,AUTD.ESDELAB,AUTD.ENLAB,AUT.ESPROGRAMAS,AUTD.IDSERVICIO,COALESCE(APOYODG_AMBITO,''))X
