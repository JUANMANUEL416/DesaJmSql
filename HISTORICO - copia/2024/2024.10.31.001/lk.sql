...
DECLARE @VLR_COPA_VAR_PR DECIMAL(14,2)
DECLARE @VLR_COPA_FIJO_PR DECIMAL(14,2)
...
BEGIN
   ...
   IF EXISTS(SELECT * FROM PPTCNT INNER JOIN AUT ON AUT.IDTERCEROCA=PPTCNT.IDTERCERO AND AUT.IDPLAN=PPTCNT.IDPLAN AND AUT.PREFIJO=PPTCNT.PREFIJO WHERE AUT.IDAUT=@IDAUT)
   BEGIN
      SELECT @COPA
      FROM PPTCNT INNER JOIN AUT ON AUT.IDTERCEROCA=PPTCNT.IDTERCERO 
                                    AND AUT.IDPLAN=PPTCNT.IDPLAN 
                                    AND AUT.PREFIJO=PPTCNT.PREFIJO 
