...
BEGIN
   ...
   IF @METODO='TRAER_NOVEDAD'     
   BEGIN         
      SELECT @PERIODO=PERIODO    
      FROM   OPENJSON (@PARAMETROS)
      WITH (           
      PERIODO  VARCHAR(20)   '$.PERIODO'
      )  

      SELECT 'OK' OK
      SELECT DISTINCT TURNOV.CODNOV AS value, TGEN.DESCRIPCION AS label
      FROM TURNOV LEFT JOIN  TGEN ON TGEN.TABLA='TSECFU' AND CAMPO='NOVEDAD' AND CODIGO=TURNOV.CODNOV
      WHERE TURNOV.PERIODO=@PERIODO
      RETURN
   END  
   IF @METODO='TRAER_NEVE'     
   BEGIN         
      SELECT 'OK' OK
      SELECT DISTINCT NEVE.IDEVENTO AS value, NEVE.DESCRIPCION AS label
      FROM NEVE
      RETURN
   END  
