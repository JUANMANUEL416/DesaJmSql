   ...
   BEGIN   
      ...
      BEGIN
         ...
         BEGIN
            ...
            AND  RPDX.CNS=@CNS_D
         END         
         PRINT'--INSERTO CIRUGIAS CON MODIFICACION'
         INSERT INTO RPDX (CNS, ID1, ID2, ID4, ID5, ID6, CANTIDAD,STRINGGRANDE1 , VALOR1, VALOR2, VALOR3, VALOR4, IDTERCERO, FECHA1,NOMBRE, STRINGMEDIO2)
        ...
        BEGIN
           ...
           WHERE RPDX.CNS=@CNS_D AND COALESCE(SER.MEDICAMENTOS,0)=0
			...
			--DETALLE DE PROCEDIMIENTO
            UPDATE RPDX SET ID2=COALESCE(C.CODIGO,SER.CODCUPS,''), STRINGGRANDE1=COALESCE(C.DESCRIPCION ,SER.DESCRIPCION_CUPS,'')
            FROM RPDX INNER JOIN SER ON SER.IDSERVICIO=RPDX.ID2
                LEFT  JOIN dbo.VWK_CUPS C ON C.IDSERVICIO=SER.IDSERVICIO AND C.IDTERCERO=@IDTERCEROHADM AND C.IDPLAN=@IDPLAN
            WHERE RPDX.CNS=@CNS_D AND RPDX.IDTERCERO='PROCEDIMIENTOS'
        END
