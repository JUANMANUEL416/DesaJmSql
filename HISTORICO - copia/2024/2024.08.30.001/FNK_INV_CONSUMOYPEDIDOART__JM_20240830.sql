...
BEGIN  
   ...
   FROM VWK_HERMANOS_INVENTARIO VK 
		INNER JOIN IEXI ON IEXI.IDARTICULO=VK.IDARTICULO
	WHERE VK.HERMANO_MAYOR=CASE WHEN @IDARTICULO='' THEN VK.HERMANO_MAYOR ELSE @IDARTICULO END 
   AND COALESCE(IEXI.EXISLOTE,0)>0
   AND IEXI.IDBODEGA= CASE @IDBODEGA WHEN '' THEN IEXI.IDBODEGA ELSE @IDBODEGA END
   GROUP BY VK.HERMANO_MAYOR, IDBODEGA

   INSERT INTO @VK (IDARTICULO, IDBODEGA, EXISTOTAL)
   SELECT VK.HERMANO_MAYOR AS IDARTICULO, IDBODEGA, 0 EXISTOTAL
   FROM VWK_HERMANOS_INVENTARIO VK 
		INNER JOIN IEXI ON IEXI.IDARTICULO=VK.IDARTICULO
	WHERE VK.HERMANO_MAYOR=CASE WHEN @IDARTICULO='' THEN VK.HERMANO_MAYOR ELSE @IDARTICULO END 
   AND COALESCE(IEXI.EXISLOTE,0)=0
   AND NOT EXISTS(SELECT * FROM @VK VKE WHERE VK.HERMANO_MAYOR=VKE.IDARTICULO)
   AND IEXI.IDBODEGA= CASE @IDBODEGA WHEN '' THEN IEXI.IDBODEGA ELSE @IDBODEGA END
   GROUP BY VK.HERMANO_MAYOR, IDBODEGA
   ...
   SELECT IDBODEGA, IDARTICULO, SUM(SALIDAS) SALIDAS, SUM(ENTRADAS) ENTRADAS, 
	...
	FROM (
		SELECT A.IDBODEGA, HM.HERMANO_MAYOR IDARTICULO, ISNULL(A.NOPRESTACION,'') AS NOPRESTACION,
		...
		FROM IMOV A WITH(NOLOCK)
         ...
         INNER JOIN IMOVH B WITH(NOLOCK) ON A.CNSMOV=B.CNSMOV 
         LEFT JOIN dbo.VWK_HERMANOS_INVENTARIO HM ON B.IDARTICULO=HM.IDARTICULO
		WHERE A.FECHACONF>= @FECHAINI 
      AND A.FECHACONF < @FECHAFIN +1 
      AND A.ESTADO=1 
      AND A.IDBODEGA=(CASE @IDBODEGA WHEN '' THEN A.IDBODEGA ELSE @IDBODEGA END)
		AND HM.HERMANO_MAYOR=CASE WHEN @IDARTICULO='' THEN HM.HERMANO_MAYOR ELSE @IDARTICULO END 
		GROUP BY A.IDBODEGA, HM.HERMANO_MAYOR, ITMO.TIPO, ITMO.AFECTAPROM, ISNULL(A.NOPRESTACION,'')
	)Y 
   ...
   INSERT INTO @RESULTADOS  
	...
	FROM @VK  T INNER JOIN IART ON IART.IDARTICULO=T.IDARTICULO
	INNER JOIN IBOD ON IBOD.IDBODEGA=T.IDBODEGA
	LEFT JOIN @IMOV X ON T.IDBODEGA=X.IDBODEGA AND T.IDARTICULO=X.IDARTICULO 
