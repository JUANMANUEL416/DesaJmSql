...
BEGIN
   ...
   BEGIN         
      ...
      SELECT @PROCESO=PROCESO,@CNSPRUEBA=CNSPRUEBA,@FLIMITE=CONVERT(DATETIME,FLIMITE),@TIPO=TIPO,@IDTIPO=IDTIPO,@RESULTADO=RESULTADO,@EQUIPO=JSON_VALUE(EQUIPO,'$.CODIGO'),
      ...
      BEGIN
         ...
         INSERT INTO KCEPRU (CNSPRUEBA, FECHA, USUARIO, EQUIPO, TIPO, IDTIPO, RESULTADO, OBSERACIONES, ESTADO, FLIMITE)
         SELECT @CNSPRUEBA, DBO.FNK_GETDATE(), @USUARIO, @EQUIPO, @TIPO, @IDTIPO, @RESULTADO, @OBSERVACIONES, 'Abierta', @FLIMITE
         SELECT 'OK'OK
         RETURN
      END
      IF @PROCESO='Editar'
      BEGIN
         ...
         BEGIN
            UPDATE KCEPRU SET EQUIPO=@EQUIPO,TIPO=@TIPO,IDTIPO=@IDTIPO,RESULTADO=@RESULTADO,OBSERACIONES=@OBSERVACIONES,FLIMITE=@FLIMITE  WHERE  CNSPRUEBA=@CNSPRUEBA AND ESTADO='Abierta'
         END
         ELSE
         BEGIN
            INSERT INTO @TBLERRORES(ERROR)
   ...
   BEGIN         
      SELECT @CNSPRUEBA=CNSPRUEBA,@NOITEM=ACCION    
      FROM   OPENJSON (@PARAMETROS)
      WITH (           
      CNSPRUEBA VARCHAR(20) '$.CNSPRUEBA',
      ...
      IF EXISTS(SELECT  * FROM KCEPRU WHERE CNSPRUEBA=@CNSPRUEBA AND ESTADO='Abierta')
      BEGIN
         UPDATE KCEPRU SET ESTADO=CASE WHEN @NOITEM=1 THEN 'Cerrada' WHEN @NOITEM= 2 THEN 'Anulada' ELSE ESTADO END
   ...
   BEGIN         
      SELECT @CNSCARGA=CNSCARGA,@FFINAL=CONVERT(DATETIME,FFINAL)      
      FROM   OPENJSON (@PARAMETROS)
      WITH (           
      CNSCARGA VARCHAR(20) '$.CNSCARGA',
