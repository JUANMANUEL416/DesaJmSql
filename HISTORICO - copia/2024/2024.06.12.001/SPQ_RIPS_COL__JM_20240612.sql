...
DECLARE  @PARAMETROS NVARCHAR(MAX)			,@MODELO VARCHAR(100)			    ,@METODO VARCHAR(100)
      ...
      ,@RAZONSOCIAL VARCHAR(20)           ,@F_FINAL VARCHAR(20)             ,@CAPITADO BIT   
  ...
  BEGIN
   ...
   BEGIN         
      ...
      WITH (           
         ...
         SELECT @PROCESO=PROCESO, @ENVIO=ENVIO, @TIPORPT=TIPORPT, @FECHA=CONVERT(DATETIME,FECHA),  @REQSEDE=CAST(CASE WHEN ISNUMERIC(REQSEDE)=1 THEN REQSEDE ELSE CASE WHEN COALESCE(REQSEDE,'')<>'SI' THEN 0 ELSE 1 END END AS BIT), @IDSEDE=IDSEDE, @IDTERCERO=COALESCE(IDTERCERO,''),
                ...
                @CODMEDICA=CODMEDICA,@MFACT=CASE WHEN MFACT ='true' THEN 1 ELSE 0 END    
         FROM   OPENJSON (@DATOS)
         WITH   ( 
      ...
      BEGIN
         ...
         BEGIN TRY           
               ...
               SELECT @ENVIO, 'N', @FECHA, @IDTERCERO, 0, @CAPITADO,@F_INICIAL, 0, @F_FINAL,COALESCE(@N_FACTURA,''), NULL, 0, 
                     ...
                     0, @USUARIO, DBO.FNK_GETDATE(), NULL, NULL, '',@CODSERVICIO,@CODMEDICA, @REQSEDE, @IDSEDE, @MFACT
         END TRY
         BEGIN CATCH
      ...
      BEGIN
         ...
         UPDATE RIPS SET FECHAENVIO=@FECHA, IDTERCERO=@IDTERCERO,CAPITADO=@CAPITADO, FECHAINICIAL=@F_INICIAL,FECHAFINAL=@F_FINAL,
                ...
                IDSEDE=@IDSEDE,TIPORPT=@TIPORPT,MFACT=@MFACT
         WHERE ENVIO=@ENVIO
      END
   ...
   BEGIN         
      ...
      BEGIN
         ...
         RETURN
      END
      SELECT @IDTERCERO=IDTERCERO,@CAPITADO=COALESCE(CAPITADO,0),@F_INICIAL=FECHAFINAL,
      ...
      IF NOT EXISTS(SELECT * FROM RIPSD WHERE ENVIO=@ENVIO)
      BEGIN
         SELECT 'KO','Envio Sin Detalles, Verfique e Intente de Nuevo'error
         RETURN         
      END
   ...
   BEGIN   
     SELECT @ENVIO=ENVIO, @IDTERCERO=IDTERCERO        
      ...
      CASE WHEN COALESCE(@VL_ENTERO,'')='SI' THEN CAST(CONVERT(INT,VALORCONSULTA)AS VARCHAR(20)) ELSE CAST(VALORCONSULTA AS VARCHAR(20)) END +','+
      CASE WHEN COALESCE(@VL_ENTERO,'')='SI' THEN CAST(CONVERT(INT,COALESCE(VALORCUOTAM,0)) AS VARCHAR(20)) ELSE CAST(VALORCUOTAM AS VARCHAR(20)) END+','+
	  CASE WHEN COALESCE(@VL_ENTERO,'')='SI' THEN CAST(CONVERT(INT,VALORNETO)AS VARCHAR(20)) ELSE CAST(VALORNETO AS VARCHAR(20)) END 
	  ,CHAR(13),'')+CHAR(10) PLANO
      FROM RRIPS_AC
      WHERE ENVIO=@ENVIO
      ...
      +','+COALESCE(ACTOQ,'1')+','+CASE WHEN COALESCE(@VL_ENTERO,'')='SI' THEN CAST(CONVERT(INT,CONVERT(DECIMAL(14,2),VALORPROCED)) AS VARCHAR(20)) ELSE VALORPROCED END,CHAR(13),'')+CHAR(10)PLANO
      ...
      CASE WHEN COALESCE(@VL_ENTERO,'')='SI' THEN CAST(CONVERT(INT,CONVERT(DECIMAL(14,2),COALESCE(VLRTOTMAT,'0.00'))) AS VARCHAR(20)) ELSE  COALESCE(VLRTOTMAT,'0') END ,CHAR(13),'')+CHAR(10)PLANO
      ...
      PRINT 'ARCHIVO @ENVIO_AM'
	   SELECT REPLACE(NUMFACTURA+','+IDPRESTADOR+','+TIPO_DOC+','+IDUSUARIO+','+COALESCE(NUMAUT,'')+','+IDMEDICAMENTO+','+TIPOMEDICAMENTO+','+
              ...
              CASE WHEN COALESCE(@VL_ENTERO,'')='SI' THEN CAST(CONVERT(INT,COALESCE(VALORTOTMEDI,0)) AS VARCHAR(20)) ELSE CAST(COALESCE(VALORTOTMEDI,'0') AS VARCHAR(20)) END,CHAR(13),'')+CHAR(10)PLANO
	   FROM RRIPS_AM 
	   WHERE ENVIO=@ENVIO
      ...
      PRINT 'ARCHIVO @ENVIO_AN'
	   SELECT  REPLACE(NUMFACTURA+','+IDPRESTADOR+','+TIPOIDM+','+IDMADRE+','+CONVERT(VARCHAR,FNACIMIENTO,103)+','+LEFT(CONVERT(VARCHAR,FNACIMIENTO,108),5)+','+COALESCE(EDADGEST,'')+','+COALESCE(CPRENATAL,'')+','+
            ...
            +LEFT(COALESCE(CONVERT(VARCHAR,FMUERTE,108),''),5),CHAR(13),'')+CHAR(10) PLANO
	   FROM RRIPS_AN
	   WHERE ENVIO=@ENVIO
      ...
      LEFT(CONVERT(VARCHAR,FECHAEGRESO,108),5),CHAR(32),'')+CHAR(13)PLANO
      ...
      PRINT 'ARCHIVO @ENVIO_AU'
		...
		SELECT REPLACE( NUMFACTURA+','+IDPRESTADOR+','+TIPO_DOC+','+IDUSUARIO+','+COALESCE(CONVERT(VARCHAR,FECHAINGRESO,103),'')+','+
             ...
             LEFT(COALESCE(CONVERT(VARCHAR,FECHASALIDA,108),''),5),CHAR(13),'')+CHAR(10) PLANO
		FROM RRIPS_AU ES
		WHERE ENVIO=@ENVIO
      ...
      CASE WHEN COALESCE(@VL_ENTERO,'')='SI' THEN  CAST(CONVERT(BIGINT,CONVERT(DECIMAL(14,2),VALORNETO,0)) AS VARCHAR(20)) ELSE COALESCE(VALORNETO,'0') END,CHAR(13),'')+CHAR(10)PLANO
	   FROM RRIPS_AF 
      WHERE ENVIO=@ENVIO
      ...
      PRINT 'ARCHIVO @ENVIO_US'
	   ...
	   SELECT REPLACE( TIPO_DOC+','+IDUSUARIO+','+CODENTIDADM+','+TIPOUSUARIO+','+COALESCE(PAPELLIDO,'')+','+COALESCE(SAPELLIDO,'')+','+
			   ...
			   SEXO+','+COALESCE(DEPHABITUAL,'')+','+COALESCE(MUNHABITUAL,'')+','+ZONAHABITUAL,CHAR(13),'')+CHAR(10) PLANO
	   FROM RRIPS_US 
      WHERE ENVIO=@ENVIO
      ...
      CAST(CONVERT(bigint,IIF(ISNUMERIC(TOTALCONCEPTO)=1,COALESCE(TOTALCONCEPTO,0),0)) AS VARCHAR(20)),CHAR(13),'')+CHAR(10)PLANO
      FROM RRIPS_AD 
      WHERE ENVIO=@ENVIO
