...
BEGIN
   ...
   BEGIN
     ...
     PRINT '@sUrl='+@sUrl
      ...
      SELECT @N_FACTURA,  @TIPODOC,@CNSFNOT,@ESTDOC, @ERRORES, @DESCSUNAT,'CONSULTA',@body , @Response

      IF @ESTDOC IN('101','102','103')
      BEGIN
         IF COALESCE(@CNSFNOT,'')=''
         BEGIN
            IF @FACTEP IN('','101','103','104A')
            BEGIN
               UPDATE FTR SET FACTEP=@ESTDOC ,ULRPTAAPI=COALESCE(@ERRORES,@DESCSUNAT) WHERE N_FACTURA=@N_FACTURA
