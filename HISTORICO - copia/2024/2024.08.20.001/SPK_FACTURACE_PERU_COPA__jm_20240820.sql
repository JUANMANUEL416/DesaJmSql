	...
	BEGIN  
		...
		BEGIN   
			...
			BEGIN
				...
				SELECT @CNSFTR, GETDATE(), 'DB', AUT.IDAREA, NULL, 
						...
						CASE WHEN COALESCE(SER.IVA,'')='SER' THEN  (SELECT ROUND(AUTD.VALORCOPAGO*(VALOR/100),0) 
			...
			BEGIN
				...
				BEGIN
					...
					SELECT @CNSFTR, GETDATE(), 'DB', AUT.IDAREA, NULL, 
							...
							CASE WHEN COALESCE(SER.IVA,'')='SER' THEN  (SELECT ROUND(AUTD.VALORCOPAGO*(VALOR/100),0) FROM  DBO.FNK_VALOR_IMPUESTO(SER.IDIMPUESTO,SER.IDCLASE,GETDATE(),AUTD.VALORCOPAGO)) ELSE NULL END,
							CASE WHEN COALESCE(SER.IVA,'')='SER' THEN  (SELECT CUENTA FROM  DBO.FNK_VALOR_IMPUESTO(SER.IDIMPUESTO,SER.IDCLASE,GETDATE(),AUTD.VALORCOPAGO)) ELSE NULL END
					FROM   AUT INNER JOIN AUTD ON AUTD.IDAUT     = AUT.IDAUT 
								INNER JOIN SER  ON SER.IDSERVICIO = AUTD.IDSERVICIO 
								LEFT  JOIN TER  ON TER.IDTERCERO  = AUTD.IDTERCEROCA
		...
		BEGIN
			...
			INSERT INTO #FTRD1(CNSFTR, FECHA, DB_CR, AREAPRESTACION, UBICACION, VR_TOTAL,
					...
					VALOR, VLR_SERVICI, VLR_COPAGOS, VLR_PAGCOMP, IDPROVEEDOR, NOADMISION,
					NOPRESTACION, NOITEM, AREAFUNCONT, N_FACTURA, SUBCCOSTO, PCOSTO, FECHAPREST, IDPLAN,IDIMPUESTO, IDCLASE,ITEM , VLRIMPUESTO, PIVA, VIVA,CUENTA_FIMPDV)
			SELECT @CNSFTR, GETDATE(), 'DB', CIT.IDAREA, NULL,VALORCOPAGO, 
				NULL, CIT.CCOSTO, SER.PREFIJO, SER.DESCSERVICIO, CIT.IDSERVICIO, NULL,
				COALESCE(CIT.CANTIDADC,1) CANTIDAD, VALORCOPAGO, VALORCOPAGO,0,
				0, CIT.IDMEDICO, @NOAUT, @NOAUT, 1, NULL, 
				@NVOCONSEC, CIT.SUBCCOSTO, CIT.VALORTOTALCOS, CIT.FECHA, CIT.IDPLAN,
						CASE WHEN COALESCE(SER.IVA,'')='SER' THEN  SER.IDIMPUESTO ELSE NULL END,
						CASE WHEN COALESCE(SER.IVA,'')='SER' THEN  SER.IDCLASE ELSE NULL END,
						CASE WHEN COALESCE(SER.IVA,'')='SER' THEN  (SELECT ITEM FROM  DBO.FNK_VALOR_IMPUESTO(SER.IDIMPUESTO,SER.IDCLASE,GETDATE(),CIT.VALORCOPAGO)) ELSE NULL END,
						CASE WHEN COALESCE(SER.IVA,'')='SER' THEN  CIT.VALORCOPAGO ELSE NULL END ,
						CASE WHEN COALESCE(SER.IVA,'')='SER' THEN  (SELECT VALOR FROM  DBO.FNK_VALOR_IMPUESTO(SER.IDIMPUESTO,SER.IDCLASE,GETDATE(),CIT.VALORCOPAGO)) ELSE NULL END,
						--CASE WHEN COALESCE(SER.IVA,'')='SER' THEN  (SELECT ROUND(VALORCOPAGO*(VALOR/100),4) FROM  DBO.FNK_VALOR_IMPUESTO(SER.IDIMPUESTO,SER.IDCLASE,GETDATE(),VALORCOPAGO)) ELSE NULL END,
						CASE WHEN COALESCE(SER.IVA,'')='SER' THEN (VALORCOPAGO -ROUND(VALORCOPAGO/1.18,2)) ELSE NULL END,
						CASE WHEN COALESCE(SER.IVA,'')='SER' THEN  (SELECT CUENTA FROM  DBO.FNK_VALOR_IMPUESTO(SER.IDIMPUESTO,SER.IDCLASE,GETDATE(),VALORCOPAGO))  ELSE NULL END            
		...
		END     
			...
			BEGIN
				...
				SELECT @CNSFTR, @COMPANIA, 'C', CASE WHEN COALESCE(@IDTERCERO_RUC,'')<>'' THEN @IDTERCERO_RUC ELSE  @IDTERCEROF END, 
						...
						@CUENTACXC,@CUENTACXC_RAD, 0 ,@CODUNG, @CODPRG, @IDSEDE,CASE WHEN COALESCE(@CNSRESOL,'')<>'' THEN 1 ELSE 0 END,COALESCE(@CNSRESOL,''),@IDFORMATOFTR,'Boleta'
			END
			ELSE
			BEGIN
