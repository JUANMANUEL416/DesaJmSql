...
BEGIN
   ...
   BEGIN         
      ...
      IF EXISTS(SELECT  * FROM NCINGRET WHERE CNSCERTI=@CNSCERTI )
      BEGIN
   ...
   BEGIN         
      ...
      WITH (           
       ...
       AND CLASE='Resta'
   END      
   IF @METODO='TRAER_CODRET'     
   BEGIN      
      IF EXISTS(SELECT * FROM TGEN  WHERE TABLA='NOMINA' AND CAMPO='CERTINGRETE')
      BEGIN
         SELECT 'OK' OK
         SELECT  CODIGO value,DESCRIPCION label FROM TGEN  WHERE TABLA='NOMINA' AND CAMPO='CERTINGRETE' ORDER BY CODIGO
         RETURN
      END
      ELSE
      BEGIN
        SELECT 'KO'KO,'No se encontraron los Códigos de retencion; Consulte con el Administrador del sistema'ERROR
        RETURN
      END
   END  
   IF @METODO='ASINGAR_CODRET'     
   BEGIN     
      DECLARE @CODCONCEPTO VARCHAR(20)
      SELECT  @CODCONCEPTO=CODCONCEPTO,@CODRET=CODRET     
      FROM   OPENJSON (@PARAMETROS)
      WITH (           
      CODCONCEPTO  VARCHAR(20)   '$.CODCONCEPTO',
      CODRET  VARCHAR(20)   '$.CODRET'
      )
      IF EXISTS(SELECT * FROM NCON WHERE CODCONCEPTO=@CODCONCEPTO)
      BEGIN
         UPDATE NCON SET CODRET=@CODRET WHERE CODCONCEPTO=@CODCONCEPTO
      END
      ELSE
      BEGIN
         INSERT INTO @TBLERRORES(ERROR)
         SELECT 'Concepto no encontrado, verifique e intente de nuevo'
      END
