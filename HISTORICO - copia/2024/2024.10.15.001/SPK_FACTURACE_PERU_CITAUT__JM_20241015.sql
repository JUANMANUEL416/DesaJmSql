IF EXISTS(SELECT NAME FROM SYSOBJECTS WHERE NAME= 'SPK_FACTURACE_PERU_CITAUT' AND XTYPE='P')
BEGIN
   DROP PROCEDURE SPK_FACTURACE_PERU_CITAUT
END
GO
CREATE  PROCEDURE DBO.SPK_FACTURACE_PERU_CITAUT
@NOAUT			VARCHAR(16),
@NIT			VARCHAR(20),
@COMPANIA		VARCHAR(2),
@IDSEDE			VARCHAR(5),
...
BEGIN
	...
	ELSE 
	BEGIN
		IF EXISTS(SELECT * FROM FTR WHERE IDTERCERO=@IDTERCEROCA1 AND ESTADO='L' AND TIPOFIN='C')
		BEGIN
   ...
   INSERT INTO #FTRD1(CNSFTR, FECHA, DB_CR, AREAPRESTACION, UBICACION, VR_TOTAL,
         ...
         VLRIMPUESTO, PIVA, VIVA,CUENTA_FIMPDV,PROCEDENCIA)
			SELECT @CNSFTR, GETDATE(), 'DB', CIT.IDAREA, NULL,VALORTOTAL, 
                ...
                (CIT.VALORTOTAL-COALESCE(CIT.VALORCOPAGO,0))/1.18, CIT.VALORTOTAL, COALESCE(VALORCOPAGO,0), 0,CIT.IDMEDICO, @NOAUT, @NOAUT, 1, NULL, @NVOCONSEC, CIT.SUBCCOSTO,
                CIT.VALORTOTALCOS, CIT.FECHA, CIT.IDPLAN,
					 ...
					 CASE WHEN COALESCE(SER.IVA,'')='SER' THEN  (SELECT ITEM FROM  DBO.FNK_VALOR_IMPUESTO(SER.IDIMPUESTO,SER.IDCLASE,GETDATE(), VALORTOTAL)) ELSE NULL END,				 
                (CIT.VALORTOTAL-COALESCE(CIT.VALORCOPAGO,0)),CASE WHEN COALESCE(SER.IVA,'')='SER' THEN 
                (SELECT VALOR FROM  DBO.FNK_VALOR_IMPUESTO(SER.IDIMPUESTO,SER.IDCLASE,GETDATE(), VALORTOTAL)) ELSE NULL END,
					 CASE WHEN COALESCE(SER.IVA,'')='SER' THEN  VALORTOTAL - (CIT.VALORTOTAL/1.18)  ELSE NULL END,
					 CASE WHEN COALESCE(SER.IVA,'')='SER' THEN  (SELECT CUENTA FROM  DBO.FNK_VALOR_IMPUESTO(SER.IDIMPUESTO,SER.IDCLASE,GETDATE(),VALORTOTAL))  
                ELSE NULL END,'CIT'
   ...
   INSERT INTO #FTRD1(CNSFTR, FECHA, DB_CR, AREAPRESTACION, UBICACION, VR_TOTAL,
         ...
         VLRIMPUESTO, PIVA, VIVA,CUENTA_FIMPDV,PROCEDENCIA)
 					SELECT @CNSFTR, GETDATE(), 'DB', AUT.IDAREA, NULL, 
							...
							CASE WHEN dbo.FNK_VALORVARIABLE('MANEJA_GARANTIA_SER') = 'SI' THEN 
								CASE WHEN dbo.FNK_VALORVARIABLE('CLASE_IPS') = 'OPTICA' THEN  CASE WHEN COALESCE(SER.GARANTIA, 0) = 0 THEN COALESCE(SER.DESCSERVICIO, '')+' '+'Producto sin Garantia'
																					...
																					ELSE SER.DESCSERVICIO
																					END,
							AUTD.IDSERVICIO, NULL, AUTD.CANTIDAD,((AUTD.VALOR-(AUTD.VALORCOPAGO/AUTD.CANTIDAD))/1.18), AUTD.VALOREXCEDENTE, 
						...
						CASE WHEN COALESCE(SER.IVA,'')='SER' THEN  AUTD.VALOREXCEDENTE  ELSE NULL END ,
						...
						CASE WHEN COALESCE(SER.IVA,'')='SER' THEN (SELECT ROUND(AUTD.VALOREXCEDENTE*(VALOR/100),2) FROM  DBO.FNK_VALOR_IMPUESTO(SER.IDIMPUESTO,SER.IDCLASE,GETDATE(),AUTD.VALOREXCEDENTE)) ELSE NULL END,
					...
					WHERE  AUT.CONSECUTIVOCIT = @NOAUT
                  ...
                  AND COALESCE(AUTD.GENERADO,0)=0
	...
	END
      ...
      WHERE 1=1
	   ...
	   INSERT INTO FTRD(CNSFTR, N_CUOTA, FECHA, DB_CR, AREAPRESTACION, UBICACION, VR_TOTAL,
				   ...
				   IDIMPUESTO, IDCLASE,ITEM , VLRIMPUESTO, PIVA, VIVA,CUENTA_FIMPDV,PROCEDENCIA)
