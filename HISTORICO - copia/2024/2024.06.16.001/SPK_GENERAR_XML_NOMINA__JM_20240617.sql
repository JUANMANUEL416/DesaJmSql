...
SELECT 
	...
	,@TiempoLaborado =CASE WHEN  DATEDIFF(DAY,FECHAINGRESO,COALESCE(FECHARETIRO,GETDATE())) <=0 THEN 1 ELSE DATEDIFF(DAY,FECHAINGRESO,COALESCE(FECHARETIRO,GETDATE())) END
...
FROM DBO.VWK_NIEPED WHERE CONSECUTIVO=@PERIODO AND NUMDOC=@NUMDOC AND HORAS>=1
