...
BEGIN  
   ...
   BEGIN  
      ...
      SELECT @NROCOMPROBANTE,@COMPANIA,'CR',  
            ...
            COALESCE(BCO.IDTERCERO,#THD.IDTERCERO),#T.CCOSTO,  
   ...
   BEGIN  
      ...
      SELECT @NROCOMPROBANTE,@COMPANIA,'CR',CASE WHEN COALESCE(BCT.BANCO,'')='' THEN CAJ.CUENTA ELSE BCT.COD_CONTABLE END ,  
            COALESCE(BCO.IDTERCERO,#T.IDTERCERO),#T.CCOSTO, UPPER(CPCJ.DESCRIPCION)+', '+UPPER(FPA.DESCRIPCION)+', '+UPPER(RTRIM(TER.RAZONSOCIAL)),  
            #THD.VALOR,#T.CNSFACJ,#T.CODCAJA,#THD.NUMERODOCUMENTO,NULL,@USUARIO,DBO.FNK_FECHA_SIN_MLS(GETDATE()),  
            NULL,LEFT(#T.AREAPRESTACION,10),'S',1,'CAJA_DEVDOC', 'Movimiento',  
            RTRIM(#T.CODCAJA)+#T.CNSFACJ,DBO.FNK_FECHA_SIN_HORA(#T.FECHA),DBO.FNK_FECHA_SIN_HORA(#T.FECHA),'CAJA',@CNSFACJ, 
            CASE WHEN #T.CODUNG = '' THEN NULL ELSE #T.CODUNG END, CASE WHEN #T.CODPRG = '' THEN NULL ELSE #T.CODPRG END
	   ...
	   BEGIN  
	      ...
	      SELECT @NROCOMPROBANTE, @COMPANIA, 'CR', 
			       CASE WHEN FPA.USAR_CUENTACAJA = 1 THEN
			          ...
			          FPA.CUENTA 
			       END,
			 CASE WHEN COALESCE(@ESLEGALIZA,0)=1 THEN COALESCE(BCO.IDTERCERO,#THD.IDTERCERO) ELSE CASE WHEN COALESCE(#TH.IDSERVICIO,'')<>'' THEN COALESCE(BCO.IDTERCERO,FCXP.IDTERCERO) ELSE COALESCE(BCO.IDTERCERO,#TH.IDTERCERO,#T.IDTERCERO) END END,
	      ...
	      GROUP BY  TER.RAZONSOCIAL,TER2.RAZONSOCIAL,TER1.RAZONSOCIAL,#THD.TIPOPAGO,
			     ...
			     CASE WHEN COALESCE(#TH.IDSERVICIO,'')<>'' THEN COALESCE(BCO.IDTERCERO,FCXP.IDTERCERO) ELSE COALESCE(BCO.IDTERCERO,#TH.IDTERCERO,#T.IDTERCERO) END,
              CASE WHEN COALESCE(#TH.IDSERVICIO,'')<>'' THEN #T.CCOSTO ELSE #TH.CCOSTO END,
              #T.CNSFACJ, LEFT(#T.AREAPRESTACION,10), FPA.DESCRIPCION, 
			   #T.CODCAJA, DBO.FNK_FECHA_SIN_HORA(#T.FECHA), #THD.NUMERODOCUMENTO, FPA.GENCHEQUE, #T.CODUNG, #T.CODPRG,#THD.TIPOPAGO,COALESCE(BCO.IDTERCERO,#THD.IDTERCERO),TER1.RAZONSOCIAL,
            CPCJ.DESCRIPCION
	   END
	   ELSE
	   BEGIN
