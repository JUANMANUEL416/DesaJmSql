...
DECLARE  @PARAMETROS NVARCHAR(MAX)			,@MODELO VARCHAR(100)			   ,@METODO VARCHAR(100)
      ...
      ,@CUOTAS SMALLINT               ,@NROCOMPROBANTE VARCHAR(20)  ,@DIASDIF SMALLINT
   BEGIN
	SELECT *
	INTO #JSON
   ...
   BEGIN
      ...
      BEGIN TRY 
         ...
         BEGIN
            ...
            BEGIN
               IF EXISTS(SELECT * FROM FTR WHERE N_FACTURA=@N_FACTURA AND DATEDIFF(DAY,F_FACTURA,DBO.FNK_GETDATE())<=7)
               BEGIN
                  EXEC SPK_ANULAR_DOCUMENTO_FACTEP @N_FACTURA,NULL,@RAZONANULA,@USUANULA
               END
               ELSE
               BEGIN
   ...
   BEGIN
      ...
      SELECT @CNSFTR=CNSFTR,@N_FACTURA=N_FACTURA,@IDSEDEF=IDSEDEF,	@F_FACTURA=F_FACTURA,@F_VENCE=F_VENCE,	@IDTERCEROF=IDTERCERO,	@IDPLAN=IDPLAN,	
          ...
          @SINCNT=CASE WHEN COALESCE(SINCNT,'false')='false' THEN 0 ELSE 1 END,@TIPOVENTA=TIPOVENTA,@CUOTAS=CUOTAS
      FROM   OPENJSON (@DATOS)
      WITH   (
  ...
  --query 2
   ...
   BEGIN 
      ...
      BEGIN
         ...
         RETURN
      END
      IF  EXISTS(SELECT * FROM FTR WHERE N_FACTURA=@N_FACTURA AND COALESCE(CONTABILIZADA,0) IN(1,2) AND @IXCOUNTRY <>'Peru')
      BEGIN
      ...
      BEGIN
         ...
         RETURN
      END
      IF @IXCOUNTRY='PERU' AND EXISTS(SELECT * FROM FTR WHERE N_FACTURA=@N_FACTURA AND COALESCE(FACTEP,'')<>'102')
      BEGIN
...
--QUERY 3
   ...
   BEGIN         
      ...
      IF EXISTS(SELECT * FROM FTR WHERE IDTERCERO=@IDTERCERO AND IDSEDE=@IDSEDEF AND CONTABILIZADA=0 AND COALESCE(FACTEP,'')='')
      BEGIN
         SELECT 'KO' KO
      END
      ELSE
   ...
   BEGIN         
      ...
      BEGIN 
         ...
         IF NOT EXISTS(SELECT * FROM HPRE WHERE HPRE.NOADMISION=@NOADMISION AND PREFIJO=@PREFIJO)
         BEGIN
   ...
   BEGIN         
      ...
      IF EXISTS(SELECT * FROM HPRED WHERE NOPRESTACION=@NOPRESTACION AND NOITEM=@NOITEM)
      BEGIN
         DELETE HPRED WHERE NOPRESTACION=@NOPRESTACION AND NOITEM=@NOITEM 

         IF EXISTS(SELECT * FROM HPRED WHERE NOPRESTACION=@NOPRESTACION)
         BEGIN
	         SELECT @VT = ROUND(SUM(HPRED.VALOR * HPRED.CANTIDAD),0), @VE = SUM(HPRED.VALOREXCEDENTE),
			         @VC = SUM(HPRED.VALORCOPAGO), @VP = SUM(HPRED.VALORPCOMP)
	         FROM   HPRED
   ...
   BEGIN         
      ...
      IF EXISTS(SELECT * FROM  HPRE INNER JOIN HPRED ON HPRE.NOPRESTACION=HPRED.NOPRESTACION AND HPRE.NOADMISION=@NOADMISION)
         AND EXISTS(SELECT * FROM HADM WHERE NOADMISION=@NOADMISION AND CERRADA=0)
      BEGIN
   ...
   BEGIN         
      ...
      IF EXISTS(SELECT * FROM  HPRE INNER JOIN HPRED ON HPRE.NOPRESTACION=HPRED.NOPRESTACION AND HPRE.NOADMISION=@NOADMISION)
         AND EXISTS(SELECT * FROM HADM WHERE NOADMISION=@NOADMISION AND CERRADA=1)
      BEGIN
   ...
   BEGIN         
      ...
      PRINT '@N_FACTURA='+@N_FACTURA
      IF EXISTS(SELECT * FROM FTR WHERE N_FACTURA=@N_FACTURA AND COALESCE(FACTE,0)=0 AND ESTADO='P' AND CONVERT(DATE,F_FACTURA)<CONVERT(DATE,GETDATE()))
      BEGIN
