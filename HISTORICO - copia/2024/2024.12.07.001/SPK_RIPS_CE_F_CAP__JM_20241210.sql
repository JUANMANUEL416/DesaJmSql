...
BEGIN  
   ...
   SELECT @CNSFCT = CNSFCT FROM FTR WHERE N_FACTURA = @N_FACTURA
      ...
      IF EXISTS(SELECT  1 FROM FTRD WHERE N_FACTURA=@N_FACTURA AND REFERENCIA<>@IDPLAN)
   BEGIN
	  PRINT 'SE VERIFICA QUE SOLO TENGA UN CONTRADO EN FTRD '-- FACTURA CAPITADA CON VARIOS PLANES
	  SELECT @IDPLAN=''
   END
   ...
   SELECT DISTINCT @ENVIO, @IDTERCERO, LEFT(AFI.TIPO_DOC,2) AS TIPO_DOC, AFI.DOCIDAFILIADO,
   CODENTIDADM = CASE WHEN COALESCE(PPT.IDALTERNA,'')='' THEN  LEFT(ISNULL(TER.IDALTERNA2,''),6) ELSE COALESCE(PPT.IDALTERNA,'') END,
   ...
   FROM   AUT INNER JOIN RIPSD ON RIPSD.NOADMISION  = AUT.IDAUT
			   ...
			   LEFT  JOIN PPT   ON PPT.IDTERCERO=TER.IDTERCERO AND PPT.IDPLAN=PLN.IDPLAN
   ...
   INSERT INTO RRIPS_AC(ENVIO,IDTERCERO,ESTADO,NUMFACTURA,IDPRESTADOR,TIPO_DOC,IDUSUARIO,
	...
	SELECT RIPSD.ENVIO, RIPSD.IDTERCERO, 'I',CASE WHEN @PGP=0 THEN  @N_FACTURA ELSE 'AC'+REPLACE(LTRIM(REPLACE(@ENVIO,'0',' ')),' ','0')+DBO.FNK_NROCONCEROS_IZQUIERDA(ROW_NUMBER() OVER (ORDER BY @ENVIO),5,'') END, @CODPREST,
			 LEFT(AFI.TIPO_DOC,2),  
			 ...
			 LEFT(CASE
					WHEN ( SELECT TOP 1 LEFT(HCAD.ALFANUMERICO,2)									-- B001. PORSALUD R.JAY 2009.05.21
                      ...
                      WHERE  HCA.IDAFILIADO = AUT.IDAFILIADO AND MPL.AMBITO IN('CE','GS')
		...
		CASE 
			WHEN (	SELECT TOP 1 TIPODX
					...
					WHERE HCA.IDAFILIADO = AUT.IDAFILIADO AND MPL.AMBITO IN('CE','GS') AND COALESCE(HCA.ANULADA,0)=0) IS NULL
			THEN 1
			ELSE
				(	SELECT	TOP 1 
							CASE TIPODX 
							...
							END
					FROM HCA LEFT JOIN MPL ON HCA.CLASEPLANTILLA = MPL.CLASEPLANTILLA
					WHERE HCA.IDAFILIADO = AUT.IDAFILIADO AND MPL.AMBITO IN('CE','GS') AND COALESCE(HCA.ANULADA,0)=0)
		END,
   ...
   SELECT @N_AC=COUNT(*) FROM RRIPS_AC WHERE ENVIO=@ENVIO AND IDTERCERO=@IDTERCERO
		SELECT HCA.FECHA, HCA.NOADMISION, HCA.IDAFILIADO, HCA.CONSECUTIVO,HCA.IDDX DXPPAL,HCA.CONSECUTIVOCIT, 
		...
		FROM HCA LEFT JOIN (
			   ...
			   INNER  JOIN CIT			ON HCA.IDAFILIADO = CIT.IDAFILIADO AND  HCA.CONSECUTIVOCIT=CIT.CONSECUTIVO AND CONVERT(VARCHAR,HCA.FECHA,102) = CONVERT(VARCHAR,CIT.FECHA,102) AND COALESCE(HCA.ANULADA,0)=0
   ...
   SELECT @DXSINHC = LEFT(DBO.FNK_VALORVARIABLE('RIPS_DX_CITA_SIN_HC'),4)
	...
	INSERT INTO RRIPS_AC( ENVIO, IDTERCERO, ESTADO, NUMFACTURA,   IDPRESTADOR, TIPO_DOC,    IDUSUARIO,
							    ...
							    VALORNETO,     NOPRESTACION, NOITEM,      CNSFTR,      N_CUOTA,       CANTIDAD,
							    PROCEDENCIA, IDDX, IDSERVICIO, CODEVAL)
SELECT @ENVIO, @IDTERCERO, 'I', CASE WHEN @PGP=0 THEN  ISNULL(RIPSD.N_FACTURA,'') ELSE 'AC'+REPLACE(LTRIM(REPLACE(@ENVIO,'0',' ')),' ','0')+DBO.FNK_NROCONCEROS_IZQUIERDA(ROW_NUMBER() OVER (ORDER BY @ENVIO)+@N_AC,5,'') END, @CODPREST,
			 LEFT(AFI.TIPO_DOC,2),  
			 ...
			 LEFT(CASE
					 WHEN (SELECT TOP 1 LEFT(HCAD.ALFANUMERICO,2)								-- B002. PORSALUD R.JAY 2009.05.21
								FROM   HCA	LEFT JOIN MDX  ON HCA.IDDX = MDX.IDDX
											LEFT JOIN MPL  ON HCA.CLASEPLANTILLA = MPL.CLASEPLANTILLA
											LEFT JOIN HCAD ON HCA.CONSECUTIVO = HCAD.CONSECUTIVO AND HCAD.CAMPO = 'FINALIDAD'
								WHERE  HCA.IDAFILIADO = CIT.IDAFILIADO AND MPL.AMBITO IN('CE','GS')
			...
			LEFT(CASE 
				...
				-- WHEN SER.IDALTERNA = '890203' THEN 'Z012' -- PORSALUD R.JAY 2009.03.31
				      WHEN COALESCE ((SELECT TOP 1 MDX.IDDX 
						       ...
						       WHERE  HCA.IDAFILIADO = CIT.IDAFILIADO AND  HCA.CONSECUTIVOCIT=CIT.CONSECUTIVO 
         ...
         --TIPODX
			CASE	WHEN COALESCE((SELECT TOP 1 TIPODX
							         FROM HCA LEFT JOIN MPL ON HCA.CLASEPLANTILLA = MPL.CLASEPLANTILLA
							         WHERE HCA.IDAFILIADO = CIT.IDAFILIADO AND HCA.CONSECUTIVOCIT=CIT.CONSECUTIVO  AND MPL.AMBITO IN('CE','GS') AND COALESCE(HCA.ANULADA,0)=0),'')=''
					THEN 1
					ELSE (SELECT TOP 1 
								CASE TIPODX 
									WHEN 'Presuntivo'   THEN 1
   ...
   SELECT @ENVIO, @IDTERCERO, 'I', CASE WHEN @PGP=0 THEN  @N_FACTURA ELSE 'AP'+REPLACE(LTRIM(REPLACE(@ENVIO,'0',' ')),' ','0')+DBO.FNK_NROCONCEROS_IZQUIERDA(ROW_NUMBER() OVER (ORDER BY @ENVIO),5,'') END, @CODPREST, 
          LEFT(AFI.TIPO_DOC,2),          
		...
		LEFT(CASE 
				...
				-- WHEN SER.IDALTERNA = '890203' THEN 'Z012' -- PORSALUD R.JAY 2009.03.31
				      WHEN COALESCE ((SELECT TOP 1 MDX.IDDX 
						       FROM   HCA	LEFT JOIN MDX ON HCA.IDDX = MDX.IDDX
								            LEFT JOIN MPL ON HCA.CLASEPLANTILLA = MPL.CLASEPLANTILLA
						       WHERE  HCA.IDAFILIADO = CIT.IDAFILIADO AND  HCA.CONSECUTIVOCIT=CIT.CONSECUTIVO AND MPL.AMBITO IN('CE','GS')
   ...
   INSERT INTO RRIPS_AP(ENVIO,IDTERCERO,ESTADO,NUMFACTURA,IDPRESTADOR,TIPO_DOC,IDUSUARIO,FECHAPROCEDIMIENTO,
                        ...
                        COMPLICACION,ACTOQ, VALORPROCED, NOPRESTACION, NOITEM, CNSFTR, N_CUOTA, 
                        CANTIDAD, PROCEDENCIA, IDDX, IDSERVICIO, CODEVAL)
   SELECT @ENVIO, @IDTERCERO, 'I', CASE WHEN @PGP=0 THEN  @N_FACTURA ELSE 'AP'+REPLACE(LTRIM(REPLACE(@ENVIO,'0',' ')),' ','0')+DBO.FNK_NROCONCEROS_IZQUIERDA(ROW_NUMBER() OVER (ORDER BY @ENVIO)+@N_AP,5,'') END, @CODPREST, 
          LEFT(AFI.TIPO_DOC,2),          
  ...
  IF @RIPS_AGRUPADOS = 'SI'
   ...
   PRINT 'CREANDO AM CE'
   PRINT 'ENTRE AQUI ERROR: '+COALESCE(@N_FACTURA,'')
   INSERT INTO RRIPS_AM(ENVIO,IDTERCERO,ESTADO,NUMFACTURA,IDPRESTADOR,TIPO_DOC,IDUSUARIO,NUMAUT,
                        ...
                        FECHAMEDICAMENTO) 
   SELECT @ENVIO, @IDTERCERO, 'I',CASE WHEN @PGP=0 THEN  @N_FACTURA 
   ELSE 'AM'+REPLACE(LTRIM(REPLACE(@ENVIO,'0',' ')),' ','0')+DBO.FNK_NROCONCEROS_IZQUIERDA(ROW_NUMBER() OVER (ORDER BY @ENVIO),5,'') END, @CODPREST, 
          LEFT(AFI.TIPO_DOC,2),  
   ...
   INSERT INTO RRIPS_AT(ENVIO, IDTERCERO, ESTADO, NUMFACTURA, IDPRESTADOR, TIPO_DOC, IDUSUARIO, NUMAUT, TIPOSERVICIO,
                        ...
                        FECHAPROCEDIMIENTO )
   SELECT @ENVIO, @IDTERCERO, 'I', CASE WHEN @PGP=0 THEN  @N_FACTURA ELSE 'AT'+REPLACE(LTRIM(REPLACE(@ENVIO,'0',' ')),' ','0')+DBO.FNK_NROCONCEROS_IZQUIERDA(ROW_NUMBER() OVER (ORDER BY @ENVIO),5,'') END, @CODPREST, 
          LEFT(AFI.TIPO_DOC,2),  
          REPLACE(AFI.DOCIDAFILIADO,'.',''), LEFT(AUT.NUMAUTORIZA,15),  
