...
BEGIN
   ...
   BEGIN 
      ...
      AND    #TH.CODCONCEPTO<>DBO.FNK_VALORVARIABLE('IDCONCEPTO_RETEFTE')
     ORDER BY NCON.NATURALEZA DESC  

      IF EXISTS(SELECT * FROM #TH WHERE COALESCE(CODCONCEPTO,'')=LTRIM(RTRIM(DBO.FNK_VALORVARIABLE('IDCONCEPTO_RETEFTE') )))
      BEGIN
         DECLARE @BASEIMP DECIMAL(14,2)
         DECLARE @CNSLIQRET VARCHAR(20)
         DECLARE @UVTLIQ DECIMAL(7,2)
         SELECT TOP 1 @BASEIMP=BASERET,@CNSLIQRET=CNSLIQRET,@UVTLIQ=UVTLIQ  FROM RETEL WHERE IDTERCERO=@NUMDOC AND NOREFERENCIA=@CNSPAGO 

         INSERT INTO MCHE(NROCOMPROBANTE,COMPANIA,TIPO,CUENTA,IDTERCERO,CCOSTO,DETALLE,VALOR,BASE_IMP,VALOR_PORCIMP,REFERENCIA1,  
         ...
         SELECT @NROCOMPROBANTE, @COMPANIA, CASE WHEN NCON.TIPO = 'Ingreso' THEN 'DB' ELSE 'CR' END,   
                ...
                NCON.CODCONCEPTO, #T.CCOSTO, 'S', #T.CODNOMINA,0), CASE  WHEN COALESCE(#TH.RECOBRO,0)=1 AND  NCON.TIPO = 'Ingreso'  THEN #TH.IDTERCERO_RECO 
                                                                   WHEN COALESCE(#TH.RECOBRO,0)=1 AND  NCON.TIPO = 'Egreso' THEN @NUMDOC
                                                                   ELSE #TH.IDTERCERO_PRF END, #T.CCOSTO, UPPER(NCON.DESCRIPCION), #TH.VALOR,  
                @BASEIMP,@UVTLIQ,@CNSPAGO,@NUMDOC,@CNSLIQRET,NULL,@USUARIO, DBO.FNK_FECHA_SIN_MLS(GETDATE()),#TH.CODCONCEPTO, #T.IDAREA,'S', 1,  
                'NOMINA_NCON', 'Movimiento', @CNSPAGO, DBO.FNK_FECHA_SIN_HORA(@FECHATRA), DBO.FNK_FECHA_SIN_HORA(@FECHATRA),   
                'NOMINA','NOMINA', @NUMDOC, #T.CODUNG, NEMP.CODPRG,#T.IDSEDE
         FROM #T INNER JOIN #TH  ON #T.CNSNIEPE     = #TH.CNSNIEPE 
                                AND #T.NUMDOC       = #TH.NUMDOC 
                 ...
                 INNER JOIN NCOND ON X.CODCONCEPTO=NCOND.CODCONCEPTO AND X.ITEM=NCOND.ITEM
         WHERE #TH.VALOR<>0 AND #TH.INFO='No'  
         AND    COALESCE(#TH.CODCONCEPTO,'')= LTRIM(RTRIM(DBO.FNK_VALORVARIABLE('IDCONCEPTO_RETEFTE') ))
         ORDER BY NCON.NATURALEZA DESC 
