     ...
     SELECT    @PROCESO        =   PROCESO ,
      ...
      WITH (	
         ...
         SELECT @IDCJFPAEFECTIVO=DBO.FNK_VALORVARIABLE('IDCJFPAEFECTIVO')
			SELECT @TOTAL = SUM(PCJ.VALOR) FROM FCJ
            ...
            BEGIN
               ...
               BEGIN
                  ...
                  BEGIN
                     ...
                     --Message('Se ha Anulado el Comprobante Original del Egreso','Anulación',icon:asterisk)
                  END
                  IF @CONTABILIZADA_FCJ = 2
                  BEGIN
                     DELETE MCHE WHERE NROCOMPROBANTE=@NROCOMPROBANTE
...
--QUERY 3
   ...
   BEGIN
      ...
      FROM OPENJSON(@PARAMETROS)WITH(
	   ...
	   BEGIN 
         SELECT FCJ.CNSFACJ, FCJ.CODCAJA,CAJ.DESCRIPCION, FCJ.CLASE_FAC, FCJ.VALORTOTAL, FCJ.COMPANIA, FCJ.IDAFILIADO,CJR.DESCRIPCION [DESCJR]
   ...
   IF @METODO = 'INSERT_FCJD_EGRESO'
	BEGIN
      SELECT  @CODCAJA = CODCAJA, @CNSFACJ = CNSFACJ, @CONCEPTO = CONCEPTO, @VLR_UNIT = VLR_UNIT , @CANTIDAD = CANTIDAD
      ...
      BEGIN
         BEGIN TRY
            INSERT INTO FCJD (CODCAJA, CNSFACJ, ITEM, CONCEPTO, VALORUNITARIO, CANTIDAD, VALORTOTAL, IDTERCERO, UNSPSC)
            SELECT @CODCAJA, @CNSFACJ, 1, @CONCEPTO, COALESCE( @VLR_UNIT, 0) , COALESCE(@CANTIDAD,0), COALESCE( @VLR_UNIT, 0) *  COALESCE(@CANTIDAD,0), @TERCERO, @UNS
         END TRY
         BEGIN CATCH
            INSERT INTO @TBLERRORES(ERROR) SELECT ERROR_MESSAGE();
         END CATCH
   ...
   IF @METODO = 'INSERTA_RECIBO'
	...
	IF @TIPO = 'E'
	BEGIN
		SELECT  @IDAFILIADO = IDAFILIADO, @OBSERVACION = OBSERVACION FROM #DATOS
		SELECT @NOMBREAFI = NOMBREAFI FROM AFI WHERE  IDAFILIADO = @IDAFILIADO
	END
		...
		BEGIN
         ...
         BEGIN TRY
			...
			SELECT @CNSFACJ =  REPLACE(SPACE(8 - LEN(@CNSFACJ))+LTRIM(RTRIM(@CNSFACJ)),SPACE(1),0)
			 ...
			 INSERT INTO FCJ (CNSFACJ,TIPO,CODCAJA,COMPANIA, CLASE_FAC,CNSACJ,ESTADO,USUARIO,SYS_ComputerName,IDTERCERO, FECHA, TIPOEGRESO, OBSERVACION, PROCEDENCIA, VALORTOTAL , USUARIOLOG
			 ,BANCO, SUCURSAL, CTA_BCO, NOADMISION, CERRADA, ARCHIVADO, IDPLAN, IDAFILIADO, NOMBRECLIENTE , DEAPERTURA)
			 
			 SELECT @CNSFACJ, 'INTERNO', @CODCAJA, @COMPANIA, 'COBRO', @CNSACJ, 'P', @CODCAJERO, @SYS_COMPUTERNAME, @IDTERCERO, DBO.FNK_FECHA_SIN_MLS(GETDATE()), null, @OBSERVACION, 'CAJA',0, @USUARIO
			, null,null,null, @NOADMISION, 0, 0, @IDPLAN, @IDAFILIADO, @NOMBREAFI , 0 
			FROM #DATOS
