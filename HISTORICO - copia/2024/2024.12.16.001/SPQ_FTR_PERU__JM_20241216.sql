...
BEGIN
   ...
   BEGIN 
     ...
     IF  EXISTS(SELECT *
	  ...
	  SELECT TOP 1 FTR.N_FACTURA,CONVERT(VARCHAR,F_FACTURA,103)F_FACTURA,FTR.VALORSERVICIOS,VIVA,VR_TOTAL,
     COALESCE(AFI.EMAIL,TER.EMAIL)CORREO,AFI.DOCIDAFILIADO,AFI.NOMBREAFI
      FROM FTR INNER JOIN FTRE ON FTR.N_FACTURA=FTRE.FTRN_FACTURA
               INNER JOIN TER  ON FTR.IDTERCERO=TER.IDTERCERO
               LEFT  JOIN AFI  ON FTR.IDAFILIADO=AFI.IDAFILIADO
      WHERE FTRE.NOTIFICADO=0
      ...
      AND CHARINDEX(COALESCE(AFI.EMAIL,TER.EMAIL),'@',1)>0
      AND CHARINDEX(COALESCE(AFI.EMAIL,TER.EMAIL),'.',1)>0
