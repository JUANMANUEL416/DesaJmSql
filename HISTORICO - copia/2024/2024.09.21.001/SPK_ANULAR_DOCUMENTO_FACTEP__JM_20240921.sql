...
BEGIN
   ...
   BEGIN
      ...
      END
      IF COALESCE(@FACTEP,'') NOT IN('OK','ERROR','Anulada')
      BEGIN
         IF @TIPODOC IN('03','01')
         BEGIN
     ...
     PRINT '@sUrl='+@sUrl
      ...
      BEGIN
         IF @TIPODOC IN('03','01')
         BEGIN
            UPDATE FTR SET ESTADO='A',RAZONANULACION=@MOTIVO,FACTEP=@ESTDOC,ULRPTAAPI=COALESCE(@ERRORES,@DESCSUNAT) WHERE N_FACTURA=@N_FACTURA
            IF EXISTS(SELECT * FROM MCP WHERE NROCOMPROBANTE=@NROCOMPORBANTE)
            BEGIN
               UPDATE MCP SET ESTADO=1,ANULADO=1 WHERE NROCOMPROBANTE=@NROCOMPORBANTE
            END
      ...
      ELSE
      BEGIN
         UPDATE FTR SET ESTADO='A',RAZONANULACION=@MOTIVO,ULRPTAAPI=COALESCE(@ERRORES,@DESCSUNAT) WHERE N_FACTURA=@N_FACTURA
      END
      IF @TIPODOC IN('03','01')
      BEGIN
         IF @PROCEDENCIA='SALUD'
         ...
         BEGIN
            ...
            BEGIN
               IF @TIPODOC='01'
               BEGIN
                  UPDATE CIT SET FACTURADA=0,N_FACTURA=NULL,CNSFACT=NULL WHERE CONSECUTIVO=@NOREFERENCIA AND N_FACTURA=@N_FACTURA
               END
               ELSE
               BEGIN
                  UPDATE CIT SET NFACTURA=NULL WHERE CONSECUTIVO=@NOREFERENCIA AND NFACTURA=@N_FACTURA
                  IF EXISTS(SELECT * FROM CIT WHERE N_FACTURA=@N_FACTURA)
                  BEGIN
                     UPDATE CIT SET FACTURADA=0,N_FACTURA=NULL,CNSFACT=NULL WHERE CONSECUTIVO=@NOREFERENCIA AND N_FACTURA=@N_FACTURA
                  END
               END
