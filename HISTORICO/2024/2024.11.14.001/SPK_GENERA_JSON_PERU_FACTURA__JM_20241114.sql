...
DECLARE @NOM_COMER_EMIS VARCHAR(125)
...
DECLARE @TEXTODETALLE VARCHAR(10)
BEGIN  
   ...
   @TEXTODETALLE=CASE WHEN TIPOFIN='N' THEN 'Copago ' ELSE '' END
   ...
   SELECT @NOM_COMER_EMIS=DBO.FNK_VALORVARIABLE('NOMBRE_COMERCIAL')
   

   SELECT @NITRECEP=CASE WHEN FTR.IDTERCERO=FTR.IDAFILIADO AND COALESCE(TER.RUC,'')<> '' AND FTR.TIPOFIN='C' THEN TER.RUC ELSE TER.NIT END ,
          ...
          @RAZONSOCIALRECEP=dbo.FNK_LIMPIATEXTO(RAZONSOCIAL,'0-9 A-Z().;:,'),@CODPOSTALRECEP='150101',@DIRECCIORECEP= dbo.FNK_LIMPIATEXTO(CASE WHEN FTR.TIPOFIN='C' THEN TER.DIRECCION ELSE COALESCE(AFI.DIRECCION,TER.DIRECCION,'') END,'0-9 A-Z().;:,'),@CIUDADRECEP=CIU.NOMBRE,
   @DEPARRECEP=DEP.NOMBRE,@DISTRIRECEP='',@PAISRECEP='PE',@CODSUNAT='0000', @MAILRECEPTOR= LOWER(CASE WHEN FTR.TIPOFIN='C' THEN COALESCE(TER.EMAIL,'') ELSE COALESCE(AFI.EMAIL,TER.EMAIL,'') END)
   ...
   SELECT @JSON=@JSON+'"NOM_COMER_EMIS": "'+COALESCE(@NOM_COMER_EMIS,@RAZONSOCIALEMI)+'",'
      ...
      ORDER BY N_CUOTA ASC
	   ...
	   BEGIN 
         SELECT @ITEM=CONVERT(varchar(3),N_CUOTA),@CANTIDAD=CONVERT(varchar(4),CONVERT(INT,CEILING(FTRD.CANTIDAD))),
                ...
                @VALOR7=CONVERT(VARCHAR(20),CONVERT(DECIMAL(14,2),ROUND(((COALESCE(VALOR,0)+COALESCE(VIVA,0))*COALESCE(FTRD.CANTIDAD,0))-CASE WHEN COALESCE(FTRD.VLR_COPAGOS,0)>0 THEN FTRD.VLR_COPAGOS ELSE 0 END ,2))),
				    @ANEXO=@TEXTODETALLE+dbo.FNK_LIMPIATEXTO(CASE WHEN @PROCEDENCIAFTR='FINANCIERO' THEN FTRD.ANEXO ELSE CASE WHEN COALESCE(SER.DESCRIPCION_CUPS,'')='' THEN SER.DESCSERVICIO ELSE SER.DESCRIPCION_CUPS END END,'0-9 A-Z'), 
