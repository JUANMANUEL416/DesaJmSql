...
BEGIN
  ...
  -- RETURN
   ...
   BEGIN
     ...
     SELECT @PORCENTAJE=(CONVERT(DECIMAL(14,6),(VALOR_EGRESOS*100/(VALOR_INGRESOS-coalesce(@VLR_SINDTO,0))))/100),@VLR_INGRESOS=VALOR_NETO 
	  FROM NIEPE WHERE CNSPAGO=@CNSPAGO AND NUMDOC=@NUMDOC2
     ...
     END,CCOSTO,IDAREA,CUENTA,FD.CNSPAGO,LINEA,CODCONCEPTO,DESCRIPCION,RUBRO,RUBRO_ID,SINDEDUC
      FROM (
      SELECT A.IDTERCERO,''NUMDOC, CANTIDAD = 1,SUM(A.VALOR)VALOR,SUM(H.VALOR_EGRESOS)VALOR_EGRESOS,
			  F.CCOSTO, F.IDAREA,
