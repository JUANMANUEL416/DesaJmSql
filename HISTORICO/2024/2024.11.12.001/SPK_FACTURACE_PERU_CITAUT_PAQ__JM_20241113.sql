CREATE OR ALTER PROCEDURE DBO.SPK_FACTURACE_PERU_CITAUT_PAQ
@NOAUT		VARCHAR(16),
@NIT			VARCHAR(20),
@COMPANIA	VARCHAR(2),
@IDSEDE		VARCHAR(5),
...
BEGIN
   PRINT 'SPK_FACTURACE_PERU_CITAUT_PAQ '+@IDAUT
	SELECT  @SYS_COMPUTERNAME = HOST_NAME(),@LIBERADA=0
	-- SACAR DE DONDE SE TOMA EL CCOSTO
	IF @EC IS NULL
			SELECT @EC = 0

   SELECT @IDTERCERO1=IDTERCEROCA,@IDTERCEROCA=IDTERCEROCA,@IDTERCEROF=IDTERCEROCA,@IDTERCERO=IDTERCEROCA
   FROM CIT WHERE CONSECUTIVO=@IDAUT

   SELECT IDTERCEROCA FROM CIT WHERE CONSECUTIVO=@IDAUT
   ...
   AND COALESCE(AUT.LISTOFACT,'PEND')='OK'
   ...
   INSERT INTO #FTRD1(CNSFTR, FECHA, DB_CR, AREAPRESTACION, UBICACION, VR_TOTAL,
		   ...
		   NOPRESTACION, NOITEM, AREAFUNCONT, N_FACTURA, SUBCCOSTO, PCOSTO, FECHAPREST, IDPLAN,IDIMPUESTO, IDCLASE,ITEM ,
         VLRIMPUESTO, PIVA, VIVA,CUENTA_FIMPDV,PROCEDENCIA,PAQUETE)
   SELECT TOP 1 @CNSFTR, GETDATE(), 'DB', CIT.IDAREA, NULL,@VLRPAQ, 
			   NULL, CIT.CCOSTO, SER.PREFIJO, SER.DESCSERVICIO, @PAQUETE, NULL,COALESCE(@CANTPAQ,1) CANTIDAD,
            (@VLRPAQ/COALESCE(@CANTPAQ,1)), @VLRPAQ, COALESCE(VALORCOPAGO,0), 0,CIT.IDMEDICO, @NOAUT, @NOAUT, 1, NULL, @NVOCONSEC, CIT.SUBCCOSTO,
            CIT.VALORTOTALCOS, CIT.FECHA, CIT.IDPLAN,
			   CASE WHEN COALESCE(SER.IVA,'')='SER' THEN  SER.IDIMPUESTO ELSE NULL END,
			   CASE WHEN COALESCE(SER.IVA,'')='SER' THEN  SER.IDCLASE ELSE NULL END,
   ...
   BEGIN   
      ...
      EXEC SPK_INSERT_TERDEAFI @IDAFILIADO, @IDCATEGORIA,'CI',@NOAUT
      ...
      END
	   ...
	   BEGIN
		   ...
		   SELECT @CNSFTR, @COMPANIA, 'C', CASE WHEN COALESCE(@IDTERCERO_RUC,'')<>'' THEN @IDTERCERO_RUC ELSE  CASE WHEN @TV='Contado' AND COALESCE(@ESPARTPERU,0) =1 THEN (CASE @IDTERCEROCA WHEN dbo.FNK_VALORVARIABLE('IDTERCEROPARTICULAR') 
				   ...
				   'CI', 'I', REPLACE(COALESCE(@OBSERVACION,''),'@N_FACTURA',@NVOCONSEC), @TV, 0, 0, 0, 0, 0, 0, 0, 0, NULL, 0, 0, 0, 0, NULL, NULL, @USUARIO,
	   ...
	   BEGIN
		   ...
		   UPDATE FTR SET ESTADO='P',CONTABILIZADA=0,NROCOMPROBANTE=NULL,NOREFERENCIA=@NOAUT, IDAFILIADO=@IDAFILIADO,PROCEDENCIA='CI',
