...
BEGIN  
   ...
   @IDPLAN=FTR.IDPLAN,@USUMEDICO='Pruebas Med',@ESPECIALIDAD='Pruebas Especialidad'
 ...
 SELECT @JSON=@JSON+'"DE_EMP_EMISORA": "CI10",'   
 SELECT @JSON=@JSON+'"CO_SEDE_SAP": "1",'   
 SELECT @JSON=@JSON+'"DE_CENTRO": "Centro_corto",'   
 SELECT @JSON=@JSON+'"CO_SOCIEDAD_SAP": "1",'   
 ...
 SELECT @JSON=@JSON+'"DE_PAIS": "'+@CODPAIS1+'",'   
 SELECT @JSON=@JSON+'"DE_PROV": "'+@CIUDAD_AFI+'",'   
 SELECT @JSON=@JSON+'"DE_DIST": "'+@DISTRITO_AFI+'",'   
 SELECT @JSON=@JSON+'"CO_REGI_SSAP": "CO_REGI_SSAP",'   
 SELECT @JSON=@JSON+'"DE_REGI": "@DE_REGI",'   
 SELECT @JSON=@JSON+'"NU_TELEF": "'+@TELEFONO_AFI+'",'   
 SELECT @JSON=@JSON+'"CO_POSTAL": "@CO_POSTAL",'   
 ...
 SELECT @JSON=@JSON+'"DE_TIPO_DEPO": "4",'   
 SELECT @JSON=@JSON+'"CO_USUA_TRAN": "@CO_USUA_TRAN",'   
 SELECT @JSON=@JSON+'"CO_ORG_VTA_SAP": "@CO_ORG_VTA_SAP",'   
 SELECT @JSON=@JSON+'"CO_CANAL_DIST": "@CO_CANAL_DIST",'   
 SELECT @JSON=@JSON+'"CO_GRU_VEN_SAP": "@CO_GRU_VEN_SAP",'   
 SELECT @JSON=@JSON+'"CO_OF_VTA_SAP": "@CO_OF_VTA_SAP",'   
      ...
      ORDER BY N_CUOTA ASC
	   ...
	   BEGIN 
         ...
         SELECT @JSOND=@JSOND+'   '
			...
			SELECT @JSOND=@JSOND+'"ID_ITEM_PREFA": "'+CAST(@N_CUOTA AS VARCHAR(10))+'",'   
			SELECT @JSOND=@JSOND+'"PK_VALE_PREFA": "PK_VALE_PREFA",'   
			SELECT @JSOND=@JSOND+'"NU_ENCUENTRO": "NU_ENCUENTRO",'   
			SELECT @JSOND=@JSOND+'"TI_ENCUENTRO": "2",'   
			SELECT @JSOND=@JSOND+'"CO_ASEGURA": "'+@DNI+'",'   
			SELECT @JSOND=@JSOND+'"CO_AUTORIZA": "'+@SITTEDS+'",'   
			SELECT @JSOND=@JSOND+'"CO_CARTA_GARA": "CO_CARTA_GARA",'   
			SELECT @JSOND=@JSOND+'"CO_MECA_SAP": "'+@IDPLAN+'",'   
			...
			SELECT @JSOND=@JSOND+'"NO_TITULAR": "'+@NITRECEP+'",'   
			...
			SELECT @JSOND=@JSOND+'"DE_PLAN_COPA": "DE_PLAN_COPA",'   
			SELECT @JSOND=@JSOND+'"CO_GARANTE": "CO_GARANTE",'   
			SELECT @JSOND=@JSOND+'"CO_SERV_ENCU": "'+@ESPECIALIDAD+'",'     
			SELECT @JSOND=@JSOND+'"CO_PLAN_COPA": "'+@IDPLAN+'",'   
			SELECT @JSOND=@JSOND+'"CO_SERV_GAST": "CO_SERV_GAST",'   
			SELECT @JSOND=@JSOND+'"IM_DEDU_GAST": "IM_DEDU_GAST",'   
			SELECT @JSOND=@JSOND+'"PC_COASEGU": "PC_COASEGU",'   
			SELECT @JSOND=@JSOND+'"CO_ESPE_SSAP": "@CO_ESPE_SSAP",'   
			SELECT @JSOND=@JSOND+'"CO_SUB_MECA": "CO_SUB_MECA",'   
			SELECT @JSOND=@JSOND+'"CO_SECTOR": "CO_SECTOR",'   
			SELECT @JSOND=@JSOND+'"CO_TIPO_ATEN": "'+@PREFIJO+'",'   
			SELECT @JSOND=@JSOND+'"ID_MEZCLA": "ID_MEZCLA",'   
			SELECT @JSOND=@JSOND+'"TI_VENTA": "TI_VENTA",'   
			SELECT @JSOND=@JSOND+'"NU_CITA_XHIS": "'+@NOADMISION+'",'   
			SELECT @JSOND=@JSOND+'"IM_PAGO": "@IM_PAGO",'   
			SELECT @JSOND=@JSOND+'"ID_TRAN_PAGO": "ID_TRAN_PAGO",'   
			SELECT @JSOND=@JSOND+'"CO_TIPO_ITEM": "CO_TIPO_ITEM",'   
			SELECT @JSOND=@JSOND+'"VA_DET_PADRE": "@VA_DET_PADRE",'   
         SELECT @JSOND=@JSOND+'   '
         SELECT @JSOND=@JSOND+' } '
         PRINT @JSOND
	   FETCH NEXT FROM XMLFTRD_CURSOR    
