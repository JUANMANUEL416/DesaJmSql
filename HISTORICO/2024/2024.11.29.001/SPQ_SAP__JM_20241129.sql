...
DECLARE  @PARAMETROS	NVARCHAR(MAX)	,@MODELO			VARCHAR(100)	,@METODO		VARCHAR(100)	,@USUARIO		VARCHAR(12)		
      ...
      ,@PRE_MED_ART DECIMAL(14,2)         ,@FEC_LOTE DATETIME              ,@LOTE VARCHAR(20) 
      ,@ID_OPE_CAB VARCHAR(20)            ,@NVOCONSEC VARCHAR(20)          ,@XmlDocumentHandle int
      ,@CO_ERROR VARCHAR(20)              ,@CO_ESTA_PREFA VARCHAR(20)      ,@DE_ERROR VARCHAR(20)
      ,@FE_COMP_SUNAT VARCHAR(20)         ,@FE_ERROR VARCHAR(20)           ,@NU_COMP_SUNAT VARCHAR(20)
      ,@PK_PREFA_PREABO VARCHAR(20)       ,@TI_COMP_XHIS VARCHAR(20)       ,@TI_ORIGEN VARCHAR(20)
      ,@EmisionFacturaSUNAT VARCHAR(MAX)  ,@NUM_LOTE VARCHAR(20)
...
BEGIN 
   ...
   BEGIN      
     ...
     BEGIN
        ...
        SELECT 'KO' KO,@METODO METODO, ERROR FROM @TBLERRORES
        RETURN
     END
     BEGIN TRY           
   ...
   BEGIN         
      ...
      SELECT @EmisionFacturaSUNAT=EmisionFacturaSUNAT
      FROM OPENJSON(@DATOS) WITH (EmisionFacturaSUNAT NVARCHAR(MAX) AS JSON)
                 
      SELECT @CO_ERROR = CO_ERROR,
      @CO_ESTA_PREFA = CO_ESTA_PREFA,
      @DE_ERROR = DE_ERROR,
      @FE_COMP_SUNAT = FE_COMP_SUNAT,
      @FE_ERROR = FE_ERROR,
      @NU_COMP_SUNAT = NU_COMP_SUNAT,
      @PK_PREFA_PREABO = PK_PREFA_PREABO,
      @TI_COMP_XHIS = TI_COMP_XHIS,
      @TI_ORIGEN = TI_ORIGEN        
      FROM   OPENJSON (@EmisionFacturaSUNAT)
      WITH   (      
         CO_ERROR VARCHAR(20)          '$.CO_ERROR',
         CO_ESTA_PREFA VARCHAR(20)     '$.CO_ESTA_PREFA',
         DE_ERROR VARCHAR(20)          '$.DE_ERROR',
         FE_COMP_SUNAT VARCHAR(20)     '$.FE_COMP_SUNAT',
         FE_ERROR VARCHAR(20)          '$.FE_ERROR',
         NU_COMP_SUNAT VARCHAR(20)     '$.NU_COMP_SUNAT',
         PK_PREFA_PREABO VARCHAR(20)   '$.PK_PREFA_PREABO',
         TI_COMP_XHIS VARCHAR(20)      '$.TI_COMP_XHIS',
         TI_ORIGEN VARCHAR(20)         '$.TI_ORIGEN'      
      )   
      
      IF EXISTS(SELECT * FROM FTR WHERE @PK_PREFA_PREABO=CONVERT(VARCHAR,CONCAT(CONVERT(INT,FTR.IDSEDE),CONVERT(INT,RIGHT(CNSFCT,LEN(FTR.IDSEDE)))),20) AND N_FACTURA<>@NU_COMP_SUNAT)
      BEGIN
         --N_FACT_EXTERNO
         UPDATE FTR SET N_FACTURA=@NU_COMP_SUNAT,PREFACTURA=N_FACTURA WHERE @PK_PREFA_PREABO=CONVERT(VARCHAR,CONCAT(CONVERT(INT,FTR.IDSEDE),CONVERT(INT,RIGHT(CNSFCT,LEN(FTR.IDSEDE)))),20)
      END
      ELSE 
      BEGIN
         INSERT INTO @TBLERRORES(ERROR)
         SELECT 'No se Encontro Documento Asociado a esta Prefactura PK_PREFA_PREABO='+@PK_PREFA_PREABO+' NU_COMP_SUNAT='+@NU_COMP_SUNAT
      END
      IF(SELECT COUNT(*) FROM @TBLERRORES)>0
      BEGIN
         SELECT 'KO' KO,@METODO METODO, ERROR FROM @TBLERRORES
