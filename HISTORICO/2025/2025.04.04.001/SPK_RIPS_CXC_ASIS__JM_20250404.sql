...
BEGIN 
   ...
   SET @PIDEAUTORIZAFACCORTE= DBO.FNK_VALORVARIABLE('PIDEAUTORIZAFACCORTE') 
   ...
   /* SE QUIETA ESTE PROCESO POR QUE ESTA DA?ANDO EL VALOR UNITARIO DEL SERVICIO DE OXIGENO*/
  ...
  -- END
   ...
   print @FORMATORIPS
   IF DBO.FNK_VALORVARIABLE('CODCUMS_FIJO')NOT IN('SER.CODCUM','IART.CODCUM','SER.CODSISPRO')
   BEGIN
      IF COALESCE(@FORMATORIPS,'')IN('01','09')-- COD CUMS SOLO
   BEGIN 
   ...
   BEGIN
      ...
      BEGIN
		   UPDATE #AM SET IDMEDICAMENTO=SER.CODCUM
		   FROM #AM A INNER JOIN HPRED D ON D.NOPRESTACION=A.NOPRESTACION AND D.NOITEM=A.NOITEM
					   INNER JOIN SER ON SER.IDSERVICIO=D.IDSERVICIO
					   LEFT JOIN IART ON D.IDARTICULO=IART.IDARTICULO
      END
      ...
      BEGIN
		   UPDATE #AM SET IDMEDICAMENTO=COALESCE(IART.CODCUM,IART1.CODCUM)
		   FROM #AM A INNER JOIN HPRED D ON D.NOPRESTACION=A.NOPRESTACION AND D.NOITEM=A.NOITEM
					   INNER JOIN SER ON SER.IDSERVICIO=D.IDSERVICIO
					   LEFT JOIN IART ON D.IDARTICULO=IART.IDARTICULO
					   LEFT JOIN IART IART1 ON D.IDARTICULO=IART1.IDSERVICIO
      END
   END
  ...
  CASE WHEN @RIPS_FEHAINIFINAL='SI' THEN CASE WHEN @PIDEAUTORIZAFACCORTE = 'SI' THEN HADMAUT.F_INICIAL ELSE DBO.FNK_FECHA_SIN_HORA(HADM.FECHA)                              END ELSE @FECHAINICIAL END F_INI,  
  CASE WHEN @RIPS_FEHAINIFINAL='SI' THEN CASE WHEN @PIDEAUTORIZAFACCORTE = 'SI' THEN HADMAUT.F_FINAL   ELSE DBO.FNK_FECHA_SIN_HORA(COALESCE(HADM.FECHAALTAMED,@FECHAFINAL)) END ELSE @FECHAFINAL END  F_FIN, 
 ...
 FROM   RIPSD   INNER JOIN FTRD   ON RIPSD.N_FACTURA     = FTRD.N_FACTURA
             ...
             LEFT JOIN HADMAUT ON HADMAUT.NOADMISION    = HADM.NOADMISION AND HADMAUT.AUTORIZACION = FTR.VENDEDOR 
   WHERE  RIPSD.ENVIO       = @ENVIO AND RIPSD.IDTERCERO=@IDTERCERO
   AND    RIPSD.PROCEDENCIA = 'SALUD'
   AND    COALESCE(FTRD.ITFC,0)         <> 2
   ELSE 
 ...
 ELSE  CONVERT (VARCHAR (15),CONVERT(DECIMAL(14,2), FTR.VR_TOTAL)) END
  ...
  BEGIN --COVID
     ...
     BEGIN
		...
		BEGIN
        UPDATE RRIPS_AF SET FECHAINICIO= CASE WHEN @FECHAINICIAL IS NULL THEN FECHAINICIO ELSE @FECHAINICIAL END, FECHAFINAL=CASE WHEN @FECHAFINAL IS NULL THEN FECHAFINAL ELSE @FECHAFINAL END   
        WHERE ENVIO=@ENVIO AND IDTERCERO=@IDTERCERO AND NUMFACTURA=@N_FACTURA
		
        UPDATE RRIPS_AU SET FECHAINGRESO= CASE WHEN @FECHAINICIAL IS NULL THEN FECHAINGRESO ELSE @FECHAINICIAL END, FECHASALIDA=CASE WHEN @FECHAFINAL IS NULL THEN FECHASALIDA ELSE @FECHAFINAL END  
        WHERE ENVIO=@ENVIO AND IDTERCERO=@IDTERCERO AND NUMFACTURA=@N_FACTURA

        UPDATE RRIPS_AH SET FECHAINGRESO= CASE WHEN @FECHAINICIAL IS NULL THEN FECHAINGRESO ELSE @FECHAINICIAL END, FECHAEGRESO=CASE WHEN @FECHAFINAL IS NULL THEN FECHAEGRESO ELSE @FECHAFINAL END   
        WHERE ENVIO=@ENVIO AND IDTERCERO=@IDTERCERO AND NUMFACTURA=@N_FACTURA
		END
        SET @I+=1
     END
