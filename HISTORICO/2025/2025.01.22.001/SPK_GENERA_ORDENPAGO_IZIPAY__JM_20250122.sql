...
BEGIN
   ...
   BEGIN
      SET @RESP =1
   END
   IF EXISTS(SELECT * FROM KPAGE WHERE CONSECUTIVO=@NOADMISION AND PROCEDENCIA=@PROCEDENCIA AND TABLA=@TABLA AND COALESCE(ESTADO,'') IN('RUNNING','SEND') AND F_EXPIRA<=GETDATE() AND @RESP=0)
   BEGIN
      PRINT 'EXISTE'
      UPDATE KPAGE SET ESTADO='EXPIRED' WHERE CONSECUTIVO=@NOADMISION AND PROCEDENCIA=@PROCEDENCIA AND TABLA=@TABLA AND COALESCE(ESTADO,'SEND') IN('RUNNING','SEND') AND F_EXPIRA<=GETDATE()
      IF @TABLA='CIT'
      BEGIN
      ...
      END
	   IF @TABLA='MAD'
      BEGIN
         UPDATE MAD SET IDKPAGE=NULL WHERE ID=@NOADMISION
      END
	   IF @TABLA='HADM'
      BEGIN
         UPDATE HADM SET IDKPAGE=NULL WHERE NOADMISION=@NOADMISION
      END
      IF @PROCESO<>'CREAR'
      BEGIN
         RETURN
      END
   END
   IF EXISTS(SELECT * FROM KPAGE WHERE CONSECUTIVO=@NOADMISION AND PROCEDENCIA=@PROCEDENCIA AND TABLA=@TABLA AND COALESCE(ESTADO,'SEND') IN('RUNNING','SEND') AND F_EXPIRA>GETDATE() AND @PROCESO='CREAR' AND @RESP = 0)
   BEGIN
   ...
   BEGIN
	  ...
	  IF @TABLA='MAD'
      BEGIN
         UPDATE MAD SET IDKPAGE=@IDKPAGE WHERE ID=@NOADMISION
      END
	  IF @TABLA='HADM'
      BEGIN
         UPDATE HADM SET IDKPAGE=@IDKPAGE WHERE NOADMISION=@NOADMISION
      END
   ...
   BEGIN
      IF @PROCESO='VALIDAR'
      BEGIN
         IF NOT EXISTS(SELECT  * FROM FNPAG WHERE IDKPAGE=@IDKPAGE)
         BEGIN
         ...
         BEGIN
            ...
            IF EXISTS(SELECT * FROM FNPAG WHERE IDKPAGE=@IDKPAGE AND COALESCE(ESTADO_OPE,'')='AUTHORISED')
            BEGIN
               UPDATE KPAGE SET ESTADO='PAID' WHERE IDKPAGE=@IDKPAGE 
            END
            ELSE
            BEGIN
               DECLARE @ID INT 
               SELECT @ID=MAX(ID) FROM FNPAG WHERE IDKPAGE=@IDKPAGE

               SELECT @RTAFNPAG = BODY FROM FNPAG WHERE IDKPAGE=@IDKPAGE AND ID=@ID
               SELECT @vads_trans_status=vads_trans_status
               FROM OPENJSON( @RTAFNPAG )
               WITH(
               ...
               ELSE
               BEGIN
                 UPDATE  KPAGE SET ESTADO=@vads_trans_status WHERE IDKPAGE=@IDKPAGE
               END
            END
            RETURN
         END
