...
BEGIN
   ...
   PRINT 'CONSULTAS CIT'
		   ...
		   numAutorizacion=COALESCE(CIT.NOAUTORIZACION,'null'),codConsulta=REPLACE(REPLACE(LTRIM(RTRIM(LEFT(SER.CODCUPS,6))),CHAR(13),''),CHAR(10),''),modalidadGrupoServicioTecSal='01',grupoServicios='01',
     ...
     --                         AND CIT.CONSECUTIVO= COALESCE (HCA.CONSECUTIVOCIT,HCA.NOADMISION) ---- VGARCIA
        ...
        PRINT 'CONSULTAS AUT'
         ...
         AND COALESCE(AUTD.VALOR,0)>0
			...
			IF EXISTS(SELECT 1 FROM @CONSULTAS)
			BEGIN
				UPDATE @CONSULTAS SET codDiagnosticoPrincipal=COALESCE(CIT.IDDX,HCA.IDDX),
				codDiagnosticoRelacionado1=HCA.DX1,
				codDiagnosticoRelacionado2=HCA.DX2,
      ...
      BEGIN
		   ...
		   SELECT @IDPRESTADOR,AUT.IDAFILIADO,AUT.NOAUT,null,fechaDispensAdmon=REPLACE(CONVERT(VARCHAR,AUT.FECHA,102),'.','-')+' '+LEFT(CONVERT(VARCHAR,AUT.FECHA,108),5),
            ...
            CASE WHEN ISNUMERIC(dbo.FNK_LIMPIATEXTO(LEFT(COALESCE(ICCN.DESCRIPCION,'0'),3),'0-9'))=1 THEN dbo.FNK_LIMPIATEXTO(LEFT(COALESCE(ICCN.DESCRIPCION,'0'),3),'0-9') ELSE 0 END,
			   COALESCE(IUNI.HOMOLOGO_RIPS,247),COALESCE(IFFA.HOMOJSON,'null'),'11',CONVERT(VARCHAR,CONVERT(INT,AUTD.CANTIDAD),10),IIF(COALESCE(AUTD.DIAS,0)=0,1,AUTD.DIAS),MED.TIPO_ID,MED.IDMEDICO,AUTD.VALOR,AUTD.VALOR*AUTD.CANTIDAD,
			   CASE WHEN COALESCE(AUTD.VALORCOPAGO,0)>0 OR COALESCE(AUT.VALORCOPAGO,0)> 0   THEN  CASE WHEN COALESCE(AFI.TIPOUSUARIO,'') IN('01','02','03')  THEN '02' ELSE '01' END ELSE @conceptoRecaudo END,
			   '04',AUTD.VALORCOPAGO,@N_FACTURA
		   FROM FTRD INNER JOIN AUT ON FTRD.NOADMISION=AUT.NOAUT AND FTRD.NOPRESTACION=AUT.IDAUT 
				       INNER JOIN AFI ON AUT.IDAFILIADO=AFI.IDAFILIADO
                   INNER JOIN AUTD ON AUT.IDAUT=AUTD.IDAUT AND FTRD.NOPRESTACION=AUTD.IDAUT AND FTRD.NOITEM=AUTD.NO_ITEM
				       INNER JOIN SER  ON AUTD.IDSERVICIO=SER.IDSERVICIO AND FTRD.REFERENCIA = SER.IDSERVICIO
                   INNER JOIN RIPS_CP ON SER.CODIGORIPS=RIPS_CP.IDCONCEPTORIPS
				       INNER JOIN IART ON SER.IDSERVICIO=IART.IDSERVICIO AND COALESCE(IART.PRINCIPAL,0)=1
     ...
     PRINT 'PROCEDIMIENTOS'
		...
		SELECT @IDPRESTADOR,CIT.IDAFILIADO,CIT.CONSECUTIVO,REPLACE(CONVERT(VARCHAR,CIT.FECHA,102),'.','-')+' '+LEFT(CONVERT(VARCHAR,CIT.FECHA,108),5),null,
				COALESCE(CIT.NOAUTORIZACION,'null'),LEFT(SER.CODCUPS,6),'02','01','02',325,'16',AFI.TIPO_DOC,AFI.DOCIDAFILIADO,CIT.IDDX,
				COALESCE(CIT.IDDX,'null'),COALESCE(CIT.IDDX,'null'),COALESCE(CIT.VALORTOTAL,0),
				 CASE WHEN COALESCE(CIT.VALORCOPAGO,0)>0 THEN  CASE WHEN COALESCE(AFI.TIPOUSUARIO,'') IN('01','02','03')  THEN '02' ELSE '01' END ELSE @conceptoRecaudo END,
      ...
      AND COALESCE(CIT.VALORTOTAL,0)>0
		...
		SELECT @IDPRESTADOR,AUT.IDAFILIADO,AUT.NOAUT,REPLACE(CONVERT(VARCHAR, AUT.FECHA, 102), '.', '-') + ' ' + LEFT(CONVERT(VARCHAR, AUT.FECHA, 108), 5),NULL
				,AUT.NUMAUTORIZA,LEFT(SER.CODCUPS,6),'02','01','02',325,'16',AFI.TIPO_DOC,AFI.DOCIDAFILIADO,AUT.DXPPAL,AUT.DXRELACIONADO,'null',COALESCE(AUTD.VALOR, 0),
				CASE WHEN COALESCE(AUTD.VALORCOPAGO,0)>0 OR COALESCE(AUT.VALORCOPAGO,0)> 0   THEN  CASE WHEN COALESCE(AFI.TIPOUSUARIO,'') IN('01','02','03')  THEN '02' ELSE '01' END ELSE @conceptoRecaudo END,
				'04'
      ...
      AND COALESCE(AUTD.VALOR,0)>0
		...
		PRINT 'PROCEDIMIENTO SALUD'
			...
			SELECT @IDPRESTADOR,HADM.IDAFILIADO,HADM.NOADMISION,REPLACE(CONVERT(VARCHAR,HPRE.FECHA,102),'.','-')+' '+LEFT(CONVERT(VARCHAR,HPRE.FECHA,108),5),null,
				HADM.NOAUTORIZACION,LEFT(SER.CODCUPS,6),COALESCE(REPLACE(TGEN.DATO2,' ',''),HADM.VIAINGRESO,'02'),'01','02',325,'16',MED.TIPO_ID,MED.IDMEDICO,COALESCE(HADM.DXINGRESO,HADM.DXEGRESO), --STORRES
      ...
      AND COALESCE(AUTD.VALOR,0)>0
        ...
        WHERE --HCA.CLASE='HC' AND --STORRES 20250315
			HCA.PROCEDENCIA='IPS'
