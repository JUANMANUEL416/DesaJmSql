...
BEGIN
   ...
   BEGIN         
      ...
      BEGIN
         SELECT 'OK'OK
         SELECT CNSFPAG,CASE WHEN COALESCE(NOCONTABILIZA,0)=0 THEN 'false' ELSE 'true' END NOCONTABILIZA,INGRESO,DBO.FNK_FECHA_GRINGA(FECHA)FECHA,
         FPAG.IDTERCERO,TER.RAZONSOCIAL,DBO.FNK_FECHA_GRINGA(FECHAPAGO)FECHAPAGO,FPAG.BANCO,BCO.DESCRIPCION,FPAG.SUCURSAL,BSU.DESCRIPCION AS DESCSUCURSAL,
         FPAG.CTA_BCO,FPAG.ITEM,FPAG.RENGLON,FPAG.BANCODOC,CODCAJA,CNSFACJ
         FROM FPAG INNER JOIN TER ON FPAG.IDTERCERO=TER.IDTERCERO
                   LEFT  JOIN BCO ON FPAG.BANCO=BCO.BANCO
                   LEFT  JOIN BSU ON FPAG.SUCURSAL=BSU.SUCURSAL AND FPAG.BANCO=BSU.BANCO
