CREATE  OR ALTER PROCEDURE DBO.SPK_FACTURACE_PERU_COPA
@NOAUT				VARCHAR(16),
@NIT				   VARCHAR(20),
@COMPANIA			VARCHAR(2),
@IDSEDE				VARCHAR(5),
...
@CNSFACJ			VARCHAR(20)= NULL,
@CODCAJA			VARCHAR(6)= NULL
WITH ENCRYPTION
AS 
...
BEGIN
	...
	IF @PROC = 'CE'
	BEGIN
      PRINT 'INGRESO POR CE'
		SELECT @NOAUT  = NOAUT ,@CODUNG=CODUNG, @CODPRG =CODPRG FROM AUT WHERE IDAUT = @NOAUT
  ...
  -- IF @PROC = 'CI'  --JEDM 20240209   COLOCO ESTE IF PQ ESTABA HACIEN UPDATE A CIT A SI NO VINIERA CON PROCEDENCIA CIT
	...
	CREATE TABLE #FTRD1(CNSFTR VARCHAR(40), N_CUOTA	int IDENTITY, FECHA datetime,
				...
				IDCIRUGIA varchar(20), CANTIDAD smallint, VALOR decimal(14,3),
	...
	BEGIN  
      PRINT 'INGRESO POR CE'
		...
		BEGIN
			...
			ELSE
			BEGIN
				print 'ingrese por el tercero @NOAUT='+@NOAUT
				DECLARE CUR_FTR CURSOR FOR
				SELECT DISTINCT AUT.IDAFILIADO, AUT.IDPLAN,AUTD.NfACTURA
				FROM AUT INNER JOIN AUTD ON AUTD.IDAUT       = AUT.IDAUT 
				WHERE AUT.NOAUT = @NOAUT 
   ...
   END
	...
	BEGIN  
				...
				BEGIN
                  ...
                  AND COALESCE(AUTD.GENERADO,0)=1
					...
					INSERT INTO #FTRD1(CNSFTR, FECHA, DB_CR, AREAPRESTACION, UBICACION, VR_TOTAL,
                     ...
                     IDPLAN,IDIMPUESTO, IDCLASE,ITEM , VLRIMPUESTO, PIVA, VIVA,CUENTA_FIMPDV)
					SELECT @CNSFTR, GETDATE(), 'DB', AUT.IDAREA, NULL, 
							AUTD.VALORCOPAGO, 
							NULL, CASE WHEN @DATOCCOSTO = 'SER:CCOSTO' THEN AUTD.CCOSTO ELSE AUT.CCOSTO END, SER.PREFIJO,
							CASE WHEN dbo.FNK_VALORVARIABLE('MANEJA_GARANTIA_SER') = 'SI' THEN 
		...
		BEGIN
         ...
         UPDATE #FTRD1 SET VIVA=VLR_SERVICI-ROUND(VLR_SERVICI/(1+(PIVA/100)),2)
         WHERE COALESCE(PIVA,0)>0 

         UPDATE #FTRD1 SET VLRIMPUESTO=(VLR_SERVICI-VIVA)
         WHERE COALESCE(PIVA,0)>0 

         UPDATE #FTRD1 SET VALOR=ROUND((VLRIMPUESTO/CANTIDAD),3)
         WHERE COALESCE(PIVA,0)>0 

         UPDATE #FTRD1 SET VALOR=ROUND((VLR_SERVICI/CANTIDAD),3)
         WHERE COALESCE(PIVA,0)=0 
		...
		END     
			...
			BEGIN
               ...
               IDPLAN=@IDPLAN,CUENTACXC= @CUENTACXC, CUENTACXC_RAD=@CUENTACXC_RAD,
					TIPOTTEC=@TTEC, F_FACTURA=@FECHAFAC, F_VENCE=@FECHAFAC, DESCUENTO = @DESCUENTO --  STORRES SE AGREGA CAMPO DE DESCUENTOPARA QUE SE ACTUALIZE
		...
		--	RETURN
            ...
            PRINT 'VOY A ACTUALIZAR CIT @NOAUT='+@NOAUT
               ...
               UPDATE CIT SET N_FACTURA=@NVOCONSEC,FACTURADA=1 WHERE CONSECUTIVO = @NOAUT
				...
				IF COALESCE(@CNSFACJ, '') <> '' AND COALESCE(@CODCAJA, '') <> ''
				BEGIN
