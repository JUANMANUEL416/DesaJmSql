...
,@USUARIO VARCHAR(12)   ,@COMPANIA VARCHAR(2)      ,@IDSEDE      VARCHAR(5)
      ...
      ,@NFINALIDAD VARCHAR(20)            ,@PREFIJO VARCHAR(20)           ,@NOM_PREFIJO VARCHAR(60)
...
BEGIN
   ...
   BEGIN         
      ...
      BEGIN
         ...
         BEGIN
            BEGIN TRY           
                  UPDATE PRE SET CIRUGIA=@CIRUGIA,MANEJAPQ=@PAQUETE,CEMDOSIAUTOM=@CEMDOSIAUTOM,LENTES=@LENTES,MINVENTARIOS=@MINVENTARIOS,M_ATENCION=@M_ATENCION,M_PROTOCOLO=@M_PROTOCOLO,REQ_PROVEED=@REQ_PROVEED, M_IMPUTABLE=@M_IMPUTABLE,REQSOLICITANTE=@REQSOLICITANTE,ESUNPROCEDIMIENTO=@ESUNPROCEDIMIENTO,IVA=@IVA,ESTADO=@ESTADO,UTILIZADOEN=@UTILIZADOEN,GRUPO=@GRUPO,FINALIDAD=@FINALIDAD,                                 NOM_PREFIJO=@NOM_PREFIJO,TIPOTRMA=@TIPOTRMA,CODRUBRO=@CODRUBRO,TAREA=@TAREA,IDAREA=@IDAREA,VIGENCIA=@VIGENCIA,NCOPIAS=@NCOPIAS,IDIMPUESTO=CASE WHEN COALESCE(@IDIMPUESTO,'')='' THEN IDIMPUESTO ELSE @IDIMPUESTO END,IDCLASE= CASE WHEN COALESCE(@IDCLASE,'')='' THEN IDCLASE ELSE @IDCLASE END ,IDPROVEEDORDEF=@IDPROVEEDORDEF
   ...
   BEGIN         
      ...
      BEGIN
         ...
         BEGIN
            ...
            UPDATE TGEN SET DATO1=@DATO1,DATO2=@DATO2,DESCRIPCION=@DESCRIPCION WHERE TABLA=@TABLA AND CAMPO=@CAMPO AND CODIGO=@CODIGO
         END
         ELSE
         BEGIN
            INSERT INTO @TBLERRORES(ERROR)
