		  ...
		  END
            ...
            END
				...
				BEGIN
					...
					SET @XML+=@E+@T+@T+@T+@T+@T+@T+@T+@T+@T+'<Value>'+COALESCE(@NOPOLIZA,dbo.FNK_LIMPIATEXTO(@IDPLAN, 'A-Z0-9'),'')+'</Value>'  -- STORRES - 20250618 -- Verificar para el futuro. Actuamnete esta en NOTIFICACION.
					SET @XML+=@E+@T+@T+@T+@T+@T+@T+@T+@T+'</AdditionalInformation>' 
				END
            ...
            BEGIN -- Solicitud Kevin Rodriguez
               ...
               BEGIN
                  ...
                  BEGIN -- FEV-RIPS Resol. 1036 de 2022 Las consultas en facturas de procedencia asistencial (archivo AC) 
				         ...
				         --SELECT @VLR_PCOMPA = SUM(VLR_COPAGOS) FROM FTRD WHERE PREFIJO IN (SELECT WORD PREFIJO FROM DBO.FNK_EXPLODE(DBO.FNK_VALORVARIABLE('PREFIJOCITAS'),',')) AND N_FACTURA=@N_FACTURA
				         --SELECT @VLR_COP = SUM(VLR_COPAGOS) FROM FTRD WHERE PREFIJO NOT IN (SELECT WORD PREFIJO FROM DBO.FNK_EXPLODE(DBO.FNK_VALORVARIABLE('PREFIJOCITAS'),',')) AND N_FACTURA=@N_FACTURA
                     ...
                     BEGIN
                        ...
                        SET @XML+=@E+@T+@T+@T+@T+@T+@T+@T+@T+'<AdditionalInformation>'  -- Anticipos a la factura / Copagos
				            IF COALESCE(@COD_ANTICIPO, '') <> '' SET @XML+=@E+@T+@T+@T+@T+@T+@T+@T+@T+@T+'<Name>'+@NOM_ANTICIPO+'</Name>'
				            ELSE SET @XML+=@E+@T+@T+@T+@T+@T+@T+@T+@T+@T+'<Name>COPAGO</Name>'

				            SET @XML+=@E+@T+@T+@T+@T+@T+@T+@T+@T+@T+'<Value>'+CAST(@VALORCOPAGO AS VARCHAR)+'</Value>' 
					         SET @XML+=@E+@T+@T+@T+@T+@T+@T+@T+@T+'</AdditionalInformation>' 

                        IF COALESCE(@VALORMODERADORA,0)>0
                        BEGIN
                           SELECT  @COD_ANTICIPO='02',@NOM_ANTICIPO='CUOTA_MODERADORA'
...
--Query2
		...
		BEGIN
			...
			--SELECT * FROM FTRD WHERE N_FACTURA='IXFE2587'
			--DECLARE @CNSFCT VARCHAR(20)='0100000841'
		...
		BEGIN -- Legalizaciones de depositos
         ...
         --IF EXISTS(SELECT 1 FROM FTRD INNER JOIN FTR ON FTR.CNSFCT=CNSFTR WHERE FTR.PROCEDENCIA = 'SALUD' AND FTRD.PREFIJO = DBO.FNK_VALORVARIABLE('PREFIJOCITAS') AND FTR.N_FACTURA=@N_FACTURA)
         ...
         BEGIN
            ...
            BEGIN
                  ...
                  SELECT 1 FROM FTRD INNER JOIN FTR ON FTR.CNSFCT=CNSFTR
                     ...
                     INNER JOIN RIPS_CP ON SER.CODIGORIPS=RIPS_CP.IDCONCEPTORIPS
                  WHERE FTR.N_FACTURA=@N_FACTURA 
                  --AND FTR.PROCEDENCIA = 'SALUD'  se retira esta condicion para que aplique parta todas
                  AND RIPS_CP.ARCHIVO='AC' 
               )
               BEGIN -- FEV-RIPS Resol. 1036 de 2022 Las consultas en facturas de procedencia asistencial (archivo AC) 
				      PRINT 'ES SALUD, POSEE CONSULTAS, VOY A DIVIDIR LOS COPAGOS'
				      DECLARE @VLR_PCOMPARTIDOS DECIMAL(14,2)

				      --SELECT @VLR_PCOMPARTIDOS = SUM(VLR_COPAGOS) FROM FTRD WHERE PREFIJO = DBO.FNK_VALORVARIABLE('PREFIJOCITAS') AND N_FACTURA=@N_FACTURA
				      --SELECT @VLR_PCOMPARTIDOS = SUM(VLR_COPAGOS) FROM FTRD WHERE PREFIJO IN (SELECT WORD PREFIJO FROM DBO.FNK_EXPLODE(DBO.FNK_VALORVARIABLE('PREFIJOCITAS'),',')) AND N_FACTURA=@N_FACTURA
				      --SELECT @VLR_COPAGO = SUM(VLR_COPAGOS) FROM FTRD WHERE PREFIJO NOT IN (SELECT WORD PREFIJO FROM DBO.FNK_EXPLODE(DBO.FNK_VALORVARIABLE('PREFIJOCITAS'),',')) AND N_FACTURA=@N_FACTURA
                  SELECT  @VLR_COPAGO = SUM(VLR_COPAGOS) FROM FTRD INNER JOIN FTR ON FTR.CNSFCT=CNSFTR
                     INNER JOIN SER ON FTRD.REFERENCIA=SER.IDSERVICIO
                     INNER JOIN RIPS_CP ON SER.CODIGORIPS=RIPS_CP.IDCONCEPTORIPS
                  WHERE FTR.N_FACTURA=@N_FACTURA 
                  ...
                  AND RIPS_CP.ARCHIVO='AC' 
				      ...
				      PRINT '@VLR_PCOMPARTIDOS >'  + COALESCE(STR(@VLR_PCOMPARTIDOS),' SIN @VLR_PCOMPARTIDOS')
				      PRINT '@VLR_COPAGO >'  + COALESCE(STR(@VLR_COPAGO),' SIN @VLR_COPAGO')
				      PRINT '@COD_ANTICIPO >'  + COALESCE(@COD_ANTICIPO,' SIN @COD_ANTICIPO')
				      PRINT '@NOM_ANTICIPO >'  + COALESCE(@NOM_ANTICIPO,' SIN @NOM_ANTICIPO')
                  ...
                  BEGIN
                     ...
                     SET @XML+=@E+@T+'</cac:PrepaidPayment>'
                  END
                  IF EXISTS(SELECT * FROM PLN WHERE IDPLAN=@IDPLAN AND TIPOSISTEMA NOT IN ('Contributivo','Prepagado'))
                  BEGIN
                     SELECT  @COD_ANTICIPO='01',@NOM_ANTICIPO='COPAGO'
                     SET @XML+=@E+@T+'<cac:PrepaidPayment>' -- Anticipos a la factura / Copagos   
                     SET @XML+=@E+@T+@T+'<cbc:ID schemeID="' + @COD_ANTICIPO + '">'+@NOM_ANTICIPO+'</cbc:ID>'
