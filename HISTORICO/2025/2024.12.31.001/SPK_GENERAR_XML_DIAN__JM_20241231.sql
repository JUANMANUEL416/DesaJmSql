...
BEGIN
	...
	BEGIN 
		...
		BEGIN -- Datos correspondiente al usuario (Paciente y Usuario de la factura)
			...
			SET @XML+=@E+@T+@T+@T+@T+@T+@T+@T+@T+@T+'<Value>'+LEFT(COALESCE(@CODHABILITA, ''),10)+'</Value>' 
			SET @XML+=@E+@T+@T+@T+@T+@T+@T+@T+@T+'</AdditionalInformation>' 
...
BEGIN
   ...
   DECLARE @VIVANOTA DECIMAL(14,2)=0
	...
	BEGIN -- Tipo de Nota de Crédito
		SELECT @TIPONOTA = '20'
		IF EXISTS(SELECT 1 FROM FNOT WHERE CNSFNOT=@CNSDOC AND COALESCE(FNOT.NOTA_SIN_FACTURA, 0)=1 AND IDCONCEPTO<>DBO.FNK_VALORVARIABLE('IDCONCEP_ANULAFTRCE')) SELECT @TIPONOTA = '22'
	END
