...
DECLARE  @PARAMETROS   NVARCHAR(MAX)	,@MODELO			VARCHAR(100)	,@METODO		VARCHAR(100)	,@USUARIO		VARCHAR(12)		,@CNSFACJ		VARCHAR(20)
		...
		,@EXONERACION	BIT				,@TIPO_EXONERACION	VARCHAR(20)		,@TIEMPOEXTRA INT
DECLARE @DOCIDAFILIADO	VARCHAR(20),	@NOMBREAFI VARCHAR(200), @NOMBRE_SEDE VARCHAR(20),	@SMS VARCHAR(MAX),		@MENSAJE VARCHAR(MAX)	,@F_EXPIRA DATETIME
		...
		,@FECHA_STRING	VARCHAR(MAX),	@NOMBRE_MED			VARCHAR(MAX)	,@NOMBRE_SER VARCHAR(max) 	
...
BEGIN 
	...
	BEGIN
        ...
        BEGIN --@CONSECUTIVO FCJ
         BEGIN TRY
            ...
            IF NOT EXISTS(SELECT 1 FROM FCJ WITH (NOLOCK) WHERE CODCAJA= @CODCAJA AND CNSFACJ= @CNSFACJ AND COALESCE(CONTABILIZADA,0)=0)
            BEGIN
   ...
   END
	...
	BEGIN 
        ...
        SELECT CIT.IDMEDICO, MED.NOMBRE, COALESCE(CIT.IDEMEDICA, MED.IDEMEDICA) IDEMEDICA, MES.DESCRIPCION ESPECIALIDAD, CIT.IDSEDE, SED.DESCRIPCION AS DESCSEDE
        FROM CIT 
...
--query3
	...
	BEGIN
		...
		IF EXISTS(SELECT 1 FROM CIT WHERE CONSECUTIVO=@CONSECUTIVO AND DATEADD(MINUTE,@MINUTOS,FECHA)<DBO.FNK_GETDATE())
		BEGIN
			INSERT INTO @TBLERRORES(ERROR)
        ...
        BEGIN TRY
			...
			UPDATE CIT SET FECHALLEGA = DBO.FNK_GETDATE(), USUARIOLLEGA = @USUARIO, NOAUTORIZACION = @NOAUTORIZACION 
