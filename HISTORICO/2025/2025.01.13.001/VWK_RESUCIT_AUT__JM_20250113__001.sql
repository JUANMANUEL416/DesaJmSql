...
CASE WHEN COALESCE(AUTD.ESDELAB,0)=1 THEN CASE WHEN COALESCE(AUTD.ENLAB,0)> 0 THEN 
      CASE WHEN EXISTS(SELECT * FROM  LING INNER JOIN LORD ON LING.NOINGRESO=LORD.NOINGRESO 
            WHERE LING.NOPRESTACION=AUT.NOAUT AND LORD.ESTADO='R') THEN 0 ELSE 1 END ELSE 1 END ELSE 0 END DX_PEND
