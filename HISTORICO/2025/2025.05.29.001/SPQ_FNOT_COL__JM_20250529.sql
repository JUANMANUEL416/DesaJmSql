...
BEGIN
  ...
  END
   ...
   BEGIN         
      ...
      END
      IF EXISTS(SELECT * FROM FNOT WHERE CNSFNOT=@CNSFNOT AND FACTE=1 AND  CONVERT(DATE,F_NOTA)=CONVERT(DATE,DBO.FNK_GETDATE()))
      BEGIN
         INSERT INTO @TBLERRORES(ERROR)
      ...
      BEGIN
         ...
         RETURN
      END
      IF EXISTS(SELECT * FROM FNOT WHERE CNSFNOT=@CNSFNOT AND (FACTE=1 OR FACTE=3) AND  CONVERT(DATE,F_NOTA)<>CONVERT(DATE,DBO.FNK_GETDATE())
                 AND MONTH(F_NOTA)=MONTH(DBO.FNK_GETDATE()))
      BEGIN
         UPDATE FNOT SET F_NOTA=DBO.FNK_GETDATE(),FACTE=1 WHERE CNSFNOT=@CNSFNOT

         SELECT 'OK'OK
         RETURN
      END
      IF EXISTS(SELECT * FROM FNOT WHERE CNSFNOT=@CNSFNOT AND (FACTE=1 OR FACTE=3 OR  FACTE=0) AND  CONVERT(DATE,F_NOTA)<>CONVERT(DATE,DBO.FNK_GETDATE())
      ...
      BEGIN
         UPDATE FNOT SET F_NOTA= CASE WHEN  CONVERT(DATE,F_NOTA)<CONVERT(DATE,DBO.FNK_GETDATE()) THEN  DBO.FNK_GETDATE() ELSE F_NOTA END ,FACTE=1 WHERE CNSFNOT=@CNSFNOT
