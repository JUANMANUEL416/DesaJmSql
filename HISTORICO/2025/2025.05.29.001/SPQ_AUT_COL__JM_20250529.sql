...
DECLARE @FECHA_AUT DATETIME, @FECHAI    VARCHAR(10), @HORA VARCHAR(5) , @NUMOBLIGA BIT
BEGIN		
	--VARIABLES PARA AUT
	...
	BEGIN
		...
		END
		IF UPPER(@PROCESO) = 'EDITAR'  AND DBO.FNK_VALORVARIABLE('SOLOUN_CODAUTORIZA')='SI' AND @NUMOBLIGA=1 AND COALESCE(JSON_VALUE(@AUT ,'$.NUMAUTORIZA'), '')<>''
		BEGIN
			...
			END
		END
		IF UPPER(@PROCESO) = 'INSERTAR'  AND DBO.FNK_VALORVARIABLE('SOLOUN_CODAUTORIZA')='SI' AND @NUMOBLIGA=1 AND COALESCE(JSON_VALUE(@AUT ,'$.NUMAUTORIZA'), '')<>''
		BEGIN
		...
		FROM OPENJSON(@AUT) --PARA GUARDAR EL SIRAS EN HACTRAND
		WITH( 
		...
		BEGIN TRY
			...
			BEGIN
				...
				BEGIN TRY--IDAREA -- JSON_VALUE(@AUT ,'$.IDCONTRATANTE.value') --IDTERCEROCA  NOAUT --NO_ITEM
					...
					IF DBO.FNK_VALORVARIABLE('INTERFAZ_LX') = 'ANNARLAB'  AND (SELECT COUNT(1) FROM AUTD INNER JOIN SER ON AUTD.IDSERVICIO = SER.IDSERVICIO WHERE AUTD.IDAUT = @CNSIDAUT AND    SER.AMBITO = 'LX' )>0
					BEGIN
					...
					BEGIN
						...
						IF ( SELECT COUNT(1) FROM HACTRAND WHERE  CNSHACTRAN	=@CNSHACTRAN AND PROCEDENCIA='CE'	AND NOREFERENCIA = @CONSECUTIVO ) >0  -- YA EXISTE EL MISMO CNSHACTRAN PARA ESA CITA SOLO ACTUALIZO EL SIRAS
						BEGIN
			...
			BEGIN
				...
				UPDATE AUT SET PREFIJO        = JSON_VALUE(@AUT ,'$.PREFIJO.value')           , IDSEDE = JSON_VALUE(@AUT ,'$.IDSEDE')                  , CCOSTO = JSON_VALUE(@AUT ,'$.CCOSTO.value') 
							...
							, FECHAVENCE = REPLACE(JSON_VALUE(@AUT ,'$.FECHAVENCE'),'-',''), FECHA = @FECHA_AUT --STORRES
							, AUTORIZADOPOR = JSON_VALUE(@AUT ,'$.AUTORIZADOPOR')  
				...
				IF EXISTS ( SELECT 1 FROM  #AUTD WHERE #AUTD.PROCESO = 'Insertar') --VALOR
				BEGIN
				...
				BEGIN
					UPDATE AUT SET CNSHACTRAN = @CNSHACTRAN, SOAT = 1 WHERE  IDAUT = @CNSIDAUT
	-- ACTUALIZO LOS DATOS DEL AMPARO 
					UPDATE HACTRAN SET 
...
--QUERY3
	...
	BEGIN
		...
		END
		SELECT 'OK' OK
		SELECT GENEROCAJA ,TIPOCAJA ,COALESCE(VALORCOPAGO,0) VALORCOPAGO, CASE WHEN AUT.IDCONTRATANTE = DBO.FNK_VALORVARIABLE('IDTERCEROPARTICULAR') THEN 1 ELSE 0 END [PARTICULAR] FROM AUT WHERE IDAUT = @IDAUT
		RETURN
	END
