...
BEGIN
   ...
   BEGIN
      ...
      SELECT @SQL='SELECT IDARTICULO FROM IMOVH WHERE CNSMOV='''+@CNSMOV+''' AND NOT EXISTS(SELECT * FROM '+@BDATOS+'.DBO.IART WHERE IMOVH.IDARTICULO=IART.IDARTICULO) '
      ...
      BEGIN
         SELECT @SQL='INSERT INTO IART (IDARTICULO, IDCLASE, IDSUBCLASE, IDGRUPO, DESCRIPCION, IDPRINACTIVO, IDFORFARM, IDCONCENTRA,
                      ...
                      CODBARRA, INSTITUCIONAL, TEMPERATURA, ICBU, RMINSTOCK, STOCKMINREQ, REQAUTCOMP,  REQRECEP, REFERENCIA, IUM, 
                      IDSERVICIO_FAC, CESTE, MIVA, PROCEDIMIENTO, MCADFRIO, BIOLOGICO, UNIRS, ALERTA, TEMPERATURA_FIN, HUMEDAD_MIN, 
                      HUMEDAD_MAX FROM IART WHERE IDARTICULO IN('+(SELECT * FROM @CREAR)+')'
          BEGIN TRY           
                EXECUTE (@SQL)              
          END TRY
          BEGIN CATCH
                  PRINT 'ERROR AL CREAR EL ARTICULO'
                  RETURN
          END CATCH
      END
