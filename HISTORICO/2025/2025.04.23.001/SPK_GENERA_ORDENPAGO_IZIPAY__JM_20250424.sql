...
BEGIN
   ...
   BEGIN
      SET @RESP =1
   END
   IF EXISTS(SELECT * FROM KPAGE WHERE  COALESCE(ESTADO,'')='EXPIRED' AND IDKPAGE=@IDKPAGE AND @PROCESO<>'CREAR')
   BEGIN
      PRINT 'ESTA EXPIRADO LIBERO LAS PRESTACIONES'
		IF @TABLA='CIT'
		BEGIN
			UPDATE CIT SET IDKPAGE=NULL WHERE CONSECUTIVO=@NOADMISION AND IDKPAGE=@IDKPAGE
		END 
		IF @TABLA='AUT'
		BEGIN
			UPDATE AUT SET IDKPAGE=NULL WHERE IDAUT=@NOADMISION AND IDKPAGE=@IDKPAGE
		END
		IF @TABLA='MAD'
		BEGIN
			UPDATE MAD SET IDKPAGE=NULL WHERE ID=@NOADMISION AND IDKPAGE=@IDKPAGE
		END
		IF @TABLA='HADM'
		BEGIN
			UPDATE HADM SET IDKPAGE=NULL WHERE NOADMISION=@NOADMISION AND IDKPAGE=@IDKPAGE
		END  
		IF @TABLA='HPREP'
		BEGIN
			UPDATE HPREP SET IDKPAGE=NULL WHERE NOPRESUPUESTO=@NOADMISION AND IDKPAGE=@IDKPAGE
		END 
      RETURN
   END
   IF EXISTS(SELECT * FROM KPAGE WHERE IDKPAGE=@IDKPAGE AND COALESCE(ESTADO,'') IN('RUNNING','SEND') AND F_EXPIRA<=GETDATE() AND @RESP=0)
   BEGIN
      PRINT 'EXISTE'
      UPDATE KPAGE SET ESTADO='EXPIRED' WHERE CONSECUTIVO=@NOADMISION AND PROCEDENCIA=@PROCEDENCIA AND TABLA=@TABLA AND COALESCE(ESTADO,'SEND') IN('RUNNING','SEND') AND F_EXPIRA<=GETDATE()
		...
		IF @TABLA='HPREP'
		BEGIN
			UPDATE HPREP SET IDKPAGE=NULL WHERE NOPRESUPUESTO=@NOADMISION
		END
      IF @PROCESO<>'CREAR'
      BEGIN
         RETURN
      END
   END
   IF EXISTS(SELECT * FROM KPAGE WHERE CONSECUTIVO=@NOADMISION AND PROCEDENCIA=@PROCEDENCIA AND TABLA=@TABLA AND IDKPAGE=@IDKPAGE AND COALESCE(ESTADO,'SEND') IN('RUNNING','SEND') AND F_EXPIRA>GETDATE() AND @PROCESO='CREAR' AND @RESP = 0)
   BEGIN
   ...
   BEGIN
      ...
      SELECT @sUrl=LTRIM(RTRIM(@sUrl))+'CreatePaymentOrder'
		...
		IF @TABLA='CIT'
		BEGIN
			SELECT @FECHA=DATEADD(MINUTE,275,FECHA) FROM CIT  WHERE CONSECUTIVO=@NOADMISION
		END
		IF @TABLA='AUT'
		BEGIN
	  ...
	  BEGIN
			INSERT INTO KPAGE(CONSECUTIVO, TABLA, PROCEDENCIA, FECHAGEN,MODO, VALOR, ESTADO,USUCOBRA, IDPLAN,REF_PAGO,RESPONSABLEPAGO,F_EXPIRA)
   			SELECT @NOADMISION,@TABLA,@PROCEDENCIA, CONVERT(SMALLDATETIME,GETDATE()),@MODO, @VALOR, 'CREATED',@USUARIO,NULL,@N_FACTURA,@IDAFILIADO,DATEADD(MINUTE,-285,@HOY)
	  ...
	  IF @TABLA='HPREP'
      BEGIN
         UPDATE HPREP SET IDKPAGE=@IDKPAGE WHERE NOPRESUPUESTO=@NOADMISION
      END
   ...
   BEGIN
      ...
      BEGIN
         ...
         BEGIN
            ...
            BEGIN
               ...
               AND TABLA=@TABLA 
               AND COALESCE(ESTADO,'') IN('RUNNING','SEND','ERROR','REFUSED') 
         ...
         BEGIN
            ...
            BEGIN          
               ...
               BEGIN
                 ...
                 IF @vads_trans_status='REFUSED' AND EXISTS(SELECT * FROM KPAGE WHERE IDKPAGE=@IDKPAGE AND F_EXPIRA<=GETDATE())
                 BEGIN
                     ...
                     BEGIN
                        UPDATE HPREP SET IDKPAGE=NULL WHERE NOPRESUPUESTO=@NOADMISION
                     END
                     UPDATE KPAGE SET ESTADO='EXPIRED' WHERE IDKPAGE=@IDKPAGE
                 END
               END
            END
            RETURN
      ...
      BEGIN
         ...
         BEGIN
            ...
            AND PROCEDENCIA=@PROCEDENCIA 
            AND TABLA=@TABLA 
            AND COALESCE(ESTADO,'') IN('RUNNING','SEND','REFUSED','') 
            AND F_EXPIRA>GETDATE()
         END
         SELECT @paymentOrderId=paymentOrderId FROM KPAGE WHERE IDKPAGE=@IDKPAGE
         SELECT @Body =' {"paymentOrderId": "'+@paymentOrderId+'"} ' 
   ...
   BEGIN
      ...
      IF @PROCESO='CREAR'
      BEGIN
		   UPDATE KPAGE SET ESTADO=@paymentOrderStatus,RESPUESTA = @response, paymentOrderId = @paymentOrderId,LINKPAGO=@urlpago,ULTIMA_RESP=@response WHERE IDKPAGE = @IDKPAGE	
      END
      ELSE 
      BEGIN
         UPDATE KPAGE SET ULTIMA_RESP=@response WHERE IDKPAGE = @IDKPAGE	
         IF @PROCESO='ANULAR'
         BEGIN
            IF @TABLA='CIT'
            BEGIN
