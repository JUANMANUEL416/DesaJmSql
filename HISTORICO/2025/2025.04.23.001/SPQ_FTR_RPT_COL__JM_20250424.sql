...
BEGIN
	...
	BEGIN         
		...
		IF @PROCESO='Rventas' --OPGRAVADA --CIT NRO_REFERENCIA CE AUT NOREFERENCIA
		BEGIN
			PRINT 'INGRESO'
			SELECT @SQL=
						...
						(SELECT SUM(CASE WHEN COALESCE(FTRD.VIVA,0)>0 THEN   (ROUND(VLR_SERVICI-CASE WHEN VLR_COPAGOS >0 THEN VLR_COPAGOS ELSE 0 END,2)  - VIVA ) ELSE  0 END)
						...
						(FTR.VALORSERVICIOS-CASE WHEN COALESCE(VALORCOPAGO,0) >0 THEN  COALESCE(VALORCOPAGO,0) ELSE 0 END) VALORSERVICIOS ,''''F_COMP_REF,''''TIPO_REF,''''NRO_SERIE,'''' NRO_REFERENCIA, FTR.PROCEDENCIA
						, CASE  FTR.PROCEDENCIA 
								WHEN  ''CE'' THEN (SELECT COALESCE(AUT.COPAGO_FIJO,0) FROM AUT WHERE FTR.NOREFERENCIA = AUT.IDAUT) 
								WHEN  ''CI'' THEN (SELECT COALESCE(CIT.VALORCOPAGO,0) FROM CIT WHERE FTR.NOREFERENCIA = CIT.CONSECUTIVO) 
								ELSE 0 END [COPAGO_FIJO]
						, CASE  FTR.PROCEDENCIA 
								WHEN  ''CE'' THEN (SELECT COALESCE(AUT.COPAGO_VARIABLE,0) FROM AUT WHERE FTR.NOREFERENCIA = AUT.IDAUT) 
								WHEN  ''CI'' THEN (SELECT COALESCE(CIT.COPA_VAR_PORC,0) FROM CIT WHERE FTR.NOREFERENCIA = CIT.CONSECUTIVO) 
								ELSE 0 END [COPAGO_VARIABLE],''''CLASNOT
			...
			AND    COALESCE(FTR.TIPOTTEC,'''') = CASE WHEN COALESCE('''+COALESCE(@TERCONTABLE,'')+''','''')='''' THEN COALESCE(FTR.TIPOTTEC,'''') ELSE '''+COALESCE(@TERCONTABLE,'')+''' END 
			AND    FTR.USUARIOFACTURA = CASE WHEN COALESCE('''+COALESCE(@USUARIOFACTU,'')+''','''')='''' THEN FTR.USUARIOFACTURA ELSE '''+COALESCE(@USUARIOFACTU,'')+''' END 
			' +@PART+ '
			UNION ALL 
			SELECT PLN.DESCPLAN,DBO.FNK_FECHA_GRINGA(FNOT.F_NOTA)F_NOTA,DBO.FNK_FECHA_GRINGA(FNOT.F_NOTA)F_NOTA,CASE FTR.TIPOFIN WHEN ''N'' THEN IIF(FNOT.CLASE=''C'',''07'',''07'') WHEN ''C'' THEN IIF(FNOT.CLASE=''C'',''07'',''07'') END TIPO,
						...
						FROM FNOTD WHERE CNSFNOT=FNOT.CNSFNOT )*-1  OPGRAVADA ,
						(SELECT SUM(CASE WHEN COALESCE(FNOTD.VALORIVA,0)=0 THEN FNOTD.VR_TOTAL  ELSE 0 END)  
						FROM FNOTD WHERE CNSFNOT=FNOT.CNSFNOT )*-1  OPNOGRAVADA ,
						(SELECT SUM(CASE WHEN COALESCE(FNOTD.VALORIVA,0)>0 THEN FNOTD.VALORIVA  ELSE 0 END)  
						FROM FNOTD WHERE CNSFNOT=FNOT.CNSFNOT )*-1,
						(FNOT.VR_TOTAL*-1),DBO.FNK_FECHA_GRINGA(FTR.F_FACTURA),CASE WHEN FTR.TIPOFIN=''N'' THEN ''03'' ELSE ''01'' END,LEFT(FTR.N_FACTURA,4)SERIE,
						RIGHT(FTR.N_FACTURA,8)NRO, FTR.PROCEDENCIA, 0 [COPAGO_FIJO], 0 [COPAGO_VARIABLE],CASE WHEN FNOT.PROCEDENCIA=''FACTURA'' THEN ''TOTAL'' ELSE ''PARCIAL'' END
			FROM FNOT INNER JOIN FTR ON FNOT.N_FACTURA=FTR.N_FACTURA
						...
						LEFT  JOIN TTEC ON FTR.TIPOTTEC=TTEC.TIPO
						LEFT  JOIN SED ON FTR.IDSEDE=SED.IDSEDE
			WHERE  F_NOTA >= '''+CONVERT(VARCHAR(10),@F_INICIAL,103)+'''
			AND    F_NOTA <  '''+CONVERT(VARCHAR(10),@F_FINAL + 1,103)+'''
			AND    COALESCE(FTR.IDSEDE,'''') = CASE  WHEN '''+COALESCE(@IDSEDEF,'Todas')+'''<>''Todas'' THEN '''+COALESCE(@IDSEDEF,'Todas')+''' ELSE COALESCE(FTR.IDSEDE,'''') END
			AND    FTR.PROCEDENCIA = CASE WHEN COALESCE('''+COALESCE(@PROCEDENCIA,'Todas')+''',''Todas'')=''Todas'' THEN FTR.PROCEDENCIA ELSE '''+COALESCE(@PROCEDENCIA,'Todas')+''' END 
			AND    FTR.IDTERCERO = CASE WHEN COALESCE('''+COALESCE(@IDTERCERO,'')+''','''')='''' THEN FTR.IDTERCERO ELSE '''+COALESCE(@IDTERCERO,'')+''' END 
			AND    FTR.IDPLAN = CASE WHEN COALESCE('''+COALESCE(@IDPLAN,'')+''','''')='''' THEN FTR.IDPLAN ELSE '''+COALESCE(@IDPLAN,'')+''' END 
		...
		BEGIN
			...
			CASE WHEN FTR.ESTADO=''P'' AND COALESCE(TIPOANULACION,'''')<>''NC'' THEN ''ACTIVA'' ELSE ''ANULADA'' END ESTADO,
			CASE COALESCE(FTR.FACTE,0) WHEN 0 THEN ''SINENVIAR'' WHEN 1 THEN ''ENCOLA'' WHEN 2 THEN ''NOTIFICADA'' WHEN 3 THEN ''CONERROR''
			WHEN 4 THEN ''NOTIFICADA'' ELSE ''OTRO'' END ESTADODIAN,FTR.CONTABILIZADA,FTR.NROCOMPROBANTE 
			FROM   FTR INNER JOIN TER ON FTR.IDTERCERO=TER.IDTERCERO
						LEFT  JOIN PLN ON FTR.IDPLAN=PLN.IDPLAN
						LEFT  JOIN AFI ON FTR.IDAFILIADO=AFI.IDAFILIADO
						LEFT  JOIN USUSU ON FTR.USUARIOFACTURA=USUSU.USUARIO
