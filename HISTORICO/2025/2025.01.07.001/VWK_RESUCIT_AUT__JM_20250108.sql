...
AS
SELECT X.CONSECUTIVOCIT,SUM(X.VLR_PROTOCOLO)VLR_PROTOCOLO,SUM(VLR_AGUDO)VLR_AGUDO,X.ESPROGRAMAS,SUM(DX_PEND)DX_PEND
FROM (
SELECT AUT.CONSECUTIVOCIT,AUT.NOAUT,AUT.ESPROGRAMAS,SUM(CASE WHEN COALESCE(AUTD.PROTOCOLO,0)= 1  THEN  AUTD.VALOREXCEDENTE ELSE 0 END)VLR_PROTOCOLO,
...
CASE WHEN COALESCE(AUTD.ENLAB,0)=1 THEN 
      CASE WHEN EXISTS(SELECT * FROM  LING INNER JOIN LORD ON LING.NOINGRESO=LORD.NOINGRESO 
            WHERE LING.NOPRESTACION=AUT.NOAUT AND LORD.ESTADO='R') THEN 1 ELSE 0 END ELSE 1 END DX_PEND
...
GROUP BY AUT.CONSECUTIVOCIT,AUT.NOAUT,AUTD.ENLAB,AUT.ESPROGRAMAS)X
