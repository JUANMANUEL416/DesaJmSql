...
BEGIN
	...
	PRINT 'SPK_RIPS_JSON_FTR_CAPIX ='+@N_FACTURA

	SELECT @CNSFCT=CNSFCT,@CPROPIO=COALESCE(COPAPROPIO,0) FROM FTR WHERE N_FACTURA=@N_FACTURA
   ...
   WHERE  EXISTS(SELECT * FROM FTRDC WHERE  FTRDC.CNSFTR=@CNSFCT AND FTRDC.NOADMISION=HADM.NOADMISION AND FTRDC.PROCEDENCIA='HADM')
	...
	BEGIN 
			...
			causaMotivoAtencion='38',CIT.VALORTOTAL,IIF(COALESCE(CIT.VALORCOPAGO,0)>0,IIF(COALESCE(AFI.TIPOUSUARIO,'')IN('01','02','03'),'02',IIF(COALESCE(AFI.TIPOUSUARIO,'')='04','01',@conceptoRecaudo)),@conceptoRecaudo),COALESCE(CIT.VALORCOPAGO,0), MED.TIPO_ID, MED.IDMEDICO,
	...
	BEGIN
		...
		causaMotivoAtencion='38',AUTD.VALOR,IIF(COALESCE(AUTD.VALORCOPAGO,0)>0,'02',@conceptoRecaudo),COALESCE(AUTD.VALORCOPAGO,0), COALESCE(MED.TIPO_ID, @TIPODOC,''), COALESCE(MED.IDMEDICO, @DOCIDAFILIADO,''),
      ...
      AND COALESCE(AUTD.VALOR,0)>0
		...
		'01',HPRED.VALOR,IIF(COALESCE(HPRED.VALORCOPAGO,0)>0,'02',@conceptoRecaudo),COALESCE(HPRED.VALORCOPAGO,0), MED.TIPO_ID, MED.IDMEDICO 
		...
		codServicio=325,finalidadTecnologiaSalud=LEFT(CASE WHEN HPRE.FINALIDAD IS NULL OR HPRE.FINALIDAD='' OR HPRE.FINALIDAD<='10'THEN '44' ELSE HPRE.FINALIDAD END,2),
		causaMotivoAtencion='38',COALESCE(HADM.DXINGRESO,HADM.DXEGRESO),COALESCE(HADM.DXSALIDA1,''),COALESCE(HADM.DXSALIDA2,''),COALESCE(HADM.DXSALIDA3,''),
		'01',HPRED.VALOR,IIF(COALESCE(HPRED.VALORCOPAGO,0)>0,'02',@conceptoRecaudo),COALESCE(HPRED.VALORCOPAGO,0),CONVERT(INT,HPRED.CANTIDAD), MED.TIPO_ID, MED.IDMEDICO 
		FROM  FTRDC INNER JOIN HADM ON FTRDC.NOADMISION=HADM.NOADMISION  
			INNER JOIN  HPRE ON HADM.NOADMISION=HPRE.NOADMISION
				INNER JOIN HPRED ON HPRE.NOPRESTACION=HPRED.NOPRESTACION AND HPRED.NOITEM = FTRDC.NOITEM
				INNER JOIN SER ON HPRED.IDSERVICIO=SER.IDSERVICIO  AND FTRDC.IDSERVICIO = SER.IDSERVICIO
	...
	BEGIN 
		...
		SELECT @IDPRESTADOR,CIT.IDAFILIADO,CIT.CONSECUTIVO,REPLACE(CONVERT(VARCHAR,CIT.FECHA,102),'.','-')+' '+LEFT(CONVERT(VARCHAR,CIT.FECHA,108),5),null,
				...
				CASE WHEN COALESCE(CIT.IDDX,'')='' THEN 'null' END,CASE WHEN COALESCE(CIT.IDDX,'')='' THEN 'null' END,CIT.VALORTOTAL,IIF(COALESCE(CIT.VALORCOPAGO,0)>0,'02',@conceptoRecaudo),'04',COALESCE(CIT.VALORCOPAGO,0),@N_FACTURA
		FROM  FTRDC 
			INNER JOIN CIT ON FTRDC.NOADMISION=CIT.CONSECUTIVO
			INNER JOIN SER ON CIT.IDSERVICIO=SER.IDSERVICIO
			INNER JOIN AFI ON FTRDC.IDAFILIADO=AFI.IDAFILIADO
		...
		BEGIN --AP
			...
			SELECT @IDPRESTADOR,AUT.IDAFILIADO,AUT.NOAUT,REPLACE(CONVERT(VARCHAR, AUT.FECHA, 102), '.', '-') + ' ' + LEFT(CONVERT(VARCHAR, AUT.FECHA, 108), 5),NULL
				...
				,AUT.DXPPAL,IIF(COALESCE(AUT.DXRELACIONADO,'')='',AUT.DXPPAL,AUT.DXRELACIONADO),AUT.DXPPAL,AUTD.VALOR,IIF(COALESCE(AUT.VALORCOPAGO,0)>0,'02',@conceptoRecaudo),FTRDC.CANTIDAD,'04'
		...
		SELECT @IDPRESTADOR,HADM.IDAFILIADO,HADM.NOADMISION,REPLACE(CONVERT(VARCHAR,HPRE.FECHA,102),'.','-')+' '+LEFT(CONVERT(VARCHAR,HPRE.FECHA,108),5),null,
         ...
         HPRED.VALOR,IIF(COALESCE(HPRED.VALORCOPAGO,0)>0,'02',@conceptoRecaudo),'04',COALESCE(HPRED.VALORCOPAGO,0),@N_FACTURA
		FROM  FTRDC INNER JOIN HADM ON FTRDC.NOADMISION=HADM.NOADMISION
					INNER JOIN AFI  ON FTRDC.IDAFILIADO=AFI.IDAFILIADO
					INNER JOIN HPRED ON FTRDC.NOPRESTACION=HPRED.NOPRESTACION AND FTRDC.NOITEM=HPRED.NOITEM
		...
		SELECT @IDPRESTADOR,HADM.IDAFILIADO,HADM.NOADMISION,REPLACE(CONVERT(VARCHAR,HPRE.FECHA,102),'.','-')+' '+LEFT(CONVERT(VARCHAR,HPRE.FECHA,108),5),null,
			HADM.NOAUTORIZACION,left(REPLACE(REPLACE(LTRIM(RTRIM(SER.CODCUPS)),CHAR(13),''),CHAR(10),''),6),COALESCE(REPLACE(TGEN.DATO2,' ',''),HADM.VIAINGRESO,'02'),'01','02',325,'16',COALESCE(MED.TIPO_ID, @TIPODOC), COALESCE(MED.IDMEDICO,@DOCIDAFILIADO),
         COALESCE(HADM.DXINGRESO,HADM.DXEGRESO), 
			CASE WHEN LEN(COALESCE(HADM.DXSALIDA1,''))<4 THEN COALESCE(HADM.DXINGRESO,HADM.DXEGRESO) END,CASE WHEN COALESCE(HADM.COMPLICACION,HADM.DXINGRESO,HADM.DXEGRESO,'')='' THEN 'null' ELSE COALESCE(HADM.COMPLICACION,HADM.DXINGRESO,HADM.DXEGRESO,'') END,
         HPRED.VALOR,IIF(COALESCE(HPRED.VALORCOPAGO,0)>0,'02',@conceptoRecaudo),HPRED.CANTIDAD, '04',COALESCE(HPRED.VALORCOPAGO,0),@N_FACTURA
		FROM  FTRDC INNER JOIN HADM ON FTRDC.NOADMISION=HADM.NOADMISION
					INNER JOIN AFI  ON FTRDC.IDAFILIADO=AFI.IDAFILIADO
					INNER JOIN HPRED ON FTRDC.NOPRESTACION=HPRED.NOPRESTACION AND FTRDC.NOITEM=HPRED.NOITEM
					INNER JOIN HPRE  ON HADM.NOADMISION=HPRE.NOADMISION AND FTRDC.NOPRESTACION=HPRE.NOPRESTACION
   ...
   END
	...
	BEGIN
		...
		CASE WHEN COALESCE(TGEN.CHECK1,0)=1 THEN DATO1 ELSE CODIGO END,COALESCE(HCA.IDDX,HADM.DXINGRESO),COALESCE(HCA.IDDX,HADM.DXINGRESO),COALESCE(HADM.DXEGRESO,HCA.DX1),
   ...
   --UPDATE @USUARIOS SET @TIPOUSU='03' WHERE 1=1
	...
	BEGIN
      ...
      UPDATE @MEDICAMENTOS SET codDiagnosticoPrincipal=UPPER(codDiagnosticoPrincipal),codDiagnosticoRelacionado=UPPER(codDiagnosticoRelacionado)
		...
		UPDATE AP
		SET 
			AP.codDiagnosticoPrincipal = CASE WHEN COALESCE(AP.codDiagnosticoPrincipal, '') = '' OR AP.codDiagnosticoPrincipal ='null' THEN COALESCE(DX.IDDX,'Z000') ELSE AP.codDiagnosticoPrincipal END,
...
--QUERY2	
    ...
    --OTROS RECIEN NACIDOS
     ...
     END
     UPDATE @CONSULTAS SET vrServicio=0,valorPagoModerador=CASE WHEN @CPROPIO=1 THEN 0 ELSE valorPagoModerador END,conceptoRecaudo=CASE WHEN @CPROPIO=1 THEN '05' ELSE conceptoRecaudo END WHERE 1=1
     UPDATE @PROCEDIMIENTOS SET vrServicio=0 ,valorPagoModerador=CASE WHEN @CPROPIO=1 THEN 0 ELSE valorPagoModerador END,conceptoRecaudo=CASE WHEN @CPROPIO=1 THEN '05' ELSE conceptoRecaudo END WHERE 1=1
     UPDATE @MEDICAMENTOS SET vrServicio=0,vrUnitMedicamento=0,valorPagoModerador=CASE WHEN @CPROPIO=1 THEN 0 ELSE valorPagoModerador END,conceptoRecaudo=CASE WHEN @CPROPIO=1 THEN '05' ELSE conceptoRecaudo END  WHERE 1=1
     UPDATE @OTROSSER SET vrServicio=0,vrUnitOS=0,valorPagoModerador=CASE WHEN @CPROPIO=1 THEN 0 ELSE valorPagoModerador END,conceptoRecaudo=CASE WHEN @CPROPIO=1 THEN '05' ELSE conceptoRecaudo END  WHERE 1=1
