IF EXISTS(SELECT NAME FROM SYSOBJECTS WHERE NAME='SPK_GENCXP_NOMINA_UNO' AND XTYPE='P')
BEGIN
   DROP PROCEDURE SPK_GENCXP_NOMINA_UNO
END

GO
CREATE   PROCEDURE DBO.SPK_GENCXP_NOMINA_UNO
@CNSPAGO          VARCHAR(20),
@COMPANIA         VARCHAR(2),
@SEDE             VARCHAR(5),
@USUARIO          VARCHAR(12),
...
DECLARE @SINDTO BIT
DECLARE @NUMDOC2 VARCHAR(20)
DECLARE @VLR_INGRESOS DECIMAL(14,2)
DECLARE @VLR_CXP      DECIMAL(14,2)
BEGIN
   ...
   FROM(
       ...
       AND    C.CLASE      = 'Salario' 
       AND    A.VALOR_NETO > 0 
       AND    NEMP.TERCEROID=@NUMDOCR
       AND  EXISTS (SELECT *  FROM NEMPFD WHERE NEMPFD.CNSPAGO=@CNSPAGO AND C.CODPRF=NEMPFD.CODPRF)
       UNION ALL
       SELECT B.CODNOMINA,
         CASE WHEN F.TIPO='Ingreso' AND EXISTS(SELECT * FROM NEMP WHERE TERCEROID=A.IDTERCERO) THEN ( SELECT TOP 1 CODPRF FROM NPRF WHERE CLASE='Salario') ELSE C.CODPRF END,
         CASE WHEN F.TIPO='Ingreso' AND EXISTS(SELECT * FROM NEMP WHERE TERCEROID=A.IDTERCERO) THEN ( SELECT TOP 1 DESCRIPCION FROM NPRF WHERE CLASE='Salario') ELSE  C.DESCRIPCION END,
  ...
  -- RETURN
   ...
   BEGIN
     ...
     SET @NUMDOC2 =NULL
     SELECT @NUMDOC2 =NEMP.NUMDOC FROM NEMP INNER JOIN NIEPE ON NEMP.NUMDOC=NIEPE.NUMDOC WHERE TERCEROID=@IDTERCERO_CUR AND NIEPE.CNSPAGO=@CNSPAGO
	  IF EXISTS(SELECT * FROM NIEPE INNER JOIN NIEPED ON NIEPE.CNSNIEPE=NIEPED.CNSNIEPE AND NIEPE.NUMDOC=NIEPED.NUMDOC
               ...
               AND NEMP.TERCEROID=@IDTERCERO_CUR)
	  BEGIN
		   SELECT @CNSFCXP=COALESCE(NIEPED.CNSFCXP,'') 
         ...
         DELETE FCXPD WHERE CNSFCXP=@CNSFCXP
	  END
	  IF COALESCE(@CNSFCXP,'')=''
	  BEGIN
     ...
     END
     SELECT @NROCOMPROBANTE=NIEPE.NROCOMPROBANTE 
     FROM NIEPE INNER JOIN  NIEPED ON NIEPE.CNSNIEPE = NIEPED.CNSNIEPE AND NIEPE.NUMDOC=NIEPED.NUMDOC
     WHERE NIEPE.CNSPAGO=@CNSPAGO 
     AND NIEPED.CODPRF=@CODPRF_CUR 
     AND NIEPED.IDTERCERO_PRF=@IDTERCERO_CUR
	 ...
	 BEGIN 
	 	  SELECT TOP 1 @NROCOMPROBANTE=NROCOMPROBANTE FROM NIEPE INNER JOIN NEMP ON NIEPE.NUMDOC=NEMP.NUMDOC
        WHERE NIEPE.CNSPAGO=@CNSPAGO
        AND NEMP.TERCEROID=@IDTERCERO_CUR
     ...
     SELECT @PORCENTAJE=(CONVERT(DECIMAL(14,6),(SUM(VALOR_EGRESOS)*100/(SUM(VALOR_INGRESOS)-coalesce(@VLR_SINDTO,0))))/100),@VLR_INGRESOS=SUM(VALOR_NETO)
     ...
     SELECT NUMDOC,CANTIDAD,CASE WHEN EXISTS(SELECT * FROM NEMPF WHERE NUMDOC=@NUMDOC2 AND CNSPAGO=@CNSPAGO) 
     ...
     END,CCOSTO,IDAREA,CUENTA,FD.CNSPAGO,LINEA,CODCONCEPTO,DESCRIPCION,RUBRO,RUBRO_ID,SINDEDUC
      FROM (
      SELECT A.IDTERCERO,''NUMDOC, CANTIDAD = 1,(A.VALOR)VALOR,(H.VALOR_EGRESOS)VALOR_EGRESOS,
			  F.CCOSTO, F.IDAREA,
      ...
      FROM   NEMPFD A INNER JOIN NPER   B ON A.CNSPAGO     = B.CNSPAGO 
                       ...
                       LEFT JOIN TER    E ON F.TERCEROID   = E.IDTERCERO 
      ...
      AND A.LIQUIDAR='Si' 
      --GROUP BY  A.IDTERCERO,F.CCOSTO, F.IDAREA,
      --       A.CNSPAGO, A.LINEA, A.CODCONCEPTO, G.DESCRIPCION,D.RUBRO,C.CLASE,D.RUBRO_ID,COALESCE(D.SINDEDUC,0),B.CODNOMINA
      UNION ALL
    ...
    BEGIN
       ...
       SELECT @CNSFCXP CNSFCXP, @ITEM ITEM, @CODCONCEPTO_CUR IDSERVICIO, @CANTIDAD_CUR2 CANTIDAD, DBO.FNK_REDONDEA_MAS_EXCEL(@VALOR_CUR2,0) VALOR, 'P' ESTADO, 
              ...
              0 VLR_IVA, DBO.FNK_REDONDEA_MAS_EXCEL(@VALOR_CUR2,0) VLR_NETO, @IDTERCERO_CUR NOLOTE, @CODCONCEPTO_CUR PREFIJO, @CCOSTO_CUR2 CCOSTO, 
...
--------------------------------aqui toca adicionar  el spk de nota debito credito  JHONATAN STEVEN ------------------------------------------------              
   ...
   IF @PORCENTAJE>0 AND EXISTS(SELECT  * FROM NPRF WHERE CODPRF=@CODPRF_CUR AND CLASE='Salario' )
   BEGIN
      PRINT 'SOY EMPLEADO'
      SELECT @VLR_CXP=VALOR FROM FCXP WHERE CNSFCXP=@CNSFCXP
      IF @VLR_INGRESOS-@VLR_CXP>0 
      BEGIN
         UPDATE FCXPD SET VALOR=VALOR+(@VLR_INGRESOS-@VLR_CXP),VLR_NETO=VLR_NETO+(@VLR_INGRESOS-@VLR_CXP) WHERE CNSFCXP=@CNSFCXP AND ITEM=1
         EXEC SPK_CALCULOVALORESCXP @CNSFCXP
      END
   END
------------------------------------------------------------YA RECALCULA EN CXP---------------------------------------------------------------------------------       
   ...
   WHERE NIEPE.CNSPAGO=@CNSPAGO
   AND NIEPED.IDTERCERO_PRF=@NUMDOCR
