...
BEGIN
   ...
   BEGIN         
      ...
      IF EXISTS(SELECT * FROM KPAGE WHERE IDKPAGE=@IDKPAGE AND ESTADO IN('RUNNING','SEND','ERROR','REFUSED'))
      BEGIN
      ...
      ELSE 
      BEGIN
		SELECT @NOADMISION = CONSECUTIVO, @TABLA = TABLA, @PROCEDENCIA = PROCEDENCIA  FROM KPAGE WHERE IDKPAGE = @IDKPAGE
         PRINT 'VOY POR EL ELSE'
		-- SELECT @NOADMISION, @PROCEDENCIA, @TABLA, @IDKPAGE
         IF EXISTS(SELECT * FROM KPAGE WHERE CONSECUTIVO=@NOADMISION AND PROCEDENCIA=@PROCEDENCIA AND TABLA=@TABLA AND COALESCE(ESTADO,'') IN ('EXPIRED') AND IDKPAGE=@IDKPAGE)
         BEGIN
            PRINT 'EXISTE'
   ...
   BEGIN         
      ...
      PRINT '@IDKPAGE='+STR(@IDKPAGE)
      IF EXISTS(SELECT * FROM KPAGE WHERE IDKPAGE=@IDKPAGE AND ESTADO IN('RUNNING','SEND','REFUSED'))
      BEGIN
         PRINT 'VOY A ANULAR'
         SELECT @CONSECUTIVO=CONSECUTIVO,@PROCEDENCIA=PROCEDENCIA,@TABLA=TABLA 
         FROM KPAGE WHERE IDKPAGE=@IDKPAGE 
         AND ESTADO IN('RUNNING','SEND','REFUSED')
         BEGIN TRY               
