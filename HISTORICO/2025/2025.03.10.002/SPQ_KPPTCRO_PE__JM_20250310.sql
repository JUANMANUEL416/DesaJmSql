...
BEGIN
   ...
   BEGIN         
      ...
      END
      BEGIN TRY   
           PRINT 'EXEC SPK_FACTURACE_PERU_HADM_PAQ '+COALESCE(@NOADMISION,'NOADMISION')+','+COALESCE(@DPR,'SI DPR')+','+COALESCE(@COMPANIA,'SIN COMPANIA')+','+COALESCE(@IDSEDE,'SIN SEDE')+','+COALESCE(@USUARIO,'SIN USUARI')+','+COALESCE(CONVERT(VARCHAR,@FECHAFACT,103),'SIN FECHA')+','+COALESCE(@TIPOFAC,'SIN TIPOFAC')+','+COALESCE(@IDTERCERO,'SIN TER')
   ...
   BEGIN         
      ...
      IF EXISTS(SELECT * FROM AUT WHERE COALESCE(FACTURADA,0) = 1 AND AUT.IDAUT = @IDAUT)
      BEGIN
         INSERT INTO @TBLERRORES(ERROR)
         SELECT 'El paquete ya se encuentra facturado.'
      END
      IF EXISTS(SELECT * FROM AUTD WHERE COALESCE(FACTURADA,0) = 1 AND AUTD.IDAUT = @IDAUT AND AUTD.NO_ITEM = @NO_ITEM)
      BEGIN
         INSERT INTO @TBLERRORES(ERROR)
         SELECT 'El item ya se encuentra facturado.'
      END
      IF(SELECT COUNT(*) FROM @TBLERRORES)>0
      BEGIN
         SELECT 'KO' OK, ERROR FROM @TBLERRORES
      ...
      BEGIN TRY           
			IF EXISTS   (SELECT * FROM AUTD WHERE  COALESCE(NOCOBRABLE,0) = 1 AND AUTD.IDAUT = @IDAUT AND AUTD.NO_ITEM = @NO_ITEM)
			BEGIN
				UPDATE AUTD SET NOCOBRABLE = 0 WHERE AUTD.IDAUT = @IDAUT AND AUTD.NO_ITEM = @NO_ITEM
				SELECT 'OK' OK
				RETURN
			END
			IF EXISTS   (SELECT * FROM AUTD WHERE  COALESCE(NOCOBRABLE,0) = 0 AND AUTD.IDAUT = @IDAUT AND AUTD.NO_ITEM = @NO_ITEM)
			BEGIN
				UPDATE AUTD SET NOCOBRABLE = 1 WHERE AUTD.IDAUT = @IDAUT AND AUTD.NO_ITEM = @NO_ITEM
				SELECT 'OK' OK
				RETURN
			END
      END TRY
