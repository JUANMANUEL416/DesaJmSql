IF EXISTS(SELECT NAME FROM SYSOBJECTS WHERE NAME= 'SPK_RPT_ANEXO_PREFAC' AND XTYPE='P')
BEGIN
   DROP PROCEDURE SPK_RPT_ANEXO_PREFAC
END
GO
CREATE PROC DBO.SPK_RPT_ANEXO_PREFAC
      ...
      BEGIN
         ...
         BEGIN
            ...
            SELECT @CNS_D, SER.PREFIJO, HPRED.IDSERVICIO, MAX(HPRED.IDARTICULO),
				   ...
				   CASE COALESCE(HPRED.CODIGOCUM,'') WHEN '' THEN CASE COALESCE(IART.CODCUM,'') WHEN '' THEN SER.CODCUM ELSE IART.CODCUM END ELSE HPRED.CODIGOCUM END
            FROM   HPRED INNER JOIN HPRE ON HPRED.NOPRESTACION = HPRE.NOPRESTACION 
                         INNER JOIN SER  ON HPRED.IDSERVICIO   = SER.IDSERVICIO
            ...
            GROUP BY SER.PREFIJO, HPRED.IDSERVICIO, SER.DESCSERVICIO, HPRED.VALOR,CASE COALESCE(HPRED.CODIGOCUM,'') WHEN '' THEN CASE COALESCE(IART.CODCUM,'') WHEN '' THEN SER.CODCUM ELSE IART.CODCUM END ELSE HPRED.CODIGOCUM END
            ORDER BY PREFIJO, IDSERVICIO, SER.DESCSERVICIO, HPRED.VALOR

            -- INSERTAMOS LOS NOCOBRABLES
            INSERT INTO RPDX (CNS, ID1, ID2, ID3, NOMBRE, VALOR1, VALOR2, VALOR3, VALOR4, IDTERCERO,STRINGGRANDE1,ID9 )
            SELECT @CNS_D, SER.PREFIJO, HPRED.IDSERVICIO, HPRED.IDARTICULO, LEFT(SER.DESCSERVICIO,150), HPRED.VALOR, SUM(HPRED.CANTIDAD), 
            ...
            AND    COALESCE(HPRED.ANULADO, 0) = 0
            AND    COALESCE(HPRED.ENCARGOS, 0) = 0
            GROUP BY SER.PREFIJO, HPRED.IDSERVICIO, HPRED.IDARTICULO, SER.DESCSERVICIO, HPRED.VALOR,CASE COALESCE(HPRED.CODIGOCUM,'') WHEN '' THEN CASE COALESCE(IART.CODCUM,'') WHEN '' THEN SER.CODCUM ELSE IART.CODCUM END ELSE HPRED.CODIGOCUM END
         ...
         BEGIN
            ...
            AND    COALESCE(HPRED.ANULADO, 0) = 0
            AND    COALESCE(HPRED.ENCARGOS, 0) = 0
            GROUP BY SER.PREFIJO, HPRED.IDSERVICIO, HPRED.IDARTICULO, SER.DESCSERVICIO, HPRED.VALOR
            ORDER BY SER.PREFIJO, HPRED.IDSERVICIO, SER.DESCSERVICIO, HPRED.VALOR
         END
         --BASE PROPIA CODCUMS 
			   ...
			   BEGIN
				  ...
				  UPDATE RPDX SET 
				   ...
				   WHERE RPDX.CNS=@CNS_D AND ID1=@PRE_OXIGENO
					...
					UPDATE RPDX SET VALOR2=(VALOR4/VALOR1),NOMBRE=SER.DESCSERVICIO+'TOTAL HORAS'+CAST(VALOR2 AS VARCHAR(4))
					FROM RPDX INNER JOIN SER ON RPDX.ID2=SER.IDSERVICIO
					 WHERE RPDX.CNS=@CNS_D AND ID1=@PRE_OXIGENO
