...
DECLARE @PROCESO VARCHAR(20)              ,@CNSFNOT VARCHAR(20)            ,@CLASE VARCHAR(20)
      ...
      ,@VIVA DECIMAL(14,2)               ,@ESTDIAN VARCHAR(20)
BEGIN
	SELECT *
	INTO #JSON
   ...
   SELECT @IXCOUNTRY=DBO.FNK_VALORVARIABLE('IXCOUNTRY')
	...
	BEGIN   
      ...
      SELECT @ESTDIAN= CASE WHEN DBO.FNK_VALORVARIABLE('IXCOUNTRY')='PERU' THEN FNOT.FACTEP ELSE 
      CASE COALESCE(FNOT.FACTE,0) WHEN 0 THEN 'SinEnviar' WHEN 2 THEN 'Enviada' WHEN 3 THEN 'ConError' WHEN 4 THEN 'Notificada' END  END
      FROM FNOT WHERE CNSFNOT=@CNSFNOT

		SELECT 'OK' OK,@CNSFNOT CNSFNOT,@ESTDIAN AS DIAN
		RETURN 
	END
   ...
   BEGIN         
      ...
      SELECT 'OK' OK, @VR_TOTAL VR_TOTAL
      RETURN 
  END
