CREATE OR ALTER VIEW VWK_PROGRAMA_MAD
AS
SELECT CIT.IDMAD, CIT.CONSECUTIVO,CIT.CONSECUTIVO AS IDAUT,DBO.FNK_FECHA_GRINGA(CIT.FECHA)FECHA,CONVERT(VARCHAR,CIT.FECHA,108) HORA,NOAUTORIZACION,AFI.TIPO_DOC,AFI.DOCIDAFILIADO,AFI.NOMBREAFI,
...
AND EXISTS(SELECT * FROM MAD WHERE MAD.ID=CIT.IDMAD)
UNION ALL
SELECT CIT.IDMAD,CIT.CONSECUTIVO,AUT.IDAUT,DBO.FNK_FECHA_GRINGA(CIT.FECHA)FECHA,CONVERT(VARCHAR,CIT.FECHA,108) HORA,NOAUTORIZACION,AFI.TIPO_DOC,AFI.DOCIDAFILIADO,AFI.NOMBREAFI,
CIT.IDMEDICO,MED.NOMBRE,MES.DESCRIPCION,CIT.CLASEORDEN,CIT.IDPLAN,
...
CASE WHEN COALESCE(AUTD.NOCOBRABLE,0) = 0 THEN  CASE WHEN COALESCE(AUTD.ESDELAB,0)=1 OR COALESCE(AUTD.APOYODG_AMBITO,'')<>'' THEN  IIF(COALESCE(AUTD.ENLAB,0)<>2,1,2) ELSE CASE WHEN  COALESCE(AUTD.ENLAB,0)<> 0 THEN COALESCE(AUTD.ENLAB,0) ELSE 2 END END ELSE 0 END RESUL,AUT.IDFARMACIA ,TGEN.DESCRIPCION AS DISTRIBUIDOR,
