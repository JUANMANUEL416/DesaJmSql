...
DECLARE  @PARAMETROS      NVARCHAR(MAX) ,@MODELO           VARCHAR(100) ,@METODO            VARCHAR(100)  ,@USUARIO         VARCHAR(12),
	   ...
	   ,@NUMERO_REFERENCIA VARCHAR(20),@ID_TRANSACCION VARCHAR(50)	, @CUENTA VARCHAR(20)	,@OPT_CHEQUE VARCHAR(20)
   ...
   END
    ...
    SELECT @CODCAJA = CODCAJA
      ...
      BEGIN TRY
		  ...
		  IF EXISTS(SELECT * FROM CAJ WHERE CODCAJA=@CODCAJA AND CLASE='Menor')
		  BEGIN
			EXEC SPK_SOL_REEMBOLSO_CAJAMENOR @CODCAJA, @SYS_COMPUTERNAME,@IDSEDE,@COMPANIA   ,  @USUARIO
		  END
		  ELSE
		  BEGIN
...
--QUERY 3
   ...
   IF @METODO = 'CONFIRMAR_RECIBO_EGRESO'
	BEGIN
      ...
      BEGIN
         BEGIN TRY
			PRINT 'INGRESO A SPK_CIERRERECIBO_CAJA ===>'
   ...
   IF @METODO = 'NUEVO_RECIBO_EGRESO'
	BEGIN
      SELECT  @CODCAJA = CODCAJA, @CNSACJ = CNSACJ, @TIPOPAGO = TIPOPAGO, @TERCERO = TERCERO , @OBSERVACION = OBSERVACION , @NUMERO_CUENTA = NUMERO_CUENTA, @FECHA = FECHA, @HORA = HORA
      FROM OPENJSON(@PARAMETROS)WITH(
	   ...
	   FECHA     VARCHAR(10)    '$.FECHA',	   HORA     VARCHAR(5)    '$.HORA'
		...
		IF (SELECT COUNT(*) FROM FCJ WHERE CLASE_FAC = 'PAGO' AND CODCAJA = @CODCAJA AND TIPOEGRESO = @TIPOPAGO AND COALESCE(CERRADA,0)  = 0 AND COALESCE(ARCHIVADO,0) = 0  AND ESTADO='P' AND FCJ.CNSACJ = @CNSACJ ) > 0
			AND (SELECT CLASE FROM CAJ WHERE CODCAJA = @CODCAJA) <> 'Principal'
		BEGIN
	...
	BEGIN
      ...
      BEGIN
         BEGIN TRY
			...
			UPDATE FCJ SET VALORTOTAL =  (SELECT SUM (VALORTOTAL) FROM FCJD WHERE FCJD.CODCAJA = @CODCAJA AND FCJD.CNSFACJ = @CNSFACJ  ) WHERE FCJ.CODCAJA = @CODCAJA AND FCJ.CNSFACJ = @CNSFACJ
			IF EXISTS (SELECT * FROM FCJ WHERE CODCAJA = @CODCAJA AND CNSFACJ = @CNSFACJ AND COALESCE(VALORTOTAL,0)<0 )
			BEGIN
