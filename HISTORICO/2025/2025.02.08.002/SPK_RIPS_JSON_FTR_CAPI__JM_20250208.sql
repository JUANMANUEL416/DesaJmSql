...
BEGIN
   ...
   IF NOT EXISTS(SELECT * FROM FTRDC WHERE CNSFTR=@CNSFCT)
   BEGIN
      PRINT 'FACTURA CAPITADA SIN RELACION DE SERVICIOS, ME REGRESO'
      RETURN
   END

   IF EXISTS(SELECT  * FROM FTRDC WHERE CNSFTR=@CNSFCT AND IDAFILIADO IS NULL)
   BEGIN 
      ...
      FROM FTRDC INNER JOIN AUT ON FTRDC.NOADMISION=AUT.NOAUT AND FTRDC.PROCEDENCIA='CE'
      WHERE FTRDC.IDAFILIADO IS NULL
      AND FTRDC.CNSFTR=@CNSFCT
   ...
   END
	...
	WHERE IDTERCERO=@IDTERINSTA
      ...
      BEGIN 
		      ...
		      causaMotivoAtencion='38',COALESCE(CIT.VALORTOTAL,0),IIF(COALESCE(CIT.VALORCOPAGO,0)>0,'02',@conceptoRecaudo),COALESCE(CIT.VALORCOPAGO,0), MED.TIPO_ID, MED.IDMEDICO
      ...
      BEGIN
			...
			causaMotivoAtencion='38',COALESCE(AUTD.VALOR,0),IIF(COALESCE(AUTD.VALORCOPAGO,0)>0,'02',@conceptoRecaudo),COALESCE(AUTD.VALORCOPAGO,0), COALESCE(MED.TIPO_ID, @TIPODOC,''), COALESCE(MED.IDMEDICO, @DOCIDAFILIADO,'') 
         ,AUT.DXPPAL, AUT.DXRELACIONADO, AUT.DXRELACIONADO2,'01'
   ...
   --            ,tipoDiagnosticoPrincipal='01'
         ...
         PRINT 'CONSULTAS HADM...'
			...
			1,COALESCE(HPRED.VALOR,0),IIF(COALESCE(HPRED.VALORCOPAGO,0)>0,'02',@conceptoRecaudo),COALESCE(HPRED.VALORCOPAGO,0), MED.TIPO_ID, MED.IDMEDICO 
         ...
         PRINT 'REVISO CANTIDADES MAYORES A 1 CONSULTAS HADM'
			...
			codServicio=325,finalidadTecnologiaSalud=LEFT(CASE WHEN HPRE.FINALIDAD IS NULL OR HPRE.FINALIDAD='' OR HPRE.FINALIDAD='10'THEN '44' ELSE HPRE.FINALIDAD END,2),
			causaMotivoAtencion='38',COALESCE(HADM.DXINGRESO,HADM.DXEGRESO),COALESCE(HADM.DXSALIDA1,''),COALESCE(HADM.DXSALIDA2,''),COALESCE(HADM.DXSALIDA3,''),
		    1,COALESCE(HPRED.VALOR,0),IIF(COALESCE(HPRED.VALORCOPAGO,0)>0,'02',@conceptoRecaudo),COALESCE(HPRED.VALORCOPAGO,0),CONVERT(INT,HPRED.CANTIDAD), MED.TIPO_ID, MED.IDMEDICO 
			FROM  FTRDC INNER JOIN HADM ON FTRDC.NOADMISION=HADM.NOADMISION  
               INNER JOIN  HPRE ON HADM.NOADMISION=HPRE.NOADMISION
					INNER JOIN HPRED ON HPRE.NOPRESTACION=HPRED.NOPRESTACION AND HPRED.NOITEM = FTRDC.NOITEM
					INNER JOIN SER ON HPRED.IDSERVICIO=SER.IDSERVICIO  AND FTRDC.IDSERVICIO = SER.IDSERVICIO
     ...
     END
      ...
      BEGIN 
		...
		SELECT @IDPRESTADOR,CIT.IDAFILIADO,CIT.CONSECUTIVO,REPLACE(CONVERT(VARCHAR,CIT.FECHA,102),'.','-')+' '+LEFT(CONVERT(VARCHAR,CIT.FECHA,108),5),null,
				COALESCE(CIT.NOAUTORIZACION,'null'),left(REPLACE(REPLACE(LTRIM(RTRIM(SER.CODCUPS)),CHAR(13),''),CHAR(10),''),6),'02','01','02',325,'16',AFI.TIPO_DOC,AFI.DOCIDAFILIADO,CIT.IDDX,
				CIT.IDDX,COALESCE(CIT.IDDX,'null'),COALESCE(CIT.VALORTOTAL,0),IIF(COALESCE(CIT.VALORCOPAGO,0)>0,'02',@conceptoRecaudo),'04',COALESCE(CIT.VALORCOPAGO,0),@N_FACTURA
		FROM  FTRDC INNER JOIN CIT ON FTRDC.NOADMISION=CIT.CONSECUTIVO
            INNER JOIN SER ON CIT.IDSERVICIO=SER.IDSERVICIO
            INNER JOIN AFI ON FTRDC.IDAFILIADO=AFI.IDAFILIADO
				INNER JOIN RIPS_CP ON SER.CODIGORIPS=RIPS_CP.IDCONCEPTORIPS
		...
		SELECT @IDPRESTADOR,AUT.IDAFILIADO,AUT.NOAUT,REPLACE(CONVERT(VARCHAR, AUT.FECHA, 102), '.', '-') + ' ' + LEFT(CONVERT(VARCHAR, AUT.FECHA, 108), 5),NULL
				,AUT.NUMAUTORIZA,left(REPLACE(REPLACE(LTRIM(RTRIM(SER.CODCUPS)),CHAR(13),''),CHAR(10),''),6),'02','01','02',325,'16',AFI.TIPO_DOC,AFI.DOCIDAFILIADO,AUT.DXPPAL,AUT.DXRELACIONADO,AUT.DXPPAL,COALESCE(AUTD.VALOR, 0),IIF(COALESCE(AUT.VALORCOPAGO,0)>0,'02',@conceptoRecaudo),'04'
      ...
      PRINT 'PROCEDIMIENTOS 1'
		...
		SELECT @IDPRESTADOR,AUT.IDAFILIADO,AUT.NOAUT,REPLACE(CONVERT(VARCHAR, AUT.FECHA, 102), '.', '-') + ' ' + LEFT(CONVERT(VARCHAR, AUT.FECHA, 108), 5),NULL
				,AUT.NUMAUTORIZA,left(REPLACE(REPLACE(LTRIM(RTRIM(SER.CODCUPS)),CHAR(13),''),CHAR(10),''),6),'02','01','02',325,'16',AFI.TIPO_DOC,AFI.DOCIDAFILIADO,AUT.DXPPAL,AUT.DXRELACIONADO,AUT.DXPPAL,COALESCE(AUTD.VALOR, 0),IIF(COALESCE(AUT.VALORCOPAGO,0)>0,'02',@conceptoRecaudo),AUTD.CANTIDAD,'04'
            ,CASE WHEN AUT.IDPLAN IN (DBO.FNK_VALORVARIABLE ('IDPLANPART'),DBO.FNK_VALORVARIABLE ('IDPLANPART2')    ----VGARCIA
                                 ,DBO.FNK_VALORVARIABLE ('IDPLANPART3'),DBO.FNK_VALORVARIABLE ('IDPLANPART4')  ----VGARCIA
                                 ,DBO.FNK_VALORVARIABLE ('IDPLANPART5'))                                       ----VGARCIA
                  THEN 0 ELSE  COALESCE(AUTD.VALORCOPAGO, 0) 
      ...
      FROM FTRDC INNER JOIN  AUT ON FTRDC.NOADMISION=AUT.NOAUT
		...
		AND AUTD.CANTIDAD > 1
			...
			SELECT @IDPRESTADOR,HADM.IDAFILIADO,HADM.NOADMISION,REPLACE(CONVERT(VARCHAR,HPRE.FECHA,102),'.','-')+' '+LEFT(CONVERT(VARCHAR,HPRE.FECHA,108),5),null,
				...
				COALESCE(HADM.DXSALIDA1,''),COALESCE(HADM.COMPLICACION,HADM.DXINGRESO,HADM.DXEGRESO),COALESCE(HPRED.VALOR,0),IIF(COALESCE(HPRED.VALORCOPAGO,0)>0,'02',@conceptoRecaudo),'04',COALESCE(HPRED.VALORCOPAGO,0),@N_FACTURA
			FROM  FTRDC INNER JOIN HADM ON FTRDC.NOADMISION=HADM.NOADMISION
                     INNER JOIN AFI  ON FTRDC.IDAFILIADO=AFI.IDAFILIADO
					      INNER JOIN HPRED ON FTRDC.NOPRESTACION=HPRED.NOPRESTACION AND FTRDC.NOITEM=HPRED.NOITEM
         ...
         PRINT 'INSERTO CANTIDAS MAYORES A UNO HADM '
			...
			SELECT @IDPRESTADOR,HADM.IDAFILIADO,HADM.NOADMISION,REPLACE(CONVERT(VARCHAR,HPRE.FECHA,102),'.','-')+' '+LEFT(CONVERT(VARCHAR,HPRE.FECHA,108),5),null,
				HADM.NOAUTORIZACION,left(REPLACE(REPLACE(LTRIM(RTRIM(SER.CODCUPS)),CHAR(13),''),CHAR(10),''),6),COALESCE(REPLACE(TGEN.DATO2,' ',''),HADM.VIAINGRESO,'02'),'01','02',325,'16',MED.TIPO_ID,MED.IDMEDICO,COALESCE(HADM.DXINGRESO,HADM.DXEGRESO), 
				COALESCE(HADM.DXSALIDA1,''),COALESCE(HADM.COMPLICACION,HADM.DXINGRESO,HADM.DXEGRESO),COALESCE(HPRED.VALOR,0),IIF(COALESCE(HPRED.VALORCOPAGO,0)>0,'02',@conceptoRecaudo),HPRED.CANTIDAD, '04',COALESCE(HPRED.VALORCOPAGO,0),@N_FACTURA
			FROM  FTRDC INNER JOIN HADM ON FTRDC.NOADMISION=HADM.NOADMISION
                     INNER JOIN AFI  ON FTRDC.IDAFILIADO=AFI.IDAFILIADO
					      INNER JOIN HPRED ON FTRDC.NOPRESTACION=HPRED.NOPRESTACION AND FTRDC.NOITEM=HPRED.NOITEM
                     INNER JOIN HPRE  ON HADM.NOADMISION=HPRE.NOADMISION AND FTRDC.NOPRESTACION=HPRE.NOPRESTACION
      ...
      BEGIN 
         ...
         INSERT INTO @HOSPITALIZACION (codPrestador,IDAFILIADO,CONSECUTIVO,viaIngresoServicioSalud,fechaInicioAtencion,numAutorizacion,causaMotivoAtencion,codDiagnosticoPrincipal,
		   ...
		   SELECT @IDPRESTADOR,HADM.IDAFILIADO,HADM.NOADMISION,COALESCE(REPLACE(TGEN.DATO2,' ',''),HADM.VIAINGRESO,'01'),
            -- REPLACE(CONVERT(VARCHAR,MIN(FTRDC.FECHA),102),'.','-')+' '+LEFT(CONVERT(VARCHAR,HADM.FECHA,108),5)
      ...
      BEGIN
           ...
           AND EXISTS(SELECT * FROM FTRDC WHERE FTRDC.NOADMISION=CIT.CONSECUTIVO AND FTRDC.PROCEDENCIA='CIT')
           UNION 
           SELECT HCA.IDAFILIADO,HCA.NOADMISION,HCA.CONSECUTIVOCIT,
                   CASE HCA.TIPODX 
							   WHEN 'Presuntivo'   THEN '01'
           ...
           WHERE HCA.CLASE='HC'
           AND HCA.PROCEDENCIA='QX'
           AND EXISTS(SELECT * FROM FTRDC WHERE FTRDC.NOADMISION=HADM.NOADMISION AND FTRDC.PROCEDENCIA='HADM')
      END
